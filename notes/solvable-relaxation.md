...
> **⟦EG-TENTATIVE⟧** **2026-08-16 entangled-generator correction** (`entangled-generator-finding.md`): the Oliver-side figures below marked with this token are tentative pending the v3 rebuild. The **solvable side is untouched** — `score(s) = s·(P(s)−1)/2` already uses the full twist, which is exactly what the correction restores to the Oliver side on matching classes. Consequence worth recording: post-correction the Oliver matching-class score F·C(c,2) = s(c−1)/2 **equals** this document's solvable score at c = P(s), so B(n) ≤ B_solv(n) is a structural invariant and a free validation check on any rebuild — run against the 289 corrected rows on 2026-08-16: 0 violations, 20 exact attainments against the two-part solvable optimum alone.
...
**The headline.** The two problems have the *same* shape space, the *same* cap formula cap_F(η) = η/(1 + √(Fη))², and the *same* balance points. The relaxation changes exactly one thing: **η = 1 always.** Oliver's ceilings (seven under v4; **⟦EG-TENTATIVE⟧** tentatively six distinct values after the entangled correction lifts classes 7, 15 to 1/8) collapse to two — 1/4 at even n and 3 − 2√2 at odd n — and the global constant improves by a factor of **2.390**, from 7 − 4√3 = 0.071797 to 3 − 2√2 = 0.171573. Sharper still: the relaxed problem's *unconditional* odd-n floor of 1/9 (§3½) **exceeds the constrained problem's conjectural worst-case ceiling**.
...
- **The "save the cyclic layer" problem vanishes.** In the Oliver world the twist competes with the foreign primes for room in the single cyclic layer (Lemma C's strip; **⟦EG-TENTATIVE⟧** under v4 it was believed to compete with the block count too, via SAFE's `dmax` stripping — the entangled-generator correction refuted that half, so the residual Oliver-side sharing is the twist–foreign coupling only). Here each block simply takes all of AGL(1, c), and nothing is shared between orbits.
...
| 7, 15 | **⟦EG-TENTATIVE⟧** 1/8 (v4: 1/9) | 3 − 2√2 | **⟦EG-TENTATIVE⟧** 1.373 (v4: 1.544) | **⟦EG-TENTATIVE⟧** tentatively η = 1/2 at F = 2 with the entangled full twist — the same shifted-prime charge as 3, 19; the v4 reading (η = 1 at F = 4, charged for the fusion count) rested on the refuted rung mechanism |
| **11, 23** | 7 − 4√3 = 0.07180 | 3 − 2√2 | **2.390** | η = 1/3: the ℓ = 3 obstruction, at F = 4 |

**Two distinct charges, and they are separable.** The first is the **shifted-prime penalty**: the chain forces the twist to be a q-power, so a prime r contributes only η = 2/D of its multiplicative group, and every ceiling is cap_F(2/D) rather than cap_F(1). The second is a **fusion-count penalty** at the residues whose optimum takes F = 4: reaching a usable η there costs a larger F, and cap_F(η) decreases in F. **⟦EG-TENTATIVE⟧** Under v4 this was visible in pure form at 7 and 15 (η = 1, the whole 1.544 charged to the fusion count); the entangled correction tentatively moves those classes to F = 2 at η = 1/2, so the pure exhibit expires and the fusion-count charge survives only at 11 and 23, compounded there with the η penalty. The relaxation removes both: the twist is the whole of C_{c−1}, so η = 1 everywhere, and F = 2 suffices because no congruence forces a larger one.
...
1. **Γ₁/Γ₂ abelian rather than cyclic.** This isolates what remains of the coprimality budget. **⟦EG-TENTATIVE⟧** As of the entangled-generator correction that is **less than this item originally said**: the claim that every F_mid, cyclic-layer twist and foreign prime must be mutually coprime is refuted — the block-permutation image is a quotient of the layer, not a subgroup, so entangled generators deliver the block count and the full twist from one generator. What cyclic still forces, against abelian, is exactly two things: no duplicated foreign prime (C_r × C_r needs abelian) and Lemma C's conjugation strip coupling the twist to the foreign primes. The earlier probe over n ≤ 70 found **no change at any n** — in hindsight partial evidence that the budget being probed was partly unreal, alongside its stated reading that Lemma C is vacuous on the winners.

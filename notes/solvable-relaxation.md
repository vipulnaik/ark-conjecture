...
**The headline.** The two problems have the *same* shape space, the *same* cap formula cap_F(η) = η/(1 + √(Fη))², and the *same* balance points. The relaxation changes exactly one thing: **η = 1 always.** Oliver's eight ceilings collapse to two — 1/4 at even n and 3 − 2√2 at odd n — and the global constant improves by a factor of **3.397**, from 0.050510 to 0.171573. Sharper still: the relaxed problem has *unconditional* floors of 1/9 and 1/16 (§3½), and **both exceed the constrained problem's conjectural worst-case ceiling**.
...
| 7, 15 | (3 − 2√2)/2 = 0.08579 | 3 − 2√2 | **2.000** | fusion unreachable — c ≡ 3 (mod 4) forces 8 \| r − 1 |
| 11 | (2 − √3)/4 = 0.06699 | 3 − 2√2 | 2.561 | η = 1/6: both obstructions |
| **23** | (5 − 2√6)/2 = 0.05051 | 3 − 2√2 | **3.397** | both obstructions *and* no fused rung |

**Two distinct charges, and they are separable.** The first is the **shifted-prime penalty**: the chain forces the twist to be a q-power, so a prime r contributes only η = 2/D of its multiplicative group, and every ceiling is cap_F(2/D) rather than cap_F(1). The second is the **fusion obstruction** at residues 7, 15 and 23, where reaching the fused rung would need a congruence on r − 1 that the residue forbids, so the shape is stuck on the unfused rung — and by the identity cap_F(η) = cap₁(Fη)/F this costs a factor of *exactly 2* at 7 and 15. The relaxation removes both: the twist is the whole of C_{c−1}, and fusion is always available because nothing else is competing for the cyclic layer.
...
**The chain's cost is entirely arithmetic, not structural.** Both problems reduce to the same balance, the same two-or-three-part shapes and the same cap formula; only η moves. That is a genuinely useful separation, because it means the framework's ceiling constants are *not* artefacts of an idiosyncratic shape space — remove the chain and the same analysis produces the same formula at η = 1. Anyone doubting the eight constants should doubt the η bookkeeping, not the geometry.
...
**A caution about direction.** δ_solv is *not* an upper bound for anything in the ARK programme, and must never be quoted as one. A solvable group without the chain carries no evasiveness conclusion, so μ_solv bounds no topological quantity; it measures the *combinatorial* headroom the chain gives up. In particular the fact that δ_solv ≥ 0.17157 generically says nothing about whether μ(n) ≥ 0.17157·C(n,2), and the 1/50 conjecture is untouched by anything here.

**One suggestive reading, offered as such.** The relaxation's global constant, 3 − 2√2 ≈ 0.1716, sits just above 1/6, and its ceiling is 1/2. The framework's conjectured floor is 1/50. So the whole distance between what Oliver groups guarantee and what solvable groups guarantee is a factor of ~3.4, while the distance between the Oliver guarantee and the *conjectured truth* for μ is a factor of ~2.5 in the other direction. The chain is expensive but not the dominant term; the dominant term is that these are all *families*, and δ(n) is a maximum over shapes that routinely beat every family ceiling (`aod` §3.3.5's warning, and the 91 values in class 11 exceeding 0.05051).

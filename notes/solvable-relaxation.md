...
*Companion note to `arithmetic-of-density.md` and `enumeration-proof.md`; this is row 2 of the hypothesis table in `orbital-evasiveness-notes.md` §1. Relaxes Oliver's chain condition to bare solvability, works out the resulting extremal problem, and reads off the price of the chain. Verification: `solvable_relaxation.py` (all checks passing; it needs the current μ table only for the comparison pass, and only there, and reports the attainment count and the class-11 share against whatever table it is given). Status: the shape space and the ceiling are **proved** below; the two generic constants of §3 are **conditional on the same Goldbach-type input the main framework uses**; the weaker floors of §3½ are **unconditional** for large n, modulo one citation flagged there; everything is **verified** over n ≤ 2600.*

> **The Oliver-side numbers are now current.** The μ table is complete over [6, 2600] and every comparison here — the equality share, the class-11 count and the **ratio distribution** (median, per-parity medians, maximum) — has been recounted against it. No ⟦PENDING-REBUILD⟧ figures remain in this document. The **solvable side is unaffected** — `score(s) = s·(P(s)−1)/2` already assumes the full twist, which is exactly what the Oliver side gets on matching classes. Consequence worth recording: the Oliver matching-class score F·C(c,2) = s(c−1)/2 **equals** this document's solvable score at c = P(s), so **B(n) ≤ B_solv(n)** is a structural invariant and a free validation check on any rebuild — 0 violations, with exact attainments wherever a fused matching class wins. *(The 289 figure is the count of rescan exceedances, which are lower bounds on the rebuilt values; a re-derivation check sees only the 18 whose own recorded witness re-scores higher. `pending-checks.md` A22 pairs the two.)*

---

## 0. The question, and why it is the right one to ask

The framework's μ(n) is a maximum over groups satisfying **Oliver's condition**: a chain Γ₂ ◁ Γ₁ ◁ Γ with Γ₂ a p-group, Γ₁/Γ₂ cyclic, and Γ/Γ₁ a q-group. That condition is not a modelling choice — it is what makes the fixed-point-set argument run, and a group failing it carries no evasiveness conclusion. But it is also, structurally, a *severe* restriction, and the framework's six ceiling constants (`aod` §3.3.5) all come from arithmetic that the chain forces: the twist on a prime block must be a q-power, so a prime r contributes only the q-part of r − 1, which is the efficiency η ≤ 1 that every ceiling is a function of.
...
**The headline.** The two problems have the *same* shape space, the *same* cap formula cap_F(η) = η/(1 + √(Fη))², and the *same* balance points. The relaxation changes exactly one thing: **η = 1 always.** Oliver's ceilings (**six** distinct values, keyed mod 12) collapse to two — 1/4 at even n and 3 − 2√2 at odd n — and the global constant improves by a factor of **2.390**, from 7 − 4√3 = 0.071797 to 3 − 2√2 = 0.171573. Sharper still: the relaxed problem's *unconditional* odd-n floor of 1/9 (§3½) **exceeds the constrained problem's conjectural worst-case ceiling**.
...
> **At a single orbit this is the framework's own S2 identity, and the agreement is exact.** Taking s = n gives density (P(n) − 1)/(n − 1), which is `arithmetic-of-density.md` §2.1's **δ_S2(n) = (Q(n) − 1)/(n − 1)** with P = Q. That is not a resemblance: the Oliver fused matching class and the solvable single orbit are the *same construction*, and the relaxation buys nothing on it — which is why **B(n) ≤ B_solv(n)** is attained exactly at the n where a fused matching class wins. So the chain's cost is zero on this shape and positive on every shape involving a foreign block, which is the whole of what the relaxation measures, visible in one formula.

> **What that proof shows and what the score claim also needs.** The above constructs an orbit worth s(P(s) − 1)/2, so score(s) is *attained*. That it is also an **upper bound** — that no solvable transitive group of degree s does better on its own pair set — is the chain-free reading of `enumeration-proof.md`'s Part C valency recursion, with the maximum valency V(s) = P(s) − 1: a solvable primitive group is affine, so a solvable transitive group of degree s has a block system whose primitive top is affine of prime-power degree, and the recursion caps the minimum orbital at s·(P(s) − 1)/2 with the bound met exactly at the shape above. That is what licenses the word "worth" here and the "proved" in the status line; it is Part C read with Oliver's chain deleted and Huppert's theorem in its place.
...
- **The "save the cyclic layer" problem vanishes.** In the Oliver world the twist competes with the foreign primes for room in the single cyclic layer (Lemma C's strip — and **only** with the foreign primes: it does not compete with the block count, since an entangled generator supplies both from one element, `aod` §3.2.3). Here each block simply takes all of AGL(1, c), and nothing is shared between orbits.
...
Verified: over non-exceptional odd n ≥ 1200 the median δ_solv is 0.16734 and the median small-part share is 0.4167, against the predicted 0.17157 and 0.41421. *(These are medians over the realised family — the winning n = 2c + r with c a prime power and r prime — not over the abstract two-part partition maximum, which runs slightly higher at 0.16882 and 0.4182 because it is free to use parts no construction supplies. Both sit below the predicted values for the same reason: the primes do not sit exactly at the balance point.)* **The balance point is the framework's own**: `aod` §3.3.5 lists x\* = (2 − √2)/2 = 0.29289 as the block share for the fused odd rung at η = 1, and 1 − 2(0.29289) = 0.41421 is the same split read from the other side. The shape is n = 2c + r\* exactly as predicted — a fused pair on one side, an unfused prime on the other, with the asymmetry between them (θ = 1/2 against θ = 1) doing all the work in setting the balance.

**The exceptional family.** A single orbit gives δ = (P(n) − 1)/(n − 1) ≈ P(n)/n, which beats the generic constant when n has a prime-power divisor above n/4 (even) or above n/5.83 (odd) — that is, n = mc with c a prime power and m ≤ 5. Such n are a **density-zero** set, their share decaying like log 5/log n; measured, the share of single-orbit winners falls 0.368 → 0.285 → 0.238 across [100,500), [500,1200), [1200,2600], against log 5/log n of 0.282 → 0.239 → 0.213. The observed multipliers are m ∈ {2, 3, 4, 5} plus three small values at m = 7 (n = 119, 721, 1211) where the two-part split fails on supply rather than on balance. **This family has no analogue in the Oliver world**, where a single fused orbit needs its block count and twist to fit the chain and cannot simply take AGL(1, c).
...
| n mod 12 | Oliver ceiling | solvable ceiling | ratio | what the chain is charging for |
|---|---|---|---|---|
| 0, 4, 6, 10 | 1/4 | 1/4 | **1.000** | nothing — η = 1 is already reachable |
| 1, 9 | 3 − 2√2 = 0.17157 | 3 − 2√2 | **1.000** | nothing |
| 3, 7 | 1/8 | 3 − 2√2 | 1.373 | η = 1/2: the twist loses the 2-part of r − 1 |
| 5 | 5 − 2√6 = 0.10102 | 3 − 2√2 | 1.698 | η = 1/3: local obstruction at ℓ = 3 |
| 2, 8 | (2 − √3)/2 = 0.13397 | 1/4 | 1.866 | η = 1/3 at even n |
| **11** | 7 − 4√3 = 0.07180 | 3 − 2√2 | **2.390** | η = 1/3: the ℓ = 3 obstruction, at F = 4 |

**Two distinct charges, and they are separable.** The first is the **shifted-prime penalty**: the chain forces the twist to be a q-power, so a prime r contributes only η = 2/D of its multiplicative group, and every ceiling is cap_F(2/D) rather than cap_F(1). The second is a **fusion-count penalty** at the residues whose optimum takes F = 4: reaching a usable η there costs a larger F, and cap_F(η) decreases in F. It is visible only at n ≡ 11 (mod 12), where it is compounded with the η penalty; no class charges the fusion count in isolation. The relaxation removes both: the twist is the whole of C_{c−1}, so η = 1 everywhere, and F = 2 suffices because no congruence forces a larger one.

The **global constant** is where the two charges compound. Oliver's is 7 − 4√3 = 0.071797, attained on all of n ≡ 11 (mod 12) — the odd residues carrying the ℓ = 3 obstruction, which forces both a cut efficiency and the larger fusion count. The relaxation's is 3 − 2√2 = 0.171573, the odd-n constant. **The chain costs a factor of 2.390 in the worst class and nothing at all in six of the twelve.**

**Empirically, against the computed table.** B_solv ≥ B_safe at all 2,187 tabulated non-prime-power n, as it must be since Oliver groups are solvable. **Recounted against the current table** (2,187 rows: [6, 2600] contiguous plus the worklist row n = 2759): the two scores **coincide at 916 of 2,187 shared n — 41.9%**, and 91 of the 119 class-11 values exceed 7 − 4√3. *(n = 2759 ≡ 11 (mod 12) is one of the 28 that do not — the range minimiser sits in the extremal class, which is where the theory says the hard cases are.)* *(That last figure is expected and is not a violation: the ceilings bound the balanced additive family, not μ itself.)* The equality share moved as predicted — the corrected matching-class score is exactly the solvable score at c = P(s), so every row whose winner is a fused matching class lands on the identity. **Recounted against the current table**: median ratio **1.062** (1.040 even, **1.145** odd), maximum **4.129 at n = 527**. Both the median and the maximum fell on the recount, as predicted — the old figures were 1.082 (1.040, 1.194) with maximum 4.275 at n = 2147, and 2147 is among the rows the correction raised. The empirical ratios sit *below* the ceiling ratios above because both sides frequently exceed their family ceilings by other shapes — the table's values are attainments, not caps.

## 5. What this says about the framework

**The chain's cost is entirely arithmetic, not structural.** Both problems reduce to the same balance, the same shape space and the same cap formula; only η moves — and, through η, which fusion count is worth paying for, since F is chosen to buy efficiency and the relaxation makes efficiency free. That is a genuinely useful separation, because it means the framework's ceiling constants are *not* artefacts of an idiosyncratic shape space — remove the chain and the same analysis produces the same formula at η = 1. Anyone doubting the ceiling constants should doubt the η bookkeeping, not the geometry. The distinction has held up under revision: the shape space, cap formula and balance points have proved robust, while what a fused class may carry in the way of twists is where the errors have been.

**It corrects a prediction in `orbital-evasiveness-notes.md` §9 item 9.** That item conjectures that a layer word with an abelian or nilpotent top should show no residue structure and "bare ceilings" (the odd value is **not** 1/9, for the reason this paragraph gives). The residue structure does vanish and the even ceiling is bare 1/4, but the odd ceiling is **3 − 2√2 = 0.17157, not 1/9**: the 1/9 reading assumes odd n needs three parts, and once the twist is unconfined a fused pair is always available, so the two-part shape wins. The item's probe never tested this, having moved only even n (56, 60, 63, 66, 70).

**A third part is a chain phenomenon, even though it wins nowhere in either world.** Under the relaxation a third part never wins, and it never wins in the Oliver world either — S4's share of the computed table is zero, the odd-n winner being two-part throughout. What differs is *why*, and that is the informative part: here a third part loses on the balance, the value falling like 1/k² with nothing to buy back, whereas there it is a way to keep η up when the congruences cut it, and is beaten only by a two-part shape with a larger fusion count that keeps η up more cheaply. **The relaxation removes the reason a third part was ever attractive, rather than removing a family that was winning** — which is what predicts where a partial relaxation would first change the answer. *(§2's box states the same thing; this paragraph once claimed three-part shapes are the generic Oliver odd-n winner, which contradicted it.)*
...
1. **Γ₁/Γ₂ abelian rather than cyclic.** This isolates what remains of the coprimality budget. **It is less than one might expect.** F_mid does *not* join a pairwise-coprimality budget: the block-permutation image is a quotient of the layer, not a subgroup, so one entangled generator supplies the block count and the full twist together. What cyclic forces, against abelian, is exactly two things: no duplicated foreign prime (C_r × C_r needs abelian) and Lemma C's conjugation strip coupling the twist to the foreign primes. A probe over n ≤ 70 found **no change at any n**, consistent with Lemma C being vacuous on every winning configuration in the table — the budget appears slack at the optimum.

...
**Empirically, against the computed table.** B_solv ≥ B_safe at every tabulated non-prime-power n, as it must be since Oliver groups are solvable. **⟦PENDING-REBUILD⟧** The ratio statistics that follow are **v4-era**: 289 Oliver rows are known low (n = 2147, the maximum-ratio row at 4.275, is among them), so on the rebuild the median ratio (1.082; 1.040 even, 1.194 odd) falls and the equality share rises. It rises sharply: on the rebuild prefix the two scores **already coincide at 796 of 1,790 shared n (44%)**, against 41% on v4 — the predicted direction, since the corrected matching-class score is exactly the solvable score at c = P(s). The empirical ratios sit *below* the ceiling ratios above because both sides frequently exceed their family ceilings by other shapes — the table's values are attainments, not caps.

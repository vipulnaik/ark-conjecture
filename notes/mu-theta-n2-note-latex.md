...
the scan's minimum over the range being attained at $n = 11183$, and a minimum of $0.045742$ at $n = 1817$ over the range where groups are exhibited individually. These are lower bounds, which is all the present argument needs; over that range the density therefore never falls below $15$ times the $\delta_0 = 1/350$ proved above. \emph{(A matching upper bound --- and hence exact values of $\mu$ --- is the object of a companion classification of the possible orbit structures together with a finite search over the resulting configurations. That is separate work, and nothing here depends on it.)}

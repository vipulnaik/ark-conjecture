...
**Shparlinski (2015) supplies the strongest unconditional results on that scale**, and they are the most directly comparable predecessors to what is proved here. Bombieri--Vinogradov gives $\theta = 1/4$ and hence $\mu(n) \ge n^{5/4+o(1)}$ \emph{unconditionally, for all large $n$} --- which already matches BBKN's ERH bound with no hypothesis at all, so that ERH statement should not be quoted as the state of the art. Using instead the Baker--Harman result on shifted primes with a large prime factor, $\theta = 0.677$ gives $\mu(n) \ge n^{1.677}$, though now for \emph{almost all} $n$ rather than all large $n$; the exponent has since been improved to $0.679$ (Runbo Li, arXiv:2508.18285, 2025).

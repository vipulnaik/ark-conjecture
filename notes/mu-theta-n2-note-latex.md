...
*Oliver's condition.* Take $\Gamma_2 = \mathbb{Z}/c$ (translations of $A$), a $c$-group; $\Gamma_1 = \mathrm{AGL}(1,c) \times (\mathbb{Z}/r)$; and $\Gamma/\Gamma_1 = C_t$, a $q$-group. Then $\Gamma_1/\Gamma_2 \cong C_{c-1} \times C_r$, which is cyclic precisely because $\gcd(c-1, r) = 1$ — this is what condition 4 of (H) secures.
...
\textbf{What the documented conditional routes reach, and where they stop.} Shparlinski also records (2014, \S5) what the Elliott--Halberstam conjecture would give on the same scale: extending the averaging in his Theorem 1 raises $5/4$ to $3/2 - \varepsilon$ for every $\varepsilon > 0$ --- the exponent BBKN obtain from Chowla, by a different and arguably weaker conjecture, since Chowla's route concerns individual progressions --- and allowing any $\alpha < 1$ in his Theorem 2 raises $1.677$ to $2 - \varepsilon$ for every $\varepsilon$, still for almost all $n$. \textbf{This is the natural comparison for the present result, and the gap is not only in the quantifier.} A bound of $n^{2-\varepsilon}$ for every $\varepsilon$ is consistent with $n^2/\log n$; what is proved here is $\delta_0 \binom{n}{2}$, a \emph{fixed positive fraction} of the maximum. The reason no strengthening of Elliott--Halberstam closes that gap is visible in the hypothesis: the $\alpha$-ladder asks for a prime factor of $r-1$ of size $r^{\alpha}$ with $\alpha$ approaching $1$, whereas (H) asks for one of size $(r-1)/d$ with $d \le 12$, linear in $r$ --- a Sophie Germain-type condition, independently twin-prime-hard, and strictly stronger than every rung rather than the ladder's limit.

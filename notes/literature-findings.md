...
**Why the μ(n) framework does not move these constants** is recorded as `pending-checks.md` A35: the restriction method needs evasiveness for every property on a sub-structure, μ(n) gives it for a class of properties on the full structure, and the two are transposed. A35 also records the one opening — whether a multi-block configuration from our shape space gives a larger fully-evasive sub-structure.

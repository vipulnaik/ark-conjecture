...
**It also settles one of the notes' open problems.** **Open Problem 2** of `orbital-evasiveness-notes.md` §11 posed exactly this: almost-all n, unconditionally, at exponent 1.67, via binary Goldbach restricted to the primes with a large prime factor of p−1. That is Corollary 3, published. The problem has been rescoped to what remains — pushing θ past Baker–Harman's 0.677, or shrinking the exceptional set at fixed θ.

**The γ = 1 endpoint reframing is precisely locatable.** Shparlinski's Theorem 2 is parameterised by α, defined by the density of primes r with P(r − 1) > r^α, where P is the largest prime divisor. Baker–Harman gives α = 0.677 unconditionally; he notes "the standard heuristic suggests that the condition of Theorem 2 holds with any α < 1", and that Elliott–Halberstam gives any α < 1. Our hypothesis (H) is effectively the **γ = α = 1 endpoint** of that ladder. Presenting (H) that way says something informative about its cost — it is the limit of a parameterised family whose current unconditional value is 0.677 — rather than asserting a barrier.

...
**The fourth row is the one most likely to be misread**, so state it carefully wherever it appears. The fallback branch has been *characterised* — a named finite residue under a stated hypothesis — not *closed*. B_refined = B_safe remains a per-n certificate rather than a theorem, and the characterisation itself is conditional at two separate points: q-pinning needs a density floor, and the p-characteristic half of the leftover needs Lemma C at a > 1, which is open. A well-characterised open problem and a solved one read alike once the boxes are written; the table is where the difference has to be visible.

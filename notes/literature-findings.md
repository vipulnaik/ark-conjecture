...
| Shparlinski §5 remark (i) | all large n | n^{3/2−ε}, every ε > 0 | Elliott–Halberstam — extends the Bombieri–Vinogradov averaging to z^{1−ε}, upgrading his Thm 1 |
...
2. **The quadratic optimisation as a load-bearing step.** cap_F(η) = η/(1 + √(Fη))² and its balance points produce the six mod-12 ceilings; they are not decoration. Behrend's radius optimisation is the nearest analogue, but it optimises a *construction* rather than a ceiling on all constructions.
...
## 19a. Brown's coset poset, and Shareshian–Woodroofe — the answer to a natural monotone-transitive family

**Found 2026-09 while answering a question about the left-regular action.** The property "S ⊆ G lies in a coset of a proper subgroup" — equivalently, the subgroup generated by S's left quotients is proper — is monotone, nontrivial and invariant under the left regular action, and its complex **is the coset complex**, homotopy equivalent to Brown's coset poset C(G).

- **K. S. Brown**, *The coset poset and probabilistic zeta function of a finite group*, J. Algebra 225 (2000) 989–1012: **P(G, −1) = −χ̃(Δ(C(G)))**, with divisibility results for that Euler characteristic. Whether χ̃ ≠ 0 at every finite group is **open**; it holds for solvable G by Hall's formula.
- **Shareshian–Woodroofe**, *Invariable generation of finite simple groups and rational homology of coset posets* (arXiv:2312.16319): **the coset poset of a finite group is never contractible**, settling Brown's question. Since non-evasive ⟹ collapsible ⟹ contractible, this makes the property above **evasive at every finite group**.
...
## 19c. How the general quadratic bounds are actually proved, and what each author's tools forced

*Read 2026-09 from Miller's tutorial (Foundations and Trends in TCS 7(4), 2011; arXiv:1306.0110, §§5.2–5.4) and from Rivest–Vuillemin directly. Worth recording because the constants in the sequence 1/16 → 1/9 → 1/4 → 8/25 → 1/3 are each explained by which fixed-point tool the author had for their sub-problem, and the explanation uses this framework's own vocabulary.*

**The KSS n²/4 argument (Miller, Prop. 5.4), increasing convention.** Let C_k be the least complexity of a nontrivial property on k vertices. Test h on the star K_{1,n−1} and the near-clique K_{n−1}:

- **h(star) = 0**: fix vertex 1 joined to everything; the restriction to n − 1 vertices is nontrivial, so D(h) ≥ C_{n−1}.
- **h(K_{n−1}) = 1**: fix vertex 1 isolated; again nontrivial, D(h) ≥ C_{n−1}.
- **Otherwise**: fill the clique on m = ⌊n/2⌋ vertices and restrict to bipartite graphs across the halves. Nontrivial, since h′(empty) ≤ h(K_{n−1}) = 0 and h′(complete bipartite) ≥ h(star) = 1. **Yao's bipartite theorem** makes it evasive, so D(h) ≥ m(n−m) ≥ (n−1)²/4.

Peel down to the largest prime p ≤ n, where C_p = C(p,2) by KSS: D(h) ≥ min{C(p,2), p²/4} = p²/4 = n²/4 − o(n²). > **The ½ is lost in the bipartite case, not at the prime-power floor.** The floor alone would give ~n²/2; the induction can stop at the third case first, and a bipartite piece has only ~n²/4 edges — the restriction discards everything inside the halves. Korneffel–Triesch reach 8/25 by the same idea one step richer: a tripartite split with parts p, p, n−2p, p ≈ 2n/5, under ℤ/p × ℤ/p × ℤ/(n−2p).

**Rivest–Vuillemin had the counting half of the fixed-point theory, and both their results show it.** For a p-group acting on a complex, χ(Δ) ≡ χ(Δ^G) mod p is *pure orbit counting* — non-invariant faces lie in orbits of p-power size — with no homology or collapsibility. With a transitive p-group on the coordinates, the only invariant subsets are ∅ and the full set, so χ̃ ≡ −1 (mod p) ≠ 0 for any nontrivial P, while non-evasiveness forces the weight enumerator to be divisible by (1 + t), i.e. χ̃ = 0. That settles prime-power **coordinate** counts via a transitive Sylow subgroup.

- **It cannot pass through a non-p layer**, which is the whole of what KSS added. At prime-power **vertex** count q the translations are a p-group but not edge-transitive — C(q,2) is never a prime power for q ≥ 4, so no p-group ever is — and the needed multiplicative part of AGL(1,q) has order q − 1, coprime to p. Counting mod p cannot see through it; it takes the Lefschetz step for the cyclic quotient applied to a complex already acyclic mod p. **KSS's contribution is the cyclic middle of the Oliver chain; the p-bottom was already Rivest–Vuillemin's.**
- **The same constraint forces their n²/16.** Needing a prime-power number of *coordinates* in the bipartite sub-problem forces a·b to be a prime power, hence equal halves of size 2^k under (ℤ/2)^k × (ℤ/2)^k acting regularly. The largest 2^k with 2·2^k ≤ n can be just over n/4, giving (n/4)² = n²/16: powers of 2 have gaps up to a factor of 2, and it is squared.

**So the history reads cleanly.** Yao freed the bipartite sub-problem from the size constraint (a cyclic group on one side plus a χ computation, at any size); KSS paired that with the density of **primes** for the peeling floor, lifting 1/16 to 1/4; later work enlarges the evasive sub-structure beyond bipartite. It is also exactly `orbital-evasiveness-notes.md` Appendix C's row 6 against row 0: prime-power *coordinate* count needs only a transitive p-group and counting; prime-power *vertex* count needs the cyclic layer and genuine topology.

> **And transitivity is not what powers the bipartite case, though it is present.** Sym(Y) × Sym(Z) is transitive on the bipartite edges, so bipartite properties are weakly symmetric. But Yao's proof uses a single *cyclic* permutation of Y, which has |Z| orbits — one fan H_z per vertex of Z — and wins by computing χ = 1 + (−1)^{k−1}C(|Z|−1, k) on that one-parameter family, which is ≠ 1 unless the property is trivial. **A t ≥ 2 argument, won by an explicit χ computation.** Korneffel–Triesch's tripartite restriction is *not* weakly symmetric: swapping the two equal parts merges the V₁–V₃ and V₂–V₃ edges, but V₃ has size n − 2p ≠ p, leaving two orbits on coordinates (transitive only at n = 3p). So the three proofs sit in three places — KSS at t = 1, Yao at t = |Z| with a simple family, Korneffel–Triesch with neither — which is the distinction `monotone-transitive-note.md` §2 draws.

**A single framework for all of these bounds** is proposed in `restriction-game.md`: each is a strategy in one restriction game, whose first level reduces to a widest path over certified sub-worlds; KSS's case analysis is that reduction run by hand, and n²/4 is the ceiling for escapes certified by transitivity (Lemma 2 there).

**Why the μ(n) framework does not move these constants** — settled, and written up in `orbital-evasiveness-notes.md` beside the threshold result. The framework's own restriction argument gives **D(h) ≥ μ(n)**, and that is sharp for it: certification by a single Oliver group reaches only a single orbital, which an adversary forces to be the minimum. Since μ(n) ≤ n²/4, it never beats Kahn–Saks–Sturtevant; it improves their explicit finite bound only by lower-order terms, at n = 2q. The better constants restrict to multi-orbital sub-structures, which need the extra symmetry layer Yao's argument exploits and the shape space does not model.
...
and the orbital size is m\* ~ n^{1+θ} — which is exactly the ladder's edge bounds in §3.6. So **the whole ladder below θ = 1 delivers a vanishing density.** The constants of §3.3 — 1/4, 3 − 2√2, 7 − 4√3 — exist only at the θ = 1 endpoint. Improving 0.679 to 0.9 moves an exponent and still leaves δ → 0, hence no dimension threshold and no evasiveness conclusion. **This is the point §3.6 currently obscures** by presenting the rungs as progress toward the endpoint: they are progress on a different quantity.
...
**A concrete point of contact with item 20's programme.** Jones–Zvonkin's projective primes are (q^n − 1)/(q − 1) prime; at **q = 2 those are exactly the Mersenne primes**, and the k = 3 analysis reaches the same condition from the other side: `three-uniform-note.md` §2.2.3 shows that at c = 2^p with 2^p − 1 prime and p ≥ 5, the semilinear group AΓL(1, c) acts **freely** on 3-subsets, so the Frobenius map multiplies the minimum orbit by exactly p. Same arithmetic input, two different questions — and the supply is governed by Lenstra–Pomerance–Wagstaff rather than by Bateman–Horn, since 2^p − 1 is exponential in p, which is precisely the exclusion Jones–Zvonkin state for their own Mersenne-like families.
...
- **Babai–Banerjee–Kulkarni–Naik, "Evasiveness and the distribution of prime numbers" (arXiv:1001.4829), Theorem 1.3.** For every graph H, the forbidden-subgraph property is (a) eventually evasive assuming the Weak Chowla Conjecture, (b) evasive for almost all n unconditionally, and (c) of query complexity C(n,2) − O(1) for all n unconditionally, improving the C(n,2) − O(n) of Chakrabarti–Khot–Shi. *Our notes had cited only (c) and (a); (b) is the stronger unconditional statement.* *In the arXiv version, part (b) is stated but its proof is "deferred to the journal version"; which version to cite for (b) should be checked.*
- **Bollobás, "Complete subgraphs are elusive", J. Combin. Theory B 21 (1976) 1–7.** Its abstract (Elsevier open archive) states that "the property of containing a complete subgraph with a given number of vertices is elusive", for an arbitrary vertex set, with no largeness condition. Triangle-freeness is the k = 3 case, so it is evasive for every n ≥ 3, and our triangle-freeness results at n = 10, 45 and 58 re-derive known cases; their value is the group mechanism, not the conclusion. *Confirmed from the abstract; the body was not read.*
- **All decreasing properties of bipartite graphs are evasive.** A Combinatorica paper, "On the recognition complexity of some graph properties" (DOI 10.1007/BF01844851; author believed to be Triesch, not confirmed from the snippet), proves by the KSS topological approach that *every decreasing graph property consisting of bipartite graphs only is elusive*. This covers bipartiteness and every sub-property of it, so the bipartiteness results in `orbital-evasiveness-notes.md` §7.4 are re-derivations of the evasiveness conclusion; their content is where bipartiteness sits on the ladder. It is also a close relative of the biclique box: there, properties inside K_{a,b}-freeness with a + b ≤ β are evasive; here, properties inside bipartiteness.
...
**Theorem 6.2 / Corollary 6.4.** If K is strongly collapsible (reducible to a point by removing dominated vertices — those whose link is a cone), then *every* group of simplicial automorphisms of K fixes a point of |K|; hence **a strongly collapsible vertex-homogeneous complex is a simplex.** The proof goes through Stong's 1984 theorem that a contractible finite T₀-space has a point fixed by all homeomorphisms, applied to the face poset. This is exactly the fixed-point property conjectured in `monotone-transitive-note.md` and `orbital-evasiveness-notes.md`'s ladder note — now a citation rather than a belief. **Consequence:** any counterexample to the monotone weakly-symmetric conjecture is non-evasive but *not* strongly collapsible.

**Proposition 6.9 / Theorem 6.8.** Every vertex-homogeneous complex K is isomorphic to the *n-th multiple* of its core (Lutz's terminology: vertex set V × {1..n}, faces = sets whose projections are faces), the core is vertex-homogeneous and minimal (no dominated vertex), and it equals the square nerve N²(K). This is precisely the *inflation* construction written into `monotone-transitive-note.md` §6 item 0 — in the reverse direction. Ours says a counterexample on G/K lifts to the regular G-set; theirs says every vertex-homogeneous complex descends to a minimal core. Both are true; the second is the one that helps a search.

**Corollary 6.13 / Conjecture 6.15.** The core of a vertex-homogeneous non-evasive complex is non-evasive. Hence **the Evasiveness Conjecture for simplicial complexes is equivalent to its restriction to minimal (dominated-vertex-free) complexes.** For a search this is a free and strong filter: a candidate with a dominated vertex is either a multiple of a smaller candidate or a simplex.

**Corollary 6.10.** A vertex-homogeneous complex with a *prime* number of vertices is minimal or a simplex.

*Applied to A₅ on 15 points:* the core of a vertex-homogeneous complex on 15 vertices has 15/m vertices with m the ≺-class size, so m ∈ {1, 3, 5, 15}. Cores on 3 or 5 vertices are prime-power cases where the conjecture is a theorem (Rivest–Vuillemin), hence simplices, hence K is a simplex. **So a counterexample on 15 points must itself be minimal.** The same argument applies at any n whose proper divisors are all prime powers — 15, 21, 33, 35, … — and makes "no dominated vertex" a necessary condition to impose in the CSP at those degrees.

**Also in the paper.** Non-evasiveness is characterised as *n-collapsibility for some n* (Def 5.1–5.3): 0-collapsible = strongly collapsible, (n+1)-collapsible = some vertex has an n-collapsible link, iteratively. The minimal such n measures the distance from strong collapsibility, and the two notions coincide in dimension ≤ n+1. So the rung between "strongly collapsible" and "non-evasive" in the ladder is itself a graded sequence, and a counterexample would have a definite *collapsibility index* ≥ 1 — a finer invariant than the ladder currently records. The paper's Example 2.13 is a 2-dimensional non-evasive complex that is not strongly collapsible (1-collapsible, not 0-collapsible), so the gap is inhabited already at dimension 2 without any symmetry.
...
Barmak–Minian (§6, citing KSS and Lutz) go further: "there exist examples of contractible (even collapsible) vertex-homogeneous complexes which are not simplices," and in §1, "the conjecture … is known to be false for collapsible complexes in general [Lutz 2001, 2002]." **However**, Benedetti–Lutz 2013 (below), with Lutz as co-author, say of the smallest known contractible example that they "do not know whether the example is collapsible or not." So the collapsible rung's status is: asserted by Barmak–Minian on citation, not confirmed for the smallest example by its own author. Recorded as *probably inhabited, unverified for the smallest instance*.
...
**Sixty vertices.** That is the regular A₅-set — exactly the "universal target" that `monotone-transitive-note.md` §6 item 0's inflation argument singled out, and the group of the Floyd–Richardson fixed-point-free disk action that Oliver's original example descends from. The known ℤ-acyclic and contractible vertex-homogeneous complexes live where the argument said a counterexample would have to be liftable to. Random discrete Morse gives best vector (1,0,0,4,8,4,0,…) after 10 runs of ~1.6 h each and the deterministic lex heuristics give the same; collapsibility undetermined.

Also in §5.11: "non-trivial vertex-homogeneous non-evasive examples cannot be obtained" by joins and the other constructions that produce contractible ones (Welker 1999). So the standard constructions are known to stop one rung short; a counterexample would need something else.

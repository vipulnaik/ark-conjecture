...
3. **The constant re-derived numerically.** The efficiency to use is $\eta \ge 1/12$ — taking $\mathrm{orb}(r,t) \ge rt/2$ regardless of the parity of $t$, which is what condition 3 conservatively guarantees; $\eta \ge 1/6$ is the true worst case but is *not* guaranteed by the condition as stated, and using it overstates the bound. With $\eta \ge 1/12$ the worst densities are $1/48$ (even) and $1/300$ (odd), so $\delta_0 = 1/350$ is safe with room. All three figures are densities relative to $\binom{n}{2}$, which is the unit the note fixes in §1 — the construction bounds arrive as $m^*/n^2$, a factor 2 away.

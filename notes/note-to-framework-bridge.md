...
*The note and the framework can drift apart in two directions — the note can quote a framework figure that has since moved, and the note can claim more than its own contents support. Both have happened; this section records the standing hazards rather than the incidents, since the incidents are fixed and the hazards are not. The largest instance to date is the 2026-08-16 entangled-generator correction (`entangled-generator-finding.md`): 289 table rows rose, the collapse certificates were voided pending repair, and the note's parenthetical citing exactness as an established separate result had to be softened to work-in-progress — both hazard types firing at once.*
...
**The note may only claim what its own contents prove.** Computing μ(n) **exactly** needs an upper bound — a proof that no Oliver group of degree n does better — which is the classification (Theorem 3.1 of the notes) together with the configuration enumeration and its collapse. §3 of the note gives constructions, hence lower bounds. The exactness claim is currently **suspended on the numbers, not on the structure**: Theorem 3.1's coprimality clause was refuted by the entangled-generator construction, the v4 table understates μ at 289 rows, and the collapse certificates are void pending the fb_common repair; the v3 rebuild is in progress. The note accordingly states lower bounds only and describes the companion classification as separate work. **Stating lower bounds costs nothing**, since the Theorem needs only Ω(n²) — and lower bounds are exactly what survived the correction untouched.

> **The failure mode that numerical checking cannot see, and the reason §4 is not enough on its own.** Condition 2 of (H) once read "n/5 ≤ c, r ≤ n/2", which for even n = c + r forces c = r = n/2 and makes the hypothesis very nearly vacuous. It survived repeated checking because **the checking was numerical and the error was not**: every verification swept x = c/n over the range encoded *in the script* — [0.2, 0.5] for even n, which correctly expresses "c is the smaller block" — and compared the resulting constant against the note's claim. Nothing ever compared the note's *prose* against the script's window. No amount of re-running the derivation closes that gap; only reading the hypothesis statement against the derivation by hand does. **Any hypothesis in the note should be read this way before circulation**, and the check is cheap: substitute the stated constraints and see whether they leave a region.

> **Three further hazards of the same family, each of which has bitten once.** A permitted parameter range set too small — condition 3 once stopped at d = 6, while d = 12 is needed at n ≡ 11. A constant claimed with too little margin — 1/(50K) against a true worst case of 0.003340 is safe by 0.2%, which is not enough to circulate. And **a units slip**: the construction bounds arrive naturally as m*/n², while every constant elsewhere is a density relative to C(n,2), a factor 2 apart. The note fixes its unit in §1 for that reason, and states the conversion where the bounds are derived. A units slip is invisible to every consistency check that works in one unit throughout — and it recurs: the two note versions once carried the *even* construction's bound in $\binom{n}{2}$ and the *odd* one in $n^2$, three lines apart. **The check that catches it is to read every displayed bound in the note and confirm each names the same unit**, which no numerical rerun will do for you.
...
| the $\delta_0$ constants by residue class, keyed **mod 12** — **six** distinct values, the entangled correction having lifted classes 7, 15 to $1/8$ and merged them into the 3, 19 row | `arithmetic-of-density.md` §3.3 | the note has the mod 12 *structure*, and now the same modulus — but not the optimised values |
...
> - **The ceilings are keyed mod 12 — the same modulus as the note's own classification.** The mod-12 split is the $\ell = 2, 3$ obstruction. *(Until the entangled-generator correction this bullet read "mod 24, not mod 12": reachability of the fused rung was thought to impose a further condition mod 8, giving seven constants. That condition priced a twist cut that no fusion forces — the rung is reachable at full twist everywhere — so classes 7, 15 rose to $1/8$ and merged into the $3, 19$ row, leaving **six** constants keyed mod 12. What still varies with the residue is the foreign block's efficiency.)*
>
> None of this touches the note's Theorem, which needs only $\Omega(n^2)$ and gets it from the unfused family. It matters only when the note's $d$-table is read as the framework's efficiency table — they now agree on the modulus as well as the classes, and disagree only on which shape is best at $n \equiv 11 \pmod{12}$, where the two-part $F = 4$ shape wins.
...
*Six constants. Classes $7, 15 \pmod{24}$ — i.e. $n \equiv 3, 7 \pmod{12}$'s other halves — formerly sat at $F = 4$, $\eta = 1$, $1/9$; the entangled correction moved them to the $F = 2$ row above, which is what collapsed the keying from mod 24 to mod 12.*
...
**The upper bound, stated as the note would.** The note gives $\mu(n) \le \binom{n}{2}$ and observes that equality forces 2-homogeneity, hence prime-power degree. The framework replaces this with an exact evaluation: every Oliver group's orbit structure is one of an explicitly enumerable list of configurations, each configuration's minimum orbital is a closed-form expression in its parameters, and $B(n)$ is the maximum over that list. The upper bound $\mu(n) \le B(n)$ is then a classification theorem, and $\mu(n) = B(n)$ needs the further fact that the optimum is realised by an actual group. Both halves are under repair after the entangled-generator correction: the classification's coprimality clause is being restated and the certificates rerun; until then the framework, like the note, asserts lower bounds.
...
3. **The constant re-derived numerically.** The efficiency to use is $\eta \ge 1/12$ — taking $\mathrm{orb}(r,t) \ge rt/2$ regardless of the parity of $t$, which is what the note's displayed bound uses. In fact $t = q$ is odd for all large $n$ (an even $t$ would mean $q = 2$ and $r = 2d + 1 \le 25$, excluded by $r \ge n/5$), so $\eta \ge 1/6$ does hold eventually and the note leaves a factor 2 on the table — deliberate crudeness, not necessity. With $\eta \ge 1/12$ the worst density is $1/300$ in **both** the even and the odd case — attained at the corner of the region where the foreign block is smallest, $r = n/5$, not at the balanced point — so $\delta_0 = 1/350$ is safe with room. *(The balanced point gives $1/48$; quoting that as the worst case is the easy slip here, since the minimising configuration is a corner and not an interior optimum.)* All three figures are densities relative to $\binom{n}{2}$, which is the unit the note fixes in §1 — the construction bounds arrive as $m^*/n^2$, a factor 2 away.
...
- **Exact values and the collapse — the numbers suspended, the structure not.** The v4-era claim ($\mu(n) = B(n)$ certified per $n$ to $2600$) is withdrawn pending the entangled-generator repair: 289 of the 2,186 rows are known to understate $\mu$, and the certificates rest on a condition now known non-necessary. The v3 rebuild and repaired certificates will restate it; until then only the lower-bound half is teasable.
...
- **The closed-form ceilings.** **Six** constants indexed by $n \bmod 12$, from $1/4$ down to $7 - 4\sqrt3$, with the extremal class $n \equiv 11 \pmod{12}$ and the global asymptotic constant $7 - 4\sqrt3 = 0.07180$ — seven values under v4, **six** after classes 7, 15 lifted to $1/8$ and merged into the $3, 19$ row; the extremal class and the global constant are unaffected by the correction. *(Conditional on the covering hypothesis, as the note's $\delta_0$ is.)*
...
2. **The mod 12 constants**, replacing $\delta_0 = 1/350$ with $\delta_0(n \bmod 12) \ge 7 - 4\sqrt3 = 0.07180$, a 25-fold improvement stated in closed form — and requiring the fused rungs, since the $F = 4$ shape is what attains it at the extremal classes.

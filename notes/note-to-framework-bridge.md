...
**The note's admissible region contains all six balance points.** Writing $x = c/n$ and $k = 1$ (even) or $2$ (odd), the density of the note's construction is
...
5. **$1/350 < 0.026117 < 0.05051$** verified, all three in units of $\binom{n}{2}$ — and their quantifiers checked to differ as stated: the middle term is global over $[6, 10^6]$, confirmed by noting that the smallest $n$ in the worklist at all is 575, so every composite non-prime-power $n < 575$ has lower bound above $0.05$, and the small values are far above the minimum ($\delta = 0.400$ at $n = 6$).

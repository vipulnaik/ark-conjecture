...
| the $\delta_0$ constants by residue class, keyed **mod 24** — **⟦EG-TENTATIVE⟧** seven under v4, tentatively six distinct values after the entangled correction lifts classes 7, 15 to $1/8$ | `arithmetic-of-density.md` §3.3 | the note has the mod 12 *structure*, not the optimised values |
...
> - **The ceilings are keyed mod 24, not mod 12.** The mod-12 classification is the $\ell = 2, 3$ obstruction; the refinement to mod 24 is a further condition mod 8 entering through the efficiency available to the foreign block at each (class, $F$). **⟦EG-TENTATIVE⟧** Under v4 there were seven constants; the entangled correction tentatively merges 7, 15 into the $1/8$ level, leaving six distinct values pending the rebuild. (The pre-correction reading — mod 8 deciding whether the fused rung is *reachable* — was the refuted mechanism: the rung is reachable at full twist everywhere, and what varies with the residue is the foreign efficiency.)
>
...
For comparison, the framework's current ceilings, keyed mod 24 and attained by fused shapes at the twelve odd classes (**⟦EG-TENTATIVE⟧** the 7, 15 row is the tentative entangled value; the others are unchanged by the correction):
...
| **7, 15** | **⟦EG-TENTATIVE⟧** **$F = 2$ entangled, $\eta = 1/2$** | $1/8 = 0.125$ (v4: $F = 4$, $\eta = 1$, $1/9$) |
...
**The upper bound, stated as the note would.** The note gives $\mu(n) \le \binom{n}{2}$ and observes that equality forces 2-homogeneity, hence prime-power degree. The framework replaces this with an exact evaluation: every Oliver group's orbit structure is one of an explicitly enumerable list of configurations, each configuration's minimum orbital is a closed-form expression in its parameters, and $B(n)$ is the maximum over that list. The upper bound $\mu(n) \le B(n)$ is then a classification theorem, and $\mu(n) = B(n)$ needs the further fact that the optimum is realised by an actual group. **⟦EG-TENTATIVE⟧** Both halves are under repair after the entangled-generator correction: the classification's coprimality clause is being restated and the certificates rerun; until then the framework, like the note, asserts lower bounds.
...
- **The closed-form ceilings.** Constants indexed by $n \bmod 24$, from $1/4$ down to $7 - 4\sqrt3$, with the extremal classes $n \equiv 11, 23$ and the global asymptotic constant $7 - 4\sqrt3 = 0.07180$ — **⟦EG-TENTATIVE⟧** seven values under v4, tentatively six after classes 7, 15 lift to $1/8$; the extremal classes and the global constant are unaffected by the correction. *(Conditional on the covering hypothesis, as the note's $\delta_0$ is.)*

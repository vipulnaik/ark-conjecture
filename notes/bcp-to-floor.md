# From (BCP) back to a density floor: the circle-method route, carried out

*Renamed 2026-09 from `sp-to-floor.md`; the hypothesis formerly written (SP) is now **(BCP)**, bounded-cofactor primes, so that its relation to **(BCG)** — bounded-cofactor Goldbach — is visible in the label: (BCG) is (BCP) plus the per-n additive clause (`arithmetic-of-density.md` §6.8). S_D is still the shifted-prime set and that phrase is kept throughout.*

*Standalone working note, companion to `shparlinski-constants.md`. **One pass by the author, plus a second reading of the algebra, the local analysis, §6.1's counterexample and every reproducible numerical claim** — not of the Siegel–Walfisz major-arc assembly, the singular-series completion at non-squarefree q, or the Vinogradov dilation lemma, which remain as labelled; each step below carries a rigor label — [PROVED HERE], [STANDARD, not rewritten], [VERIFIED COMPUTATIONALLY], or [INTERFACE to the framework's shape space]. The headline claims should be treated as a detailed proof sketch by one reader until a second reading disagrees or doesn't.*

---

## 0. The two answers, up front

The question: can the almost-all-Goldbach machinery take Hypothesis (BCP) — a **cardinality** lower bound on the bounded-cofactor set — back to a positive density floor δ₀, or is something stronger needed on that side?

**Answer 1: (BCP) as stated cannot feed the circle method, and the generic-set version of the implication is false.** The major arcs require the main term of `S_ℬ` at rationals with small denominators, which a cardinality bound leaves undetermined — and this is not a limitation of the technique: §6 constructs, for any cardinality at the (BCP) scale, a set of primes satisfying the count for which **almost all** n are unrepresentable. So no proof can proceed from the count alone while treating S_D as an arbitrary set. What remains open is whether *unconditional* facts about the actual S_D (sieve upper bounds, which do rule the counterexample's structure out) can substitute — §6.3 names that route without pursuing it.
...
*Construction and proof.* [INTERFACE + VERIFIED COMPUTATIONALLY] Take the group `Γ = (𝔽_p^k ⋊ ⟨g⟩) × (C_r ⋊ C_Q)` on `kp + r` points, where `g` is the **entangled generator** `(v_0, …, v_{k−1}) ↦ (ζ v_{k−1}, v_0, …, v_{k−2})`, `ζ` a primitive root mod p, so `⟨g⟩` is cyclic of order `k(p−1)` with `g^k = ζ·diag` — the cyclic-layer rotation with full twist that the framework's corrected shape space provides at any k. The Oliver chain: `Γ₂ = 𝔽_p^k` (bottom p-group), `Γ₁/Γ₂ = ⟨g⟩ × C_r` (cyclic — this is where `gcd(k(p−1), r) = 1` is used; **it does not hold automatically**, since in the optimised windows `r ≈ αn` is the *smaller* summand, so `r < k(p−1)` throughout. What is true is that `r` prime with `r > k` makes `r | k(p−1)` force `r | p−1`, i.e. `p ≡ 1 (mod r)`, which for `p` of linear size leaves at most `(1−α)/(kα) = O(1)` values of `p` per `n` — excluded with the other degenerate cases at cost `O(x^{1/2+ε})` in §4.3. This is the same condition as clause 4 of (BCG_{1/5}) in the short note, and it is handled the same way: by exclusion, not by automaticity), top `= C_Q` (q-group, Q prime). The orbitals, enumerated explicitly at `(p,k) ∈ {(5,2),(7,2),(5,3),(5,4),(7,4)}` and in every case matching the formulas: matching intra `k·C(p,2)` in a single orbital (full twist reached through `g^k`); cross-block `p²·(orbit structure of C_k on block-pairs)`, smallest `p²` at k = 2 and `2p²` at k = 4 (the antipodal orbit), in all cases `> k·C(p,2)` strictly, since the smallest cross-block class is `(k/2)p² > k·p(p−1)/2`, so never binding below the intra term; part-to-part cross `k·p·r`, a single orbital by joint transitivity; foreign intra `r·Q` for odd Q and `r·Q/2` for even (the `−1 ∈ C_Q` dichotomy of `shparlinski-constants.md` §1.5), and **Q is odd throughout this note**, so the bound uses `r·Q`.
...
> `#{n in slab : |E_𝔪(n)| ≥ x/(log x)³} ≪ x·(log x)^{2c+4−B} = O_A(x/(log x)^A)`. *(Using the upper bound `#ℬ ≪ x/(log x)²` cited just above, not `#ℬ ≤ x`; B is free, so the difference is bookkeeping, but the display should spend the bound it names.)*
...
## 6. Why (BCP) alone cannot do it — and what "alone" leaves open

### 6.1 The generic-set counterexample

[PROVED HERE] Suppose the implication "(cardinality at the (BCP) scale) ⟹ (almost-all representation `n = kp + r`, `k ≤ K`)" held for arbitrary sets of primes. Take `m = Π_{ℓ ≤ L} ℓ` with `L` chosen so `φ(m) ≍ log x` (so `L ≍ log log x`), fix any unit `b mod m`, and let

> `ℬ* = {r prime : r ≡ b (mod m)} ∩ window`, `#ℬ* ≍ x/(φ(m) log x) ≍ x/(log x)²` — **the (BCP) scale exactly.**
...
- **Closed form.** `δ(k,d) = (√k+√(d/2))^{−2}` against a grid search on the **corrected** objective `min{u²/k, 2αu, 2α²/d}` at nine (k,d) pairs — (1,2), (1,6), (2,2), (2,4), (2,6), (2,12), (4,6), (6,4), (12,2). Agreement to the grid resolution throughout (relative error 4·10⁻⁴ to 2·10⁻³ at 1200², shrinking with refinement), and the optimising window matches the closed-form balance point `u* = √k/(√k+√(d/2))` to three places at every pair. The class-11 margin is confirmed on the grid as well as algebraically: (4,6) at 0.0717 against (6,4) and (2,12) at 0.0669 and (12,2) at 0.0502.

...
**Not on this list, because it closed:** the converse direction — that a density floor *forces* a shifted-prime statement — is now Proposition F.4 of `enumeration-proof.md`, with the discussion in `aod` §6.7. It is elementary given F.1's machinery and needed no new input. **What it opens instead is a sharpening question**, which is a research item rather than a gap: the round trip (H) → δ₀ = 1/350 → D = 700 loses a factor ~58 against (H)'s own d ≤ 12, uniformly across both branches. Whether either direction can be tightened — a better constant in the note's central inequality, or a converse that reaches a prime rather than a prime power — decides how close to a genuine equivalence this is. Neither is needed for anything currently claimed.
...
**Requote from the run, do not carry forward**: the exact-attainment count and the class-11 exceedance share, both quoted in `solvable-relaxation.md` and both moved by the rebuild. `k3_galois.py` is *not* registered here — it takes no table and scans a fixed range, so one run per environment suffices.
...
**A discarded rung that may not be discarded for us: the fixed-residue level-of-distribution results.** Shparlinski's §5 closes by setting aside the stronger unconditional results of Bombieri–Friedlander–Iwaniec, Mikawa and Fouvry, because they restrict the residue classes a in ψ(y, m, a) and so do not suit his formulation. **Our formulation carries that restriction for free.** Everything this framework needs about shifted primes is of the form r ≡ 1 (mod Q) — the modulus varies and is large, the residue is *fixed at 1* — which is exactly the regime where those results beat Bombieri–Vinogradov (BFI reach level 4/7 for fixed a, against BV's 1/2). So the question to ask is not whether they are strong enough but whether the exclusion applies: **read BFI/Mikawa/Fouvry with a = 1 pinned, and check whether the restriction Shparlinski names is the one our formulation already satisfies.**

**A second question for the same sources: does the almost-all step survive a set of relative density ~1/log x?** Shparlinski's Theorem 2 converts a set of primes with large Q(r − 1) into a statement about almost all n by an exceptional-set argument, and it is applied to the Baker–Harman set, whose relative density is ≈ 1 − α — positive for fixed α < 1. **This supplies exactly the additive coupling that `aod` §6.7's (SP) cannot**, so it is the route back from a multiplicative hypothesis to a density floor. The obstacle is that the bounded-cofactor set has relative density only ≈ C·log D/log x, one logarithm short of positive rather than a power short. Read the exceptional-set step and ask whether it tolerates that loss: the heuristic count of representations n = c + r with r ∈ S_D is still ≫ n/log³n, so the question is whether the circle-method error terms and the set's distribution in arithmetic progressions survive — the latter being the same level-of-distribution question as the fixed-residue item above, which is why the two share sources. *If it does survive, the payoff is large: it would close the gap between (SP) and a floor for almost all n.*

*Sizing, so this is not oversold.* Bounded cofactor needs level → 1; 4/7 does not approach it, and nothing here would give the bounded-D statement. The realistic prize is the **almost-all exponents** of §3.6's ladder, where a fixed-a improvement over BV could raise a rung. Two obstacles to check first: whether the well-factorable or smooth-modulus conditions those results carry are compatible with Q ranging over prime powers, and whether their averaging is over a modulus range that reaches Q ≈ r/D at all.
...
>
> *A quantitative consequence of the same framing.* The primes with bounded cofactor have density ≈ C·log D/log x among primes — measured at D = 12 as 0.343, 0.235, 0.182 over x = 10⁴, 10⁵, 10⁶, tracking log 12/log x. So the set is twin-prime-thin, no positive-proportion statement about it can be true, and (H) is a Goldbach condition **over a set that thin** — which is the quantitative form of the "independently twin-prime-hard" remark in `aod` §3.5.
...
**The extracted hypothesis (SP) is new with the same standing.** `aod` §6.7 states the n-free multiplicative half — bounded cofactor D plus a **two-sided** relative-density estimate ρ(x), the two-sidedness being required because the coupling needs a proportional window [δ₀n, (1−δ₀)n] and |S ∩ [cx,x]| is a *difference* of cumulative counts, so a lower bound at x and an upper bound at cx are both needed (the PNT-to-Bertrand relationship). Bounded multiplicative gaps at Λ = 2/δ₀ are the weakest special case and are what a floor implies for free. The section asserts two things about the hypothesis that want checking independently: that a floor implies it (the gap half being a short argument from branch (b) plus branch (a)'s density-zero exceptional set not filling an interval), and that it does **not** imply a floor, the additive clause of (b) being inexpressible n-freely. The second needed correcting once already and is now stated more carefully: a purely multiplicative hypothesis *can* reach the additive clause, via the exceptional-set machinery of Shparlinski's Theorem 2 — an earlier draft asserted no n-free statement could, which was wrong. What blocks it at our parameters is that the machinery consumes **relative density** and the bounded-cofactor set's density is ≈ C·log D/log x rather than positive. So the honest status is *open*, not impossible, and the concrete question is filed as a T4 literature item. *Measured for context:* the largest multiplicative gap below 2·10⁶ is 1.041 at D = 12 and 1.165 at D = 2, counting from r > 10³, and shrinking with the cutoff — so (SP) at the constants a 1/25 floor needs is enormously weaker than the data suggests, and its difficulty is entirely that bounded D is the Sophie Germain endpoint.

**Three smaller steps wanting a second look.****The shape audit is done and closed one gap; record it so it is not redone.** Walking S1–S10 against the branch split: S3, S4, S5, S6, S7 and **S9** carry a foreign part and land in (b); S8 and S10 are killed so nothing exists to quantify over; S1 and S2 land in (a). **S9 (fused outside class) is the gap it found** — the first draft bounded a foreign part's contribution by r·Q with r ≤ n, which is the unfused case; a fused foreign class contributes F·r·Q and needs the **joint** bound F·r ≤ n, the same care the matching branch already required. Stated in the proof now. *Note this is the second time the joint-versus-separate bound has been the defect* (the other produced the spurious 2/δ₀²), which makes it the thing to check first on any restatement. Also recorded there: branch (a) is **wider than S1 ∪ S2**, since multi-class all-matching shapes such as n = 640 = 1·256 + 3·128 are not named in the census but land in (a) correctly — the proof splits on presence of a foreign part rather than on the census, and a shape-by-shape proof would have missed them.
...
2. **Progress. Phase 0 is complete.** Status is per file, and the distinction that matters is the sorry count rather than the compile — a sketch full of sorries compiles perfectly happily:
   ...
   Between the first two the note's whole arithmetic layer is proved; what stays conditional is (H) and Oliver, neither formalisable. **Next Lean work is phase 1: `Basic.lean`'s sorries**, where the mod-12 ceiling table and cap algebra live — and its statements carry the ladder correction while its proofs have never been attempted, so nothing there has been pre-checked by a compiler.

...
4. **The ceiling table's η values.** The F side of §3.3.5's joint optimum is closed — cap_F(1) = 1/(1 + √F)² bounds each F-slice with no arithmetic input — but the η side is measured over a sample of n rather than derived, and the class-11 entry turns on 676 > 675. Nothing computed depends on it, these being family ceilings and hence floors for μ; the exposure is to the asymptotic story. → **T6**
...
- **Whether to build groups at all, and how often.** Preconditions existing is not the same as the group existing. The n = 10 and n = 12 exhaustive batteries plus eight hand-built configurations from n = 12 to 315 are the current evidence; a decision about whether to add occasional GAP spot-checks at new shapes is a priorities call — but **S7 at F ≥ 3 has moved from least-evidenced curiosity to the shape that sets the class ceiling at four residues** (`aod` §3.3.5), so a construction at F = 4 with a cut twist is now the single most valuable spot-check available, and S4 is second.
...
### T6. The ceiling table's η values are measured, not proved

`aod` §3.3.5 is a joint optimum over (F, η), and the **F side is closed**: cap_F(η) = η/(1 + √(Fη))² together with η ≤ 1 gives cap_F(1) = 1/(1 + √F)² as a hard ceiling on the whole F-slice, which excludes F ≥ 8 with no arithmetic input at all, and the parity constraint at odd n leaves only F ∈ {2, 4, 6}. At residues 7 and 15 the tabulated value *is* cap₄(1), the absolute ceiling at F = 4, so those rows cannot be improved by any F at any η.

**The η side is not closed.** The guaranteed efficiencies in that table are the minimum over roughly 30–60 sampled n per class in [6·10⁴, 9·10⁴] of the best η available, with c ≡ 3 (mod 4) so the matching term stays at F·x². That is evidence, not proof. Three things a human should check:

- **The measurement is of a guarantee, not a best case**, and getting this backwards is easy: an earlier version asked only whether η was *available* without checking that the matching side still attained its share, and produced a table contradicting §3.3.5 where §3.3.5 was right. The current figures reproduce §3.3.5's η = 1/12 at F = 2 for class 23, which is the check that the correction took — any re-derivation should reproduce it too.
- **The class-11 entry rests on a very thin margin.** 7 − 4√3 beats (2 − √3)/4 by way of 26 > 15√3, i.e. **676 > 675**. Any slip in the measured η at that class flips the comparison, and the decimals hide how close it is.
- **A congruence derivation would replace the sampling.** The mod-8 mechanism is understood — 4c ≡ 4 rather than 6 (mod 8) sends r from 1 to 3 (mod 8) and drops v₂(r − 1) to 1 — so the reachable η per (class, F) should be derivable rather than measured. Doing that is a page of congruence bookkeeping and would put the whole table on the same footing as its F side.

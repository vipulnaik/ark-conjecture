...
>
> **Measured, and close to tight.** Computing lcm(orbital sizes) from each recorded witness via the G.3 formulas, over 605 sampled n across [6, 10^6]: **median n^3.70, 10th percentile n^3.66, range n^2.64 to n^4.56, and not one row below n^2.5**. Against the actual orders the lcm accounts for nearly all: n^3.98 vs n^4.07 at n = 2759, n^3.60 vs n^3.66 at n = 30,030, n^3.68 vs n^3.74 at n = 510,510 — a gap of about 0.06 in the exponent. **On every configuration the table records, divisibility alone already forces Ω(n^{3.3+}).**
>
> **So what is owed is a lemma, not a search** — and `orbital-evasiveness-notes.md` **Theorem 2.1's upper-bound argument is the template**, as Vipul observed. That argument uses nothing about Oliver groups beyond *solvable, hence not 2-homogeneous off prime-power degree, hence t >= 2*; what it uses instead is that **the action comes from vertices**, which constrains the orbital shapes. Applying it here:
>
> **Transitive case.** Each u-orbital has a common valency d, so |Ω| = n*d/2 with sum_i d_i = n-1 over the t orbitals. So **all orbitals equal <=> all valencies equal <=> t | n-1 and d = (n-1)/t**, giving |Ω| = C(n,2)/t — the bad case exactly. Meeting the floor as well forces **t <= 1/delta**. *The bad case is therefore a transitive group of rank t+1 <= 1/delta + 1 with all valencies equal.* It is not vacuous — C_n at odd n has every valency 2 and every orbital of size n — but there delta = 2/(n-1) -> 0. **Small rank and equal valencies pull against each other, and that tension is the lemma.**
>

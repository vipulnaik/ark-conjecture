...
*The one leg of μ(n) = B(n) with no per-n check. The certificate side is verified at all 2,186 rows; attainment is verified at **seven**, the largest n = 308. `verify_witness.g` is the right tool and needs no change — what it needs is entries.*
...
**The battery has the blind spot that was in the ladder.** Its matching classes use F ∈ {1, 2, 3, 5} — **no even F ≥ 4**. That is now the family attaining the class ceiling at residues 7, 11, 15 and 23 (`aod` §3.3.5), and it is the same omission that kept even F out of `ladder_verify.py`'s S7 loop, from the same source: the belief that a fusion count had to be a prime power. Closing it costs minutes.

**Step 1 — add five entries and rerun.** All at n ≤ 575, so they run at battery speed. The first is the smallest even F ≥ 4 anywhere; the next three are the smallest F = 4 winners at the three low-cap residues where F = 4 sets the ceiling; the last exercises F = 6, where the fusion count occupies two primes in the cyclic layer rather than one.

```gap
[ "p=5 q=2: 4x5",                 40 ],   # n =  20, smallest even F >= 4
[ "p=43 q=41: 4x43 + 1x83*",    3403 ],   # n = 255, class 15 (mod 24)
[ "p=43 q=5: 4x43 + 1x151*",    3612 ],   # n = 323, class 11
[ "p=103 q=3: 4x103 + 1x163*", 13203 ],   # n = 575, class 23
[ "p=47 q=2: 6x47",             6486 ],   # n = 282, F = 6, two primes in the layer
```

No code change is needed: `ConstructionTwists` already strips the fusion count's primes from the twist — at `4x139 + 1x619*` it returns d = 69, stripping the 2 from 138 because F_mid = 4 sits in the cyclic layer — and `PredictedTerms` already keys the within-class cross coefficient on the parity of F. A Python build of that configuration returns orbitals 38364, 38642, 63757, … with m\* = 38364 = B(1175) exactly, so the shape does realise its score; the battery simply has never been asked.

...
3. **The ladder's reach — now carrying more than it did, and promoted for that reason.** The contiguous table is complete to 2600 and the ladder to 10⁶ with an exact minimum at n = 2759; extension of the *table* is discretionary. The **ladder** is not merely evidence any more: Corollary E.6 derives μ(n) = B(n) from the ladder floor being above 1/25, so `ladder_verify.py`'s correctness is now load-bearing for a theorem-side statement about a million values rather than for a floor. Re-read its scoring — every rung must be an admissible configuration scored no higher than SAFE — before relying on that. What remains is that it is *finite*: nothing here bears on n beyond 10⁶. → **R0**, **R7**, **R1** after any discretionary batch.

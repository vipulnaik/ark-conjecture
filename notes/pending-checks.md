...
2. **Progress.** **Phase 0 is complete.** `Note.lean` compiles on the laptop at **0 errors, 0 sorries** (10 of 10 discharged: six by import from `ArkCore`, four written against Mathlib), and `ArkCore.lean` compiles in both environments at 0 sorries. Between them the note's whole arithmetic layer is proved; what remains conditional is (H) and Oliver, neither formalisable here. `blockValue_lower` moved *into* `ArkCore` (pure ℕ) and is compiled there. Next Lean work is **phase 1: `Basic.lean`'s sorries**, which is where the ceiling table and cap algebra live. *Import mechanics, since this cost a round:* Lean resolves `import ArkCore` through `LEAN_PATH` and lake's build dirs and loads the **compiled `.olean`**, so co-locating the sources does nothing and `PATH` is irrelevant. Either `lake env lean -o ArkCore.olean ArkCore.lean` then `LEAN_PATH=$PWD ./leancheck.sh Note.lean` — **on one line** (exporting `LEAN_PATH` first does not take), and **without** `:$LEAN_PATH` (unset variable ⟹ empty path entry ⟹ rejected) — or move the files under the lake library's source dir and import `Ark.ArkCore`. The README carries both recipes and all three gotchas.

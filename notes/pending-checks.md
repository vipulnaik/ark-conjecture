...
**A second question for the same sources: does the almost-all step survive a set of relative density ~1/log x?** Shparlinski's Theorem 2 converts a set of primes with large Q(r − 1) into a statement about almost all n by an exceptional-set argument, and it is applied to the Baker–Harman set, whose relative density is ≈ 1 − α — positive for fixed α < 1. **This supplies exactly the additive coupling that `aod` §6.8's (SP) cannot**, so it is the route back from a multiplicative hypothesis to a density floor. The obstacle is that the bounded-cofactor set has relative density only ≈ C·log D/log x among primes — in the integers, 1/log²x against the 1/log x that both almost-all Goldbach and Shparlinski's input enjoy. **Binary Goldbach for almost all even n is a precedent that 1/log x is no barrier**; nothing is precedent for a summand at 1/log²x. *But the sharper question is not about density at all.* The exceptional-set argument consumes **distribution in arithmetic progressions** — major arcs are built from it, minor arcs from an exponential-sum bound — and (SP) supplies only a count. So read the step to establish **what equidistribution clause a sufficient version of (SP) would need**, rather than only whether the density suffices; expect the answer to be a level-of-distribution statement about the modulus Q, which is the same question as the fixed-residue item above and why the two share sources. *If it does survive, the payoff is large: it would close the gap between (SP) and a floor for almost all n.*

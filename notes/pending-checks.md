...
# 5. the documents against the table; this replaces the pending figures
...
**What is owed: a further independent reading**, worth more than another pass by either previous reader. Parts A–J and Part 0 have had a second reading by someone with no prior exposure; it found no error in the steps it examined, confirmed B′'s socle argument, and **every finding it returned was site 3**. It did not reach the site-4 defect that sat in the theorem statement, the SAFE cap and `fb_common`'s condition (4) throughout — so a reading running sites 1–3 should be assumed to leave site 4 untouched.
...
2. **Progress.** **A draft with every `Note.lean` sorry discharged is ready for a laptop run** (10 of 10: six by import from `ArkCore`, four written against Mathlib). The imported half is certain — `exact` applications of theorems compiled with zero sorries in two environments; the Mathlib half is numerically verified but not yet toolchain-checked, and Mathlib name drift is the expected failure mode, not false statements. `blockValue_lower` moved *into* `ArkCore` (it is pure ℕ) and is compiled there. **Run `leancheck.sh` on it and report the sorry count**, which should now be 0 for `Note.lean` as well. *Import mechanics, since this cost a round:* Lean resolves `import ArkCore` through `LEAN_PATH` and lake's build dirs and loads the **compiled `.olean`**, so co-locating the sources does nothing and `PATH` is irrelevant. Either `lake env lean -o ArkCore.olean ArkCore.lean` then `LEAN_PATH=$PWD ./leancheck.sh Note.lean` — **on one line** (exporting `LEAN_PATH` first does not take), and **without** `:$LEAN_PATH` (unset variable ⟹ empty path entry ⟹ rejected) — or move the files under the lake library's source dir and import `Ark.ArkCore`. The README carries both recipes and all three gotchas.

Before that run, **all three files compile**, and the distinction that matters is the sorry count, not the compile: `Note.lean` and `Basic.lean` compile on the laptop against Mathlib with **nonzero sorries** — sketches, statements as the deliverable — while **`ArkCore.lean` compiles in both environments with zero sorries, every proof complete** against core Lean 4.15.0 — covering the ℕ half of phases 0 and 1: `central_even`/`central_odd`, Lemma D1, the capacity bound, F.1 in squared form, `orb_full`, and every `decide` table. **One portability lesson already banked:** `List.mem_cons_self` takes explicit arguments in core 4.15.0 and implicit ones under the laptop's toolchain, so `List.mem_cons_self a rest` compiles in exactly one of the two environments. Three sites now use `simp` instead. **The convention for this file is therefore: prefer a tactic to a named library lemma wherever the goal is trivial**, since the named form is what carries the version dependence. The likeliest next offenders are the `Nat.mul_le_mul_left` / `Nat.mul_le_mul_right` / `Nat.mul_lt_mul_left` family, whose argument order and explicit/implicit split have both moved historically — if a future laptop run fails, look there first. The container recipe for a bare toolchain is in the README (direct GitHub tarball; elan's release server is off the allowlist; **Mathlib stays unreachable there**, so Mathlib-side work is laptop work). Next: compile `Basic.lean` on the laptop, which has never been run anywhere; then start discharging `Note.lean`'s sorries, importing the ℕ statements from `ArkCore` rather than re-proving them.

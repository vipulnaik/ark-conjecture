...
> | `a18_verify.py` on the **exact** table | all passes, worst UB/B **0.8276 at n = 56**; D2′'s range-scoped half now covers to 24,236. The script prints the theorem threshold for each floor: 0.02516 → n ≥ 1582, **0.04621 → n ≥ 471** |
...
**Added by the 2026-09 audit, and none of them touches a computed value.** Each is now fixed in the documents; they are recorded here because the *pattern* is what recurs.

...
**Literature — answered** (`literature-findings.md` entry 23): bipartiteness's complex is a known wedge of spheres (Chari; Jonsson's book); triangle-freeness is covered by Bollobás 1976, "Complete subgraphs are elusive" — **from the title only; the paper's scope (all n or large n) still needs confirming** (a further search found only citations, none stating its scope); also found: every decreasing property of bipartite graphs is evasive (Combinatorica, DOI 10.1007/BF01844851, author to confirm), and BBKN Theorem 1.3(b)'s proof is deferred to their journal version; and BBKN Theorem 1.3 adds evasiveness for almost all n, unconditionally.

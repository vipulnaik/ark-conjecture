...
2. **Bound the other classes.** With a 2-transitive permuter the nonzero-offset classes may be small enough to restore m\* ≤ |O|/2 by a different route.
   ...
   > **Most of this route may already be written down, in a code comment rather than in the document.** `mu_enumerate_v2.py`'s `parts_for` skips fused foreign parts and justifies it by **domination rather than by D2's divisibility step**: independent translations are impossible (F copies of C_c generate C_c^F, cyclic only at F = 1), and *diagonal* translations, though admissible, preserve the difference y − x, so the same-position pairs form a class of size only ~F·c/2 — "which the twist can never merge (it fixes 0)". That argument never mentions the degree of the block-permuting group, so on its face it survives exactly the case A18 attacks. **The clause to press on is the parenthetical**: the twist fixes 0 and so cannot move an offset-zero pair off the diagonal, but a 2-transitive *permuter* acts on the block index, and what has to be checked is that it cannot merge the offset-zero class into a larger one either — the class is (block-pair orbital)·c, so a 2-transitive permuter makes it C(F,2)·c rather than (F/2)·c, and whether that still gives m\* ≤ |O|/2 is the actual question. Worked case E of `enumeration-proof.md` is the same computation at F = 2.
   >
   > Two things follow. The code comment and this item are currently **disconnected**, so the enumerator's exclusion rests on an argument the document does not make and the document's lemma rests on a step the code does not use; whichever survives, they should be made the same argument. And if the domination reading does close the gap, it closes it *for the shape space as implemented*, which is the stronger and more useful outcome — it would mean the enumeration is not missing shapes even though D2's stated proof does not cover them.

...
- **Which constrains the shape of any proof of (H)/(SP), independently of the density accounting.** Bounded cofactor needs moduli `Q ≍ r/D`, inside Friedlander–Granville's failure window — but the framework never needs a per-modulus asymptotic there (the count of `r ≤ x` in one class mod `Q ≍ r/D` is `O(D)`, bounded, so an asymptotic is meaningless). (SP) is a count **aggregated over Q**, which irregular moduli do not disturb. **So (H) and (SP) cannot be derived from a level-of-distribution statement of the usual asymptotic form at level ≈ 1, because that statement is false; they must come from a counting or sieve argument tolerant of irregular moduli.**

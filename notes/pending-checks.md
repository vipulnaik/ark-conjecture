...
> **The computational programme is complete to its planned bounds.** `mu_enumerate_v2.py` has the table contiguous over every non-prime-power n ≤ **2,600**; `ladder_verify.py` has scanned to **10⁶**; R1 runs clean against both; and R7 is **vacuous at the conjectured floor of 1/25**, every worklist entry pruning on its own lower bound with no survivor to compute. So R0, R1 and R7 are not a backlog: **a further run of those happens only if a bug is found or the range is deliberately extended**, and in either case the trigger is a change to the code or a decision about scope, never an outstanding obligation. **R8 is the exception and the one computational item genuinely owed** — the attainment leg of μ(n) = B(n) is checked at twelve values against the certificate side's 2,186, and its battery has never been run on the fusion counts that now set four of the class ceilings. It sits in §1 with the other runs, since it is a run: `gap -q -A verify_witness.g`, with its step 1 entries already in the file.

> **An independent read pass (August 2026) found no mathematical error and thirteen documentation-state defects, all now applied.** It confirmed B′'s socle argument and read Parts D, D2, E, E′, E″, F and G; it did not reach `fb_common.py`, the literature items, or any script beyond `eta_derive.py` and the two verifiers. §2 is scoped to what it left. One change it forced is structural rather than textual: **rows above n = 2,600 in `mu_table_safe_v4.csv` are worklist rows, not range**, so every distributional figure is quoted over the contiguous prefix — see R0.
...
*The one leg of μ(n) = B(n) with no per-n check: the certificate side is verified at all 2,186 rows, attainment at **twelve**, the largest n = 575. `verify_witness.g` is the tool; step 1 below is ready to run.*
...
**What the August 2026 pass settled, and what it did not.** Parts A–J and Part 0 have now had a reading by someone who had not read them before, and the three failure sites above were the checks it ran. It found **no mathematical error** in any structural step; in particular **B′'s socle argument is confirmed** — Step 0 does need irreducibility plus C_G(V) = V, exactly as written, and both cases close — so the "one reading, no independent scrutiny" flag on B′ is discharged. Every finding it did return was failure site 3, a claim contradicting an artefact nobody had compared it against, which is the site this file predicted would be most productive and the one a reader catches most cheaply.

Two things remain, and they are narrower than the original item. **A second independent reading is still worth more than a third pass by either previous reader** — the argument for independence does not stop applying once two readers agree, and the framework's record is of errors surviving one reading each. And **the pass was a machine reading, so it is weakest exactly where the failure mode is "an argument that reads as plausible"** — it verified constants and recomputed tables freely, and had no way to notice a step whose plausibility is doing the work. That is the residual human item.
...
*Human, for the same reason as T1: the value is in the independence.* **The August 2026 read pass did not touch this item at all** — `fb_common.py` was not in the working set, so the eight necessity arguments have still had exactly one reading. Of everything in §2a this is now the largest unread surface, and it is the whole trusted base for μ(n) = B(n) on the certified range.

### T4. Literature: one high-upside investigation, four smaller owed items, framing deferred

*Untouched by the August 2026 read pass, which stayed inside the documents; every item below still stands as written. Four passes are written up in `literature-findings.md`, which carries a reference convention — every citation of our own documents is prefixed `` `aod` ``, `` `notes` ``, `` `ep` ``, and a bare § belongs to a cited paper. Two candidate follow-ups — running our CSP against Angel–Borja's surviving types, and chasing the two-orbital criterion computationally — are **deliberately not on the list**: the exhaustive n = 10 and n = 12 batteries already validate the machinery more strongly than either would.*

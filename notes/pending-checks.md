...
2. **When `solvable_relaxation.py` finishes at 10⁶** — it now reports progress, so the decision to wait or optimise can be made from the first heartbeat. **Stage announcements and a work-weighted heartbeat go to stderr** (`QUIET=1` to suppress), and an up-front estimate prints before the loop: **78 s of the 79 s at N = 85,000 is the B_solv loop**, which is ~O(n) per value and ~O(N²) overall, so **N = 10⁶ is about 3 h**. *The heartbeat measures work, not values: the instantaneous rate falls from 4,200 to 1,100 values/s across an 85k run, so a count-based ETA drifts low for the whole run — wrong in the reassuring direction, which is the one direction a "should I wait?" tool must not be.* If 3 h is too long, the loop is the only thing worth touching, and the same interval-and-prime-power-list treatment that gave `mu_ladder_exact.py` 4.8× applies to it.

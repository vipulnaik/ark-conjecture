...
> **✔ PASS — all four parts, every expectation met.** Part A: all seven asserted negatives return `fail`, all four positives pass. **Part B is the result that matters** — over degrees 6..11, **108 Oliver against 52 not**, so the predicate separates a real population rather than saying 0 always, and **eleven of the negatives are solvable**: T(8,42), T(8,45), T(8,46), T(8,47), T(9,23), T(9,25), T(9,26), T(9,28), T(9,29), T(9,30), T(9,31), orders 216 to 1296. Those are the informative ones, since part A's insoluble negatives fail for a reason too cheap to test much — and their existence answers a question the framework had not asked directly: **solvability does not imply Oliver, and the gap is wide**, roughly a third of transitive groups at degrees 8 and 9. Part C: 71 shapes compared under both predicates, no disagreement in the direction that would be a bug. Part D: all four broken chains rejected, D2′ accepted, and **the two verdict columns diverge exactly as designed** — D1's witness fails while the search finds a chain at q = 3, D2's fails while the search finds one at q = 2. That divergence is the item's real product: it is the first output in the battery where the `oliver` column carries information rather than a constant. *(GAP has no implicit string concatenation — two adjacent literals across a line break are a parse error, not a join. That cost the first D run; fixed.)*

> **Two cautions when reading the part B table, both about what it does *not* show.** (1) **No insoluble group is Oliver, and that column proves nothing** — the chain is solvable-by-solvable-by-solvable, so Oliver ⇒ solvable is a theorem and the zero is forced. Only the **solvable** failures are evidence. (2) The solvable failures sit **only at degrees 8 and 9** in range — 4 of 45 and 7 of 30 — while every solvable transitive group at degrees 6, 10 and 11 is Oliver. Do not read that as "the chain is free away from prime powers": the range is short, and degrees 8 and 9 are exactly where a solvable group has room to be built from three primes without a usable normal layer. **A non-Oliver group is not a counterexample home either** — at prime-power degree a Sylow subgroup is transitive and Oliver regardless (`monotone-transitive-note.md` Proposition 2), so these degrees are settled whatever the individual groups do. The table is a check on the predicate, not a result about degrees.

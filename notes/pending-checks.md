...
> | `wide_cert.py 100000` | **90,297 of 90,299 (99.998%)**; weakest B_lo density 0.020004, permitted s ≤ 6. The residue is two values, **n = 50,817 and n = 89,697** — see below |
> | `wide_cert.py 100000 --no-theorems` | **90,292**, i.e. five more unresolved, and that is the expected disagreement rather than a defect: the five are bare pairs that E.3(ii) resolves and the mode declines to |
>
> **The two values `wide_cert.py` leaves open are a named residue, not a rounding artefact.** Both are the shape 2c + r\* with c a safe prime, r = (c−1)/2 and leftover **L = c** — exactly the case E.3(ii)'s global promotion declines, since the (r, r) re-reading would re-type the leftover c-block as a second foreign part equal to the first. They were previously reported as closed, by conditions that capped a p-characteristic part with a twist strip; that cap bounds what a *group* realises, not the SAFE score the collapse is about, so the closure proved μ = B_refined there and not B_refined = B_safe. Under the flat cap the certificate answers the second question and these two survive. **The earlier 100.00% was an artefact of testing the wrong quantity, not a stronger result.** Settling them needs the true B(n), far past the computed table; nothing else below 10⁵ is open.
>

...
**One live item it leaves behind**, worth carrying separately since it is not about the presentation at all — and now with data attached (`johnson-presentations.md` §5 records the measurement: our witnesses are **polynomial**, log|Γ|/log n ≈ 3.5–4.5 typical, against BBKN's **superpolynomial** (n/log n)·log log n, e.g. 10³⁶·⁹³⁵ times larger at n = 510,510 while its orbital is 8,000× *smaller*). §5's open question, **what is the minimum order of an Oliver group on m points whose minimum orbital is ≥ δ·C(m,2), for m not a prime power?** The necessary bound is Ω(m²) and is tight at prime powers (AGL(1, m) has order 2·C(m,2)), but the multi-block constructions use groups polynomially larger — the bottom layer alone is c^F — and nothing in the framework explains the excess. Either much smaller Oliver groups achieve constant δ at composite m, which would widen the class of G the argument reaches, or Ω(m²) is far from achievable off prime powers and a better lower bound exists. `mu_exact.py` could settle it over the computed range by recording each winning configuration's group order beside its density — a small change to a script that already enumerates the configurations.

...
The smallest open cases of the right kind. **n = 6 is settled by Kahn, Saks and Sturtevant in the original 1984 paper** — not by their prime-power theorem, since 6 is not a prime power, but as a separate case they dispose of alongside it, with the Oliver machinery doing most of the work and a small residue checked by hand; C(6,2) = 15 coordinates keeps that feasible. *(Engström's 2006 abstract states it directly: the conjecture "was proved for n a prime power, and n = 6, … by Kahn, Saks, and Sturtevant." This file once said simply "KSS settle n = 6", which reads as though the prime-power theorem did it, and then over-corrected to "by exhaustive computation, not KSS"; both were wrong in different ways.)* 7, 8, 9 are prime powers, where the conjecture *is* a theorem. **n = 10 = 2·5** is the first composite non-prime-power beyond 6, and it is *arithmetically strong*: 5 is a Fermat-adjacent prime, 5 − 1 = 4 is a 2-power, and the wreath construction AGL(1,5)≀C₂ achieves density 20/45 = 0.444, close to the ceiling of 1/2 — which holds for *all solvable* groups at non-prime-power n, not merely Oliver ones (`solvable-relaxation.md` Proposition 1 — stated there over all solvable groups of degree n, transitivity not imposed, matching this framework's own shape space), so the headroom above 0.444 is genuinely small rather than an artefact of the chain condition.
...
> **Correction (2026-09): the global test is a group test after all.** The trivial group is Oliver — trivial p-group, trivial cyclic layer, trivial top — its fixed complex is Δ_P, and its condition is exact, so **χ(Δ_P) = 1 is the Γ = 1 row of the trivial-top condition in §2.2**. What keeps it out of the CSP is only the orbital cap: the trivial group has t = C(10,2) = 45 orbitals against MAXT = 12. So the "independence" above is between the *capped* battery and the uncapped trivial group, and the lesson of §5.4 sharpens: the skeleton satisfied every group condition with t ≤ 12 and died at t = 45. Since χ-resistance does not pass to subgroups (`orbital-evasiveness-notes.md` §7), small groups are not weaker tests, and the Oliver classes between the cap and the trivial group — 183 of them at n = 10, with 13 ≤ t ≤ 45 — are a band the pipeline never evaluates.
...
*Independent reproduction (2026-09).* A separately written Oliver predicate (`oliver_tom.g` in the notes directory: largest normal p-subgroup, then the normal closure of the non-q Sylow subgroups), run on the S₁₀ table of marks in a freshly built GAP 4.14, finds 1,593 classes, **1,294 Oliver, and 1,111 with at most 12 orbitals — exactly the TOM emission of §8.5**. That is a second check on `IsOliverTop` beyond the negative-control battery, and it supplies the uncapped list: 183 Oliver classes at n = 10 with 13 ≤ t ≤ 45. Of the 1,294, 699 are trivial-top (exact), 577 have top prime 2, and 21 top prime 3.
...
- **n = 6** (Kahn–Saks–Sturtevant 1984, as a separate case — *not* via the prime-power theorem, 6 not being a prime power) reproduces.
...
**A natural property shows the cone escape at every transitive group.** Bipartiteness at n = 6 and 10, checked against every Oliver subgroup class (`bip_fixed_chi_scan.py`): **all 12 and all 24 transitive Oliver groups have cone complexes** — some orbital can be added to every bipartite union — so they are blind to it at every rung, χ, congruence and Smith alike. The killing is done entirely by intransitive groups: 1,156 of them at n = 10, and 1,004 within the t ≤ 12 battery, so bipartiteness could never have survived the CSP. At the t = 2 group of §7.4 it takes the branch K₅,₅ ∈ P. *And a cheap strengthening follows*: evaluating a known candidate against the whole uncapped list of 1,294 groups took 8 seconds for bipartiteness, so a **post-check battery** — every SAT candidate against every Oliver class, not only the capped CSP set — fits naturally between the CSP and the global χ test. It cannot enter the CSP, whose encoding is exponential in t, but as a filter on candidates it costs nothing.

...
**The group theory.** Parts A, B, B′, C, D and D2 of `enumeration-proof.md` are statements about *which groups exist* and about their block structure. None mentions pairs. They transfer unchanged, and so does the shape space: chain primes (p, q), orbits, matching and foreign blocks, fusion counts F = F_mid·F_top, and the cyclic-layer budget in its corrected form — Lemma C's twist–foreign strip and the no-duplicate-foreign rule; the v4 reading that F_mid itself joins a pairwise-coprimality budget is refuted at k = 2 and does not transfer because it was never true.
...
> **What that competition is worth, at both arities.** The S4 / S5 / S7 comparison determines which shape realises the *three-part family*. Under v4 the two-part S7 at F = 4 held the ceiling at 7, 11, 15, 23; post-correction it holds only 11, 23, with 7, 15 going to S7 at F = 2 at full twist — a shape inside this comparison. So read §5.6.4 as settling the internal order of the three-part shapes at 11, 23, and as identifying the class winner at 7, 15.
...
The other congruence effect is the one `aod` §3.2.3 called the c mod 8 law. Under v4 a cyclic-layer fusion (F_mid = 2) was believed to force the twist down to the **odd part** of c − 1; the entangled-generator construction refutes the forcing at k = 2, and the mechanism (z^F = full twist) is arity-agnostic, so the cut is **never forced at k = 3 either** — the cost table below records the *price of the cut where taken*, not a price of fusion, and the residual price of fusion is Lemma C's foreign coupling. Writing 2^v ‖ c − 1:
...
- **S7** (swap in the *cyclic* layer): under v4, the twist was cut to the odd part at the cost of §5.6.2; post-correction the entangled layer keeps the full twist (subject only to Lemma C's strip against the foreign prime) — and **q is free**, so the foreign block may choose the top prime that maximises its own η₃. S7 pays nothing on the matching side.
...
**The trade is the same in shape as at k = 2, and different in magnitude — read under v4's cut.** Post-correction the matching-side ratio is **1 : 1 at every c and both arities** (neither shape pays a twist cut), so S7's free q decides outright wherever Lemma C does not bite, and the table below describes only the cut-taking regime. Under v4: at k = 2 the ratio S5 : S7 is 2^{v−1}, so at c ≡ 3 (mod 4) it is 1 and S7 wins on the free q; at k = 3 the ratio is 2^{v−1} or **2^v**, so:
...
> **Under v4 the c ≡ 3 (mod 4) escape narrowed to c ≡ 11 (mod 12)**: a third of odd blocks giving S7 its free-q win at k = 2, a quarter at k = 3, S5's advantage one factor of two larger on the rest. Post-correction that narrowing is moot — S7 wins its side everywhere — and S5's remaining case is the one where the matching twist shares a prime with the foreign block, so that Lemma C strips S7's layer while S5's top-layer swap leaves the twist alone. Whether S5 converts still turns on finding r with a large 2-part of r − 1 — the Fermat-like supply of `aod` §4.3, unchanged.
...
> **The F = 4 rows follow from the k = 3 machinery with nothing new assumed, which is worth stating because it was flagged as open.** Three things transfer verbatim. The **parity constraint** — at odd n with c odd and r an odd prime, F·c + r = n forces F even — is arithmetic of the decomposition and mentions no arity. The **mod-8 supply mechanism** likewise: the shape determines r mod 8 from n mod 8, and v₂(r − 1) follows (under v4 this was routed through c ≡ 3 (mod 4); post-correction c mod 4 is freed at F = 2 and the residue arithmetic is carried by the shape alone — the 11, 23 rows come out unchanged, and 7, 15 gain the η = 1/2 reading recorded above). And the **η values** are the same, since η₃ = η/2 uses the same twist t and the same prime r, so `aod` §3.3.4a's derivation applies as corrected. What differs is only the cap formula, and there the arity enters as the extra √κ inside each term.
...
5. Finish rows 7, 15 and 23 of the ceiling table (§5.7). **Row 23 done** — set by the two-part F = 4 shape at both arities, outside the three-part tie entirely; see the box after §5.7's table. **Rows 7, 15 revised** by the entangled-generator correction: S7 at F = 2, η = 1/2, β₃ = 1/16 in the κ_c = 2 column. The k = 2 rekey they inherit from is settled (`aod` §3.3.4, confirmed independently); what remains open is only whether the arity-agnostic transfer of the full-twist mechanism has a k = 3 subtlety not visible at k = 2.

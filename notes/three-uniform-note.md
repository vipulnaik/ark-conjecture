...
| §2.2.2 the semilinear criterion | **proved**, necessity included; two clauses out of computational reach |
...
> - **d > 1 and m = a** (which the theorem requires). Frobenius acts on C_d by x ↦ x^{2}, trivial only if d | 2^{a/m} − 1 = 1. So for d > 1 the product is non-abelian, C_a cannot join the middle layer, and **Γ/Γ₁ ≅ C_a must be a q-group — a must be a prime power q^e.**
>
> **Corollary (Oliver-constrained).** For a block with d > 1, the Galois part raises the minimum **iff p = 2, gcd(d, 6) = 1, and a = q^e with q ≥ 5**, the gain being exactly **q**.
>
> So the answer to "do the conditions survive Oliver's condition" is **yes, with one tightening**: gcd(a, 6) = 1 becomes the strictly stronger *a is a prime power with q ≥ 5*. The unconstrained criterion admits a = 35; Oliver's does not, since C₃₅ is not a q-group.
...
> - **On a matching block, intermediate values are useless.** §2.2.2 shows a proper Galois subgroup (1 < m < a) never raises the minimum, because the fixed field 𝔽_{p^{a/m}} then has at least p² ≥ 4 elements and supplies a stable 3-set. So **the only values worth writing are m = 1 and m = a**, and m = a is worth taking only under the Oliver-constrained criterion — p = 2, gcd(d, 6) = 1, a = q^e with q ≥ 5 — at a cost analysed in §4.3.
>

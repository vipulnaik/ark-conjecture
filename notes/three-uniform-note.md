...
**The group theory.** Parts A, B, B′, C, D and D2 of `enumeration-proof.md` are statements about *which groups exist* and about their block structure. None mentions pairs. They transfer unchanged, and so does the shape space: chain primes (p, q), orbits, matching and foreign blocks, fusion counts F = F_mid·F_top, and the cyclic-layer budget in its corrected form — Lemma C's twist–foreign strip and the no-duplicate-foreign rule; F_mid itself does **not** join a pairwise-coprimality budget — at either arity, since the fusion image is a quotient of the layer rather than a subgroup.
...
> **What that competition is worth, at both arities.** The S4 / S5 / S7 comparison determines which shape realises the *three-part family*. The two-part S7 at F = 4 holds the ceiling at 11, 23 only; at 7, 15 the winner is S7 at F = 2 at full twist — a shape inside this comparison. So read §5.6.4 as settling the internal order of the three-part shapes at 11, 23, and as identifying the class winner at 7, 15.
...
| n mod 12 (residues mod 24) | shape | η | **s\*/n**, κ_c = 2 | s\*/n, κ_c = 3 | **β₃** (κ_c = 2) | β₃ (κ_c = 3) | k = 2 cap (δ) |
|---|---|---|---|---|---|---|---|
...
| 7, 15 | S7 at F = 2, entangled (v4: F = 4) | 1/2 (v4: 1) | 0.50000 | 0.55051 | **1/16 = 0.06250** (v4: 1/18) | (5 − 2√6)/2 = 0.05051 | 0.12500 (v4: 0.11111) |
...
5. Finish rows 7, 15 and 23 of the ceiling table (§5.7). **Row 23 done** — set by the two-part F = 4 shape at both arities, outside the three-part tie entirely; see the box after §5.7's table. **Rows 7, 15**: S7 at F = 2, η = 1/2, β₃ = 1/16 in the κ_c = 2 column, inherited from the settled k = 2 keying (`aod` §3.3.4). What remains open is only whether the arity-agnostic transfer of the full-twist mechanism has a k = 3 subtlety not visible at k = 2.

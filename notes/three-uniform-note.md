...
| §2.2.2 the semilinear criterion | **proved**, necessity included; two clauses out of computational reach. **The Oliver-constrained corollary was wrong and is corrected** — the tightening is a layer split, not "a is a prime power" |
...
> - **d > 1 and m = a** (which the theorem requires). Frobenius itself acts on C_d by x ↦ x², nontrivially whenever d > 1, so the *generator* of C_a cannot join the middle layer. But that does not force **all** of C_a upstairs, and the layer assignment has to be worked out rather than asserted.
>
> > **The split, done properly.** Let C_{a′} ≤ C_a be the part of the Galois group that joins the cyclic layer. Its generator is Frob^{a/a′}, which acts on C_d by x ↦ x^{2^{a/a′}} and centralises C_d exactly when **d | 2^{a/a′} − 1**. Given that, Γ₁ = 𝔽_c ⋊ (C_d × C_{a′}) has abelian quotient C_d × C_{a′} over Γ₂, which is **cyclic iff gcd(d, a′) = 1**; and Γ₁ is normal in Γ because Frobenius normalises C_d and centralises C_{a′}. The remaining quotient is Γ/Γ₁ ≅ C_{a/a′}, which must be the top q-group. So the requirement is not on a but on the existence of a suitable split:
> >
> > > **∃ a′ | a with (i) d | 2^{a/a′} − 1, (ii) gcd(d, a′) = 1, and (iii) a/a′ a prime power q^e.**
> >
> > The case a′ = 1 recovers the naive reading "a itself is a prime power", but it is only one branch.
>
> **Corollary (Oliver-constrained).** For a block with d > 1, the Galois part raises the minimum **iff p = 2, gcd(d, 6) = 1, and a admits a split as above** — with q, the top prime, being the least prime divisor of a/a′; the gain (the factor of §2.2.2's theorem, which needs m = a) is the least prime divisor of **a**.
>
> So the answer to "do the conditions survive Oliver's condition" is **yes, and the tightening is weaker than it first appears.** The naive tightening — gcd(a, 6) = 1 becoming *a is a prime power* — is **wrong**, and its stated test case is a counterexample to it. Take **a = 35, d = 31, c = 2³⁵**: since 31 | 2⁵ − 1 | 2^{35/7} − 1, put a′ = 7 in the cyclic layer, where C₃₁ × C₇ ≅ C₂₁₇ is cyclic (gcd(31, 7) = 1), leaving Γ/Γ₁ ≅ C₅. That is a genuine Oliver chain with top prime 5, m = a = 35, and gcd(35, 6) = gcd(31, 6) = 1 — so the gain applies and the factor is 5. **C₃₅ never has to be a q-group; only the part of it left on top does.**
>
> > **Why this matters more than a corrected predicate usually would.** The error's direction is the dangerous one at k = 3: it *under*-counts admissible Galois blocks, and §5.8 records that a k = 3 scoring which under-credits the Galois part is not a loose upper bound but not an upper bound at all. Anything downstream that enumerated blocks by "a = q^e with q ≥ 5" — the census's m-note in §4.2, §6.1's escape condition, and any future k = 3 enumerator — is enumerating a strict subset of what exists. The qualitative conclusions are unaffected: gains still need characteristic 2, still need gcd(a, 6) = 1 and gcd(d, 6) = 1, q | a still holds so §4.3's top-prime coupling stands unchanged, and a ≤ log₂ n still keeps the escape at O(n/log n). What changes is only which blocks qualify. Tracked as item 8 of `pending-checks.md` §2b.
...
> - **On a matching block, intermediate values are useless.** §2.2.2 shows a proper Galois subgroup (1 < m < a) never raises the minimum, because the fixed field 𝔽_{p^{a/m}} then has at least p² ≥ 4 elements and supplies a stable 3-set. So **the only values worth writing are m = 1 and m = a**, and m = a is worth taking only under the Oliver-constrained criterion of §2.2.2 — p = 2, gcd(d, 6) = 1, and a admitting the layer split there (**not** merely a = q^e; see the correction box) — at a cost analysed in §4.3.
>
...
**Why it is an escape rather than a routine improvement.** It is available only when p = 2, gcd(d, 6) = 1 and a admits §2.2.2's layer split, so the matching block must be 2^a with gcd(a, 6) = 1 — the blocks 32, 128, 2048, 8192, … together with the composite-a blocks the split admits (a = 35 among them), a set of density zero and, being Mersenne-adjacent in the twist condition, thin even among those. *(The block set here is larger than the superseded "a a prime power" reading gave; the count below is unaffected, since it is driven by a ≤ log₂ n either way, but the enumeration in §5.7's rows and any k = 3 enumerator must use the corrected predicate.)* And it comes with the coupling of §4.3: taking it fixes the top prime at q, and every foreign block in the configuration then needs q | r − 1.

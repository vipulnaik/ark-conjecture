...
> **What this note imports from k = 2, and one hazard in it.** The rung mechanism transfers arity-agnostically: z^F is the full twist — a statement about the group, not the pairing — so a cyclic-layer fusion costs nothing on the matching side at either arity, and the residual cost is Lemma C's twist–foreign coupling. **Gotcha: do not import a coprimality budget on F_mid.** There is none, at either arity. Unaffected by any of this: the orb₃ law and κ₃ arithmetic (§2), the Kantor classification and everything downstream (§3), the Galois machinery and its global-q coupling (§§2.2, 4.3), Lemma B′ and the supply analysis (§4.4, §5.5), the allocation formula and balance-point identity (§5.2, §5.7's κ_c = 2 identity), and §5.8.
...
**The group theory.** Parts B, B′, D and D2 of `enumeration-proof.md`, together with Part 0, are statements about *which groups exist* and about their block structure. None mentions pairs, and they transfer unchanged. *(Parts A and C do not: A's term types are a partition of k and C's counting bound is pair-specific — see §8, which is the authority on the part-by-part status.)* So does the shape space: chain primes (p, q), orbits, matching and foreign blocks, fusion counts F = F_mid·F_top, and the cyclic-layer budget in its corrected form — Lemma C's twist–foreign strip and the exclusion of duplicated foreign primes — which is a *domination* (two foreign orbits of one prime share a diagonal C_r and carry a same-position class of size r, Lemma D2 in its non-permuted form), not a non-existence; F_mid itself does **not** join a pairwise-coprimality budget — at either arity, since the fusion image is a quotient of the layer rather than a subgroup.
...
*Why.* The translations act on 3-subsets with trivial setwise stabiliser — a nonzero translation fixing a 3-set would make it a union of cosets of a nontrivial additive subgroup, and 𝔽_c has none — contributing a factor c. The stabiliser of a 3-set inside the twist ⟨ζ⟩ is cyclic of order m, acting freely on the nonzero elements and fixing 0; so the 3-set is a union of free m-orbits plus possibly 0, giving m | 3 (if 0 ∉ S) or m | 2 (if 0 ∈ S) — hence m ≤ 3 and m | d, and the largest available is the largest such m dividing d. Orbit size is c·d/m, minimised by taking m as large as possible. ∎ *(The step needing care is that a stabiliser of order m forces the 3-set to be a union of ⟨ζ^{d/m}⟩-orbits — at m = 3 a set {x, ζ′x, ζ′²x} with ζ′ of order 3, at m = 2 a set containing an antipodal pair.)*
...
>
> **Corollary (Oliver-constrained).** For a block with d > 1, the Galois part raises the minimum **iff p = 2, gcd(a, 6) = 1, gcd(d, 6) = 1, and a admits a split as above** — with q, the top prime, the least prime divisor of a/a′; the gain (the factor of the theorem, which needs m = a) is the least prime divisor of **a**.
>
> > **The gcd(a, 6) = 1 clause is not implied by the split and must be carried explicitly.** A split can exist at an a the theorem excludes: take **a = 10, d = 31**, where a′ = 2 gives a/a′ = 5 prime with 31 | 2⁵ − 1 and gcd(31, 2) = 1 — a perfectly good Oliver chain — yet gcd(10, 6) = 2, so the 𝔽₄ escape of the theorem supplies a Galois-stable minimal orbit and there is **no gain at all**. Any predicate testing only "p = 2, gcd(d, 6) = 1, split exists" therefore over-credits, which by §5.8's trap is the *safe* direction here but still wrong. `k3_galois.py` tests all four conditions.
>
...
>
> > **Two quantities that are not the same, and this is the next trap.** The theorem's **gain** is lpf(a); the split's **top prime** is q = lpf(a/a′). At a a prime power these coincide, which is why the two are easy to conflate without visible damage. At composite a they diverge, and since §4.3 couples the top prime to every foreign block in the configuration — each needing q | r − 1 — the divergence is usable: at a = 35 the twist d = 31 splits with a′ = 7 giving **q = 5**, while d = 127 splits with a′ = 5 giving **q = 7**, and the gain is lpf(35) = 5 either way. So **the twist choice selects the top prime at fixed block size**, a degree of freedom the naive predicate cannot express, since a = q^e determines q uniquely.
>
> > **Where the distinction actually bites, which is further out than it sounds.** The block sizes it adds are exactly the a coprime to 6 that are not prime powers: **a ∈ {35, 55, 65, …}**, all of which have lpf(a) = 5, so the added blocks are low-gain. But the block size determines n — the shape is n = c = 2^a — so the distinction is not diluted across many n: at **n = 2³⁵** the naive predicate credits gain 1 where the truth is 5, and the bound is a factor of 5 too small, hence not an upper bound. The smallest affected n is therefore 2³⁵ ≈ 3.4 × 10¹⁰, far beyond any table, which is why this is a theorem-statement issue rather than a computational one. The predicate itself is `k3_galois.py`, registered among the companion files of `pending-checks.md`.
...
> **Consequence for the GL-versus-ΓL reading — and J0a is a different question, which this note has elsewhere conflated with it.** Two distinct issues share the neighbourhood:
>
> - **Is the Galois part worth adding?** GL(1, c) against ΓL(1, c) — whether a block should be scored at orb(c, d, 1) or orb(c, d, a). That is what §2.2.2 answers, and the answer flips between arities: inert at k = 2, worth lpf(a) at k = 3 under the stated conditions.
> - **J0a proper, which is whether the stabiliser lies in ΓL(1, c) *at all*.** `enumeration-proof.md`'s Part B has a false step assuming a semilinear stabiliser, and its own extraspecial example — 3^{1+2} ≤ GL(3,7) — is a stabiliser **outside** ΓL(1, c) whose realised minimum exceeds orb(c, d) by a factor 9, **at k = 2**. So J0a is not dormant at k = 2 and never was; what is dormant there is the Galois question.
>
> The first issue is the one this section settles. A k = 3 programme inherits it in a sharp form — the semilinear reading is worth lpf(a) exactly on blocks with p = 2, gcd(a, 6) = 1 and gcd(d, 6) = 1, subject to §2.2.2's Oliver-constrained layer split — and, since larger orbits are *better* for us, that is an opportunity rather than a hazard, but it means the shape space must be indexed by ΓL twists rather than GL twists. It inherits J0a proper too, unchanged and unresolved, exactly as k = 2 has it. *A trap on the way: it is tempting to conclude from a handful of prime-power examples that min₃ is insensitive to the Galois part — that conclusion is false, and the examples suggesting it (c = 8, 9, 16, 25, 27, 49) all satisfy one of the escape clauses above, so a sample chosen for convenience will reliably mislead.*
...
> **At k = 2 the third argument is inert**, which is why the k = 2 documents never needed it: `enumeration-proof.md` J0a shows orb₂(c, d, m) = orb₂(c, d, 1) for every m. The same three-factor reading applies there — τ = 2 in characteristic 2 (the translation x ↦ x + 1 swaps a pair, which is `orb`'s `char2` flag), θ = 2 when 2 | d, and γ ≡ m always. **So the k = 2 formula is this one with the Galois factor forced to be trivial**, and the general-k statement is that τ contributes the p-part of k (capped by p^a), θ contributes the largest j | d dividing k or k − 1, and γ is inert exactly when p ≥ k. (The "or k − 1" carries the 0-containing sets — at k = 3 that is exactly the antipodal-pair-plus-origin case — and is where a naive j ≤ k reading first fails, at k = 5; `general-k-note.md` §2.2.)
...
> **In fact the degree count can be replaced by a proof, and it is three lines.** Take a cross orbit of the 2 + 1 type: two points in a class of block size c and one in another part of size s′. Its orbit contains, for each pair in the two-point block's own pair-orbit, at least s′ ≥ 2 choices of the third point, so it has size at least 2·orb₂(c, d) ≥ c·d/2 ≥ c·d/κ₃ whenever κ₃ ≥ 2 — and when κ₃ = 1 the intra term is c·d·m, while the cross orbit is at least 2·orb₂ ≥ c·d, so the comparison needs only s′ ≥ m, which holds since s′ ≥ r ≥ 5 > a for every block in range. The 1 + 1 + 1 type is larger still, being a product of three part sizes. For a cross orbit **within** one class the F blocks are permuted transitively, which multiplies by a further F. **So the minimum is an intra term at every configuration, with no appeal to asymptotics** — the degree count says the margin is a factor of order n, and this says the sign of the comparison never turns over at small n either.

*Caveat on the census, not on the binding.* The statement above is about which term binds within a configuration; that the census of §4.2 lists every admissible configuration is a separate claim, and is the measured one (§10 item 1).
...
| **S7** | middle-layer-fused matching + outside, **any F** | exists | orb₃(r, t, 1) when the foreign twist is small | measured 0.0013 at n = 33 | Same shape; a small foreign twist is punished harder, orb₃ being linear in d with no C(c,2) cap to rescue it. **The F-split survives the arity.** At k = 2, F = 2 carries 10/24 and F = 4 carries 2/24 — F = 4 setting the ceiling at n ≡ 11 (mod 12) alone, at both arities (§5.7). The κ_c = 3 tie at class 11 stands. `general-k-note.md` §4 extends this to every k ≥ 3 |
...
**What it does *not* consume** is the *term-type* comparison of `aod` §3.2 — which of intra, within-class cross and between-orbit binds — since at k = 3 the answer is always "the intra term of the weakest part". **Everything else survives**: there are still ceilings by residue class (§5.7), still the mod-12 keying of those ceilings with §5.6.3's mod-24 refinement inside the cut-taking regime, and the balance points are literally k = 2's. What looked at first like the disappearance of the whole optimisation is the disappearance of one of its two ingredients.
...
> **What that competition is worth, at both arities.** The S4 / S5 / S7 comparison determines which shape realises the *three-part family*. The two-part S7 at F = 4 holds the ceiling at n ≡ 11 (mod 12) only; on the other odd classes the winner is S7 at F = 2 at full twist — a shape inside this comparison. So read §5.6.4 as settling the internal order of the three-part shapes at class 11, and as identifying the class winner elsewhere.
...
The other congruence effect concerns the **odd part** of c − 1. **A cyclic-layer fusion does not force the twist down to it** — z^F is the full twist, at either arity (`aod` §3.2.3) — so the table below is the *price of the cut where a configuration takes it*, not a price of fusion; the price of fusion itself is Lemma C's foreign coupling. Writing 2^v ‖ c − 1:
...
**and mod 24 refines it *for a block that has taken the cut***, because v is not determined by c mod 12: c ≡ 11 and 23 (mod 24) both have v = 1 and are the good class; c ≡ 5 (mod 24) has v = 2 while c ≡ 17 has v = 4, so their fusion costs differ by a factor of four.

> **The contrast with k = 2 is a narrowing — inside the cut-taking regime only.** Since no fusion forces the cut at either arity, this comparison governs configurations that *take* it; the per-block congruence content that applies unconditionally at k = 3 is §5.6.1's mod-3 penalty. Within the cut-taking regime: at k = 2 the good class is **c ≡ 3 (mod 4)**, every one of c ≡ 3, 7, 11 (mod 12) equally good since only v matters, while at k = 3 the class c ≡ 11 (mod 12) is strictly better than c ≡ 7 because the mod-3 penalty separates them. So the governing modulus rises from 4 to 12, with mod 24 pinning v. **Gotcha: none of this keys the *ceiling* table, at either arity.** That is mod 12 (`aod` §3.3.4), and a mod-24 refinement appearing there is a sign the cut has been assumed forced.
...
- **S7** (swap in the *cyclic* layer): the layer keeps the full twist, subject only to Lemma C's strip against the foreign prime — and **q is free**, so the foreign block may choose the top prime that maximises its own η₃. S7 pays nothing on the matching side.
...
**The matching-side ratio is 1 : 1 at every c and both arities**, neither shape paying a twist cut, so S7's free q decides outright wherever Lemma C does not bite. Inside the cut-taking regime the trade has the same shape at both arities and differs in magnitude: at k = 2 the ratio S5 : S7 is 2^{v−1}, so at c ≡ 3 (mod 4) it is 1 and S7 wins on the free q; at k = 3 it is 2^{v−1} or **2^v**, so:
...
> **S7 wins its side everywhere**, so the narrowing of the c ≡ 3 (mod 4) escape to c ≡ 11 (mod 12) applies only inside the cut-taking regime (a third of odd blocks giving S7 its free-q win at k = 2, a quarter at k = 3, S5's advantage one factor of two larger on the rest). S5's remaining case is the one where the matching twist shares a prime with the foreign block, so that Lemma C strips S7's layer while S5's top-layer swap leaves the twist alone. Whether S5 converts turns on finding r with a large 2-part of r − 1 — the Fermat-like supply of `aod` §4.3.
...
| n mod 12 | shape | η | **s\*/n**, κ_c = 2 | s\*/n, κ_c = 3 | **β₃** (κ_c = 2) | β₃ (κ_c = 3) | k = 2 cap (δ) |
|---|---|---|---|---|---|---|---|
| 0, 4, 6, 10 | S3, two parts | 1 | 0.50000 | 0.55051 | **0.12500** | 0.10102 | 0.25000 |
| 2, 8 | S3, two parts | 1/3 | 0.36603 | 0.41421 | **0.06699** | 0.05719 | 0.13397 |
| 1, 9 | S7 at F = 2 | 1 | 0.58579 | 0.63397 | **0.08579** | 0.06699 | 0.17157 |
| 3, 7 | S7 at F = 2, entangled | 1/2 | 0.50000 | 0.55051 | **1/16 = 0.06250** | (5 − 2√6)/2 = 0.05051 | 0.12500 |
| 5 | S7 at F = 2 | 1/3 | 0.44949 | 0.50000 | **0.05051** | 0.04167 | 0.10102 |
...
>
> **What does move it is the mod-3 penalty**, and it moves it *away* from the middle rather than towards it: a κ_c = 3 matching block is less efficient, so by §5.2 it needs a **larger** share — 0.55051 against 0.50000, 0.63397 against 0.58579. The one row where it lands exactly on 1/2 is class 5.
>
...
**The F = 4 row resolves what would otherwise be a provisional reading of the odd rows.** At k = 2 those rows turn on a tie between the fused and unfused three-part rungs — cap_B(1/4) = cap_C(1/2) identically, a coincidence at η = 1/2 and nowhere else — and on which c mod 8 the argmax sat, with §5.6.4's S5 : S7 matching ratio able to disturb it. At class 11 the ceiling is set by the two-part F = 4 shape, outside the tie, at both arities. On the class {3, 7} the winner is S7 at F = 2 at full twist, settled at k = 2 by an independent scan of real configurations per residue class; the k = 3 cells inherit the same (F, η) with η₃ = η/2, giving β₃ = 1/16 at κ_c = 2.
...
> **Caveat, and it is a real difference from k = 2 — now half-closed.** At k = 2 this kind of statement is backed by `mu_enumerate_v3.py`, which enumerates the whole shape space, plus `brute.py` as an independent check. **`mu3_menu.py` (2026-09) is the counterpart, and it goes exactly half the distance**: it scores *every shape in §4.2's census* at every n in a range and writes `b3_census`, replacing "best in the family searched by hand at one n" with "best over the census, systematically". What it does **not** do is close §10 item 1, because the census is not known to be complete at k = 3 — §4 re-analyses the k = 2 shape list rather than deriving one. So **B₃^census ≥ μ₃ requires the census to be complete, which is open**, and the column is named `b3_census` and never `mu3`.
>
> **Which way the inequality runs, since it is easy to state the wrong half.** Two directions with two different hypotheses:
>
> > **μ₃ ≥ B₃^census** given that every census shape is **realisable** by an Oliver group (the Part E analogue, which at k = 3 has no counterpart to the GAP battery);
> > **μ₃ ≤ B₃^census** given that the census is **complete**, which is §10 item 1 and open.
>
> So the useful reading is the first: `b3_census` is a **lower** bound on μ₃ modulo realisability, and the open completeness question is what stops it being an upper one. Neither hypothesis is established at k = 3, which is why the column is named for what it is.
>
> *What is sound in it is the scoring.* The orbit law is proved (modulo one routine stabiliser step) and verified on 104 triples; and §4.1's "only intra terms bind" is proved in three lines by the degree count, so a configuration's score is min over classes of F·orb₃ with **no term-type comparison at all** — which is why the enumerator is short where the k = 2 one is long. *Independently reproduced on writing it:* every value in §6.2's table (orb₃(32,31,5) = 4960, orb₃(32,31,1) = 992, orb₃(101,25,1) = 2525, and all seven r-rows), and the full-density degrees of §3.1 — 5, 8 and 32 attain C(c,3) while 16 does not, Kantor's classification showing up in the arithmetic rather than being assumed by it.
>
> > **One defect found in the first run, and it is §4.3's own warning made concrete.** The script credited a Galois part of order m without requiring m to be a power of the top prime q — but the Frobenius sits in the **top layer**, which is a q-group, so it must be. At **295 of 306** rows crediting m > 1, the top prime was something other than 5 while a 32-block was credited m = 5. Fixed; **266 rows fell, all downward**, and n = 133 did not move, because there q = 5 = m is exactly the coincidence §6.2 is about. *That the error was an over-credit rather than an under-credit is the dangerous direction here — §5.8 records that a k = 3 scoring which under-credits the Galois part is not a loose upper bound but not an upper bound at all, and the mirror of that is that an over-credit is not a loose lower bound either.*
...
| **`aod` §3** (ceilings) | **partly transfers.** The *term-type* comparison does not (§4.1) and neither do the δ constants (§3). The ceilings by residue class (keyed mod 12), §5.6.3's mod-24 refinement of the cut cost, and the balance points all do — recomputed as §§5.6–5.7, with β₃ = m\*₃/n² replacing δ |
...
3. **The Galois part is invisible at k = 2, and not for a deep reason — and J0a is dormant at k = 2 as well, though for a different reason again.** §2.2.2 shows the minimum on 2-sets is unchanged by *adding* the Galois part at every block, because the fixed field 𝔽_p always contains a 2-subset; at k = 3 that fails exactly in characteristic 2, and the question acquires teeth. So if the k = 2 shape space were ever indexed by ΓL rather than GL twists, nothing would change, which is worth knowing. **J0a proper is a different and still-live question at k = 2**: it asks whether a block's stabiliser lies in ΓL(1, c) at all, and Part B's own 3^{1+2} ≤ GL(3,7) example shows a stabiliser outside it beating orb(c, d) by a factor 9 at k = 2. The two were conflated in earlier drafts of this note; the k = 3 finding is about the first, and leaves the second exactly where the k = 2 documents have it.
...
5. The ceiling table's odd rows are now settled (§5.7). Class 11 is set by the two-part F = 4 shape at both arities, outside the three-part tie entirely; the class {3, 7} is S7 at F = 2, η = 1/2, β₃ = 1/16 in the κ_c = 2 column, inherited from the settled mod-12 k = 2 keying (`aod` §3.3.4).
   **The table is now measured rather than only derived** — `ceiling_rederive3.py` (2026-09) scans real configurations under a generic-family filter and takes the per-(class, κ_c) sup, the k = 3 counterpart of `ceiling_rederive.py`. Over n ∈ [7000, 14000]: **no cell is exceeded**, and **every one of the twelve classes attains its tabulated ceiling to within 0.1% in exactly one κ_c column** (classes 2 and 8 in both). Nine of the twenty-four cells fall short, and they are precisely the *other* column at each class — e.g. class 0 reaches 0.10100 against 0.10102 at κ_c = 3 but only 0.06697 against 0.12500 at κ_c = 2, which is the κ_c = 2 sup landing on the **η = 1/3 value** rather than the η = 1 one.
   **So the open question sharpens.** What remains is not "is the table right" but **whether each class's unreachable column is arithmetically obstructed or merely thin in supply** — at class 0, κ_c = 2 the η = 1 entry needs r − 1 = 2·q^e with q ≥ 5, which is a supply condition of the same family as the safe primes, not obviously an obstruction. Distinguishing the two cases is what a k = 3 analogue of `aod` §3.3.8's escape analysis would do, and it is the concrete form of "whether the arity-agnostic transfer has a k = 3 subtlety not visible at k = 2".
...
7. **k ≥ 4.** The orbit law generalises — with the twist factor being the largest j | d dividing k or k − 1, not the naive max{m ≤ k : m | d}, which fails from k = 5 (`general-k-note.md` §2.2) — so the same analysis runs; the threshold degrades to Θ(n²) against C(n,k) throughout, and the shape ranking presumably inverts further.

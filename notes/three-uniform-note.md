...
**The group theory.** Parts B, B′, D and D2 of `enumeration-proof.md`, together with Part 0, are statements about *which groups exist* and about their block structure. None mentions pairs, and they transfer unchanged. *(Parts A and C do not: A's term types are a partition of k and C's counting bound is pair-specific — see §8, which is the authority on the part-by-part status.)* So does the shape space: chain primes (p, q), orbits, matching and foreign blocks, fusion counts F = F_mid·F_top, and the cyclic-layer budget in its corrected form — Lemma C's twist–foreign strip and the no-duplicate-foreign rule; F_mid itself does **not** join a pairwise-coprimality budget — at either arity, since the fusion image is a quotient of the layer rather than a subgroup.

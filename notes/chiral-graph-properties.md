...
| §6 what is open | a to-do list; **n = 5 and n = 9 are now closed**, so the frontier is n = 13 |
...
> **And it predicts where the interesting examples live — but n = 5 is settled, from another direction.** n = 5 is the smallest prime power where the factor is paid, and it is where the chiral analysis found a candidate: the down-closure of one A₅-orbit of Hamiltonian cycles is ℚ-acyclic with H̃₁ = ℤ/2, homotopy equivalent to the real projective plane ℝP² (see `pending-checks.md` R10).
>
> **That candidate cannot be completed, and the proof is already computed elsewhere.** A chiral graph property on 5 vertices is exactly a monotone property of the 10 pairs invariant under A₅ acting on pairs — which is **T(10,7)**, the first group at which `monotone-transitive-note.md` §3's criterion fails, and whose invariant monotone properties that note enumerates exhaustively: **3,176 of them, of which 112 have χ = 1 and 0 are non-evasive**. Since non-evasive ⇒ ℤ-acyclic ⇒ χ = 1, that is a complete search. **So every nontrivial monotone chiral graph property on 5 vertices is evasive**, ℝP² notwithstanding — ℚ-acyclicity with H̃₁ = ℤ/2 is not ℤ-acyclicity, and the exhaustive search is what confirms nothing else at n = 5 does better. The two notes reach this by different routes and should be read together: §4 here says *why the natural group falls short at n ≡ 1 (mod 4)*, and that note says *what happens at the resulting group*.
>
> **So the chiral frontier is n = 13**, the next prime ≡ 1 (mod 4). The remaining members are the primes **13, 17, 29** and the prime powers **25, 169** — **not n = 9**, which the box above settles, and **not n = 5**, which the search above settles. **The n where the natural group falls one orbital short are the chiral world's analogue of the non-prime-power n in the S_n world**, and for the same structural reason. (§5: this is where the analogy stops — unlike the non-prime-power case, the shortfall does not propagate into the composite analysis, because a *fused* class recovers the full twist.)
...
4. **The chiral floor conjecture.** δ ≥ 1/25 becomes δ_chi ≥ 1/50 by Theorem 1, but §5 suggests the truth is δ_chi ≥ 1/25 unchanged; the ladder of `ladder_verify.py` would need the same sign check per family to confirm it, and by (F2) the fused rungs should pass unaltered. *One thing transfers for free: the collapse theorem of `enumeration-proof.md` Part E‴ is a statement about SAFE scores, which do not see parity, and μ_chi ≤ B_safe holds since subgroups of A_n are Oliver groups. So wherever the chiral ladder clears 1/25, μ_chi(n) = B_safe(n) = μ(n) — the halving of Theorem 1 is then not paid at all — and the only place the two can differ above the line is where the chiral constructions fall short of B_safe, i.e. an unfused c ≡ 1 (mod 4) block at even n (§5), which is a supply question rather than a structural one.*
5. **R10** (`pending-checks.md`) is answered — no chiral half of the Hamiltonian-cycle complex is ℤ-acyclic at any n ≡ 1 (mod 4) — and **n = 9 is closed twice over**: §4's box gives a 2-transitive Oliver group inside A₉, so every nontrivial monotone chiral property there is evasive. **n = 5 is now closed as well, and completely**: chiral properties there are the monotone A₅-invariant families on the 10 pairs, and `monotone-transitive-note.md` §6 enumerates all 3,176 of them with **none non-evasive**, which subsumes R10's Hamiltonian-cycle case and every other candidate at that n. Where a chiral candidate could still live is the n at which the factor of two is paid and no exhaustive search has been run: the primes **13, 17, 29** and c = **25**, **169**.

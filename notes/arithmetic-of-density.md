...
> ⟦PENDING-1E5-EXACT-RUN⟧ **Status of the computed range: R0/R1 complete, and the frontier has moved to 36,848.** `mu_exact.py` has replaced the enumerator's output with a **fully contiguous** table over every eligible n — composite, non-prime-power — in **[6, 36848]**, all **32,860** of them, with no gaps and **no worklist-driven tail at all**, agreeing with `mu_enumerate_v3.py` at every common value. The R1 battery passes on it (`validate_table_v3.py` 26 PASS / 0 FAIL; `converse_check.py` 0 violations; the S2 identity clear at every row). Further extension is **discretionary**, not owed. *The prefix/tail discipline is dormant rather than retired:* it applies again the moment an extension fills higher n from a worklist, since the worklist selects by low score, and aggregates would then have to be requoted over the contiguous part only. **Measured figures below that name [6, 2600] are scoped to the older frontier and remain correct as scoped statements; they have not all been recomputed at the new one.**
>
...
>
> **⟦PENDING-1E5-EXACT-RUN⟧ — the one tag that is about *this* table rather than a script.** `mu_exact.py` is running on to **n = 100,000**. Figures marked with it are measurements taken over a **narrower** frontier — most of them the older contiguous range [6, 2600], a few the 36,848 one — and every one of them will move when that run lands: winner counts and shares, per-shape medians, the census columns, the low-density tail, and any "over the table" aggregate. **They are not wrong and they are not provisional**, in the way a ⟦PENDING-REBUILD⟧ figure was: each is correct as a scoped statement about the range it names, which is why each names it. The tag marks that the *scope* is stale, not the number. Structural claims, closed forms, ceilings and every theorem in these documents are unaffected — they quantify over all n and no table extension can touch them. **On completion:** rerun `check_doc_figures.py --pass pending`, which lists every tagged site with the quantity recomputed at the new frontier, requote each in place, and delete the tag. *Do not retire the tag piecemeal:* a half-requoted document is worse than a fully stale one, because the range words stop being a reliable guide to which population a figure came from.
>
...
> ⟦PENDING-1E5-EXACT-RUN⟧ *Verified.* Over all **743** one-part winners in the contiguous range, the predicted density 1/F agrees with the computed value to O(1/n), with no exceptions — indeed |δ − 1/F|·n < 0.95 on every row, and every row matches the exact form (c−1)/(Fc−1). By F: 227 rows at F = 2 with median density 0.4996, 167 at F = 3 with median 0.3327, 130 at F = 4 with 0.2493, 106 at F = 5 with 0.1993, **60** at F = 7 with 0.1421. Maxima 0.49981, 0.33308, 0.24971, 0.19969, 0.14252 against 1/F = 0.5, 0.3333, 0.25, 0.2, 0.1429. (The remaining **53** sit at F = 6, 8, 9, 11, 13, 17 — F = 10 and F = 19 no longer occur.)

> ⟦PENDING-1E5-EXACT-RUN⟧ *Verified, with one correction the corrected shape space forces.* Of the 743 one-part winners over the contiguous range to n = 2,600 (2,186 rows), **738 have ω(n) = 2 and five do not** — n = 78, 222, 282, 894 and 1434, all of shape `6x p`. **The exceptions are exactly the composite-F fusions**: F = 6 is available only because F_mid need not be a prime power, and n = F·c then carries three prime factors. Under a shape space with F a q-power the statement "every one-part winner has ω(n) = 2" is a theorem — n = F·c with c a p-power has ω(n) ≤ 2 — so this is a place where the correction changes a structural claim and not only a count. Of the 1,210 values with ω(n) = 2, **738** are one-part winners and the other **472** do better with a split.
...
> ⟦PENDING-1E5-EXACT-RUN⟧ *Verified, and tight.* Maximum observed density among two-part winners is **0.24939** and **no two-part winner exceeds 1/4**, over all 1,443 of them in the contiguous range; median 0.1682. The three-part half of this check has no population left to run on — the family is empty on the completed table (`enumeration-proof.md`'s census) — so the 1/9 bound for three parts is now a statement about a shape that exists and never wins, not a measured margin.
...
> ⟦PENDING-1E5-EXACT-RUN⟧ **Recounted over the completed table**, all 395 cyclic-layer winners over the contiguous range [6, 2600]. The c mod 8 distribution is **{1: 92, 3: 100, 5: 101, 7: 96}** — flat to within 5%, and **49.6% of the odd-c rows sit at c ≡ 1 (mod 4)**. The prediction was that the congruence would disappear rather than merely weaken, and at four residues within a few percent of each other over 395 winners it has: what remains is sampling noise, not a residual preference.
...
> ⟦PENDING-1E5-EXACT-RUN⟧ **What bounds an individual row is cap_F(η), not its class ceiling.** Against the completed table over [6, 2600], **185 of the 1,443 one-foreign winners exceed their own class's tabulated δ₀**. In class 11 the *median* winner sits at 1.14× the tabulated cap and **34 of 60** exceed it, with a maximum of 2.81×. So δ₀ is a guarantee for one specific balanced shape and not a description of typical behaviour; "generic ceiling" is the wrong reading of it. What *is* an upper bound, and holds without exception, is cap_F(η) evaluated at the configuration's own F and η: **0 of 1,443 rows exceed it**, and over the **590** fused-class-plus-foreign winners the maximum observed is **0.17131 at n = 2025** (`2x593 + 1x839*`) — **99.8% of cap_2(1) = 3 − 2√2**, so that ceiling is not merely an upper bound but a nearly attained one.
...
⟦PENDING-1E5-EXACT-RUN⟧ **The prediction shows up in the table, on both of its halves.** Two forces act on the observed floor in opposite directions, and separating them is the point. Extending the range lowers it, since a wider range can only find weaker values. Adding configurations raises it, since B(n) is a maximum — so a shape-space correction lifts the whole curve at once. Over the contiguous range [6, 2600] the floor is **0.048039 at n = 2183** (v4's 0.045742 at n = 1817 rises to ≥ 0.0594 under the entangled-generator correction; 2183 is not among the 289 raised rows, so its value carried through the rebuild unchanged and is now confirmed as the minimum). So the *within-table* floor moved up when the space was corrected, while the mechanism below — which is about which n the engines reach, not about how well they are scored — is unaffected. The thirds of the range behave as the argument requires:

| n | ω(n) = 2 share | median smallest cofactor F | min density |
|---|---|---|---|
| [6, 800) | 64.9% | 4 | 0.05703 |
| [800, 1500) | 53.6% | 5 | 0.06065 |
| [1500, 2600] | 50.1% | **7** | **0.04804** |

Two effects, not one. The ω(n) = 2 population thins, as predicted; and **among the values that remain, the smallest prime-power cofactor grows**, so the 1/F the multiplicative engine delivers shrinks even where the engine applies. n = 2183 = 37·59 illustrates the mechanism: ω(n) = 2, so a fused class exists, but only at F = 37, worth 1/37 ≈ 0.027 — which loses to `6x251 + 1x677*` at 0.048039. (Under the v2 shape space the winner there was the three-class 1297\* + 443 + 443 at 0.041107; the cyclic-layer block count is what lifted it.) **⟦PENDING-REBUILD⟧** n = 2183 is the table floor itself: v4's floor row n = 1817 rises to ≥ 0.0594 (`3x256 + 1049*` at full twist), while 2183 is not among the raised rows. 2183 is n ≡ 11 (mod 12), the doubly-obstructed class — so the floor tentatively returns to that class, and the 1817 exception becomes historical pending the rebuild.

Two consequences, and both should temper how the computed range is read.

⟦PENDING-1E5-EXACT-RUN⟧ **The observed density floor should drift downward.** Fully 55.4% of the current table has ω(n) = 2, so more than half the computed values are served by an engine whose reach halves over the next few decades of n. The median of 0.1994 is propped up by a population that thins. The evidence is two-sided and worth keeping apart: *within* the exactly-computed range each extension lowers the floor, which is the prediction, while shape-space repairs keep lifting the contiguous range's floor — to 0.045742 at n = 1817 under v4, and to 0.048039 at n = 2183 under the entangled-generator correction — corrections to the scoring and not counter-observations. Beyond the table the ladder of §5 stays above the exactly-computed floor's own value in the table's range (ladder 0.04621 at n = 2759 against the table's 0.048039 at n = 2183 — the ladder's argmin lies outside the computed range, which is why the two differ), which is the recovery showing through rather than a contradiction.
...
**What the branch-and-bound has left to do.** The worklist holds **44,091** entries — every n whose family score falls below the asymptotic ceiling 7 − 4√3 — but **none below 1/25**, so it has nothing left to eliminate against the conjecture as stated, and the adaptive run against a floor of 0.04 returns an **empty** below-floor list without computing a single B(n). Its remaining use is sharpening: the operative question is **"does B(2759) exceed 0.04621"**. *The low tail is short: exactly **two** entries fall below 0.05 — 2759 and 2183 — and both are already in the μ table with B equal to the ladder's own score, so there is no third value anywhere below 10⁶ where an exact B would move the floor. The next entries up are 0.05051 (n = 2639) and 0.05516 (n = 3239).* B is already known at the lowest of them: **B(2759) = 175,813 equals the ladder's score there**, which is what fixes the minimum below 10⁶ exactly; computing B at the other nine would raise the verified floor above 1/25 with real margin.
...
**The binding value is 0.04621 at n = 2759, in the [10³, 10⁴) decade, and the descent is now visibly one decade wide.** Every other decade clears **0.057**, and the minima rise monotonically on either side of the binding one — 0.05703 below it, 0.05829 and 0.06391 above. So the middle range where neither the computation nor the asymptotics reaches is a single decade, its worst value clears 1/25 by 15.5%, and that value is not merely a bound but an exact B. *(The two upper decade minima moved substantially on the corrected scoring — [10⁴, 10⁵) from 0.04801 to 0.05829 — which is what turned "one decade wide" from a reading of understated values into a statement about δ.)*
...
**Measured.** Scoring every unequal odd-p configuration at full diagonal twist: **none wins** — confirmed directly against the current table, where **0 of 2,186 winners have matching classes of two different block sizes, at any p**. (How close they come is measured in `shape-counting.md` §5.) Together with the earlier observation that no winner in the table has two matching classes of different sizes, that is the empirical side; the ceiling above is the structural side, and it says why.
...
⟦PENDING-1E5-EXACT-RUN⟧ **Hence the fusion shapes drop from the asymptotic statement, and the reason is a bounded cofactor rather than a bounded ω(n).** In the single-class case the argument is now one line from §2.1's identity: δ_S2(n) = (Q(n) − 1)/(n − 1), so δ_S2 ≥ δ₀ holds exactly when Q(n) ≥ δ₀·n — that is, when n lies in the finite union ⋃_{F ≤ 1/δ₀} F·{prime powers}, a **density-zero** set. The general fused shape is no better off: any F_i > 1 needs F_i equal blocks of prime-power size, so n again carries a prime-power divisor at a cofactor the feasibility criterion bounds. Fused winners — winners with any F > 1 — are 61.0% of the contiguous range, of which 27.0% pair a fused class with a foreign prime, propped up by small n, and the share recedes with the prime powers rather than with ω(n). This is what licenses the N_add count of §6.4.
...
3. ⟦PENDING-1E5-EXACT-RUN⟧ **Predict the 1/12 shortfall from the singular series.** The odd/even split is **6.7% of odd and 0.08% of even** values below 1/12, over the contiguous range. Both engines' availability is computable heuristically, so this compares the whole framework of this document against measurement rather than testing any single family.
...
6. ⟦PENDING-1E5-EXACT-RUN⟧ **The fused family at ω(n) = 2 but bad splitting.** **472 of the 1,210** values with ω(n) = 2 do better with a split than with fusion, which happens when the smallest prime-power cofactor F is large. The distribution of F over ω(n) = 2 integers is classical, so predicting the **738/472** division is a clean test. (Both figures must come from the same slice: 738 + 472 = 1,210 over the contiguous range [6, 2600]. A pair that does not sum to the ω(n) = 2 count is a slice mismatch, and a stale pair can still sum correctly against an older range and so look right in isolation. Note the one-part winner count 743 is **not** 738: five one-part winners have ω(n) = 3.)

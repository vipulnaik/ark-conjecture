...
> **One computed instance of each live shape**, from the current table (**⟦REQUOTE-ON-EXTENSION⟧** — first instances are small-n and in practice stable), so that every row below can be checked against a concrete configuration. The starred part is the foreign block.
>
...
- what comes back is (BCP) *together with* a per-n additive clause — which is to say (BCG), by the identity of §6.8 — and not a single n-free statement; **(BCP) alone is strictly weaker**, and §6.8(iv) makes the gap precise: at a polylogarithmic ρ it reaches n^{2−ε} for almost all n on its own, and the residue is a bounded-cofactor condition on the **companion** n − r.
...
#### 6.8 Hypothesis (BCP): the multiplicative half as a standalone statement
...
> **The name, and what it buys.** **BCP = bounded-cofactor primes**, sharing its first two letters with **BCG = bounded-cofactor Goldbach** (§3.5) because it *is* that hypothesis with the additive clause deleted. The two names now display the relation the prose has always asserted:
>
> > **(BCG) = (BCP) + the additive clause, per n.**
>
> — clause 3's bounded cofactor d ≤ 12 is common to both and defines S_D below; clause 1's decomposition n = F·c + r is what (BCG) has and (BCP) does not. **(BCP) is therefore strictly weaker**, which §6.9 turns into a precise gap.
>
> *And (BCP) takes no -AL / -AA suffix, which is informative rather than an omission.* (BCG) needs one because it quantifies over n, and the uniformity trap of §3.5 lives in that quantifier; **(BCP) has no n in it**, so the trap cannot arise there. The absence of the suffix marks exactly where the difficulty sits — in the additive clause, not in the cofactor condition. *(The label was **(SP)** until 2026-09, for "shifted prime". S_D is still the shifted-prime set and the phrase is kept throughout, so the pointer to the Erdős–Pomerance–Shparlinski literature survives in words; what changed is that the label now carries the structural relationship instead of the provenance.)*
...
**(i) Bounded multiplicative gaps are the weakest case: ρ = 1/π(x).** "Every interval [x, Λx] contains a member of S_D" is exactly **(BCP_{D, 1/Λ, 1/π(x)})** — the window ratio is c = 1/Λ and the requirement is one member, not a proportion. Everything below is stated against this baseline.

**(ii) A density floor implies that baseline, with one constant in both roles.** If μ(n) ≥ δ₀·C(n,2) for all large n then **(BCP_{D, c, 1/π(x)}) holds with D = D(δ₀) = 2(1 − √δ₀)²/δ₀ and every c < δ₀/2**. Branch (b) supplies a member of S_D in [δ₀(n−1)/2, n] for almost all n; if the window [cx, x] held none, then every n whose whole r-range sits inside it fails (b), and those n fill [2cx/δ₀, x], an interval of positive proportion, which branch (a)'s O(x/log x) integers cannot cover.
...
**(iv) What (BCP) at that ρ would give, and where the obstruction actually sits.** With ρ(x) ≍ 1/log x, the route back is the exceptional-set argument of Shparlinski's Theorem 2, which converts a set of shifted primes with large Q into a statement about almost all n — supplying branch (b)'s additive clause. **Read against the proof, the density question answers *yes*, and the obstruction is somewhere else entirely.**
...
**(c) (BCP) as a statement about arbitrary sets is refuted; for the actual S_D it remains open. [ELEMENTARY, and the three-way status is the point.]** The note constructs, at exactly the (BCP) scale, a set of primes concentrated in one class mod `m` with `φ(m) ≍ log x` for which almost all `n` are unrepresentable. So:

| statement | before | after |
|---|---|---|
| (BCP) suffices, treating S_D as an arbitrary set of that cardinality | open | **refuted** |
| (BCP) suffices for the actual S_D | open | **open** |
| the obstruction is the companion exponent, not S_D's thinness | §6.8(iv) | unchanged, reinforced |

The middle row is the one to keep. The counterexample's structure is **unconditionally absent** from S_D — Selberg/Brun–Titchmarsh upper bounds for the pair `{Q, dQ+1}` in a residue class cap its concentration well below the (BCP) count — so what is refuted is the *generic* implication, and the route that survives is transference against the sieve majorant (`pending-checks.md` A25). **Reading (c) as "we now know (BCP) does not suffice" would overstate it in precisely the direction the note is careful about.**

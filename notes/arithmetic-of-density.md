...
**It also forces an arithmetic statement, and that one is short.** The shape-counting above is about *which* systems; this is about what any of them must supply. It is `enumeration-proof.md` Proposition F.4, and it is the precise sense in which the reduction runs both ways.
...
The mechanism is that the score is a **minimum** over orbitals, so every part must clear δ₀·C(n,2) on its own; a part does that only by being large *and* carrying a large twist, and both convert directly into arithmetic. For a foreign part, r·Q ≥ δ₀n²/2 with r ≤ n forces Q ≥ δ₀n/2, and Q | r − 1 < r ≤ n then bounds the cofactor.

*The "almost all" is load-bearing rather than a hedge.* The alternative is an all-matching configuration, and that branch constrains n itself: every class needs c_i ≥ δ₀n, which forces **n = M·p^b with M ≤ 1/δ₀**. Such n have density zero, so the exceptional set is genuinely negligible — but only asymptotically, and slowly: the density falls off like H_M/log n, and at n ≤ 10⁵ some **44%** of integers are still within a factor 25 of a prime power. On that branch the conclusion survives with r − 1 replaced by p^b − 1 **at the same bound 2/δ₀**, so *some* statement holds with no exceptional set; it is only the clean prime form that needs one. (Getting the same constant on both branches takes one piece of care: the class contributes F·c·d, a three-factor product, and F·c is bounded by n *jointly*. Bounding F and c separately spends δ₀ twice and gives 2/δ₀² — a real loss, not a real feature of the branch.)

> **One dependency to keep visible.** The conclusion is about a prime-**power** divisor, and it must be: with Q merely a divisor of r − 1 the statement is vacuous, since Q = r − 1 always works. What confines the foreign twist to a prime power is its living in the top q-group — a layer-assignment claim, and those have the worst record here (`pending-checks.md` T8). The Proposition has had one reading.
>
> **Measured against the table, and the constant that comes back is 12.** Across all 2,186 contiguous rows the three predicted inequalities hold without exception: (r − 1)/Q ≤ 2/δ at all **1,409** foreign primes in winning witnesses; r ≥ δ·n at every one; and M ≤ 1/δ at all **777** one-part winners. Two of the tightest cases are worth naming. The all-matching bound is essentially *attained* at n = 2594 = 2·1297, where M/(1/δ) = 0.9996 — the same row §6.1 finds sitting on the feasibility boundary, so two independent constraints bind at once there. And the largest cofactor (r − 1)/Q occurring anywhere in the table is **exactly 12**, at n = 221 with r = 157 and Q = 13. That is (H)'s own d ≤ 12, reached from the other direction and with no reference to it: the hypothesis's constant is not an artefact of how it was written down but the value the optima actually use. *The Proposition's bound is meanwhile loose by a factor ≈ 4 against that* — the global floor gives D = 44 where 12 suffices — which is where a sharpening would start. **⟦PENDING-REBUILD⟧** The counts are v4-era and `converse_check.py` requotes them; the v5 partial frontier reproduces the 12 at the same witness with no violations. Note the slack figure tracks the *floor*, so it is range-dependent on any correct table and a change in it is not a finding.

**So (H) and a density floor are equivalent up to constants.** (H) supplies r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350; the converse takes δ₀ back to a bounded cofactor, so the round trip returns **D ≤ 700** rather than d ≤ 12 — on both branches, the constant being uniform. Three differences survive it, and naming them is what keeps "equivalent" honest: the converse yields a prime-**power** divisor where (H) supplies a prime; it holds for **almost all** n where (H) is stated for all; and the cofactor loosens by a factor ≈ 58, which is 2/δ₀ against d = 12. **What does not degrade is the shape of the statement** — a shifted prime with a large prime-power divisor at *bounded cofactor*, rather than at exponent θ < 1. That is exactly the quantity §3.6's ladder measures, and exactly where Elliott–Halberstam stops short. So the framework is not merely *implied by* a shifted-prime hypothesis: at the θ = 1 end it **demands** one, which is the strongest available reason to think no substantially weaker arithmetic input can deliver a positive density floor.

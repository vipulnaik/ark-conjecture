...
>
> S6 has no instance in the current range. The pair to compare is **S5 at n = 459 against S7 at F = 2 at n = 99**: both are `2×c + r*`, and they are distinguished only by the top prime — q = 2 in the first, where the twist stays full and the Fermat prime 257 keeps the foreign block at η = 1, and q = 13 in the second, where the pre-correction scoring cut the twist to the odd part of 22 — the entangled generator keeps the twist full at any q, so the two shapes now differ only in foreign efficiency and S5's q = 2 reading is dominated at odd q; the census split is pending the rebuild.
...
| **S4** | two matching + outside, n = 2c + r\* | additive | **⟦PENDING-REBUILD⟧** 15 of the 16 are exceeded by entangled S7-at-F=2 readings (all but n = 1529); expect ~0 after the rebuild | **exists → 12/24** (all odd n); **wins → 0**. Within the three-part family it was, under the retired c mod 8 law, the argmax at residues 7, 15 and 23 mod 24 (`three-part-family-split.md`, now deprecated — with the full twist the fused rung beats it at every c), and those are exactly the residues whose class ceiling is attained by the two-part F = 4 shape, which beats the whole family there. **The widest existence/winning gap in the census**: available at essentially every odd n and beaten everywhere — by the fused rung where that rung is reachable, and by F = 4 where it is not | §3.2 |
...
| **253** | c = 73 ≡ 1 (8)<br>r = 107, r−1 = 2·53 | 2628 — binds on **intra**, C(73,2) | **5256** — binds on **intra**, 2·C(73,2) at the **full** twist 72 | 214 — binds on the **foreign** block at q = 2, orb(107, 2) | **5256 = min( 2·C(73,2), 73², orb(107,53) ) = min(5256, 5329, 5671)**, the inter term being 15622. This row is the correction's cleanest exhibit: c ≡ 1 (mod 8) with the fused class at its **full** twist, which the retired c mod 8 law forbade outright. It was 2783 under v4 — the largest single ratio in the exceedance list, 1.889 |
...
**Reading the losing entries.** The three readings share the same n, c and r and differ only in how the two c-blocks are treated, so the columns are directly comparable and the shortfalls are informative. At n = 273 the unfused reading loses by a factor 1.67 because it forgoes the doubling; the top-layer reading loses by a factor 53 because forcing q = 2 leaves the foreign block a twist of order 2 where q = 53 would give order 53. At n = 253 cyclic fusion is worth its full factor of 2 — 5256 against the unfused 2628 — *at c ≡ 1 (mod 8)*, which is exactly the configuration the retired law scored at 1314; the fused intra term now clears the foreign term 5671 rather than falling to half the unfused value. At n = 531 the top-layer reading is worth exactly twice the unfused one, 18632 against 9316, since the twist is untouched and nothing else binds.

> **A case the correction makes uninteresting, worth one line so it is not looked for.** At n = 247 (c = 73, r = 101, r − 1 = 4·25) the fused and unfused readings now score **identically** at 2525, because the foreign term orb(101, 25) binds below both intra terms; fusion is free but buys nothing. That n was this table's third instance before the correction, on the strength of a harm that does not occur. Its v3 winner is neither reading but a different shape, `4x41 + 1x83*` at 3280.
...
| **B** | two c-blocks fused in the **cyclic layer** + foreign | **S7 at F = 2** (no congruence on c; formerly stated as needing c ≡ 3 (mod 4)) | 2x² | 2η/(√2+2√η)² | √η/(√2+2√η) |
...
**Reachability imposes nothing beyond mod 12, and the mod-24 keying was an artifact.** Rung B — cyclic-layer fusion — was believed to need the twist on the c-blocks to be odd, i.e. c ≡ 3 (mod 4); the entangled construction refutes that (§3.2.3), so **the matching side imposes nothing**. The consequence for the keying is larger than a loosened condition:
...
*(Four is now literal: the twist clause has been deleted, leaving shape, balance, efficiency and coherence.)*

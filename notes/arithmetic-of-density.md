...
>
> **Two pending tags, and the distinction is the point.** ⟦**PENDING-LADDER-REBUILD**⟧ marks figures blocked on `ladder_verify.py` to 10⁶ — the unconditional floor, its argmin, and the range over which the four-family ladder certifies it. That run is the current bottleneck and nothing else. ⟦**PENDING-REBUILD**⟧ now marks only figures awaiting a **certificate or script rerun** that has not happened yet — coverage counts, collapse shares, shape-scan totals — plus the one distributional figure that cannot be regenerated from the CSV at all. Figures that were pending on the μ-table rebuild have been resolved against the completed table and carry no tag.
>
...
> *Verified* (`ladder_verify.py`), over every composite non-prime-power n in range — all residues, no eligibility filter — with **no value below 1/25**. **⟦PENDING-LADDER-REBUILD⟧** *The floor, its argmin, the scanned range and the margin over 1/25 are run outputs; requote them from the current scan rather than from here. Interim, corrected scoring: δ ≥ 0.0462 over n ≤ 10⁵, attained at n = 2759, margin 1.16 over 1/25.*
...
| global floor δ ≥ 1/25 (**⟦PENDING-LADDER-REBUILD⟧** exact value from the current scan) | ladder range | computed (§5), from the four-family ladder alone |
...
**⟦PENDING-LADDER-REBUILD — scoped⟧** *This table is the **worklist scan to 10⁶**, a different artifact from the μ table, and it has not been rerun. What the completed μ table settles is only its overlap: over [10³, 2600] the minimum is **0.048039 at n = 2183**, and the `0.04574 at 1817` entry does not survive the corrected cap, that row now scoring ≥ 0.0594. Whether 1817 remains the decade minimum out to 10⁴ is a question for the rerun, not for this table. The per-block floor table and the block-by-block recovery narrative are outputs of the superseded scan; regenerate them from the rerun before quoting any row.*
...
> **δ ≥ 0.04453, at n = 11183.** **⟦PENDING-LADDER-REBUILD⟧** *Pre-correction scan; R7 requotes it. The corrected-scoring run reaches n ≤ 10⁵ so far with floor **0.04621 at n = 2759**, so the figure is expected to rise — `ladder_verify.py`'s families understate, which is why the 1/25 conjecture is unaffected either way.*
...
> **Conjecture (global density floor).** For every composite non-prime-power n, **μ(n) ≥ C(n,2)/25**, i.e. δ(n) ≥ 0.04 — verified unconditionally over the four-family ladder's range (**⟦PENDING-LADDER-REBUILD⟧** — the exact floor, its argmin and the range are outputs of the current scan; interim, corrected scoring: δ ≥ 0.0462 over n ≤ 10⁵ at n = 2759); and asymptotically **δ(n) ≥ 7 − 4√3 − o(1) = 0.071797…**, the extremal residue being **n ≡ 11 (mod 12)**, the odd class carrying the ℓ = 3 obstruction.
<!-- /DUP -->
...
| 0.04453 (a representative ladder-floor value; **⟦PENDING-LADDER-REBUILD⟧**) | 4.739 | 4 | 22 | **122** |

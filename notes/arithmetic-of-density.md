...
**So (H) and a density floor are equivalent up to constants.** (H) supplies r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350; the converse takes δ₀ back to a bounded cofactor, so the round trip returns **D ≤ 700** rather than d ≤ 12, uniformly across both branches. Three differences survive it, and naming them is what keeps "equivalent" honest: the converse yields a prime-**power** divisor where (H) supplies a prime; it holds for **almost all** n where (H) is stated for all; and the cofactor loosens by a factor ≈ 58. **What does not degrade is the shape of the statement** — a shifted prime with a large prime-power divisor at *bounded cofactor*, rather than at exponent θ < 1. That is exactly the quantity §3.6's ladder measures, and exactly where Elliott–Halberstam stops short. So the framework is not merely *implied by* a shifted-prime hypothesis: at the θ = 1 end it **demands** one, which is the strongest available reason to think no substantially weaker arithmetic input can deliver a positive density floor.

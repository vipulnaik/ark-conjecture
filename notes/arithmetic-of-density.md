...
> **Status of the computed range: R0/R1 complete.** The table covers **every eligible n — composite, non-prime-power — in [6, 2600]**, all 2,186 of them, with no gaps and no worklist-driven tail, and the R1 battery passes on it (`validate_table_v3.py` 24 PASS / 0 FAIL; `converse_check.py` clean; the S2 identity clear at every row). Further extension is **discretionary**, not owed. *The prefix/tail discipline stays live as a method rather than as a caveat on these numbers:* a discretionary extension that fills higher n from a worklist re-creates a biased tail, since the worklist selects by low score, and aggregates would then have to be requoted over the contiguous part only.
>
...
>
> **The ladder run is also complete, so both numerical bottlenecks are cleared.** `ladder_verify.py` to 10⁶ under the corrected scoring: **44,091 worklist entries, floor δ ≥ 175813/3804661 = 0.046209898… attained at n = 2759, nothing anywhere below 1/25**, and the adaptive run against a floor of 0.04 returns an empty below-floor list — so the conjecture's finite half needs no μ(n) computation at all in that range. The ⟦PENDING-LADDER-REBUILD⟧ tags introduced to isolate that bottleneck have been discharged with it.
>

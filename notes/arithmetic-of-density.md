...
> **The multiplicative engine.** A single **fused class** — F blocks of size c with n = F·c and c a prime power — achieves density **1/F**. It requires n to have a prime-power divisor of small cofactor, which for the commonest case F = F_top a q-power means ω(n) ≤ 2; a composite F_mid relaxes that to ω(n) ≤ 3 at a handful of values. It is the only structure that breaks density 1/4.
>
...
> **Hypothesis (H).** Write (F, η) for the fusion count and efficiency that attain n's class ceiling, as tabulated by residue mod 12 in §3.3.5 — so **F = 1** at even n, **F = 2** at n ≡ 1, 3, 5, 7, 9 (mod 12), and **F = 4** at n ≡ 11 (mod 12) — and put **d = 2/η**. Then every sufficiently large n admits a prime q, a prime r and a prime power c with
>
...
> **Two attributions are tracked here and they come apart, which is the point of the last two columns.** The arithmetic inputs are not ours and are not new; what varies is *who noticed that this framework consumes them*. BBKN state the θ = 1/2 and ERH rungs; Shparlinski isolates the max-min as a named function and supplies the unconditional 1/4 rung and the almost-all 0.677 rung; **ours are the 0.679 update, the observation that the whole picture is one parameter θ, and the identification of (H) as its θ = 1 endpoint.** A reader should see at a glance that the ladder is a survey with two original entries rather than a contribution.
>
...
Hypothesis (H) of §3.5 is the **θ = 1 endpoint** of this ladder — more informative than calling it a barrier, since it places the hypothesis on a scale with a known current value rather than in a separate category.

**But "endpoint" is not "limit of the ladder", and the distance is larger than the θ column suggests.** E–H gives θ = 1 − ε for almost all n, for every fixed ε; (H) differs in two ways. The quantifier is one (every large n, not almost all). The substantive one is the *kind* of statement: every rung asks for a prime factor of r − 1 of size r^θ, so the family delivers n^{2−ε} and no member delivers a **constant fraction** of n² — a bound of that form is consistent with n²/log n. (H) instead asks for a prime factor of size (r − 1)/d with d ≤ 12, **linear in r**, which yields Ω(n²) with an explicit constant. A linear-sized prime factor is a Sophie Germain-type condition, independently twin-prime-hard, and strictly stronger than every rung rather than the limit of them. Pushing ε → 0 does not close this, so E–H is not "almost (H)". *§6.7 runs the implication backwards and shows the gap is not an artefact of how (H) is stated:* a density floor δ₀ **forces** a shifted prime with a prime-power divisor at bounded cofactor, so the bounded-cofactor condition is not merely sufficient for a floor but necessary for one, and no rung with θ < 1 can supply it.
...
**Hence the fusion shapes drop from the asymptotic statement.** A shape with any F_i > 1 needs F_i equal blocks of prime-power size, so n carries a prime-power divisor whose cofactor is bounded by the feasibility criterion; in the extreme single-class case n = F·c that is ω(n) ≤ 2 when F is a q-power and ω(n) ≤ 3 at the composite-F_mid fusions. Fused winners are 39.6% of the computed table, but that share is propped up by small n: the ω(n) ≤ 2 population thins like log log n / log n (§4), from 64.9% below 800 to 28.5% near 10⁶. So the fusion shapes cover a **density-zero** set of n, which is what licenses the N_add count of §6.4.
...
**So (H) and a density floor are equivalent up to constants — with the factorisation above as the fine print.** (H) supplies r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350; the converse takes δ₀ back to a bounded cofactor, so the round trip returns **D ≤ 700** rather than d ≤ 12. Four differences survive it, and naming them is what keeps "equivalent" honest: the converse yields a prime-**power** divisor where (H) supplies a prime; it holds for **almost all** n where (H) is stated for all; the cofactor loosens by a factor ≈ 58; and what comes back is (SP) *together with* a per-n additive clause rather than a single n-free statement, since (SP) alone is strictly weaker than (H) — §6.8(iv) makes that last gap precise: (SP) at a polylogarithmic ρ reaches n^{2−ε} for almost all n on its own, and the residue is a bounded-cofactor condition on the **companion** n − r. **What does not degrade is the shape of the statement** — a shifted prime with a large prime-power divisor at *bounded cofactor*, rather than at exponent θ < 1. That is exactly the quantity §3.6's ladder measures, and exactly where Elliott–Halberstam stops short. So the framework is not merely *implied by* a shifted-prime hypothesis: at the θ = 1 end it **demands** one, which is the strongest available reason to think no substantially weaker arithmetic input can deliver a positive density floor.
...
*A structural remark on D.* Cofactors are essentially even: an odd d needs Q(r) = 2^k, so r = d·2^k + 1 is Fermat-like and such r are very rare — among S_12 below 2·10⁶ the odd cofactors d = 1, 3, 5, 7, 9, 11 contribute 5, 6, 4, 3, 5 and 2 members against 7,755 at d = 2 alone. So D is in effect a bound on the even cofactors, and (H)'s d ≤ 12 permits six of them.

...
**The standing check.** p = 3 is the one value that could put an unequal shape inside the competitive range, so **whenever the δ ≤ 1/16 tail is recounted, check it once against p = 3**. That is the whole residue of what this section previously left open.

...
>
> *A rung deliberately not on the ladder.* Shparlinski notes stronger unconditional results of Bombieri–Friedlander–Iwaniec, Mikawa and Fouvry, and sets them aside: they constrain the residue classes a to which ψ(y, m, a) applies. The omission is a considered judgement rather than an oversight — the residue-class restriction is the obstacle to check first if a stronger unconditional rung is ever proposed from that literature.
...
> Suppose μ(n) ≥ δ₀·C(n,2) for all large n. Then for almost all n there is a prime **r ≥ δ₀·n** and a **prime-power divisor Q of r − 1** with **(r − 1)/Q ≤ 2/δ₀** — together with the additive statement that n decomposes with r as a part.

**What makes a converse available at all** is that the score is a *minimum* over orbitals, so every part must clear δ₀·C(n,2) on its own; a part does that only by being large **and** carrying a large twist, and both conditions convert directly into arithmetic. For a foreign part, r·Q ≥ δ₀n²/2 with r ≤ n forces Q ≥ δ₀n/2, and Q | r − 1 < r ≤ n then bounds the cofactor.

**One dependency to keep visible.** The conclusion is about a prime-**power** divisor, and it must be: with Q merely a divisor of r − 1 the statement is vacuous, since Q = r − 1 always works. What confines the foreign twist to a prime power is its living in the top q-group — a layer-assignment claim, and those have the worst record in this framework (`pending-checks.md` T8). The Proposition has had one reading.

**The "almost all" is load-bearing rather than a hedge.** The alternative is an all-matching configuration, and that branch constrains n itself: every class needs c_i ≥ δ₀n, which forces **n = M·p^b with M ≤ 1/δ₀**. Such n have density zero, so the exceptional set is negligible — but only asymptotically, and slowly: the density falls off like H_M/log n, and at n ≤ 10⁵ some **44%** of integers are still within a factor 25 of a prime power. On that branch the conclusion survives with r − 1 replaced by p^b − 1 **at the same bound**, so *some* statement holds with no exceptional set; only the clean prime form needs one. (Getting the same constant on both branches takes one piece of care: the class contributes F·c·d, a three-factor product, and F·c is bounded by n *jointly*. Bounding F and c separately spends δ₀ twice and gives 2/δ₀² — a loss in the derivation, not a feature of the branch.)

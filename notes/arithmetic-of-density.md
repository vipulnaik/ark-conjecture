...
3. **The ceiling splits by residue class mod 12, for both parities.** **⟦EG-TENTATIVE⟧** The local obstructions at ℓ = 2 and ℓ = 3 split it mod 12 — and those are the only two moduli that can obstruct, because each system is three linear polynomials so ω(ℓ) ≤ 3 < ℓ for ℓ ≥ 5. **Six distinct constants result**, from 1/4 down to 7 − 4√3 = 0.07180 (§3.3). *Before the entangled-generator correction a further condition mod 8 — whether the odd family could reach its fused rung — refined the odd classes to mod 24 and gave seven constants. That condition priced a twist cut that no fusion forces, so it is gone: at F = 2 the residual 2-adic dependence is only mod 4, and the genuine mod-8 condition at F = 4 is constant on its mod-12 class (§3.3.4).*
...
>
> S6 has no instance in the current range. The pair to compare is **S5 at n = 459 against S7 at F = 2 at n = 99**: both are `2×c + r*`, and they are distinguished only by the top prime — q = 2 in the first, where the twist stays full and the Fermat prime 257 keeps the foreign block at η = 1, and q = 13 in the second, where the pre-correction scoring cut the twist to the odd part of 22 — **⟦EG-TENTATIVE⟧** the entangled generator keeps the twist full at any q, so the two shapes now differ only in foreign efficiency and S5's q = 2 reading is dominated at odd q; the census split is pending the rebuild.
...
| **S4** | two matching + outside, n = 2c + r\* | additive | **⟦EG-TENTATIVE⟧** 15 of the 16 are exceeded by entangled S7-at-F=2 readings (all but n = 1529); expect ~0 after the rebuild | **exists → 12/24** (all odd n); **wins → 0**. Within the three-part family it is the argmax at residues 7, 15 and 23 mod 24 (`three-part-family-split.md`), but those are exactly the residues whose class ceiling is attained by the two-part F = 4 shape, which beats the whole family there. **The widest existence/winning gap in the census**: available at essentially every odd n and beaten everywhere — by the fused rung where that rung is reachable, and by F = 4 where it is not | §3.2 |
...
- **§3.3 turns the ceilings into a table indexed by n.** **⟦EG-TENTATIVE⟧** Which ceiling applies at a given n is decided by which local obstructions bite — a condition **mod 12**, from ℓ = 2 and ℓ = 3, and no other prime can obstruct. That gives **six constants**, of which the smallest — 7 − 4√3 = 0.071797, shared by n ≡ 11 (mod 12) — is the number §5's floor is stated against. *(Before the entangled-generator correction this read mod 24 and seven constants, the extra modulus coming from a condition on n mod 8 governing which fusion rung was reachable. That condition was an artifact of the refuted twist cut; see §3.3.4.)*
...
>
> **⟦EG-TENTATIVE⟧** *This box previously read "a cyclic group has a unique subgroup of each order, so the twist must be coprime to 2: d is the odd part of c − 1". That inference is invalid — it treats the fusion image as a subgroup — and the entangled construction refutes it explicitly at n = 33, 78 and 105 (`entangled-generator-finding.md`). The correction is the subject of §3.2.3.*
>
...
>
> **⟦EG-TENTATIVE⟧** That check, as stated, caught half the error. It was applied here to conclude that the cost is real at the cyclic layer and spurious at the top layer — and the cost is spurious at **both**, because the argument being audited assumes the fusion image is a *subgroup* of the cyclic layer ("a cyclic group has a unique subgroup of each order") when it is a **quotient**. So the right standing check is one level deeper: *whenever an argument constrains a layer by what subgroups it contains, ask whether the object being placed there is a subgroup or a quotient.* This is the third instance of that confusion in the programme — after the n = 308 bug and the SAFE `dmax` strip — which is enough to make it a checklist item rather than an incident.
...
>
> **⟦EG-TENTATIVE⟧** **What this does *not* buy is a cheaper block swap, and that is the correction.** Full 2-homogeneity normally costs the whole multiplicative group C_{c−1}; at c ≡ 3 (mod 4) it costs only the odd index-2 subgroup, so the factor 2 in c − 1 is not needed inside the block. This section long concluded that the freed 2 is what the cyclic layer spends on F_mid = 2, and hence that **the fused rung is reachable exactly when the block does not need the 2 for itself** — a congruence condition on c. That conclusion is false. A block swap realised as an **entangled generator** takes z of order F·d acting as a block rotation whose step-multipliers generate 𝔽_c^×; then z^F is the full twist, and the block-permutation image is a *quotient* of the cyclic layer, not a subgroup of it. So a cyclic-layer fusion costs nothing on the matching side at **any** c, and no congruence on c is required for the fused rung.
>
> **⟦EG-TENTATIVE⟧** **What c mod 4 does instead is steer.** At F = 2, 2c ≡ 2 or 6 (mod 8) according to c mod 4, so r = n − 2c is reachable at **two** residues mod 8 where the old law reached one. At F = 4 it steers nothing, since 4c ≡ 4 (mod 8) for every odd c. That asymmetry — and not any property of c itself — is what separates the residue classes: the two F = 2 options differ by 4 mod 8 and so agree mod 4, which means the freeing can turn r ≡ 1 (mod 8) into r ≡ 5 (mod 8), buying η = 1/2, but can never produce an r ≡ 3 (mod 4) and so never buys η = 1 or η = 1/3 where those were unavailable. Hence the classes needing η = 1/2 gained under the correction and no others did (§3.3.4a, §3.3.5).
>
...
**⟦EG-TENTATIVE⟧** **The c mod 8 law is retired.** It priced a twist cut that no fusion forces, so the fused intra term is 2·C(c,2) at every odd prime power c and the cyclic-layer rung is strictly better than the unfused C(c,2) **everywhere**, with no congruence and no tie case. The old four-row table is worth keeping in its corrected form, because the classes it named are still the ones where the retired penalty was largest and hence where its absence is most visible in the computed table:
...
| 1 | 8 \| c−1 | **none** | Lemma C, ditto — and the retired penalty was largest here, so this is where the correction shows up most in the table |
...
| **253** | c = 73 ≡ 1 (8)<br>r = 107, r−1 = 2·53 | 2628 — binds on **intra**, C(73,2) | **5256** — binds on **intra**, 2·C(73,2) at the **full** twist 72 | 214 — binds on the **foreign** block at q = 2, orb(107, 2) | **5256 = min( 2·C(73,2), 73², orb(107,53) ) = min(5256, 5329, 5671)**, the inter term being 15622. **⟦EG-TENTATIVE⟧** This row is the correction's cleanest exhibit: c ≡ 1 (mod 8) with the fused class at its **full** twist, which the retired c mod 8 law forbade outright. It was 2783 under v4 — the largest single ratio in the exceedance list, 1.889 |
...
**Reading the losing entries.** The three readings share the same n, c and r and differ only in how the two c-blocks are treated, so the columns are directly comparable and the shortfalls are informative. At n = 273 the unfused reading loses by a factor 1.67 because it forgoes the doubling; the top-layer reading loses by a factor 53 because forcing q = 2 leaves the foreign block a twist of order 2 where q = 53 would give order 53. **⟦EG-TENTATIVE⟧** At n = 253 cyclic fusion is worth its full factor of 2 — 5256 against the unfused 2628 — *at c ≡ 1 (mod 8)*, which is exactly the configuration the retired law scored at 1314; the fused intra term now clears the foreign term 5671 rather than falling to half the unfused value. At n = 531 the top-layer reading is worth exactly twice the unfused one, 18632 against 9316, since the twist is untouched and nothing else binds.

> **⟦EG-TENTATIVE⟧** **A case the correction makes uninteresting, worth one line so it is not looked for.** At n = 247 (c = 73, r = 101, r − 1 = 4·25) the fused and unfused readings now score **identically** at 2525, because the foreign term orb(101, 25) binds below both intra terms; fusion is free but buys nothing. That n was this table's third instance before the correction, on the strength of a harm that does not occur. Its v3 winner is neither reading but a different shape, `4x41 + 1x83*` at 3280.
...
> *Verified by direct orbit computation rather than by reading the formula.* Building the group at c = 7 with two blocks, independent translations, a diagonal twist of order 3 (the odd part of c − 1 — a valid choice, though no longer a forced one) and a block swap gives orbital sizes **{42, 49}** summing to C(14,2) = 91: intra 2·orb(7,3) = 42 and within-class cross **49 = c²**, not 2c². That is the F-keyed rule, and it agrees with the orbital count of `orbital-evasiveness-notes.md` §9.7, whose ⌊F/2⌋ term gives exactly one within-class cross orbital at F = 2.

**⟦EG-TENTATIVE⟧** The bold values are what `mu_table_safe_v5_code_v3.csv` records, under the witnesses `p=137 q=2: 2x137 + 1x257*`, `p=83 q=53: 2x83 + 1x107*` and `p=73 q=53: 2x73 + 1x107*`. Note what the first two show together: **both** reach the full twist at c ≡ 1 (mod 8) — one through the top layer, where the retired law never applied, and one through the cyclic layer, where it was believed to. That the two now agree is the correction in one line.
...
|---|---|---|---|
| **q odd** (cyclic-layer, S7 at F = 2) | 80 | **⟦EG-TENTATIVE⟧** **no congruence: c mod 8 = {1: 14, 3: 23, 5: 23, 7: 17}**, i.e. **37 at c ≡ 1 (mod 4)**, plus 3 at p = 2 | unrestricted |
| **q = 2** (top-layer, S5) | 15 | 5 / 4 / 4 / 2 across 7 / 5 / 3 / 1 mod 8 — **no congruence** | only **1** and **3** |

**⟦EG-TENTATIVE⟧** **So the split is by q and by nothing else.** Both rows now show no congruence on c, which is a stronger form of this subsection's original claim rather than a retreat from it: under the retired law the cyclic-layer route was thought to be rationed by c ≡ 3 (mod 4) and hence to share the odd-n asymptotics with S4 as a co-carrier on a quarter of all c each. With the twist cut gone, **the cyclic-layer rung is available at every c** and S4 wins nowhere in the current range — it carried 2 rows under v4 and 0 under v3. S5 remains a third, supply-limited route that ignores c entirely and is rationed by u instead.

> **⟦EG-TENTATIVE⟧** *The counts above are provisional*, taken from `mu_table_safe_v5_code_v3.csv` at 806 rows (n ≤ 1000) rather than the full range; the c mod 8 shape of them is what matters and is not in doubt — 46% of cyclic-layer winners now sit at c ≡ 1 (mod 4), against 9% under v4. Requote on the completed rebuild via `check_doc_figures.py`.
...
| **B** | two c-blocks fused in the **cyclic layer** + foreign | **S7 at F = 2** **⟦EG-TENTATIVE⟧** (no congruence on c; formerly stated as needing c ≡ 3 (mod 4)) | 2x² | 2η/(√2+2√η)² | √η/(√2+2√η) |
...
**⟦EG-TENTATIVE⟧** **Reachability imposes nothing beyond mod 12, and the mod-24 keying was an artifact.** Rung B — cyclic-layer fusion — was believed to need the twist on the c-blocks to be odd, i.e. c ≡ 3 (mod 4); the entangled construction refutes that (§3.2.3), so **the matching side imposes nothing**. The consequence for the keying is larger than a loosened condition:
...
So the ceilings are keyed **mod 12**, and the same modulus decides local solubility (§3.5.4) — the two questions that this document long kept apart because they answered differently. Before the correction, classes 7 and 15 took F = 4 while 3 and 19 took F = 2; since 15 ≡ 3 and 7 ≡ 19 (mod 12), that grouping cut across mod-12 classes, and it was the table's **only** genuine mod-24 dependence. Removing the twist cut merged 7, 15 into the 3, 19 row (§3.3.5) and the dependence went with it. *Confirmed empirically: over n ∈ [24000, 48000] the per-residue sup agrees in cap, F and η across every pair {a, a+12} (`ceiling_rederive.py`). The old "100% or 0% over 15,000 values per residue, no boundary cases" measurement was of the superseded condition and is void.*
...
**⟦EG-TENTATIVE⟧** **The 2-adic condition.** This step formerly took c ≡ 3 (mod 4) as *required* for even F, on the ground that the fusion count occupies the prime 2 in the cyclic layer and cuts the twist to the odd part of c − 1. The entangled construction refutes the requirement (§3.2.3), and the derivation is **strengthened rather than repaired**, because the two F values that matter behave differently:

> - **F = 4** — 4c ≡ 4 (mod 8) for *every* odd c, so F·c is determined mod 8 with **no premise about c at all**. The F = 4 rows below are therefore exactly as they were, which is why the classes whose optimum takes F = 4 came through the correction unchanged.
> - **F = 2** — 2c ≡ 2 or 6 (mod 8) according to c mod 4, both now available. So r = n − 2c is reachable at **two** residues mod 8, and the F = 2 rows gain an option they previously lacked. Since the two differ by 4 and hence agree mod 4, the gain is confined to converting r ≡ 1 (mod 8) into r ≡ 5 (mod 8) — i.e. to buying η = 1/2, never η = 1 or η = 1/3.
...
> **Where mod 8 is not enough, and why it no longer forces a deeper keying.** At r ≡ 1 (mod 8) the rule above gives only v ≥ 3. The exact value needs mod 16: **⟦EG-TENTATIVE⟧** with c mod 4 free, 4c ≡ 4 or 12 (mod 16), so r ≡ n − 4 or n − 12 (mod 16) — two options where the superseded reading forced 4c ≡ 12 — and **n mod 16 is not determined by n mod 24** either way. The class splits, now with the better of the two options available at each n: v = 3 and η₂ = 1/4, or v = 4 and η₂ = 1/8. That affects the F = 4 entry at three classes, 5, 13 and 21. **None of them is a class whose optimum takes F = 4**, so the loosening is harmless here and the guarantee at those cells is unchanged or better. Of the classes that do take F = 4, **11 and 23 have r ≡ 3 or 7 (mod 8), hence v = 1 exactly** — the smallest value available, pinned, and undisturbable by any deeper modulus, which is why they survived the correction untouched. Classes 7 and 15 have left the F = 4 group entirely, to an F = 2 reading at η = 1/2 (§3.3.5), so no class whose optimum takes F = 4 is exposed to the mod-16 refinement. **The binding cells are precisely those where the 2-adic valuation is pinned at the bottom, and there it is pinned at v = 1 for both mod-24 residues of the class** — which is why no modulus deeper than 12 appears in the table, though mod 8 appears in the derivation (§3.3.4).
...
| n mod 12 (residues listed mod 24) | which rung attains the cap | shape | η | **x\* = c/n** | x\* | cap, closed form | cap |
|---|---|---|---|---|---|---|---|
...
> **⟦EG-TENTATIVE⟧** **Where F = 4 beats F = 2, and why it is a mod-8 effect — restated after the correction.** The argument formerly ran: at n ≡ 7 (mod 8), F = 2 *needs* c ≡ 3 (mod 4) to keep its twist, forcing 2c ≡ 6 (mod 8) and hence r ≡ 1 (mod 8), so 8 | r − 1 and the efficiency is cut hard — at residue 23 to η = 1/12, capping at (7 − 2√6)/50 = 0.042020. **The premise is refuted** (§3.2.3): c mod 4 is free, so at F = 2 both 2c ≡ 2 and 6 (mod 8) are available and r reaches **two** residues mod 8 rather than one.
>
> What survives, and it is the whole of the F = 4 case: 4c ≡ 4 (mod 8) for every odd c, so at F = 4 the residue r ≡ n − 4 (mod 8) is pinned with no condition on c at all, giving v₂(r − 1) = 1 and no 2-adic cut. **At 11 and 23 that still beats every F = 2 reading**, because the two F = 2 options there both land on r ≡ 1 (mod 4), so with the ℓ = 3 obstruction the best F = 2 can reach is η = 1/6 and cap₂(1/6) = 0.050510 < 7 − 4√3. So the D = 24 obstruction remains a property of F = 2 at those two classes.
>
> **At 7 and 15 the conclusion inverts, and those classes leave the F = 4 group.** There the freed c mod 4 puts r ≡ 5 (mod 8) within reach, so F = 2 attains η = 1/2 and cap₂(1/2) = 1/8, above the F = 4 value 1/9. **The resulting (F, η, x\*, cap) is identical to that of classes 3 and 19**, so 7 and 15 do not merely gain a better constant — they merge into that row, and the odd part of the table loses a constant. The final sentence of this box — that the F = 4 rows need c ≡ 3 (mod 4) for the matching term to stay at F·x² — is deleted: the matching term is F·x² unconditionally.
>
> *Confirmed independently of the congruence argument.* `ceiling_rederive.py` scans real configurations of the generic family — c a prime ≥ 5, twist a prime power at q ≥ 5, F even — and takes the empirical sup of density per residue class, with the four escapes of §3.3.8 excluded. Over n ∈ [45000, 90000] the sup at classes 7 and 15 is 0.124987 and 0.124985 against 1/8, attained at F = 2, η = 1/2 and c/n = 0.24999, 0.25001 against the predicted x\* = 1/4 — and **both witnesses sit at c ≡ 1 (mod 4)**, the branch the retired law excluded. At 11 and 23 the same scan returns 0.071791 and 0.071793 against 7 − 4√3 = 0.071797, attained at F = 4, η = 1/3, x\* = 0.13398 against 0.13397: **unchanged, as the F = 4 argument requires.** All eight other odd classes reproduce their tabulated constants to four places from below. *(Run without the escape filter and the sup exceeds the cap at 5, 11, 17 and 23 — by 1.48× at class 11 — every time via c or oddpart(r−1) a power of 3. Those are §3.3.8's documented O(n/log n) routes, and their appearance is a check on the filter rather than on the table.)*

> **⟦EG-TENTATIVE⟧** **The ties are mostly gone, because they were artefacts of the twist cut.** A tie means two shapes return the same *value*. The tie recorded here at 23 — fused and unfused agreeing at (5 − 2√6)/2 whenever c ≡ 5 (mod 8), because the odd part of c − 1 is (c−1)/4 and the fused reading returns exactly C(c,2) — **no longer occurs**: with the full twist the fused reading returns 2·C(c,2) at every c, so it strictly beats the unfused one and the tie condition on c is vacuous. (This is visible in the validator, whose retired congruence check formerly counted a tie population at c ≡ 5 (mod 8).) The identity cap_B(1/4) = cap_C(1/2) at 7 and 15 is a genuine coincidence of the cap formula at η = 1/2 and stands as arithmetic, but it no longer describes competing *reachable* shapes. Ties in the remaining sense — the binding term being one that fusion does not touch, as at n = 247 where a foreign term binds below both intra terms — still occur and are where equal values now come from. Which reading realises that family, and in what proportions, is worked out in `three-part-family-split.md`.

> **What the merged row hides, and why it is still one row.** **⟦EG-TENTATIVE⟧** *The table now has **six** constants, not seven: classes 7 and 15 joined the 1/8 row when the entangled correction freed c mod 4 (see the box above), so the four residues 3, 7, 15, 19 share it.* The rows group residues sharing a cap, an η and a shape, and they have always grouped residues whose *foreign* residue differs — at (F = 2, η = 1) the four residues 1, 9, 13, 21 split into r ≡ 3 and r ≡ 7 (mod 8) yet share a row. The 11/23 merge is the same convention: with F = 4 the mod-8 condition drops out, both take η = 1/3, and the row entries are identical. Two things they do **not** share, needed downstream:
>
...
**⟦EG-TENTATIVE⟧** Six distinct ceilings, keyed mod 12. **The global asymptotic constant is the smallest of them: 7 − 4√3 = 0.071797, attained at n ≡ 11 (mod 12)** — the two residues carrying the ℓ = 3 obstruction at odd n, which caps their efficiency at η = 1/3 even once F = 4 removes the 2-adic cut. That is the constant §5's floor is stated against.

> **Why it is a tie between two residues rather than a single worst class.** Taking F = 4 removes any mod-8 dependence from the *outcome* — 4c ≡ 4 (mod 8) for every odd c, so r ≡ n − 4 (mod 8) is pinned and gives v₂(r − 1) = 1 at both 11 and 23 — leaving only the ℓ = 3 obstruction, which sees n mod 12 and cannot distinguish them. So the two halves of n ≡ 11 (mod 12) are equal. **⟦EG-TENTATIVE⟧** *This paragraph formerly explained the tie as a local coarsening, mod 24 back to mod 12 at the F = 4 classes only, against a table that was otherwise mod 24. The correction makes the whole table mod 12 (§3.3.4), so the tie is no longer the exception it was written as — it is what every class now does.* The check the paragraph proposed still stands and is now sharper: **a ceiling table that separates any a from a + 12 must be getting it from a mod-8 condition**, and the only surviving one, at F = 4, is constant on the class.
...
>
> **⟦EG-TENTATIVE⟧** *A fifth clause has been deleted rather than weakened.* It required c ≡ 3 (mod 4) at F ≥ 2, so that the matching class keeps its full 2-homogeneity and its intra term stays at F·x²; the entangled construction gives the full twist at any c (§3.2.3), so **the intra term is F·x² unconditionally** and the clause imposes nothing. Note the direction: the deleted clause held at about half of all solutions, so removing it is a **factor 2 gain in supply** for every fused shape, with no change to the local analysis.
>
...
**⟦EG-TENTATIVE⟧** *(Four is now literal: the twist clause has been deleted, leaving shape, balance, efficiency and coherence.)*
...
The efficiencies η = 2/d this yields are exactly the η column of §3.3's ceiling table: d = 2 gives η = 1 at n ≡ 1, 9 (mod 12) and at the unobstructed even classes, d = 4 gives η = 1/2 at 3 and 7 (mod 12), i.e. **⟦EG-TENTATIVE⟧** at 3, 7, 15, 19 (mod 24), d = 6 gives η = 1/3 at 5 and at the even classes 2 and 8, and d = 12 gives η = 1/6 at 11 **under F = 2** — the two tables being the same fact read from the two ends. **⟦EG-TENTATIVE⟧** At the **two** residues where F = 4 attains the ceiling — 11 and 23 — the relevant η is §3.3.4a's and the d it calls for is 6; classes 7 and 15 now take F = 2 at d = 4, alongside 3 and 19. So d = 4 covers four residues rather than two, and d = 12 survives in clause 3's list only as the F = 2 value at class 11, which the F = 4 optimum supersedes.
...
**⟦EG-TENTATIVE⟧** *Why the list and the ceilings are now keyed alike, at mod 12.* These remain **different questions**, and the distinction is still worth keeping: the admissible-d table above is about **local solubility** — which systems are not identically obstructed — decided by ℓ = 2 and ℓ = 3, hence mod 12; the ceiling table of §3.3 is about **which rung is reachable**. What has changed is that the second question no longer answers at a finer modulus. Reachability formerly imposed an extra condition on n mod 8 at odd n, giving mod 24 — but that condition was the refuted twist cut, and with it gone the F = 2 rows depend on n only mod 4 and the surviving mod-8 condition at F = 4 is constant on its mod-12 class (§3.3.4). So the two tables coincide in keying while remaining distinct in content, and (H) still quantifies over local solubility with condition 2 absorbing reachability. *Two questions agreeing on a modulus is a coincidence of this family, not an identification: nothing here says a wider shape space would keep them aligned.*

**⟦EG-TENTATIVE⟧** *The twist clause is gone, and what it was tracking is now a free parameter.* This paragraph formerly showed the clause to be automatic at the eight F = 2 residues (c ≡ 3 mod 4 following from n ≡ 3 mod 8 and r ≡ 5 mod 8) and a genuine condition at the four F = 4 residues, holding at about half of all solutions there. With the twist cut refuted the clause imposes nothing anywhere and has been deleted from (H). Its arithmetic inverts into the steering statement of §3.3.4a: **at F = 2, c mod 4 chooses between two reachable r mod 8; at F = 4, 4c ≡ 4 (mod 8) for every odd c, so c mod 4 is free and inert.** The supply consequence is a factor 2 gained at each fused class. With clause 3's values, **d ∈ {2, 4, 6, 12} suffices to attain every ceiling**; larger d describe alternatives rather than requirements.
...
# F = 4 rows: residues 11, 23 at (D, x*) = (6, 0.13397).  **⟦EG-TENTATIVE⟧** Classes 7 and
...
**⟦EG-TENTATIVE⟧** *The extra congruence the fused rung was thought to need does not exist.* Rung B — the cyclic-layer one, S7 at F = 2 — was stated to require c ≡ 3 (mod 4); the entangled construction removes the requirement (§3.2.3), so **no congruence on c is imposed at either rung** and rung B′ was never subject to one. The arithmetic this paragraph gave remains true as a *consequence* rather than a condition: at n ≡ 11 (mod 24), r = Dq + 1 with q odd gives r ≡ 5 (mod 8), and n ≡ 3 (mod 8) then makes c = (n − r)/2 ≡ 3 (mod 4) — so that value of c happens to arise, and nothing breaks if it does not. What is lost is the tidy coincidence between the mod-24 split and the split between rungs; the mod-24 keying now comes from the foreign block's r − 1 alone (§3.3.4a).
...
**The prediction shows up in the table, on both of its halves.** Two forces act on the observed floor in opposite directions, and separating them is the point. Extending the range lowers it, since a wider range can only find weaker values. Adding configurations raises it, since B(n) is a maximum — so a shape-space correction lifts the whole curve at once. Over the contiguous range to n = 2600 the floor is **⟦EG-TENTATIVE⟧** **tentatively 0.048039 at n = 2183** (v4's 0.045742 at n = 1817 rises to ≥ 0.0594 under the entangled-generator correction; 2183 is not among the 289 raised rows, so its v4 value stands as the working minimum pending the rebuild). So the *within-table* floor moved up when the space was corrected, while the mechanism below — which is about which n the engines reach, not about how well they are scored — is unaffected. The thirds of the range behave as the argument requires:
...
**But the vanishing is logarithmic, which is why they are conspicuous at computed sizes.** Measured against the ceilings (**⟦EG-TENTATIVE⟧** now keyed mod 12), the fraction of odd n whose ceiling the route exceeds:
...
| S4 | **wins nowhere**; **⟦EG-TENTATIVE⟧** the argmax of the three-part family at 23 mod 24, where F = 4 beats the family (at 7 and 15 the F = 2 entangled reading now beats both, and at 11 F = 4 does) | §3.2 |
| **S7 at F = 2** | carries odd n — **⟦EG-TENTATIVE⟧** **10/24**, the residues 1, 3, 5, 7, 9, 13, 15, 17, 19, 21 mod 24 (i.e. all of n ≡ 1, 3, 5, 7, 9 mod 12) | §3.2 |
| **S7 at F = 4** | carries odd n — **⟦EG-TENTATIVE⟧** **2/24**, the residues 11, 23 mod 24 (i.e. n ≡ 11 mod 12) | §3.3.5 |
...
Since the multiplicative engine vanishes in density, the asymptotic behaviour of μ(n) for almost all n is set by the additive families, whose ceilings are the **⟦EG-TENTATIVE⟧** mod-12 table of §3.3 and whose availability is a Bateman–Horn question. In particular the ladder constants of §5 of the notes — the §3.3 constants — are the right asymptotic quantities, and the fused family's 1/2 and 1/3 are not, however dominant they look in the table.
...
> **⟦EG-TENTATIVE⟧** **tentatively 0.048039 at n = 2183 = 37·59**, witness `6x251 + 1x677*` (v4's floor **0.045742 at n = 1817**, witness `p=389 q=173: 1x1039* + 2x389`, rises to ≥ 0.0594 under the corrected caps),
...
> **Why the extremal residue is mod 12 — as, now, is everything else.** **⟦EG-TENTATIVE⟧** The local obstructions at ℓ = 2 and ℓ = 3 see only n mod 12, and since the entangled-generator correction nothing in the ceiling table sees anything finer (§3.3.4): at F = 2 the 2-adic dependence is mod 4, and the genuine mod-8 condition at F = 4 is constant on its class. *This paragraph formerly had to explain why the extremal class was coarser than the rest of the table; there is no longer a discrepancy to explain.* The minimum over all residues is **7 − 4√3 = 0.071797**, attained on **n ≡ 11 (mod 12)**.
...
> *The floor rows want recomputing as the table extends.* At the conjectured floor of 1/25, L = 5.000 and the part cap is **k ≤ 5** — a substantial narrowing of the search this table sizes, since the count grows steeply in L. The exactly-computed floor — **⟦EG-TENTATIVE⟧** tentatively 0.048039 at n = 2183, giving L = 4.562, k ≤ 4, F ≤ 20 (v4's 0.045742 at n = 1817 gave L = 4.676, N(δ₀) = 112) — wants these rows re-derived after the rebuild. Recompute the affected rows rather than the whole table. (The 122 and 164 above, and the 112 here, are direct enumerations of the criterion, checked against the 24 / 65 / 83 rows by the same code.)
...
> **⟦EG-TENTATIVE⟧** for most n ≡ 11 (mod 12), **the system n = 4c + r with c a prime power, r prime and r − 1 = 6q^e for a prime power q^e, is solvable with c/n near (2 − √3)/2** — *the clause "c ≡ 3 (mod 4)" has been dropped (the twist is full at any c, §3.2.3), and the residue widened from 23 mod 24 to 11 mod 12 since the two halves are now equal*
...
- **The gap between consecutive ceilings bounds ε**, and it is not uniform: 0.0048 at n ≡ 11 (mod 12) — the gap from 7 − 4√3 down to (2 − √3)/4, as computed above; the older figure of 0.0085 was the gap under the superseded F = 2 optima — but the classes where two rungs tie — 7 and 15, where cap_B(1/4) = cap_C(1/2) exactly — admit **no** such ε, and there the disjunction genuinely cannot be collapsed.

...
> *Verified, and tight.* Maximum observed density is 0.24939 among two-part winners and 0.10912 among three-part winners; **no two-part winner exceeds 1/4 and no three-part winner exceeds 1/9**, over 1,393 and 16 rows respectively. Medians 0.1695 and 0.0978.
...
**⟦PENDING-REBUILD⟧** *The per-block floor table and the block-by-block recovery narrative are outputs of the superseded scan; regenerate them from the rerun before quoting any row.*
...
6. **The fused family at ω(n) = 2 but bad splitting.** **437 of the 1,210** values with ω(n) = 2 do better with a split than with fusion, which happens when the smallest prime-power cofactor F is large. The distribution of F over ω(n) = 2 integers is classical, so predicting the 773/437 division is a clean test. (Both figures must come from the same slice — the 754/323 pair quoted elsewhere is the older n ≤ 2,298 range.)

...
>
> **The E–H exponents are approached, not attained, and this is not pedantry.** Elliott–Halberstam is quantified *for any fixed ε > 0* (level up to z^{1−ε}), so each application buys θ = 1/2 − ε/2 and the bound is **n^{3/2−ε} for every ε**, never n^{3/2} on the nose — the same form BBKN's Chowla row carries. Likewise Theorem 2's α ranges over values *below* 1, giving n^{2−ε} for every ε, which is what "2 − o(1)" abbreviates. The `+o(1)` in the unconditional n^{5/4+o(1)} is the up-to-subpolynomial-loss convention (the o(1) may be negative) and so does **not** cancel against this. *Owed check:* Shparlinski's §5 states the remark without writing an exponent, so the ε-form above is inference from how E–H is quantified rather than a quoted bound; worth confirming against the published version.
>
> **Why this matters for what (H) is claiming, and it is not merely a quantifier upgrade.** n^{2−o(1)} is consistent with n²/log n, or n^{2−1/log log n}, and the E–H route sits on exactly that side: it delivers, for each fixed ε, an exponent below 2, and no member of the family delivers a *constant fraction* of n². What (H) buys is **Ω(n²) with an explicit constant** — δ₀·C(n,2) — which is on the other side of that barrier and is not reachable by pushing α → 1 at all. The structural reason: α → 1 asks for a prime factor of size r^α for exponents approaching 1, whereas (H)'s d ≤ 12 asks for a prime factor of size **(r−1)/d, linear in r**. A linear-sized prime factor of r − 1 is a Sophie Germain-type condition, independently twin-prime-hard, and is strictly stronger than every rung of the α-ladder rather than its limit point. So E–H is not "almost (H)": it approaches the exponent from below and stops short of the quantity (H) asserts.
>
> **Elliott–Halberstam buys two different things, and they were previously one row.** Shparlinski's §5 remark contains two independent consequences, and they sit on opposite sides of the quantifier column. (i) E–H extends the averaging of the Bombieri–Vinogradov input up to z^{1−ε}, which replaces the 5/4 of his Theorem 1 with **3/2 for all large n** — the same exponent the Chowla row buys, by a different route. (ii) Separately, E–H allows any α < 1 in his Theorem 2, whose unconditional α = 0.677 comes from Baker–Harman, giving **n^{2−o(1)} for almost all n**. Compressing these into one row previously attached the 2−o(1) exponent to the *all large n* quantifier, which is wrong and wrong in our favour.
>
> **The 3/2 rungs are reached by two non-equivalent conjectures, and Shparlinski says which is likely harder.** Chowla's route applies to *individual* progressions and so may be more difficult to establish than Elliott–Halberstam; the E–H route asks only for distribution on average. So the θ = 1/2 rung is better supported than a single conjectural row would suggest — two independent paths reach it — and the honest reading is that 3/2-for-all-large-n is the more robust conjectural target, while 2−o(1) is available only in the almost-all regime.
>
> **The quantifier column is not decoration.** The strongest exponents all carry quantifier costs: 1.677 and its conditional 2−o(1) hold for almost all n (exceptional set O(x^{0.354}(log x)⁴) unconditionally), and the 3/2 rungs carry O((log x)⁴). On the *all large n* row the unconditional state of the art is 5/4. Omitting the quantifiers would mislead **in our favour**, which is worse than misleading against us — and the E–H row did exactly that until it was split.
>
...
>
> *A rung deliberately not on the ladder.* Shparlinski's §5 closes by noting stronger unconditional results of Bombieri–Friedlander–Iwaniec, Mikawa and Fouvry, and then sets them aside: they constrain the residue classes a to which ψ(y, m, a) applies, which makes them hard to use here. Worth recording that the omission is his considered judgement rather than an oversight — if anyone tries to add a stronger unconditional rung from that literature, the residue-class restriction is the obstacle to check first.

Hypothesis (H) of §3.5 above is the **θ = 1 endpoint** of this ladder. Stating it that way is more informative than calling it a barrier: it places the hypothesis on a scale with a known current value rather than in a separate category. *But "endpoint" should not be read as "limit of the ladder", and the separated rows make the two distinct gaps visible.* E–H delivers θ = 1 − ε for **almost all** n, for every fixed ε. (H) differs from that in **two** ways, not one: the quantifier (every large n, not almost all), and — the more substantive difference — the *kind* of statement, since (H) asks for a prime factor of r − 1 of size linear in r, giving Ω(n²) with a constant, where the ladder only ever gives n^{2−ε}. The second gap is not closed by any amount of pushing ε → 0. (H) is the ladder's endpoint in the sense of sitting at θ = 1 on the same scale; it is *not* the ladder's limit.

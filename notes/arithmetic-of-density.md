...
*Branch (b) of Proposition F.4 is, stripped of n, a statement about a set of primes. Fixing its two parameters rather than quantifying over them makes the implications below precise about which version is meant where.*

Write **Q(r)** for the largest prime-power divisor of r − 1, fix an integer **D ≥ 1** and a positive function **ρ**, and set

> **S_D = { r prime : (r − 1)/Q(r) ≤ D }.**

> **Hypothesis (SP_{D,ρ}).** For all large x, |S_D ∩ [1, x]| = ρ(x)·π(x)·(1 + o(1)).

The estimate is **two-sided** by construction, and that is what makes it usable: the coupling needs members of S_D in a window proportional to n, and |S_D ∩ [cx, x]| is a *difference* of cumulative counts, so an upper bound at cx is needed as much as a lower bound at x. This is the relationship the prime number theorem has to Bertrand's postulate — the two-sided asymptotic gives π(x) − π(cx) ≫ x/log x for fixed c < 1, which no lower-bound-only statement does.
...
**(i) Bounded multiplicative gaps are the special case ρ = ρ_gap.** "Every interval [x, Λx] contains a member of S_D" is (SP_{D,ρ}) with the weakest ρ that forces one member per window, namely **ρ_gap(x) = (log x / log Λ)/π(x)** — a count growing like log x rather than like x/log²x. Everything below is stated against this baseline.

**(ii) A density floor implies (SP) at that baseline, with one constant in both roles.** If μ(n) ≥ δ₀·C(n,2) for all large n then **(SP_{D, ρ_gap}) holds with D = Λ = 2/δ₀**. Branch (b) supplies a member of S_D in [δ₀(n−1)/2, n] for almost all n; if some [x, (2/δ₀)x] held none, every n in its upper part would fail (b), and branch (a)'s n have density zero and so cannot fill an interval. The same inequality bounds the cofactor and forbids the gap, which is why D and Λ coincide.

**(iii) Empirical and Hardy–Littlewood support is for far more than the baseline: ρ(x) ≍ 1/log x.** The count of r = dQ + 1 with Q a prime power is a Bateman–Horn system for each d, predicting |S_D ∩ [1,x]| ~ C_D·li₂(x) with C_D a singular-series sum over the admissible cofactors, hence **ρ(x) = (C_D + o(1))/log x**. Measured against li₂(x) at x = 2·10⁶: **C_2 ≈ 0.692, C_12 ≈ 2.26, C_50 ≈ 4.02**, each still drifting down slowly as such counts do. The D = 2 case is a clean check on the prediction rather than on the framework — it is Sophie Germain, whose constant should be 2C₂/2 = C₂ ≈ 0.660 after the r ≤ x versus q ≤ x/2 halving, against 0.692 measured, agreement to about 5%. So the supported hypothesis is (SP_{D, C_D/log x}), which exceeds ρ_gap by a factor of order x/log³x.

**(iv) What (SP) at that ρ would give, and the one link that is open.** With ρ(x) ≍ 1/log x, the route back to a floor is the exceptional-set machinery behind Shparlinski's Theorem 2, which converts a set of shifted primes with large Q into a statement about almost all n — supplying exactly branch (b)'s additive clause. It would yield **δ(n) ≥ δ₀ for almost all n**, weaker than (H)'s conclusion for *all* large n but the natural counterpart of an almost-all hypothesis. **The open link is whether that machinery tolerates ρ ≍ 1/log x.** It is applied in the literature to the Baker–Harman set {r : Q(r) ≥ r^α}, whose relative density is ≈ 1 − α — positive for each fixed α < 1, and **vanishing exactly as α → 1**. Bounded cofactor is the α = 1 endpoint, so its density is one logarithm short of what the machinery is known to consume. Not a power short: the heuristic count of representations n = c + r with r ∈ S_D remains ≫ n/log³n, and S_D is not concentrated low — at c = 0.04 the window [0.04x, x] holds 92.5% of the cumulative count at x = 2·10⁶, against 96% for the primes. **So the obstacle is the size of ρ, not its localisation**, and the question is filed as a reading task (`pending-checks.md` T4).

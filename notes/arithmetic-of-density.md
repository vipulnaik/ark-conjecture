...
| **S5** | **top-layer**-fused matching + outside; forces q = 2, hence η = 1/u | hybrid | **21** (1.0%) | **exists → 12/24** (all odd n — q = 2 fusion is always available, just usually at a useless η); **wins → 0**. An escape: winning needs η = 1/u with u small, i.e. r = 2^a·u + 1, which is O(log n) candidates per n and so O(n/log n) values | §3.3, §4.3 |
...
| 2, 8 | k = 1, no fusion question | S3 | 1/3 | **(√3 − 1)/2** | 0.36603 | **(2 − √3)/2** | 0.13397 |
...
**S7 at F ≥ 3** is an escape rather than a carrier, and §4.3 counts it with the other routes at O(n/log n). Its parity structure is what restricts it: at odd n, F·c even with F odd forces c to be a power of 2, leaving O(log n) choices per n. **S7 at F = 2 is a different matter entirely** — it is the odd-n fused rung and carries 5/12 of all n; it is listed under §4.5's live shares, not here.

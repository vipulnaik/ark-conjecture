...
**(ii) A density floor implies that baseline, with one constant in both roles.** If μ(n) ≥ δ₀·C(n,2) for all large n then **(SP_{D, c, 1/π(x)}) holds with D = 2/δ₀ and c = δ₀/2**. Branch (b) supplies a member of S_D in [δ₀(n−1)/2, n] for almost all n; if some window held none, every n in its upper part would fail (b), and branch (a)'s n have density zero and so cannot fill an interval. The same inequality bounds the cofactor and fixes the window, which is why D and 1/c coincide up to the factor 2.
...
**(iv) What (SP) at that ρ would give, and the one link that is open.** With ρ(x) ≍ 1/log x, the route back to a floor is the exceptional-set machinery behind Shparlinski's Theorem 2, which converts a set of shifted primes with large Q into a statement about almost all n — supplying exactly branch (b)'s additive clause. It would yield **δ(n) ≥ δ₀ for almost all n**, weaker than (H)'s conclusion for *all* large n but the natural counterpart of an almost-all hypothesis. **The open link is whether that machinery tolerates ρ ≍ 1/log x.** It is applied in the literature to the Baker–Harman set {r : Q(r) ≥ r^α}, whose relative density among primes is ≈ 1 − α — positive for each fixed α < 1, and **vanishing exactly as α → 1**. Bounded cofactor is the α = 1 endpoint, so ρ is one logarithm short of what the machinery is known to consume.

*How short, in the currency the circle method actually uses.* Counted inside the integers rather than inside the primes, and measured at x = 2·10⁶: the primes have density 0.0745 ≈ 1.08·(1/log x), the Baker–Harman set 0.0253 ≈ 0.37·(1/log x), and **S_12 0.0127 ≈ 2.67·(1/log²x)**. So the almost-all Goldbach theorem and Shparlinski's input both operate at 1/log x per summand, while S_D sits at 1/log²x — the twin-prime order, one further logarithm down. **Binary Goldbach for almost all even n is a genuine precedent that 1/log x is no barrier**, since its summands are exactly that thin; what has no precedent is a summand at 1/log²x.

*And density is probably not the whole of what is consumed, which is the more useful caution.* The exceptional-set argument needs its input set **distributed in arithmetic progressions**, not merely counted — the major arcs are built from that distribution, and the minor arcs from an exponential-sum bound. (SP_{D,c,ρ}) as stated supplies a count and nothing else, so it is very likely **insufficient as it stands** even at ρ ≍ 1/log x, and a sufficient version would have to add an equidistribution clause for S_D. That clause is a level-of-distribution statement about the modulus Q, which is where this framework's arithmetic lands every time. The heuristic count of representations n = c + r with r ∈ S_D remains ≫ n/log³n, and the window is not the difficulty — at c = 0.04 it holds 92.5% of the cumulative count against 96% for the primes — so **neither the window nor the raw density is where this fails; the missing hypothesis is equidistribution**. Filed as a reading task (`pending-checks.md` T4).

> **The exponent and the density compete for one resource.** This is the second reason §3.6's ladder stops short of a constant, independent of the level-of-distribution barrier recorded there. Any α < 1 buys positive relative density and yields n^{1+α}; α = 1 buys Ω(n²) and costs the density the coupling runs on. So n^{2−ε} and Ω(n²) are not adjacent points on one scale, and (SP) at ρ ≍ 1/log x is exactly the statement that sits in the gap.

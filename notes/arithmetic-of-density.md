...
**The observed density floor should drift downward.** Fully 55.4% of the current table has ω(n) = 2, so more than half the computed values are served by an engine whose reach halves over the next few decades of n. The median of 0.1994 is propped up by a population that thins. The evidence is two-sided and worth keeping apart: *within* the exactly-computed range each extension lowers the floor, which is the prediction, while the shape-space repair lifted the current table's floor to 0.045742 at n = 1817, which is a correction to the scoring and not a counter-observation. Beyond the table the ladder of §5 reaches 0.04453 below 10⁶, attained at n = 11183 — above the exactly-computed floor's own value in the table's range, which is the recovery showing through rather than a contradiction.

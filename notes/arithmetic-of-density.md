...
> *Verified over [6, 10⁶].* Over all **124,502** one-part winners, the predicted density 1/F agrees with the computed value to O(1/n), with no exceptions — |δ − 1/F|·n < 0.95 on every row, and every row matches the exact form (c−1)/(Fc−1). By F: 41,706 rows at F = 2 (median 0.5000), 28,814 at F = 3 (0.3333), 22,173 at F = 4 (0.2500), 18,048 at F = 5 (0.2000), **9,846** at F = 7 (0.1429); maxima 0.50000, 0.33333, 0.25000, 0.20000, 0.14286 against 1/F. The remaining **3,915** sit at F = 6 (7), 8 (40), 9 (67), 11 (2,100), 13 (1,700) and 17 (1) — F = 10 and F = 19 do not occur, and the only F that is not a prime power is 6, at the same seven n as before.

> *Verified over [6, 10⁶], with one correction the corrected shape space forces.* Of the 124,502 one-part winners, **124,495 have ω(n) = 2 and seven do not**, all of shape `6x p` (the five below 2,600 are n = 78, 222, 282, 894 and 1434) — the same seven at 10⁶ as at 159,027, so the F = 6 family does not grow. **The exceptions are exactly the composite-F fusions**: F = 6 is available only because F_mid need not be a prime power, and n = F·c then carries three prime factors. Under a shape space with F a q-power the statement "every one-part winner has ω(n) = 2" is a theorem — n = F·c with c a p-power has ω(n) ≤ 2 — so this is a place where the correction changes a structural claim and not only a count. Of the **288,726** values with ω(n) = 2, **124,495** are one-part winners and the other **164,231** do better with a split — but ω(n) = 2 is the wrong slice for that question, being neither necessary (the seven F = 6 winners) nor sufficient (98,870 of those 288,726 have n/Q(n) > 25 and could not clear 1/25 by fusion at all); §4.1 and §8 item 6 put it in terms of the cofactor n/Q(n).
...
> *Verified over [6, 10⁶], and tight.* Maximum observed density among two-part winners is **0.25000** to five places and **no two-part winner exceeds 1/4**, over all 796,763 of them; median 0.1707. The three-part half of this check has no population left to run on — the family is empty on the completed table (`enumeration-proof.md`'s census) — so the 1/9 bound for three parts is now a statement about a shape that exists and never wins, not a measured margin.
...
> **Recounted over [6, 10⁶]**, all 306,544 cyclic-layer winners at F = 2 (306,254 with odd c; the rest are 2-power blocks). The c mod 8 distribution is **{1: 76,787, 3: 77,591, 5: 75,976, 7: 75,900}** — flat to within 2.2%, and **49.9% of the odd-c rows sit at c ≡ 1 (mod 4)**. The prediction was that the congruence would disappear rather than merely weaken, and at four residues within two percent of each other over three hundred thousand winners it has: what remains is sampling noise, not a residual preference.
...
> **What bounds an individual row is cap_F(η), not its class ceiling.** Over [6, 10⁶], **19,665 of the 796,763 one-foreign winners exceed their own class's tabulated δ₀**. In class 11 the *median* winner now sits at **1.00×** the tabulated cap and **7,950 of 52,038** exceed it, with a maximum of 2.81× — the class has settled onto its ceiling as the range grew (on the 2,600-frontier table the median was 1.14× and 34 of 60 exceeded; at 159,027 it was 0.99× — small-n excess that the singular series predicts should recede, and it has). So δ₀ is a guarantee for one specific balanced shape and not a description of typical behaviour; "generic ceiling" is the wrong reading of it. What *is* an upper bound, and holds without exception, is cap_F(η) evaluated at the configuration's own F and η: **0 of 796,763 rows exceed it**, and over the **423,102** fused-class-plus-foreign winners the maximum observed is **0.171573 at n = 999,685** (`2x292801 + 1x414083*`) — **cap₂(1) = 3 − 2√2 to six places**, so that ceiling is not merely an upper bound but an attained one. *(This is the row whose six-decimal density column rounded across the cap and produced the checker's one-in-a-million false positive, `pending-checks.md` A20b.)*
...
**The prediction shows up in the table, on both of its halves.** Two forces act on the observed floor in opposite directions, and separating them is the point. Extending the range lowers it, since a wider range can only find weaker values. Adding configurations raises it, since B(n) is a maximum — so a shape-space correction lifts the whole curve at once. Over the contiguous range [6, 10⁶] the floor is **175813/3804661 = 0.046210 at n = 2759** (`2x653 + 1x1453*`); on the earlier 2,600-frontier table it was 0.048039 at n = 2183, and extending the range past 2759 lowered it exactly once, to the ladder's own global minimum, after which no value below 10⁶ came within 0.0018 of it. So the *within-table* floor moved up when the space was corrected and down when the range was extended, while the mechanism below — which is about which n the engines reach, not about how well they are scored — is unaffected. The thirds of the range behave as the argument requires:

| n | ω(n) = 2 share (proxy) | n/Q(n) ≤ 25 share | median cofactor n/Q(n) over ω = 2 | min density |
|---|---|---|---|---|
| [6, 333,334) | 33.9% | 32.1% | 9 | **0.04621** |
| [333,334, 666,667) | 30.6% | 27.3% | 9 | 0.06654 |
| [666,667, 10⁶] | 29.6% | 26.0% | 11 | 0.06679 |
...
**The observed density floor should drift downward — and then stop.** Over [6, 10⁶] **28.4%** of the table has n/Q(n) ≤ 25 and S2 actually wins at **13.5%** (it won 34% at the 2,600 frontier and 16.5% at 159,027; the ω(n) = 2 proxy reads 31.3%, was 55.4%), so the engine whose reach thins like 1/log n is already serving one row in seven; the median density is 0.1713 (was 0.1994). The evidence is two-sided and worth keeping apart: *within* the exactly-computed range extension lowered the floor exactly as predicted — from 0.048039 at n = 2183 to 0.046210 at n = 2759 the moment the range passed 2759 — while shape-space repairs lifted it (to 0.045742 at n = 1817 under v4, to 0.048039 at 2183 under the entangled-generator correction), corrections to the scoring and not counter-observations. But past 2759 the drift *stopped*: no n in (2759, 10⁶] comes within 0.0018 of the floor, and the per-third minima rise (0.04621 / 0.06654 / 0.06679). That is the §5 recovery — the class-23 additive ceiling taking over as representations near the balance point become plentiful — showing through inside the exactly-computed range rather than only in the ladder's extrapolation of it.
...
> **The finite-range minimum is set by whichever engine happens to be weakest at the extreme value, and that alternates.** Over the contiguous range [6, 10⁶] the smallest density is

> **175813/3804661 = 0.046210 at n = 2759 = 31·89, witness `2x653 + 1x1453*`** — the exact table now covers the whole range the ladder scan once bounded, and contains the argmin rather than stopping short of it; the runner-up is 0.048039 at n = 2183 (`6x251 + 1x677*`), and nothing else below 10⁶ is under 0.05 —
...
One criterion replaces the three that might be imposed separately, and it is strictly sharper than their conjunction. It gives k ≤ 1/√δ₀ (Proposition F.1) by taking every F_i = 1, and it gives **F ≤ 1/δ₀** — note this is *not* 1/√δ₀, which is the natural-looking but wrong bound: at δ₀ = 1/9 a single class of **nine** fused blocks is feasible. The tightest instance in range is `2x499979` at n = 999,958, where √2 = 1.4142136 against 1/√δ = 1.4142142, a slack of **0.0000006** — the Theorem 2.1 family n = 2·(prime) sits *on* the boundary and approaches it as n grows (at n = 2594, `2x1297`, the slack was 0.0003); the fused single classes at large F are the next tightest; `19x61` is admissible at n = 1159 with slack 0.034, though that n is won by `4x173 + 1x467*` at 0.088683. (The floor value — 0.046210 at n = 2759 — is lower but its winner's fused class is small, so it is not the binding test — the criterion binds where F is large *relative to* δ, not where δ is least.) **Checked against every winner over [6, 10⁶]: 0 of 921,265 violate it.**
...
3. **Predict the 1/12 shortfall from the singular series.** The odd/even split is **11.7% of odd (49,183 of 421,284) and one single even value (of 499,981)** below 1/12, over [6, 10⁶] — at the 2,600 frontier it was 6.7% and 0.08%, and 10.0% at 159,027, so the odd shortfall keeps *growing* with the range while the even one has vanished, which is itself the prediction to test: even n have the 1/4 class with a two-part shape always in supply, odd n pay the fusion count. Both engines' availability is computable heuristically, so this compares the whole framework of this document against measurement rather than testing any single family.
...
6. **When does the single fused class lose to a split?** Over [6, 10⁶], S2 wins at 124,502 values and loses to a two-part configuration at the rest; the natural predictor is the cofactor F = n/Q(n), since S2 is worth ≈ 1/F and a split near the class ceiling is worth δ₀ ≈ 0.07–0.25. The distribution of n/Q(n) over the integers is classical, so predicting the S2 win count from it is a clean test. *(The earlier form of this item sliced by ω(n) = 2 — 738 / 472 at the 2,600 frontier, 23,736 / 28,146 here — which was the pre-entangled reading of when a fused class exists; seven S2 winners have ω(n) = 3, and 98,870 ω(n) = 2 values have cofactor above 25, so the slice is the wrong one. Slice by n/Q(n).)*

...
> *The floor rows want recomputing as the table extends*, and are keyed to the exactly-computed floor rather than the conjectured one — **⟦PENDING-REBUILD⟧**. The recomputation apparatus, and which rows it touches, is in `shape-counting.md` §3.
...
> **N_add(δ₀) = Σ_{k=2}^{K} k = K(K+1)/2 − 1,  K = ⌊δ₀^{−1/2}⌋** — **quadratic in L, hence ≈ 1/(2δ₀)**. *(k = 1 is excluded: an additive one-part shape with a foreign part is n = r itself, a prime, which is outside the domain. The counts below are stated inclusive of it and so are one high at every row; the growth rate is what the section uses.)*
...
**Hence the fusion shapes drop from the asymptotic statement, and the reason is a bounded cofactor rather than a bounded ω(n).** In the single-class case the argument is now one line from §2.1's identity: δ_S2(n) = (Q(n) − 1)/(n − 1), so δ_S2 ≥ δ₀ holds exactly when Q(n) ≥ δ₀·n — that is, when n lies in the finite union ⋃_{F ≤ 1/δ₀} F·{prime powers}, a **density-zero** set. The general fused shape is no better off: any F_i > 1 needs F_i equal blocks of prime-power size, so n again carries a prime-power divisor at a cofactor the feasibility criterion bounds. Fused winners — winners with any F > 1 — are 59.2% of [6, 159,027], of which 42.7% pair a fused class with a foreign prime (the single fused class, S2, is the other 16.5%); the S2 share is what recedes with the prime powers rather than with ω(n), and it has — from 34% of the 2,600-frontier table to 16.5% here — while the fused-plus-foreign share has grown from 27% to 43%, since a fused class beside a foreign prime needs only c a prime power, not n. This is what licenses the N_add count of §6.4.

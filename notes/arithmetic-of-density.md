...
| **S10** | outside block with r = q | — | never at F = 1 | at F = 1 never exists — normality kills the twist. **F ≥ 2 is open** and is the residue of D2's domination argument (`enumeration-proof.md` Part D2) | — |
...
| S8, S10 at F = 1 | cannot occur | excluded by D1 and normality |
...
> **These are real minima, and the early-exit trap is worth naming.** `ladder_verify.py` may stop scanning once a value clears a threshold, and with that threshold set at 0.9 × the class cap it returns 0.9 × cap verbatim rather than the true minimum — six consecutive blocks all reporting 0.04546 = 0.9 × 0.05051 is the signature, an artefact sitting exactly where the signal is. With the cutoff at the asymptotic constant instead, nothing below 0.050510 is truncated, and the rise above it is direct evidence for §4's prediction that the envelope climbs as the ω(n) = 2 population thins.
...
*Reading the conjecture.* The asymptotic constant is η/(1 + k√η)² at η = 1/6, k = 2 — the balanced family at the one residue that is both doubly obstructed and stuck on the unfused rung. The other half of n ≡ 11 (mod 12) reaches 0.06699 (§3.3). The asymptotic half says the *worst* n eventually reach what the balanced family guarantees; it is a floor, and individual n exceed it freely.
...
One criterion replaces the three that might be imposed separately, and it is strictly sharper than their conjunction. It gives k ≤ 1/√δ₀ (Proposition F.1) by taking every F_i = 1, and it gives **F ≤ 1/δ₀** — note this is *not* 1/√δ₀, which is the natural-looking but wrong bound: at δ₀ = 1/9 a single class of **nine** fused blocks is feasible, and the tightest instance in range is `19x61` at n = 1159, where √19 = 4.359 against 1/√δ = 4.393 — essentially *on* the boundary. (The floor value 0.045742 at n = 1817 is lower but its winner is unfused and nowhere near the bound, so it is not the binding test.) **Checked against every winner in the table: 0 of 1,666 violate it.**
...
> *The floor row wants recomputing as v4 extends.* At the current floor of **0.045742** (n = 1817) one has L = 4.676, k ≤ 4 and F ≤ 21 — the k and F columns unchanged, and N(δ₀) at that L a little above 83, not recomputed here. Recompute the row rather than the whole table.
...
**Measured, over v4.** Scoring every unequal odd-p configuration at full diagonal twist: **654 of 1,666 values admit one, none wins, none comes within 10% of B(n)**, and the best ratio anywhere is 0.236·B at n = 1007. Together with the earlier observation that no winner in the table has two matching classes of different sizes, that is the empirical side; the ceiling above is the structural side, and it says why.

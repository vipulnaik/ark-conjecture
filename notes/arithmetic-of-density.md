...
**And it forces a shifted-prime statement of the same shape as the ladder's rungs — the θ = 1 counterpart of Elliott–Halberstam.** This is `enumeration-proof.md` Proposition F.4, and it is the precise sense in which the reduction runs both ways. Suppose μ(n) ≥ δ₀·C(n,2) for all large n. Because the score is a *minimum* over orbitals, every part of the winning configuration must clear δ₀·C(n,2) on its own, and a part can only do that by being large **and** carrying a large twist. Chasing the inequalities gives, with **D = 2/δ₀**:

> for almost all n there is a prime **r ≥ δ₀n** together with a **prime-power divisor q^e of r − 1** for which **(r − 1)/q^e ≤ D**

— together with the additive statement that n decomposes with r as a part. The "almost all" is doing real work but is not a hedge: the alternative is an all-matching configuration, which forces n = M·p^b with M ≤ 1/δ₀, and such n have density zero. (Density zero **slowly** — of order H_M/log n, so at n ≤ 10⁵ some 44% of integers are still within a factor 25 of a prime power. The exclusion is asymptotic and no computation will display it.) In that branch the conclusion survives with r − 1 replaced by p^b − 1 and D by 2/δ₀², so a statement holds unconditionally; it is only the clean *prime* form that needs the exceptional set removed.

**So (H) and a density floor are close to equivalent, with constants lost in both directions.** (H) asks for r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350. The converse takes δ₀ back to a bounded cofactor D = 2/δ₀, so the round trip from (H) returns not d ≤ 12 but D ≤ 700, and through the all-matching branch D ≤ 245,000. Three differences survive the round trip and are worth naming, since they are what "close to" is hiding: the converse yields a prime-**power** divisor where (H) supplies a prime; it yields "for almost all n" where (H) is stated for all; and the cofactor bound loosens by the square of the density constant. **What does not degrade is the shape of the statement** — a shifted prime with a large prime-power divisor, at bounded cofactor rather than at exponent θ < 1 — which is exactly the quantity §3.6's ladder measures and exactly where E–H stops short. The framework is therefore not merely *implied by* a shifted-prime hypothesis; at the θ = 1 end it is **equivalent to one up to constants**, which is the strongest reason to think no substantially weaker arithmetic input can give a positive density floor.

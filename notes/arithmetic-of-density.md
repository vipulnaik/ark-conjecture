...
| global floor δ ≥ 1/25 (**⟦PENDING-REBUILD⟧** exact value from the current scan) | ladder range | computed (§5), from the four-family ladder alone |
...
**⟦PENDING-REBUILD — scoped⟧** *This table is the **worklist scan to 10⁶**, a different artifact from the μ table, and it has not been rerun. What the completed μ table settles is only its overlap: over [10³, 2600] the minimum is **0.048039 at n = 2183**, and the `0.04574 at 1817` entry does not survive the corrected cap, that row now scoring ≥ 0.0594. Whether 1817 remains the decade minimum out to 10⁴ is a question for the rerun, not for this table. The per-block floor table and the block-by-block recovery narrative are outputs of the superseded scan; regenerate them from the rerun before quoting any row.*
...
> **δ ≥ 0.04453, at n = 11183.** **⟦PENDING-REBUILD⟧** *Pre-correction scan; R7 requotes it. The corrected-scoring run reaches n ≤ 10⁵ so far with floor **0.04621 at n = 2759**, so the figure is expected to rise — `ladder_verify.py`'s families understate, which is why the 1/25 conjecture is unaffected either way.*
...
| 0.04453 (a representative ladder-floor value; **⟦PENDING-REBUILD⟧**) | 4.739 | 4 | 22 | **122** |

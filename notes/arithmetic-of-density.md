...
> **Status of the computed range: two contiguous tables, agreeing wherever they overlap, and B = μ at every row by theorem.** `mu_exact.py`'s exhaustive search is contiguous over every eligible n — composite, non-prime-power — in **[6, 55,814]**, all **50,062** of them; `mu_ladder_exact.py` (menu + S6/S11 scored exactly, certified above C(n,2)/25 by Theorem E.5 and `ladder-completeness.md` Proposition 1) is contiguous over **[6, 159,027]**, all **144,299**, with **no uncertified row**. The two agree on the integer `mu_bound` at all 50,062 common n (`validate_table_v3.py --baseline`: ALL equal, 0 higher, 0 lower; 95 recorded witnesses differ, all ties). Since every row clears 1/25 — the minimum over both is **175813/3804661 at n = 2759** — Corollary E.6 makes every value μ(n), not a bound on it. The R1 battery passes on both (26 PASS / 0 FAIL). Further extension is **discretionary**; `mu_ladder_exact.py` runs to 10⁶ in ~3 h single-threaded. *The prefix/tail discipline is dormant rather than retired:* it applies again the moment an extension fills higher n from a worklist. **Measured figures in these documents are scoped to the 144,299-row range [6, 159,027] unless a narrower range is named beside them.**
>
...
>
> **The ⟦PENDING-1E5-EXACT-RUN⟧ tag is retired.** It marked measured figures whose *scope* was stale — taken over the older contiguous range [6, 2600], or the 36,848 one — pending a run past 10⁵. That run is the 159,027-row ladder-exact table, legitimate for the purpose because Corollary E.6 makes its certified values μ(n) exactly; every tagged aggregate has been recomputed against it and requoted in place with its range stated. The convention it enforced remains: **any winner count, share, median or tail figure names the range it was measured over**, and `check_doc_figures.py --pass scope` flags a range-scoped claim that does not. Structural claims, closed forms, ceilings and theorems quantify over all n and were never in scope.
>
> **The ladder run is also complete, so both numerical bottlenecks are cleared — and the ladder now carries more than a floor.** `ladder_verify.py` to 10⁶ under the corrected scoring: **44,091 worklist entries, floor δ ≥ 175813/3804661 = 0.046209898… attained at n = 2759, nothing anywhere below 1/25**, and the adaptive run against a floor of 0.04 returns an empty below-floor list — so the conjecture's finite half needs no μ(n) computation at all in that range. **Since that floor exceeds 1/25, Corollary E.6 of `enumeration-proof.md` Part E‴ upgrades it: μ(n) = B(n) at every eligible n ≤ 10⁶, by theorem, with no B(n) computed anywhere in the argument.** The ⟦PENDING-LADDER-REBUILD⟧ tags have been discharged.
>
...
>
> **The numerical rebuild is done.** ⟦PENDING-REBUILD⟧ figures were read off a table predating the
> current SAFE cap; `mu_exact.py`'s completed run has replaced it, and `check_doc_figures.py` against
> that output is the pass that requotes them. What remains tagged is only what a *script* has still to
> regenerate — certificate coverage counts and shape-scan totals — plus the one distributional figure
> the CSV cannot supply (the orbital count t is not a column). Unmarked figures from the current table
> are correct as lower bounds and may rise on any extension. Structural and asymptotic claims
...
**Which hypothesis each constant depends on.** Every ceiling in this document is row 1 of the table in `orbital-evasiveness-notes.md` §1 ("Which hypothesis is doing which work"): the six constants come from the **shifted-prime condition**, which is Oliver's chain and not solvability, while the existence of a full-density block — the thing being optimised — comes from **2-homogeneity at k = 2** and not from the chain. Relaxing the chain collapses the six constants to two (`solvable-relaxation.md`); raising k removes the block instead. The two are independent, and no constant here survives the first relaxation unchanged.
...
> **The multiplicative engine, in closed form.** A single **fused class** — F blocks of size c with n = F·c, c a prime power and F > 1 — achieves density **1/F**, and after the entangled-generator correction F is an arbitrary integer, carried by the cyclic layer. So the shape is available at **every n that is not a prime power** (take c = Q(n), the largest prime-power divisor, and F = n/Q(n) > 1), with no condition on ω(n) at all. Optimising over c gives an identity rather than a bound:
>
> **δ_S2(n) = (Q(n) − 1)/(n − 1)** for every non-prime-power n.
>
> It is the only structure that breaks density 1/4, and it *wins* only when the cofactor n/Q(n) is small — which is where the density-zero statement of §6.5 comes from, not from any bound on the number of prime factors.
>
...
4. **One global floor covers everything, over exactly the domain where the multiplicative engine exists.** Conjecturally **δ(n) ≥ 1/25 for every composite non-prime-power n** — the excluded values being precisely those with no S2 reading at all, by item 1's identity, so the domain is structural rather than stipulated — — and below 10⁶ this is **verified unconditionally** by the four-family ladder (§5). *Current run (corrected scoring, complete to 10⁶): floor **0.04621 at n = 2759**, with no entry below 1/25 anywhere. The interim figure from the corrected scoring is δ(n) ≥ 0.0462 at every composite non-prime-power n ≤ 10⁵, attained at n = 2759.* The bound is a lower bound at every step, so it needs nothing from the table. The extremal residue is **n ≡ 11 (mod 12)** (§3.3), which is where the search has landed at every stage.
5. **The multiplicative engine covers a density-zero set, so asymptotically the additive engine is everything.** The engine *exists* at every non-prime-power n (item 1's identity), so what thins is not its availability but its value: δ_S2 ≥ δ₀ demands Q(n) ≥ δ₀n, and a bounded cofactor times the prime powers is a density-zero set. So the asymptotic behaviour of μ is governed entirely by the Hardy–Littlewood side, and the observed density floor should be expected to drift downward as the fused family's reach recedes.
...
> **One computed instance of each live shape**, from the current table (**⟦PENDING-REBUILD⟧**), so that every row below can be checked against a concrete configuration. The starred part is the foreign block.
>
...
> | S4 | — | no winner in the computed range | — | — |
> | S5 | n = 475 | `p=109 q=2: 1x257* + 2x109` | 11772 | 0.104570 |
> | S7 at F = 2 | n = 99 | `p=23 q=13: 1x53* + 2x23` | 506 | 0.104308 |
> | S7 at F ≥ 3 | n = 143 | `p=2 q=23: 3x32 + 1x47*` | 1081 | 0.106471 |
>
> S6 has no instance in the computed range, and S4 none either — the fused rung beats it at every c. The pair to compare is **S5 against S7 at F = 2**: both are `2×c + r*`, distinguished only by the top prime. The twist is full at either q, so the two differ only in foreign efficiency, and S5's forced q = 2 makes it the dominated reading at odd q.
...
> **Frontier.** The table is **complete over all 144,299 eligible n — composite, non-prime-power — in [6, 159,027]**: no gaps and no worklist-driven tail, so every aggregate below is a genuine range aggregate rather than a prefix statistic. *The prefix/tail discipline stays live as a method, not as a caveat on these numbers:* an extension that fills higher n from a worklist re-creates a biased tail (the worklist selects by low score), and aggregates would then have to be requoted over the contiguous part only. Any figure quoted after such an extension needs its range restated.
>
> **Provenance of every measured figure in this document.** Structural columns, closed forms and asymptotic verdicts are stable and can be read as stated. **Winner counts, percentages and per-shape medians are not** — they are measurements of the current table over its **contiguous range** [6, 159,027], they move with every extension, and they are what `validate_table_v3.py` prints back for copying. Two gotchas govern them. **A count taken over the whole CSV is not a count over the range** whenever the CSV carries a worklist tail: such rows are selected by low ladder score, so they skew every share (on the 2,600-frontier table they held 4 of the 20 three-part winners and 21 of the 71 F = 4 ones). The current tables have no tail. And **the winner column is sensitive to the SAFE cap**, which is not shape-neutral: crediting a fused class F·C(c,2) regardless of its twist inflates fused shapes specifically and can empty a census row that a tightened cap restores.
...
| **S2** | fused matching class, n = F·c with **F > 1** | multiplicative | **743** (34.0%) | **exists = 1**; **wins → 0** — the two limits come apart completely here. It exists at every non-prime-power n (F is an arbitrary integer), with δ_S2 = (Q(n) − 1)/(n − 1); it wins only where the cofactor n/Q(n) is small | §2.1, §4.1 |
| **S3** | matching + outside, n = c + r\* | additive | **853** (39.0%) | **exists → all even n *plus a positive proportion of odd n***; **wins → all six even classes**. All even n conjecturally, and at odd n the shape survives with c = 2^a, giving n = 2^a + r\* — which Romanov puts at positive lower density and Erdős's covering congruences keep bounded away from all of odd n. So this is the one row where existence strictly exceeds winning by a positive density rather than by a rate: the odd instances exist but are almost never good enough near the balance point to win, and even on the even side existence converges faster than winning | §3.1, §4.3 |
| **S4** | two matching + outside, n = 2c + r\* | additive | **0** — empty on the completed table; the v4 baseline's 16 all migrated to the fused two-part rung | **exists → 12/24** (all odd n); **wins → 0**. The cyclic-layer fused rung beats it at **every** c, since 2·C(c,2) > C(c,2) with no congruence in play (§3.2.3), so it is dominated inside the three-part family as well as outside it. **The widest existence/winning gap in the census**: available at essentially every odd n and beaten everywhere | §3.2 |
| **S5** | **top-layer**-fused matching + outside; forces q = 2, hence η = 1/u | hybrid | **21** (1.0%) | **exists → 12/24** (all odd n — q = 2 fusion is always available, just usually at a useless η); **wins → 0**. An escape: winning needs η = 1/u with u small, i.e. r = 2^a·u + 1, which is O(log n) candidates per n and so O(n/log n) values. *The share falls in **bursts**, not monotonically, and a decline test applied to it will fire spuriously: one usable r serves a window of n proportional to r, so a single prime such as r = 12289 = 3·2¹² + 1 supplies a run of consecutive winners (59 of the 80 winners on the exact table, running out to n ≈ 2.4×10⁴) and the share then sits flat until it is exhausted. The limit is 0 as a liminf-and-Cesàro statement; the sequence of shares is not decreasing* | §3.3, §4.3 |
...
| **S7** | **middle**-layer-fused matching + outside; **F = 2 is the odd-n fused rung B** | hybrid | **395** at F = 2; **58** at F = 4; **102** at F = 3, **14** at F = 6 | **F = 2: exists → all odd n**; **wins → five of the six odd classes**, n ≡ 1, 3, 5, 7, 9 (mod 12), whose class ceiling it attains. At n ≡ 11 (mod 12) the ceiling belongs to F = 4, so F = 2 does not win there even though it attains the three-part family's own best. **F ≥ 3: exists → every n; wins → one odd class at F = 4** (n ≡ 11 (mod 12)) **and two even classes at F = 3** (n ≡ 2, 8 (mod 12), where it ties with S3). Two regimes, split by the parity of F — and the odd-F regime splits again by the parity of **n**. At **odd F and odd n** the shape n = F·c + r needs F·c even, hence c = 2^a: O(log n) block sizes per n, a genuine escape reaching O(n/log n) values, and it wins nowhere. At **odd F and even n**, c is an ordinary odd prime, the supply is a full Hardy–Littlewood system, and the escape reasoning does not apply. There F = 3 loses at the unobstructed even classes, where S3's cap 1/4 beats 0.13397 — but at **n ≡ 2, 8 (mod 12)**, where the ℓ = 3 obstruction cuts S3 to η = 1/3, **cap₃(1) = cap₁(1/3) = 0.13397 exactly**, so F = 3 at full efficiency is a **co-winner of those two classes** and its winning share is a positive proportion of all n rather than 0. Odd F ≥ 5 is excluded at even n too, by cap₅(1) = 0.09549 < 0.13397. At **even F** there is no restriction at either parity: at odd n, F·c is automatically even and c ranges over all prime powers, so the supply is a full Hardy–Littlewood system. **F = 4 attains the class ceiling at n ≡ 11 (mod 12)** (§3.3.5), where it beats every F = 2 rung because 4c ≡ 4 rather than 6 (mod 8) pins r and removes the 2-adic cut on the foreign efficiency | §3.2, §3.3.5, §4.3 |
...
| **S11** | one matching class + **two** outside blocks, n = c + r₁\* + r₂\* | hybrid, two foreign | **0** | **exists → thin** (both r_j − 1 = k_j·q^{e_j} at a common q with k_j ≤ 12); **wins → 0**: it caps at **1/16**, attained only at (η₁, η₂) = (1, ½) with r₂ = 2r₁ − 1, below every class ceiling. Together with S6 it is the only live shape outside `ladder_verify.py`'s menu, which is why it is named — a ladder shortfall above 1/25 has to be one of the two | `ladder-completeness.md` §3 |
| **S12** | two matching classes of **distinct** block size, same p | multiplicative | **0** | **exists**, **wins → 0** and never at δ > 1/25: the merged fused class of F = F₁p^{a−b} + F₂ blocks of size p^b dominates term by term and is admissible above the floor. It is a shape, not a family of its own | `ladder-completeness.md` §2 |

> **The verdict column reports two different limits, and they can differ sharply.** **Exists** is the proportion of n at which *some* admissible configuration of the shape can be built. **Wins** is the proportion at which one attains B(n). For S1, S2 and S6 the two coincide because the shape is rare and dominant where it occurs; for S8 and S10 both are zero by theorem, and for S9 existence is positive while winning is zero. For the odd-n family they diverge: S4, S5 and S7 at F = 2 all exist at essentially every odd n, and which of them wins is decided by the ceiling comparison of §3.3, so their winning shares partition the odd classes while their existence shares all equal 1/2. Even where the two limits agree, existence converges faster — a representation becomes available well before it becomes good enough near the balance point to beat everything else.
>
...
> **S4 and S7 at F = 2 are not co-carriers: the fused rung dominates.** Both realise the same family n = 2c + r, and the c mod 8 law that once split them between the two is void — a cyclic-layer fusion keeps the full twist at **every** c (§3.2.3), so 2·C(c,2) > C(c,2) with no congruence in play and S4 wins nowhere. **The fused rung here is S7 at F = 2, not S5.** S5 fuses by a *top-layer* involution, which keeps the full twist and obeys no congruence on c at all, but forces q = 2 and so is rationed by the foreign block instead — which is what makes it an escape rather than a carrier. The analysis is in §3.2.
>
...
Let n = F·c with c a power of p and F a fusion count (F = F_mid·F_top, only F_top a q-power). The single fused class has intra-orbital F·C(c,2), within-class cross (F or F/2)·c² — F for odd F, F/2 for even F — and no other terms, so

> m\* = F·C(c,2) = F·c(c−1)/2, and **δ = m\*/C(n,2) = (c−1)/(Fc−1) → 1/F.**

Two things follow immediately. Since a fused class needs F ≥ 2, the engine cannot exceed **1/2**. And since F is an arbitrary integer carried by the cyclic layer, the shape needs only that n have *some* prime-power divisor c > 1 with n/c > 1 — that is, it is available at **every non-prime-power n**, with no condition on ω(n).

To maximise, take c to be **Q(n), the largest prime-power divisor** of n, and F = n/Q(n) — which is what makes δ_S2 an identity rather than a bound.

> *Verified over [6, 159,027].* Over all **23,743** one-part winners, the predicted density 1/F agrees with the computed value to O(1/n), with no exceptions — |δ − 1/F|·n < 0.95 on every row, and every row matches the exact form (c−1)/(Fc−1). By F: 7,875 rows at F = 2 (median 0.5000), 5,486 at F = 3 (0.3333), 4,244 at F = 4 (0.2500), 3,462 at F = 5 (0.2000), **1,863** at F = 7 (0.1428); maxima 0.50000, 0.33333, 0.25000, 0.19999, 0.14285 against 1/F. The remaining **813** sit at F = 6 (7), 8 (40), 9 (67), 11 (398), 13 (300) and 17 (1) — F = 10 and F = 19 do not occur, and the only F that is not a prime power is 6.

> *Verified over [6, 159,027], with one correction the corrected shape space forces.* Of the 23,743 one-part winners, **23,736 have ω(n) = 2 and seven do not**, all of shape `6x p` (the five below 2,600 are n = 78, 222, 282, 894 and 1434). **The exceptions are exactly the composite-F fusions**: F = 6 is available only because F_mid need not be a prime power, and n = F·c then carries three prime factors. Under a shape space with F a q-power the statement "every one-part winner has ω(n) = 2" is a theorem — n = F·c with c a p-power has ω(n) ≤ 2 — so this is a place where the correction changes a structural claim and not only a count. Of the **51,882** values with ω(n) = 2, **23,736** are one-part winners and the other **28,146** do better with a split — but ω(n) = 2 is the wrong slice for that question, being neither necessary (the seven F = 6 winners) nor sufficient (15,255 of those 51,882 have n/Q(n) > 25 and could not clear 1/25 by fusion at all); §4.1 and §8 item 6 put it in terms of the cofactor n/Q(n).

**Why fusion is worth a factor of F.** F *unfused* equal parts of size c give min(C(c,2), c²) = C(c,2) ≈ n²/(2F²), density 1/F². Fusing them replaces the mutual capping by a single intra term F·C(c,2), density 1/F. So fusion buys exactly F, which is why reduction (R1) of the proof document — merge equal-size classes whenever the merged count is admissible, as a q-power in the top layer or under the coprimality budget in the cyclic one — is the single most valuable simplification in the search, and why the enumeration's winners are so often a single fused class.

**Terminology, since the next few sections depend on it.** A configuration is n = Σᵢ Fᵢcᵢ. Each summand is a **class**: Fᵢ blocks of size cᵢ, with cᵢ a prime power and **Fᵢ = F_mid,ᵢ·F_top,ᵢ** — only F_top a power of the top prime q, F_mid carried by the cyclic layer subject to its coprimality budget. So a fusion count is any integer, and composite F occurs (`6x p` at n = 78, 222, 282, 894 and 1434). A class is **unfused** when Fᵢ = 1 — a single block. The `parts` column of the table counts **classes**, not blocks, and Proposition F.1's k is that count, whether or not the classes are internally fused.

So fusion is an axis *within* a configuration rather than a separate kind of configuration, and the two engines above are a first approximation rather than a dichotomy. Mixed shapes are the norm rather than the exception: **493 of the 1,590 two-class winners (27.0% of the table)** pair a fused class with an unfused foreign prime. The minimum over the contiguous range [6, 2600] is n = 2183 at 0.048039, witness `6x251 + 1x677*` (the global minimum over n ≤ 10⁶ is lower, 0.0462099 at n = 2759 — §5) — also a fused class plus a foreign prime; v4's minimum n = 1817 rises to ≥ 0.0594 under the corrected caps (`3x256 + 1049*`). (No three-class winner contains a fused class.) Fusion carries a cost only through the layer that supplies it, and which layer that is decides everything: a **top-layer** factor F_top constrains q, since F_top must be a q-power and the foreign twist is then a q-power of the same q; a **cyclic-layer** factor F_mid constrains nothing on the top prime at all, leaving the foreign efficiency free. The witness at n = 1817, `1x1039* + 2x389` at q = 173, is the second kind — F = F_mid = 2 with q odd, which a q-power-only reading of the fusion count cannot produce.
...
> *Verified over [6, 159,027], and tight.* Maximum observed density among two-part winners is **0.24999** and **no two-part winner exceeds 1/4**, over all 120,556 of them; median 0.1695. The three-part half of this check has no population left to run on — the family is empty on the completed table (`enumeration-proof.md`'s census) — so the 1/9 bound for three parts is now a statement about a shape that exists and never wins, not a measured margin.
...
- **§§3.5–3.6 say what is actually being assumed.** §3.5 draws the distinction the rest of the section relies on — these are *parametric* Goldbach-type systems, not fixed Bateman–Horn ones — and states **Hypothesis (BCG)**, the single hypothesis everything conditional in this document rests on. §3.6 places (BCG-AL) on the ladder of shifted-prime results, where it is the θ = 1 endpoint against a current unconditional 0.679.
...
**One n of each, with the terms.** The clearest way to see that these are three genuinely different shapes is to score all three readings at the same n and watch a different one win each time. Every row is n = 2c + r, and the bold entry is the value `mu_bound` records:
...
| **531** | c = 137 ≡ 1 (8)<br>r = 257, r−1 = 2⁸ | 9316 — binds on **intra** C(137,2) | **274** — r − 1 has no odd prime factor, so at any odd q the foreign twist is trivial and orb(257, 1) = 257 binds; the rung exists and is worthless | **18632** — binds on **intra**, 2·C(137,2) | **18632 = min( 2·C(137,2), 137² ) = min(18632, 18769)**, the foreign and cross terms being 32896 and 70418. The two live terms differ by **0.7%**, but that near-tie is *structural* rather than a balance effect: a fused class always has intra = c(c−1) against within-class cross = c², so they sit a factor c/(c−1) apart at every c |
...
The bold values are what `mu_table_safe_v5_code_v3.csv` records, under the witnesses `p=137 q=2: 2x137 + 1x257*`, `p=83 q=53: 2x83 + 1x107*` and `p=73 q=53: 2x73 + 1x107*`. Note what the n = 531 and n = 253 rows show together: **both** reach the full twist at c ≡ 1 (mod 8) — one through the top layer and one through the cyclic layer. The two layers agree, which is the content of §3.2.3 in one line.
...
|---|---|---|---|
| **q odd** (cyclic-layer, S7 at F = 2) | 395 | **no congruence: c mod 8 = {1: 92, 3: 100, 5: 101, 7: 96}**, i.e. **193 at c ≡ 1 (mod 4)** against 196 at c ≡ 3, plus 6 at c even (p = 2) | unrestricted |
| **q = 2** (top-layer, S5) | 21 | 8 / 6 / 5 / 2 across 7 / 5 / 3 / 1 mod 8 — **no congruence** | only **1** (16 rows) and **3** (5 rows) |
...
> **Recounted over [6, 159,027]**, all 44,665 cyclic-layer winners at F = 2 (44,467 with odd c; the 198 others are 2-power blocks). The c mod 8 distribution is **{1: 11,105, 3: 11,306, 5: 10,983, 7: 11,073}** — flat to within 3%, and **49.7% of the odd-c rows sit at c ≡ 1 (mod 4)**. The prediction was that the congruence would disappear rather than merely weaken, and at four residues within a few percent of each other over forty-four thousand winners it has: what remains is sampling noise, not a residual preference.
...
> **η = 1 has two sources, and the second is easy to miss.** The formula reads η = 1 as "u = 1, or a = 1 with u a prime power". The second disjunct is the familiar one — r a safe prime or r = 2q^e + 1 — and it is tempting to treat it as the only one and conclude that 4 | r − 1 caps η at 1/2. It does not: at u = 1 the efficiency is full for **every** a, because the whole of r − 1 is then a 2-power and q = 2 reaches all of it. These are the **Fermat** primes. In the computed table the branch produces **16 winners of shape `2×c + 257*`** over the completed table, with densities from 0.10319 up to **0.16138** (n = 639, `2x191 + 1x257*`) against 0.08579 for the classes they sit in — at n = 475, 531, 555, 559, 583, 595, 639, 651, 679, 703, 711, 715, 735, 759, 795 and 799. (Four n that look like members are not: 451 and 819 are won by F = 4 shapes, `4x71 + 1x167*` and `4x139 + 1x263*`; 459 by `4x73 + 1x167*`; and 783 by `2x197 + 1x389*`.) `ladder_verify.py`'s `EFF` array computes η from r − 1 directly and so covers this case automatically; it is only in prose that the branch tends to get dropped.
...
>
> So B′ clears (5 − 2√6)/2 = 0.050510 — the **old class-23 figure**, which is cap₄(1/6), equivalently the unfused rung C at η = 1/6, and *not* cap₂(1/6) — exactly when **u ≤ 9**, i.e. when r = 2^a·u + 1 for one of five small odd u. Measured instead against the worst class ceiling the F = 2 rungs actually reach, cap₂(1/6) = (2 − √3)/4 = 0.066987, the condition tightens to **u ≤ 5**; either way the family is exponential and the conclusion below is unchanged. That is an exponential family: it supplies O(log n) candidate r per n, hence reaches O(n/log n) values of n by the counting of §4.3 — the same tier as the other escapes, and **it therefore leaves every ceiling in the table below untouched.** The observed u are 1 and 3 and nothing else: 16 winners at r = 257, 5 at r = 769 = 3·2⁸ + 1. The Fermat branch above is this rung at u = 1, which is why reading that branch as an O(1) phenomenon tied to the five known Fermat primes understates it — the family is r = 2^a·u + 1 with u a small odd prime power.
...
|---|---|---|---|---|---|---|---|
| 0, 4, 6, 10 | k = 1, no fusion question | S3 | 1 | **1/2** | 0.50000 | **1/4** | 0.25000 |
| 2, 8 | k = 1 **tied with F = 3** | S3, **and S7 at F = 3** | 1/3 (S3); 1 (F = 3) | **(√3 − 1)/2** | 0.36603 | **(2 − √3)/2** | 0.13397 |
| 1, 9 | **B alone** | S7 at F = 2 | 1 | **(2 − √2)/2** | 0.29289 | **3 − 2√2** | 0.17157 |
| **3, 7** | **B alone** | S7 at F = 2 | 1/2 | **1/4** | 0.25000 | **1/8** | 0.12500 |
| 5 | **B alone** | S7 at F = 2 | 1/3 | **(√6 − 2)/2** | 0.22474 | **5 − 2√6** | 0.10102 |
| **11** | **F = 4** | S7 at F = 4 | 1/3 | **(2 − √3)/2** | 0.13397 | **7 − 4√3** | 0.07180 |

> **The two obstructed even classes have a second attainer, and it is not an escape.** At n ≡ 2, 8 (mod 12) the ℓ = 3 obstruction cuts S3 to η = 1/3, giving cap₁(1/3) = (2 − √3)/2. The shape n = 3c + r with c an odd prime and r a **safe** prime is unobstructed there — mod 3 it forces the safe-prime residue rather than a vanishing singular series — and cap₃(1) = 1/(1 + √3)² is the *same constant*, by cap_F(η) = cap₁(Fη)/F at F = 3, η = 1. So the cap is attained by two structurally different shapes at these two classes, and the tie is exact rather than approximate. **This is the only place an odd fusion count sets a ceiling**, and it is confined to even n: at odd n odd F forces c = 2^a and is an escape (§4.3), while at the unobstructed even classes S3's 1/4 beats 0.13397 outright. Odd F ≥ 5 is excluded everywhere by cap₅(1) = 0.09549 < 0.13397. *Measured:* over n ∈ [15000, 30000] a congruence-free scan of the generic family returns sups of 0.13397 (F = 1, η = 1/3) and 0.13395 (F = 3, η = 1) at both classes, and on the exact table 1,261 of the 1,352 F = 3 winners are even n, every one at n ≡ 2, 8 (mod 12), taking about 37% of those two classes near n = 2×10⁴ with the share rising. *Gotcha: "odd F is always an escape" is true only at odd n, and the sentence's own arithmetic — F·c even forcing c = 2^a — is what silently selects that half.*

> **How fast the ceiling is approached.** These caps are suprema, attained only in the limit: at each n the loss is set by how close an admissible configuration sits to the balance point, and for class 11 that loss is **Θ(log³n/n)** with the leading constant given by the same singular series as §3.4. Worked out and tested against both the ladder to 10⁶ and exact B(n) to 10⁴ in **`approach-rate-note.md`**, which is kept separate because its distributional half is a Cramér-model heuristic rather than a hypothesis of the kind this document carries. Nothing here depends on it.
...
> **At odd n the fusion count must be even, and the search over F is finite.** With c odd and r an odd prime, n = F·c + r forces F even: odd F makes Fc odd and r = n − Fc even. The F-search then closes on an argument needing no arithmetic at all — since η ≤ 1, **cap_F(1) = 1/(1 + √F)²** bounds the entire F-slice, and that is below 7 − 4√3 for every F ≥ 8. So only F ∈ {2, 4, 6} can attain any of these caps, and §3.3.4a gives their efficiencies. **Gotcha: cap₄(1) = 1/9 is the absolute ceiling of the F = 4 slice, and it is not the ceiling of any class** — every class that could otherwise take F = 4 at full efficiency reaches 1/8 through F = 2 at η = 1/2 instead, since 2c ≡ 2 or 6 (mod 8) puts r ≡ 5 (mod 8) within reach. A row quoting 1/9 as a class cap has read an F-slice bound as a class bound.
...
>
> **At n ≡ 11 (mod 12), F = 4 wins**: the two F = 2 options both land on r ≡ 1 (mod 4), so with the ℓ = 3 obstruction the best F = 2 can reach is η = 1/6 and **cap₂(1/6) = (2 − √3)/4 = 0.066987 < 7 − 4√3 = 0.071797**. The margin is 0.0048, which is what §6.6's disjunction-collapse consumes. *Gotcha: 0.050510 = (5 − 2√6)/2 is **not** cap₂(1/6) — it is cap₄(1/6), equal to the unfused rung C at the same η, and it is the old class-23 figure. Quoting it here overstates the F = 4 margin by a factor of four; the next rung down from 7 − 4√3 is (2 − √3)/4, as §6.6 records.* The D = 24 obstruction is a property of F = 2 in that class.
>
> **At n ≡ 3, 7 (mod 12) it loses.** There the choice in c mod 4 puts r ≡ 5 (mod 8) within reach, so F = 2 attains η = 1/2 and cap₂(1/2) = 1/8, above the F = 4 value 1/9. Both mod-24 halves of each class reach it — which is what makes these two classes a single row of the table, and the odd part of it six constants rather than seven.
>
...
>
> *Confirmed independently of the congruence argument.* `ceiling_rederive.py --mod12` scans real configurations of the generic family — c a prime ≥ 5, twist a prime power at q ≥ 5, F even — and takes the empirical sup of density per residue class, with the four escapes of §3.3.8 excluded; `--no-filter` puts them back, and is the control showing the filter is what separates the two. Over n ∈ [45000, 90000] the sup at classes 7 and 15 is 0.124987 and 0.124985 against 1/8, attained at F = 2, η = 1/2 and c/n = 0.24999, 0.25001 against the predicted x\* = 1/4 — and **both witnesses sit at c ≡ 1 (mod 4)** — the branch a c-congruence would have excluded. At 11 and 23 the same scan returns 0.071791 and 0.071793 against 7 − 4√3 = 0.071797, attained at F = 4, η = 1/3, x\* = 0.13398 against 0.13397: **unchanged, as the F = 4 argument requires.** All eight other odd classes reproduce their tabulated constants to four places from below. *(Run without the escape filter and the sup exceeds the cap — on the shipped `--no-filter` log over n ∈ [8000, 16000], at classes 3, 5, 7 and 11 (mod 12), by 1.74× at class 11 — every time via c or oddpart(r−1) a power of 2 or of 3. Which classes fire is range-dependent, so requote against the log rather than carrying a class list forward; that they are §3.3.8's documented O(n/log n) routes is not, and their appearance is a check on the filter rather than on the table.)*
...
> **What a row hides, and why it is still one row.** The rows group residues sharing a cap, an η and a shape, and they group residues whose *foreign* residue differs — at (F = 2, η = 1) the mod-24 residues 1, 9, 13, 21 split into r ≡ 3 and r ≡ 7 (mod 8) yet share the n ≡ 1, 9 (mod 12) row. Every row is like this: a mod-12 class contains two mod-24 halves with different foreign arithmetic and the same cap. Two things the halves do **not** share, needed downstream:
>
> - **The Bateman–Horn system differs between a class's two mod-24 halves.** At n ≡ 11 (mod 12) the optimum needs r ≡ 7 (mod 24) on the half n ≡ 11 and r ≡ 19 (mod 24) on the half n ≡ 23 — the *ceiling* is the same, by the mod-12 law, but the singular series and hence the supply constants are computed per residue, not per row. §3.5's counting keeps them separate.
> - **The second-best shape is a tie of two rungs, and it is the same tie in both halves.** Both fall to (2 − √3)/4 = 0.066987 if the F = 4 optimum is unavailable, reached at **F = 2** with η = 1/6 and equally at **F = 6** with η = 1/2 — cap₂(1/6) = cap₆(1/2) = (2 − √3)/4 identically, by cap_F(η) = cap₁(Fη)/F. *An earlier form of this bullet gave F = 2 to the half n ≡ 11 (mod 24) and F = 6 to the half n ≡ 23, on the grounds that the latter's F = 2 rung is worthless at η = 1/12. That is a mod-24-era reading: with c mod 4 free (§3.2.3), r ≡ 5 (mod 8) is reachable at F = 2 in **both** halves, so both get η = 1/6 there. The row's own gotcha applies to it — a separation of a from a + 12 must come from a mod-8 condition, and every mod-8 condition here is constant on the class.* An argument needing the runner-up therefore needs the tie, not a split; §7's disjunction-collapse is one such, and the 0.0048 margin is unchanged. *Confirmed by scan rather than by the congruences that produced the correction:* `ceiling_rederive.py --runners` takes the sup per fusion count and per mod-24 half over n ∈ [30000, 60000], and returns **0.066981 (F = 2, η = 1/6) and 0.066978 (F = 6, η = 1/2) at n ≡ 11, against 0.066973 and 0.066972 at n ≡ 23** — both routes live in both halves, all four within 2·10⁻⁵ of (2 − √3)/4, with F = 4 at 0.071791 and 0.071788 above them and F = 8 far below at 0.0481. The per-F sup is what the ceiling scan cannot supply, since that maximises over F and hides a runner-up at another one.
>
> Empirically the two behave alike, as equal caps predict: over n ∈ [2·10⁴, 1.2·10⁵] their ladder medians are 0.07047 and 0.07035, their fractions below cap 64.6% and 65.6%, and the regenerated worklist splits 1823 : 1824. The older observation that low-δ values concentrate at n ≡ 23 was largely a **selection effect** — that worklist was thresholded at 0.050510, which was class 23's own ceiling under the *unfused* reading, so 23 appeared on merely falling short of that cap while 11 had to fall 25% below a higher one.
...
Every entry is a unit in ℤ[√d] over a small denominator, as it must be, since only F and η enter. The pairings come from **cap_F(η) = cap₁(Fη)/F**, immediate from the closed form: one rung's value at η is exactly half the next rung's at 2η. Visible here as **3 − 2√2** at (F = 2, η = 1) against **(3 − 2√2)/2** at (F = 4, η = 1/2), and **5 − 2√6** at (F = 2, 1/3) against **(5 − 2√6)/2** at (F = 4, 1/6) — the latter being what n ≡ 11 (mod 12) falls to if the F = 4 optimum is unavailable.
...
> - the ℓ = 2 residues when r = 2^a·u + 1 with u a small odd prime power, so that rung B′ is available at a usable η — the u = 1 (Fermat) case being the commonest, with 16 instances in range at r = 257;
> - the **odd** residues when a cyclic-layer-fused class of **odd** F — 3 or 5 blocks — is available, which at odd n forces c = 2^a and so is an escape. **Even F is not on this list**: at odd n, F·c is automatically even, c is unrestricted, and F = 4 is not an escape but the shape that *sets* the ceiling at 11 (mod 12) (§3.3.5). **Nor is odd F at even n**: there c is an ordinary odd prime, the supply is a full Hardy–Littlewood system, and F = 3 *attains* the ceiling at n ≡ 2, 8 (mod 12) rather than escaping above it (§3.3.5).
>
...
> **What bounds an individual row is cap_F(η), not its class ceiling.** Over [6, 159,027], **7,097 of the 120,556 one-foreign winners exceed their own class's tabulated δ₀**. In class 11 the *median* winner now sits at **0.99×** the tabulated cap and **1,826 of 7,408** exceed it, with a maximum of 2.81× — the class has settled onto its ceiling as the range grew (on the 2,600-frontier table the median was 1.14× and 34 of 60 exceeded, small-n excess that the singular series predicts should recede). So δ₀ is a guarantee for one specific balanced shape and not a description of typical behaviour; "generic ceiling" is the wrong reading of it. What *is* an upper bound, and holds without exception, is cap_F(η) evaluated at the configuration's own F and η: **0 of 120,556 rows exceed it**, and over the **61,640** fused-class-plus-foreign winners the maximum observed is **0.17157 at n = 157,221** (`2x46049 + 1x65123*`) — **cap₂(1) = 3 − 2√2 to five places**, so that ceiling is not merely an upper bound but an attained one.
...
**The uniformity trap.** Even granting the asymptotic for each n, deducing positivity for *all* large n needs the error term controlled uniformly in n, and that is not a free assumption: Friedlander and Granville showed that sufficiently uniform versions of Hardy–Littlewood-type conjectures are **false**. So the honest form of a hypothesis here is either "for all large n" taken as an axiom — which is what (BCG-AL) below does, and which is of Goldbach difficulty — or "for all but O(x^θ) of n ≤ x", which is weaker, closer to known technology, and sufficient for a density statement about μ.  **These two honest forms are now named: (BCG-AL) and (BCG-AA) of §3.5.3.** This paragraph is the reason the split exists — Friedlander–Granville is what makes the first unreachable from a fixed system and the second reachable.

#### 3.5.3 The hypothesis (BCG), in two quantifier strengths

**The hypothesis, stated.** Everything conditional in this document rests on one statement, which is worth writing out rather than gesturing at, since its exact shape is what §§3.3–3.4 have been determining. It is stated in **two variants that differ only in their quantifier over n**, because they have sharply different status: one is believed but unreachable from any fixed prime-tuple system, the other is reachable and is what the analytic route actually delivers.

*(The name is **bounded-cofactor Goldbach**: "bounded cofactor" is clause 3's d ≤ 12, the same condition that defines the set S_D of §6.8, and "Goldbach" is the additive-decomposition difficulty of clause 1. The name deliberately avoids the letter H, which in the literature means Schinzel's hypothesis — `literature-findings.md` uses it in that sense, and the collision was the reason to rename.)*

> **Hypothesis (BCG-AL)** *(all large n)*. Write (F, η) for the fusion count and efficiency that attain n's class ceiling, as tabulated by residue mod 12 in §3.3.5 — so **F = 1** at even n, **F = 2** at n ≡ 1, 3, 5, 7, 9 (mod 12), and **F = 4** at n ≡ 11 (mod 12) — and put **d = 2/η**. Then every sufficiently large n admits a prime q, a prime r and a prime power c with
>
...
> 3. **the efficiency** — r = d·q + 1 with **d ∈ {2, 4, 6, 12}**, the value being the one §3.3.4a makes available at n's (class, F): d = 2 at the unobstructed even classes and at n ≡ 1, 9 (mod 12); **d = 4 at n ≡ 3, 7 (mod 12)**; d = 6 at n ≡ 2, 8 (mod 12), at n ≡ 5, and at **n ≡ 11**; d = 12 at none of the ceiling-setting cells, since at class 11 the F = 4 reading at d = 6 beats the F = 2 rung that would need it;
...
>
> **The F = 4 clause is not a refinement but a load-bearing one.** The two residues that attain the global constant 7 − 4√3, and hence set §5's asymptotic floor, are 11 and 23 — both F = 4. A version of (BCG) quantifying only over n = 2c + r at odd n does not deliver δ₀ at those two classes, so it does not imply the floor it is invoked for.

> **Hypothesis (BCG-AA)** *(almost all n)*. Clauses 1–4 exactly as above, but required only for all but **O(x/(log x)^A)** of the n ≤ x, for every fixed A > 0.

**The two variants have different status, and the difference is the point of stating both.**

**(BCG-AL) is believed, and is implied by no fixed Bateman–Horn system.** The empirical case is overwhelming — §3.5.1's counts run to eight figures with no exception and agree with the singular-series prediction to the percent — but belief is not the issue. Each n presents its *own* prime-tuple system, and passing from "each system has infinitely many solutions" to "every large n has one" requires an asymptotic uniform in the system's coefficients, which is exactly what §3.5.2 records **Friedlander–Granville** as refuting: no such uniformity holds in the range needed, for any fixed system. So (BCG-AL) is not a corollary of Bateman–Horn, however strong the numerics; it is a statement of the same genus as Goldbach, believed on the same kind of evidence and open for the same reason.

**(BCG-AA) is implied by a fixed system.** Dropping to almost all n removes the uniformity demand — an exceptional set of density zero absorbs precisely the n where the individual system misbehaves — and what remains is a Bateman–Horn statement for the finitely many pair systems {Q, dQ + 1} with d ∈ {2, 4, 6, 12}, plus Siegel–Walfisz uniformity to polylogarithmic moduli. Call that package **(BH-SW)**. Then

> **(BH-SW) ⟹ (BCG-AA)** — *a one-pass result of the companion note, whose standing is set out at §6.9(e); the (AL)/(AA) split above does not depend on it.* The implication is cell by cell: the (F, d) assignment that the circle-method route selects at each residue class agrees with clause 3's table at all six cells — (1, 2) at n ≡ 0, 4, 6, 10; (1, 6) at 2, 8; (2, 2) at 1, 9; (2, 4) at 3, 7; (2, 6) at 5; **(4, 6) at 11**, the load-bearing one. The route is worked out in `sp-to-floor.md`; the agreement is not approximate and not up to constants, and the analytic side has no access to the shape space that produced clause 3.

**Which variant a downstream statement needs is not a detail.** The finite half of §5's floor conjecture needs neither. The asymptotic half comes in two forms, and they take different inputs: *for all large n* needs (BCG-AL) and is therefore no easier than Goldbach; *for almost all n* needs only (BCG-AA), and is therefore a theorem modulo a fixed prime-tuple conjecture. Every conditional claim in this document should name which one it is using, and the ones that say only "(BCG)" are errors of statement.
...
The efficiencies η = 2/d this yields are exactly the η column of §3.3's ceiling table: d = 2 gives η = 1 at n ≡ 1, 9 (mod 12) and at the unobstructed even classes, d = 4 gives η = 1/2 at n ≡ 3, 7 (mod 12), d = 6 gives η = 1/3 at 5 and at the even classes 2 and 8, and d = 12 gives η = 1/6 at 11 **under F = 2** — the two tables being the same fact read from the two ends. At the one class where F = 4 attains the ceiling — n ≡ 11 (mod 12) — the relevant η is §3.3.4a's and the d it calls for is 6. So d = 4 covers n ≡ 3, 7 (mod 12), and d = 12 survives in clause 3's list only as the F = 2 value at class 11, which the F = 4 optimum supersedes.

Every class has at least one admissible d, so **(BCG) is locally soluble at every n** — local solubility is a property of the clauses and is common to both variants —, and by §3.3's uniform bound its singular series is bounded below by an absolute constant rather than merely being positive. What is unproven is existence, not local solubility.

*Why the list and the ceilings are keyed alike, at mod 12 — and in what sense that is not a coincidence.* These are **different questions**: the admissible-d table above is about **local solubility**, which systems are not identically obstructed, decided by ℓ = 2 and ℓ = 3; the ceiling table of §3.3 is about **which rung is reachable**, which turns on the 2-adic structure of r − 1 and on F. They answer at the same modulus — at F = 2 reachability depends on n only mod 4, and the mod-8 condition at F = 4 is constant on the mod-12 class (§3.3.4). (BCG) quantifies over local solubility, with condition 2 absorbing reachability.

**The alignment is now known to be more than an accident of these two tables** (§6.9(b)): a third derivation with no access to either — solving the congruence for a circle-method window, using only conditions on `r − 1`'s divisor structure — selects the same (F, d) at every one of the twelve classes. Three routes agreeing cell by cell is evidence that the mod-12 keying belongs to the arithmetic of `r = dQ + 1` rather than to any one construction.

**The gotcha survives that, in weakened form, and is worth keeping.** What is now supported is that *these* assignments coincide; what is still unsupported is transferring an arbitrary fact from one table to the other because "both are mod 12". Nothing above says a wider shape space would keep them aligned — and the shape space has already been wrong once in the permissive direction (`pending-checks.md` A24).

*What c mod 4 does, given that (BCG) imposes nothing on it.* It steers, per §3.3.4a: **at F = 2, c mod 4 chooses between two reachable r mod 8; at F = 4, 4c ≡ 4 (mod 8) for every odd c, so c mod 4 is free and inert.** That is the whole of its role — a parameter to be chosen, never a condition to be met. With clause 3's values, **d ∈ {2, 4, 6, 12} suffices to attain every ceiling**; larger d describe alternatives rather than requirements.

*(BCG) is not of Goldbach type alone.* n = c + r with both prime is Goldbach-like; r = dq + 1 with both prime is a Sophie Germain condition, independently twin-prime-hard. Neither implies the other and (BCG) demands both on the same variable, in both variants. Nor is it implied by Chowla, or Chowla by it — §3.6 places (BCG-AL) as the **θ = 1 endpoint** of the shifted-prime ladder, which is a scale on which the current unconditional value is 0.679.
...
**Consequence for how the collapse is certified — and how far that consequence reaches.** A family whose members are available at a density-zero set of n cannot be dispatched by a *per-shape* argument, because at the sparse n where it *is* available it may well be the optimum. That is why `enumeration-proof.md` Part E′ certifies the collapse per n rather than per shape. **What the per-shape route cannot do, a per-density one can:** Part E‴'s Theorem E.5 bounds the SAFE score of *every* fallback configuration, whatever its shape and however sparse its supply, by C(n,2)/25 — so above that density the collapse needs neither a shape analysis nor a certificate. The sparsity discussed here is why the intermediate attempts failed, not an obstruction to the statement that closed it: **a bound on the value is available where a bound on the supply is not.**
...
> **Two attributions are tracked here and they come apart, which is the point of the last two columns.** The arithmetic inputs are not ours and are not new; what varies is *who noticed that this framework consumes them*. BBKN state the θ = 1/2 and ERH rungs; Shparlinski isolates the max-min as a named function and supplies the unconditional 1/4 rung and the almost-all 0.677 rung; **ours are the 0.679 update, the observation that the whole picture is one parameter θ, and the identification of (BCG) as its θ = 1 endpoint.** A reader should see at a glance that the ladder is a survey with two original entries rather than a contribution.
>
> **One row is a trap.** The ERH rung is still quoted in some presentations as the state of the art for all large n; it is not, since Shparlinski's Theorem 1 reaches n^{5/4+o(1)} unconditionally. Any comparison against "the ERH bound" is against a superseded baseline.
>
> **Elliott–Halberstam supplies two rungs, not one, and neither exponent is attained.** Shparlinski's §5 remark has two independent consequences on opposite sides of the quantifier column: extending the averaging of the Bombieri–Vinogradov input up to z^{1−ε} raises his Theorem 1 from 5/4 to 3/2, for all large n; separately, allowing any α < 1 in his Theorem 2 raises 0.677 to 1, for almost all n. Both are approached rather than reached — E–H is quantified for any *fixed* ε > 0, so each application buys θ = 1/2 − ε/2 or α = 1 − ε, giving n^{3/2−ε} and n^{2−ε} for every ε. This is the same form BBKN's Chowla row carries, and the `+o(1)` in the unconditional n^{5/4+o(1)} does not offset it: that is the up-to-subpolynomial-loss convention, where the o(1) may be negative.
>
> **The θ = 1/2 rung has two independent conjectural paths, and Shparlinski says which is likely harder.** Chowla's route concerns *individual* progressions; E–H asks only for distribution on average, so the Chowla route may be the more difficult to establish. The rung is therefore better supported than a single conjectural row suggests, and 3/2-for-all-large-n is the more robust conjectural target — 2 − ε being available only in the almost-all regime.
>
> **The quantifier column is not decoration.** The strongest exponents all carry quantifier costs: 1.677 and its conditional 2 − ε hold for almost all n (exceptional set O(x^{0.354}(log x)⁴) unconditionally), and the 3/2 rungs carry O((log x)⁴). On the *all large n* row the unconditional state of the art is 5/4. Omitting the quantifiers would mislead **in our favour**, which is worse than misleading against us.
>
> *Source checks owed.* Three, all against primary sources rather than Shparlinski's framing: the attribution of the θ = 1/4 rung to Bombieri–Vinogradov; the Chowla row, which is a conjecture-type rather than a specific citation; and the ε-form of the two E–H rungs, since §5 states those improvements without writing an exponent, making the form above an inference from how E–H is quantified.
>
> *A rung deliberately not on the ladder — but the reason may not bind here.* Shparlinski notes stronger unconditional results of Bombieri–Friedlander–Iwaniec, Mikawa and Fouvry, and sets them aside: they constrain the residue classes a to which ψ(y, m, a) applies. That is a considered judgement about *his* formulation. **This framework only ever needs r ≡ 1 (mod Q)** — modulus varying and large, residue fixed at 1 — which is exactly the regime where those results beat Bombieri–Vinogradov, so the restriction he excludes them for is one we already satisfy. Whether that converts into a rung is open and is filed as a literature item (`pending-checks.md` T4); the realistic prize is the almost-all exponents rather than θ = 1, since bounded cofactor needs level → 1 and these reach 4/7.

Hypothesis (BCG-AL) of §3.5 is the **θ = 1 endpoint** of this ladder — more informative than calling it a barrier, since it places the hypothesis on a scale with a known current value rather than in a separate category.

**But "endpoint" is not "limit of the ladder", and the distance is larger than the θ column suggests.** E–H gives θ = 1 − ε for almost all n, for every fixed ε; (BCG-AL) differs in two ways. The quantifier is one (every large n, not almost all). The substantive one is the *kind* of statement: every rung asks for a prime factor of r − 1 of size r^θ, so the family delivers n^{2−ε} and no member delivers a **constant fraction** of n² — a bound of that form is consistent with n²/log n. (BCG-AL) instead asks for a prime factor of size (r − 1)/d with d ≤ 12, **linear in r**, which yields Ω(n²) with an explicit constant. A linear-sized prime factor is a Sophie Germain-type condition, independently twin-prime-hard, and strictly stronger than every rung rather than the limit of them. Pushing ε → 0 does not close this, so E–H is not "almost (BCG-AL)". *§6.7 runs the implication backwards and shows the gap is not an artefact of how (BCG) is stated:* a density floor δ₀ **forces** a shifted prime with a prime-power divisor at bounded cofactor, so the bounded-cofactor condition is not merely sufficient for a floor but necessary for one, and no rung with θ < 1 can supply it.

**The ceiling on the route is technological, not conjectural — and this distinguishes it from Chowla's 1/2.** Chowla's exponent is the value a *conjecture* buys; beating it means assuming something else. Baker–Harman's is the current output of a *method*, and the method's limit is a **level-of-distribution barrier**: results of this shape rest on Brun–Titchmarsh on average, i.e. on controlling primes in progressions to moduli beyond x^{1/2} — exactly the gap between Bombieri–Vinogradov and Elliott–Halberstam. The exponent moves whenever that control does, and it has: 0.677 became **0.679** on Maynard's triple-convolution estimates, in the lineage that took Bombieri–Friedlander–Iwaniec's x^{29/56} to Maynard's x^{11/21} and Lichtman's x^{17/32}. So θ → 1 is not a separate wish; it *is* level → 1, which is Elliott–Halberstam. *And there is a second, independent reason the ladder stops short of a constant, which §6.8(iv) sets out — though not the one it is natural to guess. The almost-all machinery that converts a set of shifted primes into a statement about almost all n runs on the set's **cardinality alone** (Balog–Sárközy), so thinning the input as θ → 1 costs a power of log x and not the argument. What is capped is the **companion**: that machinery certifies only a sub-linear prime factor in n − r, at any input density, so it yields n^{1+γ} with γ < 1 and never a constant fraction of n². So n^{2−ε} and Ω(n²) are not adjacent points on one scale.*
...
> *Verified* (`ladder_verify.py`), over every composite non-prime-power n in range — all residues, no eligibility filter — with **no value below 1/25**. *Current scan, complete to 10⁶: floor **0.04621 at n = 2759**, a margin of **1.16×** over 1/25. Requote from the scan rather than from here. Interim, corrected scoring: δ ≥ 0.0462 over n ≤ 10⁵, attained at n = 2759, margin 1.16 over 1/25.*
...
| collapse B_refined = B_safe **proved** | **n ≤ 10⁶** | by theorem: Part E‴ caps the fallback branch at δ = 1/25 and the ladder clears 0.04621 unconditionally throughout, so no B(n) is computed anywhere in the argument |
| collapse B_refined = B_safe certified (independent route) | n ≤ 100,000 | computed from lower bounds (Part E″), at **every** composite non-prime-power value, **90,299 of 90,299 (100.00%)** — *confirmed unchanged by the post-repair rerun*; now corroboration rather than the proof |
...
*Both shapes of the multiplicative engine live or die on the same condition, and it is **not** ω(n) ≤ 2.* A fused class needs n = F·c with c a prime power — and, under the entangled construction, **F any integer** (§3.2.3): the block-permuting rotation and the full twist come from one generator, so nothing confines F to a prime power. Every non-prime-power n has such a decomposition, with c = Q(n) the largest prime-power divisor and F = n/Q(n); what the engine delivers there is δ_S2 = (Q(n) − 1)/(n − 1) ≈ 1/F (§2.1). So the engine's *reach* is not a divisibility pattern but a **bounded cofactor**: it clears a floor δ₀ exactly when n/Q(n) ≤ 1/δ₀, i.e. n ∈ ⋃_{F ≤ 1/δ₀} F·{prime powers}. ω(n) = 1 is **S1**, the trivial case at δ = 1; every other n is a candidate for **S2**, the fused family proper, with F small or not.

> **The earlier form of this paragraph read "F and c both prime powers, i.e. ω(n) ≤ 2", which is the pre-entangled condition and is false as a precondition.** The census (`enumeration-proof.md`, S2 row: *n = F·c with F > 1*) already carries the corrected reading; seven S2 winners in [6, 159,027] have ω(n) = 3, all at F = 6 (§2.1's box), and 15,255 of the 51,882 ω(n) = 2 values in that range have n/Q(n) > 25, so S2 alone could not carry them over 1/25 — ω(n) = 2 is neither necessary nor sufficient. Where ω(n) = 2 is quoted below it is a *proxy* that tracks the bounded-cofactor share closely (both are 30–35% over most of the table) and was the variable measured before the correction; the bounded cofactor is the one the argument is about.

A bounded cofactor is a **density-zero condition**: ⋃_{F ≤ K} F·{prime powers} has at most K·π(N) + O(K√N) = O(K·N/log N) members below N, so its share falls like K/log N. The prime powers alone are O(N/log N). So S1 vanishes fastest, S2 next, and neither survives into the asymptotic picture.
...
> *Verified, on the variable that matters and on the proxy.* Fraction of composite non-prime-power n with **n/Q(n) ≤ 25** — the values S2 alone can carry over 1/25 — by dyadic block: **65.8%** on [10³, 2·10³), 47.0% on [5·10³, 10⁴), 33.5% on [5·10⁴, 10⁵), 26.3% on [5·10⁵, 10⁶), 24.6% on [10⁶, 2·10⁶); at cofactor ≤ 16 the same blocks read 52.9 / 38.2 / 27.6 / 21.8 / 20.4%. The ω(n) = 2 proxy on the same blocks: 52.3 / 43.1 / 35.0 / 29.8 / 28.5% — it undercounts at small n (where composite F like 6 admit many values) and overcounts at large n (where an ω = 2 value's cofactor is usually too big to be worth anything), crossing near 5·10⁴.

**The prediction shows up in the table, on both of its halves.** Two forces act on the observed floor in opposite directions, and separating them is the point. Extending the range lowers it, since a wider range can only find weaker values. Adding configurations raises it, since B(n) is a maximum — so a shape-space correction lifts the whole curve at once. Over the contiguous range [6, 159,027] the floor is **175813/3804661 = 0.046210 at n = 2759** (`2x653 + 1x1453*`); on the earlier 2,600-frontier table it was 0.048039 at n = 2183, and extending the range past 2759 lowered it exactly once, to the ladder's own global minimum, after which no value below 159,027 came within 0.0018 of it. So the *within-table* floor moved up when the space was corrected and down when the range was extended, while the mechanism below — which is about which n the engines reach, not about how well they are scored — is unaffected. The thirds of the range behave as the argument requires:

| n | ω(n) = 2 share (proxy) | n/Q(n) ≤ 25 share | median cofactor n/Q(n) over ω = 2 | min density |
|---|---|---|---|---|
| [6, 53,009) | 39.6% | 41.1% | 8 | **0.04621** |
| [53,009, 106,018) | 34.8% | 33.3% | 9 | 0.06362 |
| [106,018, 159,027] | 33.5% | 31.3% | 9 | 0.06391 |

*(On the 2,600-frontier table the same thirds read 64.9% / 53.6% / 50.1% for the ω(n) = 2 share and 4 / 5 / 7 for the cofactor, with the floor 0.04804 in the last third — the first third of the current table already sits below where that table ended.)*

Two effects, not one — and they are the same variable seen twice. The bounded-cofactor population thins, as predicted; and **among the values that remain, the cofactor n/Q(n) grows**, so the 1/F the multiplicative engine delivers shrinks even where the engine still wins. (The ω(n) = 2 share thins in step, as a proxy should.) n = 2759 = 31·89 illustrates the mechanism: ω(n) = 2, so a fused class exists, but only at F = 31, worth 1/31 ≈ 0.032 — which loses to `2x653 + 1x1453*` at 0.046210, itself foreign-bound at the boundary cofactor 12. n = 2183 = 37·59, the runner-up at 0.048039, is the same story one step up: F = 37 is worth 0.027 and `6x251 + 1x677*` beats it. (Under the v2 shape space the winner at 2183 was the three-class 1297\* + 443 + 443 at 0.041107; the cyclic-layer block count is what lifted it.) n = 2759 is the table floor itself: v4's floor row n = 1817 rises to ≥ 0.0594 (`3x256 + 1049*` at full twist), while 2183 is not among the raised rows. 2183 is n ≡ 11 (mod 12), the doubly-obstructed class — so the floor tentatively returns to that class, and the 1817 exception becomes historical pending the rebuild.

Two consequences, and both should temper how the computed range is read.

**The observed density floor should drift downward — and then stop.** Over [6, 159,027] **35.2%** of the table has n/Q(n) ≤ 25 and S2 actually wins at **16.5%** (it won 34% at the 2,600 frontier; the ω(n) = 2 proxy reads 36.0%, was 55.4%), so the engine whose reach thins like 1/log n is already serving a sixth of the computed values; the median density is 0.1713 (was 0.1994). The evidence is two-sided and worth keeping apart: *within* the exactly-computed range extension lowered the floor exactly as predicted — from 0.048039 at n = 2183 to 0.046210 at n = 2759 the moment the range passed 2759 — while shape-space repairs lifted it (to 0.045742 at n = 1817 under v4, to 0.048039 at 2183 under the entangled-generator correction), corrections to the scoring and not counter-observations. But past 2759 the drift *stopped*: no n in (2759, 159,027] comes within 0.0018 of the floor, the per-third minima rise (0.04621 / 0.06362 / 0.06391), and the ladder to 10⁶ finds nothing lower. That is the §5 recovery — the class-23 additive ceiling taking over as representations near the balance point become plentiful — showing through inside the exactly-computed range rather than only in the ladder's extrapolation of it.
...
| a cyclic-layer-fused class of F = 3 or 5 blocks, **at odd n only** (even F is not an escape at either parity; odd F at *even* n is not one either — it is a ceiling-setting shape at n ≡ 2, 8 (mod 12), §3.3.5) | **S7 at F ≥ 3, odd F, odd n** |
...
*This is the source of the census's one existence/winning gap of positive density.* The 2^a + r\* route is S3 at odd n, so S3 **exists** on all even n plus a positive proportion of odd n, while **winning** on the even half alone — the odd instances are available almost everywhere and effective almost nowhere. Every other gap in that column is a difference of rate rather than of density.
...
**S7 at F ≥ 3** is an escape rather than a carrier **only in two of its three regimes**, and §4.3 counts those with the other routes at O(n/log n). The parity structure is what decides which: at **odd n with odd F**, F·c even forces c to be a power of 2, leaving O(log n) choices per n — an escape. At **even n with odd F** the supply is a full Hardy–Littlewood system and F = 3 ties S3 for the ceiling at n ≡ 2, 8 (mod 12), so that regime is a **carrier on those two classes** and wins a positive proportion of all n. At **even F** the shape is the odd-n ceiling-setter at n ≡ 11 (mod 12). **S7 at F = 2 is a different matter entirely** — it is the odd-n fused rung and carries 5/12 of all n; it is listed under §4.5's live shares, not here.

**S5** is a census shape but behaves as an escape, for a reason on the other side of the configuration: top-layer fusion forces q = 2, so the foreign efficiency is 1/u with u the odd part of r − 1, and reaching a useful η needs r = 2^a·u + 1 with u small. That is an exponential family, O(log n) candidates per n, hence O(n/log n) values — §3.3 and §4.3. **Its share arrives in bursts rather than declining**, since each usable r serves an interval of n proportional to r; the census row records why a monotone-decline test is the wrong instrument for it.
...
| S2 | **(i)** thins | needs a bounded cofactor n/Q(n) ≤ 1/δ₀, a density-zero set of n (the entangled construction takes any F; the old ω(n) = 2 gate is superseded) — §4.1 |
| S3 | carries even n — **1/2** | §3.1; the counting check of §3.8 |
| S4 | **wins nowhere**: the cyclic-layer fused rung beats it at every c (§3.2.3), and at n ≡ 11 (mod 12) the two-part F = 4 shape beats the whole three-part family | §3.2 |
| **S7 at F = 2** | carries odd n — **five of the six odd classes**, n ≡ 1, 3, 5, 7, 9 (mod 12) | §3.2 |
| **S7 at F = 4** | carries odd n — **one odd class**, n ≡ 11 (mod 12) | §3.3.5 |
...
The live shares sum to 1: **1/2 to S3** (the six even classes) and **5/12 to S7 at F = 2** (five of the six odd classes), leaving **1/12 to S7 at F = 4** at n ≡ 11 (mod 12). S4, S5, S7 at F ≥ 3 other than F = 4, and the escape routes take no share of the limit. Note that for the first two this is a matter of being **beaten**, not of being absent: both are available at a positive density of n — S5 at essentially every odd n, S7 at F ≥ 3 at essentially every even one — and it is their ceilings that lose. Only the O(n/log n) subset where they clear their residue's ceiling shows up at all.
...
**Where the floor lives.** The worst residues are **n ≡ 11 (mod 12)**, with δ₀ = 7 − 4√3 = 0.07180 — the odd classes carrying the ℓ = 3 obstruction. Almost every value that has ever set a running floor has been in n ≡ 11 (mod 12): of §4.1's eight record-setters under the corrected table, seven are ≡ 23 (mod 24), and the one exception was v4's table floor **n = 1817 ≡ 17 (mod 24)**, i.e. 5 (mod 12) — under the entangled-generator correction 1817 rises to ≥ 0.0594 and the floor returns to n = 2183 ≡ 23 (mod 24), restoring the doubly-obstructed class as the argmin's home. (The pre-repair descent's record-setters were all ≡ 11 (mod 12), eight at 23 and three at 11 mod 24.) Finite-n record holders are low from *supply* failure rather than from a low ceiling, so class membership of the argmin is weaker evidence than the ceiling table itself.

> **The finite-range minimum is set by whichever engine happens to be weakest at the extreme value, and that alternates.** Over the contiguous range [6, 159,027] the smallest density is

> **175813/3804661 = 0.046210 at n = 2759 = 31·89, witness `2x653 + 1x1453*`** — the same value the ladder finds as the global minimum to 10⁶ (below), so the exactly-computed range now *contains* the argmin rather than stopping short of it; the runner-up is 0.048039 at n = 2183 (`6x251 + 1x677*`), and nothing else below 159,027 is under 0.05 —

a **fused class beside a foreign block** at **n ≡ 23 (mod 24)**. It is **foreign-bound**: the binding term is orb(1453, 121) = 175,813 at q = 11, against an intra term of 2·C(653,2) = 425,756 and larger cross terms; the runner-up 2183 is foreign-bound too, on orb(677, 169) = 114,413 at q = 13 against 6·C(251,2) = 188,250. **Which term binds is a property of the record-holder, not of the record**, so it changes silently whenever the argmin does — re-derive it from the witness rather than carrying it forward. *(The previous holder, v4's n = 1817, was intra-bound at a twist cut by an F_mid strip; that strip is not a condition, and 1817 now scores 0.091483 at the full twist.)* The runner-up mechanism is multiplicative granularity: the next value up, n = 2303, sits at 0.059833 on `6x251 + 1x797*`, and among the pure fused classes the low rows are those whose smallest prime-power cofactor is large, where the engine delivers only 1/F and nothing additive competes. Which of the two owns the record at a given frontier is not stable, and **the record-holder's mechanism is a range-scoped claim that expires silently** — `check_doc_figures.py --pass scope` catches numeric range assertions but not claims about mechanisms, which is the failure mode `pending-checks.md` A5 records. Re-read this paragraph at every extension. Asymptotically neither mechanism is the story: the class-23 additive ceiling takes over, which is the §3.7 handover seen from the floor's side.

`ladder_verify.py` computes, for each n, the best density achievable by four explicit families, scanning the block size over a window wide enough to contain every balance point — now **x ∈ [0.10, 0.85]** and fusion counts 3..16 ∪ {25}; the earlier window [0.10, 0.55] clipped the winning c at 274 tabulated n, understating them by up to 1.8×, none of which touched the minimum (`pending-checks.md` R7a). Over all composite non-prime-power **n ≤ 10⁶** (78 minutes, at the old window; the rerun at the corrected one can only raise entries) the smallest value is
...
>
> The run to 10⁶ at the corrected scoring is complete: **44,091** worklist entries, minimum **0.04621 at n = 2759**, and no entry anywhere below 0.04. Both corrections moved it in the same direction — the entangled repair, and then the S7 branch keeping its full twist — and the second lifted the [10⁴, 10⁵) decade clear: its minimum is now 0.05829 at n = 22139, where the understating scan reported 0.04801 at n = 11183.
>
...
>
> *The same care is owed to the rest of the scan, and it survives it.* `ladder_verify.py` prints to five places, so any entry could sit up to 5·10⁻⁶ below what it shows. The lowest printed value among the other 45,389 entries is 0.04801 at n = 11183, hence at worst 0.048005 — still clear of 175813/3804661 by more than 0.0017, so neither the minimum nor its uniqueness depends on the printing precision. *(That runner-up is itself a scoring artefact and will rise on the rerun — n = 11183 reaches at least 0.0657 on `6x1171 + 1x4157*` once the S7 branch keeps its full twist. The minimum and its uniqueness are unaffected, since every entry can only rise and n = 2759 is already tight against B.)*
>
...
>
> **What the minimiser looks like, and why it is the hard case.** The witness is `1x1453* + 2x653` at q = 11 — a fused pair of 653-blocks beside a foreign prime r = 1453 — and the binding class is the **foreign intra orbital r·Q = 1453 × 121 = 175813**, not the matching term (2·C(653,2) = 425756, more than twice as large). The foreign twist has order Q = 121 = 11², a prime power as Lemma B′ requires, and its cofactor is **(r − 1)/Q = 1452/121 = 12** — exactly (BCG)'s d ≤ 12, at the boundary. So the worst n in a million is worst because its foreign block is as inefficient as the hypothesis permits and no more.
...
> **The fused rungs double the intra term and nothing else, so they are invisible wherever the foreign block binds.** At every odd residue where a fused rung is reachable, the configuration realising the *class minimum* binds on its foreign term, not its intra term — checked directly at each. Fusing cannot move a value whose bottleneck is the foreign block, so what fusion changes at most of them is the value at some *other* n in the class, which then displaced the old minimum. At n ≡ 11 (mod 24) nothing displaced it.
>
...
|---|---|---|---|---|
| [0, 10⁵) | **0.04621** | **2759** | 0.04582 | 9911 |
| [10⁵, 2·10⁵) | 0.05603 | 173627 | 0.06027 | 120791 |
| [2·10⁵, 3·10⁵) | 0.05928 | 275267 | 0.06202 | 235127 |
| [3·10⁵, 4·10⁵) | 0.06336 | 371123 | 0.06257 | 326423 |
| [4·10⁵, 5·10⁵) | 0.06522 | 441059 | 0.06375 | 415127 |
| [5·10⁵, 6·10⁵) | 0.06439 | 500627 | — | — |
| [6·10⁵, 7·10⁵) | 0.06579 | 623087 | — | — |
| [7·10⁵, 8·10⁵) | 0.06581 | 773843 | — | — |
| [8·10⁵, 9·10⁵) | 0.06651 | 873287 | — | — |
| [9·10⁵, 10⁶) | 0.06547 | 946259 | — | — |
...
> **The finite constant was 1/50 and the gap between the two forms is the interesting part.** 1/50 was set when the observed floor was near 0.026 and falling with every extension of the range, so it was a modest safety margin under a moving target. The target then stopped moving down and moved up, and every fall turned out to be a scoring deficiency rather than a property of μ: the pre-repair shape space read n = 2291 at 0.037524 where the true value is 0.068522, and the pre-repair ladder read n = 8927 at 0.02516 where the corrected families reach 0.0554 — still on the worklist, but at more than double the old score. **The drift is one-directional for a structural reason.** B(n) is a *maximum* over admissible configurations, so a missing shape can only depress it — and a missing shape has no witness, hence nothing in the pipeline can detect one, whereas an over-credit is caught by re-deriving μ from the recorded witness. The errors that survive to be found later are therefore biased toward those that lower the floor, which is why the ratchet has run upward and why 1/25 should also be read as likely loose.
...
**The asymptotic half is exactly what (BCG) buys — and which variant is granted decides the quantifier it comes back with.** Granting (BCG-AA) instead yields the same bound for **almost all** n, which is materially cheaper: (BCG-AA) follows from a fixed Bateman–Horn system (§3.5.3), whereas (BCG-AL) does not follow from any, so the almost-all form of the asymptotic half is a theorem modulo a prime-tuple conjecture while the every-large-n form is of Goldbach difficulty. Granting Hypothesis (BCG-AL) of §3.5, every sufficiently large n admits a representation at its own class ceiling, so δ(n) ≥ δ₀(n) − o(1) ≥ 7 − 4√3 − o(1) with the minimum at n ≡ 11 and n ≡ 23 (mod 24). The finite half — δ ≥ 1/25 at *every* composite non-prime-power n — is not implied by either variant of (BCG), both being eventual statements; it is verified to 10⁶ by the ladder and supported by the margin below.

The constant 1/25 is deliberately loose, though less so than 1/50 was. Two things are absorbed into the margin: the finite exceptional set of §3.5, whose members fall back on whatever configuration they can find, and the windowing loss of §3.4, which costs a factor Θ(√ε) when the balance point is not exactly available. The measured margin is 15.5% — the ladder's floor is 175813/3804661 = 0.046209… against 1/25 = 0.04 — so 1/25 is close to what the current families actually deliver, and a sharper constant would want either B computed at the next several worklist entries or a fifth family.
...
> **The ladder is tight wherever both are known, and what makes that true is the S7 branch keeping its full twist.** Joining the worklist against the computed table — `validate_table_v3.py --ladder`, which reads both files and recomputes nothing — the ladder never exceeds B(n), which is the correctness half and the one the floor rests on, and it **equals** B(n) at every one of the **28** values in both. So on the measured range the four families do not merely bound δ(n), they attain it, and the ladder's advantage is entirely in cost: O(n/log n) per value against the enumeration's n^2.9. That is the empirical basis for reading the floor out to 10⁶ as an estimate of δ rather than only as a bound.
>
> **The gotcha, and it is the one that decides whether the previous paragraph is true.** A fused class's twist must **not** be cut by the block count. The tempting argument — the rotation C_F_mid and the twist's cyclic part share one cyclic layer, which has a unique subgroup of each order — treats a *quotient* of the layer as a subgroup of it; an entangled generator supplies rotation and full twist together at any F_mid (§3.2.3). Cutting the twist anyway is safe in the ladder's own direction, since understating a family lowers a max over families and leaves every floor a valid lower bound — but it is not free, and it does not fail loudly. It bites hardest at even F with c ≡ 1 (mod 4), where the strip removes the 2 from c − 1 and halves the intra term, which is exactly the fused-plus-foreign shape that wins at the arithmetically weakest n. Under the cut the ladder fell short of B at 16 of the 37 values then joined, by up to 1.81×, and every shortfall was of that shape; with the full twist it is tight at all 28 that survive into the corrected worklist — the count falls because entries that were only in the worklist by virtue of being understated are no longer in it. **A ladder that understates is still a valid floor, so nothing in §5's minimum depended on this — what depended on it was every statement about the *shape* of the low tail**, since an understated value is indistinguishable from a genuinely weak one in a worklist. *The rerun bears that out precisely: the floor and its argmin are unchanged, both having been tight against B already, while the worklist shed 1,299 entries and two of the four decade minima rose.*
>
> The check also runs in the other direction: a ladder value *above* B(n) would mean a family scored too generously, a shape missing from the enumeration, or a broken collapse.
>
> **This became true only after the S7 family was given its layer split**, and the gotcha is worth keeping. A fused class of F blocks does not put every prime of F into the cyclic layer: F = F_mid·F_top with F_top a power of the top prime q (`enumeration-proof.md` G.2), so the foreign twist must avoid the primes of **F_mid**, not of F. Excluding all of F is a valid lower bound but discards every configuration whose foreign twist needs a prime dividing F — in practice **q = 2 with F even**, the Fermat-prime branch. It cost exactly one value in range: the ladder read **n = 935** at 0.04898 against B(935) = 0.07534, on `6x113 + 1x257*` with F_top = 2, F_mid = 3, binding on orb(257, 256) = 32,896. The repair lifts 935 above the asymptotic ceiling and 1007 from 0.06444 to 0.06494, changes nothing else below 5·10⁴, and leaves the global floor where the full corrected run puts it, 0.046210 at n = 2759.

**The ladder alone now settles the range**, without the branch-and-bound needing to compute a single B(n) — and above 1/25 its value is not merely a floor but, up to one named exception, the value itself: `ladder-completeness.md` shows the menu contains a B-optimal configuration at every such n except where a two-foreign shape at a common top prime wins, which caps at 1/9 (even n) or 1/16 (odd n), loses to every class ceiling under (BCG-AL) at ε ≤ 0.129, and never wins on the table. The ladder equals B at all 50,062 exactly-computed values, and `mu_ladder_exact.py` — the same menu scored exactly plus the two-foreign shapes, certified by Theorem E.5 — equals `mu_exact.py` at every one of them and extends the contiguous table to 159,027. **And above 1/25 B is μ** (Corollary E.6), so at every n ≤ 10⁶ the ladder's value is a lower bound on μ(n) that is *attained* wherever it has been checked. Every one of the 44,091 worklist entries scores at least 0.04621, so M never falls below that and no survivor exists to examine. *Run against the 1/25 floor, the adaptive search returns an **empty** below-floor list — nothing in the range requires a μ(n) computation at all.* What the search is for is *sharpening* — computing B at the lowest entries to raise the verified floor — rather than establishing it. An illustrative run against an earlier, weaker ladder shows the shape of the descent:

> M → 0.041812 (n = 575) → 0.041107 (n = 2183) → 0.037524 (n = 2291) → 0.029282 (n = 3059) → 0.026117 (n = 3239), each step an n whose family score fell below the standing minimum. Under the corrected families none of those five is a candidate at all: 2291 reaches 0.068522, 3059 reaches 0.083906 and 3239 reaches 0.055155 (all exact B, in the table), and the descent terminates immediately.
>
...
**What survives the enumeration defect, and what does not.** Every step used the table from below: `ladder_verify.py` scores explicit constructions, and B(n) ≤ μ(n) at every n where B(n) > C(n,2)/25 by Theorem E.5 — which is every n ≤ 10⁶, since the ladder's floor is 0.04621. So

> **min { μ(n)/C(n,2) : n ≤ 10⁶ composite, not a prime power } = 175813/3804661 = 0.046209898…, uniquely at n = 2759**

is proved, the ladder alone supplying every value's lower bound: every one of the 10⁶ values scores at least that, with the minimum at **n = 2759 = 31·89**, the runner-up being **0.04804 at n = 2183** and no third entry below 0.05. In particular **δ(n) ≥ 1/25 throughout**, with 15.5% to spare. And at the argmin the bound *is* a value: **B(2759) = 175,813** is in the table, equal to the ladder's own score there, so the minimum is determined exactly rather than bounded — which is what §5's box above states and what makes n = 2759 the unique global minimiser rather than merely the lowest entry.

**What the branch-and-bound has left to do: nothing.** The worklist holds **44,091** entries — every n whose family score falls below the asymptotic ceiling 7 − 4√3 — but **none below 1/25**, so it has nothing to eliminate against the conjecture as stated, and the adaptive run against a floor of 0.04 returns an **empty** below-floor list without computing a single B(n). Its sharpening role is also discharged: B = μ is known exactly at *every* n ≤ 159,027, the low tail is **two** entries below 0.05 — 2759 and 2183, then 0.05051 at n = 2639 and 0.05516 at n = 3239 — all inside that range with B equal to the ladder's score, and Corollary E.6 makes the ladder's value a lower bound on μ at every n ≤ 10⁶ regardless. So the verified floor to 10⁶ is 175813/3804661, exactly, with no exact-B computation left that could move it.

> **Probe before committing to any of them.** Finding one configuration that clears a threshold is sub-second; proving optimality is what costs hours. A targeted scan over the two-part census shapes, scored with `mu_enumerate_v3.py`'s own `value()`, reproduced B(n) exactly at all eleven worklist values where B was independently known.

`mu_enumerate_v3.py --floor M --adaptive` runs the loop as one job: it seeds at M·C(n,2) so any configuration above the floor rejects n immediately, prunes candidates whose lower bound has risen above the current floor, computes B(n) exactly only for survivors, and adopts a lower value as the new floor — which in turn tightens Proposition F.1's part-count cap ⌊1/√M⌋ for everything after it.
...
**The binding value is 0.04621 at n = 2759, in the [10³, 10⁴) decade, and the descent is now visibly one decade wide.** Every other decade clears **0.057**, and the minima rise monotonically on either side of the binding one — 0.05703 below it, 0.05829 and 0.06391 above. So the middle range where neither the computation nor the asymptotics reaches is a single decade, its worst value clears 1/25 by 15.5%, and that value is not merely a bound but an exact B — and, with the exact table now contiguous to 159,027 and Corollary E.6 in hand, the first two decades and most of the third are *computed* rather than bounded: there is no middle range left below 1.6·10⁵ at all. *(The two upper decade minima moved substantially on the corrected scoring — [10⁴, 10⁵) from 0.04801 to 0.05829 — which is what turned "one decade wide" from a reading of understated values into a statement about δ.)*

## 6. Running the implication in both directions

Corollary 3.2 of the notes is an equivalence, so a lower bound on μ yields an additive prime statement — and, read the other way, an arithmetic hypothesis yields a bound on μ. §§6.1–6.8 run it **backwards**, from a floor to what it forces; §6.9 reports the **forward** direction, where a fixed prime-tuple hypothesis delivers the floor. The two meet: §6.7 shows a floor *demands* a shifted prime with a large prime-power divisor at bounded cofactor, and §6.9 shows that hypothesis *delivers* the floor, so the equivalence is not merely formal. It is worth being exact about *which* statement, because the natural reading is too strong.
...
**What it does force is a covering statement over a finite set of systems.** At density δ₀ the search bounds are all effective, so the possible **shapes** form a finite set depending on δ₀ alone. Getting that set right matters, since the covering statement is only as strong as the set is small — and §§6.1–6.5 are the work of getting it right.

**And it forces an arithmetic statement, which is a different kind of consequence.** The covering statement says *which systems*; the arithmetic one says what any of them must supply — a shifted prime with a large prime-power divisor, at bounded cofactor. That is the converse of the whole framework, so it is set out at the end of the section, in §6.7, where it can be read against §6.6's covering form.
...
One criterion replaces the three that might be imposed separately, and it is strictly sharper than their conjunction. It gives k ≤ 1/√δ₀ (Proposition F.1) by taking every F_i = 1, and it gives **F ≤ 1/δ₀** — note this is *not* 1/√δ₀, which is the natural-looking but wrong bound: at δ₀ = 1/9 a single class of **nine** fused blocks is feasible. The tightest instance in range is `2x79507` at n = 159,014, where √2 = 1.414214 against 1/√δ = 1.414218, a slack of **0.000004** — the Theorem 2.1 family n = 2·(prime) sits *on* the boundary and approaches it as n grows (at n = 2594, `2x1297`, the slack was 0.0003); the fused single classes at large F are the next tightest; `19x61` is admissible at n = 1159 with slack 0.034, though that n is won by `4x173 + 1x467*` at 0.088683. (The floor value — 0.046210 at n = 2759 — is lower but its winner's fused class is small, so it is not the binding test — the criterion binds where F is large *relative to* δ, not where δ is least.) **Checked against every winner over [6, 159,027]: 0 of 144,299 violate it.**
...
| 1/9 | 3.000 | 3 | 9 | **24** |
| 1/16 | 4.000 | 4 | 16 | **65** |
| 0.051813 (`19x61`, an admissible fused class at n = 1159) | 4.393 | 4 | 19 | **83** |
| 0.04621 (the ladder floor to 10⁶) | 4.652 | 4 | 21 | **112** |
| 0.04 = 1/25 (conjectured) | 5.000 | 5 | 25 | **164** |

> *The floor row is keyed to the exactly-computed floor*, which is now the ladder's own global minimum to 10⁶ (175813/3804661 at n = 2759, inside the contiguous range) and so cannot move on extension below 10⁶ — only a scoring correction at n = 2759 itself would move it. The apparatus is in `shape-counting.md` §3.
...
**What bounds the shape is a density ceiling — for the unfused case.** Take the unfused two-class case c = p^a > c′ = p^b, so c′ ≤ c/p and c + c′ ≤ n, whence c′ ≤ n/(p+1). The configuration's minimum is at most the smaller class's intra term, ≤ C(c′,2), so

> **δ ≤ (c′/n)² ≤ 1/(p+1)²** — 1/9 at p = 2, **1/16 at p = 3**, 1/36 at p ≥ 5.

So an *unfused* unequal-size shape is infeasible above density 1/9 whatever p is, and above 1/16 unless p = 2. Among unfused shapes only p = 3 can compete anywhere near the computed floor, and only inside the δ ≤ 1/16 tail.

> **The bound is about the unfused reading, and fusing the smaller class breaks it at p = 2.** The derivation prices the smaller class at C(c′,2); a *fused* class of F′ blocks of size c′ is worth F′·C(c′,2), which is exactly the factor the ceiling does not carry. At p = 2 that is enough to clear 1/9: **n = 640 = 1·256 + 3·128** scores min(C(256,2), 3·C(128,2), 3·128², 256·384) = 24,384, **δ = 0.1192**. So "no unequal shape is feasible above 1/9" holds for the unfused reading only, and the one-size counts of §6.1 and §6.4 are exact above 1/9 only once fused unequal shapes are accounted for. **Gotcha: a ceiling derived for one reading of a shape does not transfer to its fused reading, and fusion is precisely the axis that multiplies an intra term.**
>
> *What that costs, which is little.* At odd p the same computation stays below roughly 1/(8p), so only p = 2 escapes; and a fused unequal p = 2 shape needs n to be a sum of two distinct 2-power multiples, which is a density-zero condition of the same kind as §6.5's other escapes. So these join the escape list rather than the covering accounting — but they must join it explicitly, since the ceiling argument above does not exclude them.

**Measured.** Scoring every unequal odd-p configuration at full diagonal twist: **none wins** — confirmed directly against the current table, where **0 of 144,299 winners over [6, 159,027] have matching classes of two different block sizes, at any p** (S12 in the census; dominated above 1/25 by the merged fused class, `ladder-completeness.md` Proposition 1). (How close they come is measured in `shape-counting.md` §5.) Together with the earlier observation that no winner in the table has two matching classes of different sizes, that is the empirical side; the ceiling above is the structural side, and it says why.

**So the one-size presupposition is false but harmless.** Unequal-size shapes exist; they are escapes rather than competitors, exactly like the fusion shapes and the c = 2^a family of §6.5, and they sit outside the ceiling accounting for the same reason — the *unfused* ones by the density ceiling above, the *fused* p = 2 ones by their density-zero supply. Two consequences for the counts below: at **δ₀ > 1/9 the one-size counts are exact up to that escape set**; below 1/9 the covering set gains a partition factor, tabulated in the box that follows. The n = 551 = 256 + 167\* + 128 configuration — two matching classes at 2⁸ and 2⁷, cyclic layer C₂₅₅ × C₁₂₇ × C₁₆₇ — is §6.5's second escape and the worked instance.

**The standing check.** Two values could put an unequal shape inside the competitive range: **p = 3 unfused**, which the ceiling admits only inside the δ ≤ 1/16 tail, and **p = 2 fused**, which clears 1/9 but is supply-limited. So whenever the low-density tail is recounted, check it against both. That is the whole residue of what this section leaves open.

> **What the partition factor costs below 1/9, in one line.** A shape records not only its parts but which matching parts share a size, so below 1/9 the covering set gains a **partition factor of about two** at the operative densities — 24 → 24 and 65 → 67 on the all-shapes counts, 15 → 24 for the additive disjunction at the conjectured floor. Three things survive it unchanged, and they are what matter here: **finiteness** (the number of distinct sizes is bounded by k ≤ 1/√δ₀, so every shape is still one Bateman–Horn system, in several size variables rather than one); **the ceiling table of §3.3** (an unequal shape's smaller class is capped at (c′/n)² ≤ 1/(p+1)², *below* the equal-size shape of the same part count, so such shapes enlarge the covering set without raising any class ceiling); and **the parity reduction** (every matching size is a power of the same odd p, hence odd). The growth becomes exp(c·δ₀^{−1/4}), worse than quadratic but far below the raw count's exp(2.53·δ₀^{−1/3}).
>
> So the exposure is to the specific numbers of §6.1 and §6.4 below 1/9, not to the structure of the argument. The full table, the penalty's derivation, and the boundary analysis that fixes its top row are in `shape-counting.md` §4.
...
**The raw count is a weighted partition problem**, N(δ₀) summing m₁(M) + 1 over multisets M with Σ√F ≤ L. It grows like **exp(2.53·δ₀^{−1/3})** — subexponential in 1/δ₀, so halving the floor does not square the disjunction — by Meinardus' theorem applied to parts {√F}, whose Dirichlet series is ζ(s/2). Convergence is slow and the asymptotic overstates the exact count by about 11× at the densities of interest, so **use the exact sums in range and the asymptotic only for the growth**. Derivation, fit and the exact counts: `shape-counting.md` §§1–2.

**Applying the reductions changes the constant, and one theorem changes the count.** Reduction (a) is already in N. Reduction (b) halves it. What drops from the covering set for a **density** reason is exactly one family: the shapes with **no foreign part** — the single fused class S2 and its multi-class relatives, which need n itself to carry a prime-power divisor at a bounded cofactor (§6.5). Every shape *with* a foreign part stays: n = F·c + r puts the prime-power divisor on n − r, not on n, and such shapes are available at a positive-density set of n for every F. So the covering set is **N_foreign(δ₀) = N(δ₀) − #{multisets}** — every feasible shape with at least one foreign part — and parity is exact on it: with every block size odd, n ≡ Σ_a F_a + i ≡ Σ_{all parts} F (mod 2), so **a shape serves exactly the n of the parity of its total fusion count**.

> **The earlier form of this subsection dropped every fused shape from the asymptotic covering set**, on the reasoning that fusion shapes "cover a density-zero set of n". That is true of S2 and false of S7: n = 2c + r\* needs c a prime power, not n. The purely additive count it quoted — 15 systems at 1/25, 9 odd / 6 even — excluded the very shapes §3 shows set the odd ceilings, and §6.6's own collapse onto n = 4c + r named a shape the count did not contain. The table made the inconsistency visible: fused-plus-foreign shapes win 42.7% of [6, 159,027], a share that has *grown* from 27% at the 2,600 frontier. The corrected counts are below; the *structure* of the argument — finitely many explicit systems, computable from δ₀ alone — is untouched.

| δ₀ | L | raw N(δ₀) | **with a foreign part** | at odd n | at even n | *purely additive (old count)* |
|---|---|---|---|---|---|---|
| 1/9 | 3.000 | 24 | **9** | 6 | 3 | 6 |
| 1/16 | 4.000 | 65 | **25** | 11 | 14 | 10 |
| 0.046210 (verified to 10⁶) | 4.652 | 112 | **42** | 22 | 20 | 10 |
| 0.04 = 1/25 (conjectured) | 5.000 | 164 | **66** | **37** | **29** | 15 |

**And then Proposition 1 of `ladder-completeness.md` cuts this to a fixed short list, unconditionally, wherever δ₀ ≥ 1/25.** Above C(n,2)/25 the B-optimal configuration is in the ladder's menu — S2, S3, or S7 at F ≤ 16 (S4/S5 being the same systems as S7 at F = 2) — or is S6 or S11, two foreign primes at a common top prime with or without one matching class; nothing else. If μ(n) ≥ δ₀·C(n,2) then the optimum reaches δ₀, so the covering disjunction may be taken over those shapes alone. Dropping S2 (no foreign part) and applying parity — c and every r odd, so n = Fc + r is odd iff F is even, n = Fc + r₁ + r₂ is odd iff F is odd, r₁ + r₂ is even — the covering set at δ₀ = 1/25 is

> **odd n: n = Fc + r for F ∈ {2, 4, …, 16} (8 systems) and n = Fc + r₁ + r₂ for F ∈ {1, 3, 5, 7, 9} (5 systems) — 13 in all;**
> **even n: n = Fc + r for F ∈ {1, 3, …, 15} (8), n = r₁ + r₂ (1), and n = Fc + r₁ + r₂ for F ∈ {2, 4, 6, 8} (4) — 13 in all,**

with the F-bounds set by the feasibility criterion (√F + 1 ≤ 5 gives F ≤ 16; √F + 2 ≤ 5 gives F ≤ 9), and F ≤ 16 agreeing with Proposition 1's own bound cap_F(1) > 1/25 ⟺ F ≤ 16 — two derivations of the same cut-off. The two-foreign systems cap at 1/9 (S6) and 1/16 (S11), so at any floor above 1/16 they drop and the set is the **eight** F·c + r systems of n's parity. *(These are counts of systems in the sense of §6.3(a); the raw 164 and the 66 remain the right numbers for how many shapes are* feasible*, which is what §6.1 sizes.)*
...
- **all blocks the same size** — one diagonal C_{c−1}, cyclic, with the top q-group coming free from permuting the blocks. This is n = F·c, the multiplicative engine, unconditional and available at every non-prime-power n — what is scarce is not the shape but a small cofactor (§2.1);
...
**Hence the shapes with no foreign part drop from the asymptotic statement, and the reason is a bounded cofactor rather than a bounded ω(n).** In the single-class case the argument is one line from §2.1's identity: δ_S2(n) = (Q(n) − 1)/(n − 1), so δ_S2 ≥ δ₀ holds exactly when Q(n) ≥ δ₀·n — that is, when n lies in the finite union ⋃_{F ≤ 1/δ₀} F·{prime powers}, a **density-zero** set. A shape of several matching classes and no foreign part is no better off, n being a sum of bounded multiples of prime powers. **A fused class beside a foreign prime is entirely different**: n = F·c + r\* needs c a prime power and r prime, a condition on n − r and not on n, available at positive density for every F — it is the odd-n carrier of §3, and it stays in the covering set. The table says the same: fused winners — winners with any F > 1 — are 59.2% of [6, 159,027], of which the single fused class S2 is 16.5% and *falling* (from 34% at the 2,600 frontier) while the fused-plus-foreign share is 42.7% and *rising* (from 27%). This is what licenses §6.4's N_foreign count and what the old "purely additive" count got wrong.4.

> **The old form of this argument gated the shape on ω(n) ≤ 2, and that is no longer even true**, let alone the reason: after the entangled-generator correction F need not be a prime power, and n = 78 = 6·13 is a three-prime witness at F = 6. The conclusion is unchanged and the route to it is shorter.
...
- **The density-zero families must be excluded by hand**, being bounded by no class ceiling: n a prime power (S1), a bounded cofactor n/Q(n) ≤ 1/δ₀ (S2 — not ω(n) = 2, §4.1), and the escapes of §4.3 — c a power of 2, c or (r−1)/2 a power of 3, and r = 2^a·u + 1 with u small. Each is O(n/log n) or thinner, so "most n" survives, but the statement is about the complement of an explicit sparse set.
- **The gap between consecutive ceilings bounds ε**, and it is not uniform: 0.0048 at n ≡ 11 (mod 12) — the gap from 7 − 4√3 down to (2 − √3)/4, as computed above — while at n ≡ 3, 7 (mod 12) the ceiling is rung B at η = 1/2 and the gap to the next rung down, cap₄(1) = 1/9, is ε < 1/8 − 1/9 ≈ 0.0139 — wider than at class 11, not absent. *(The identity cap_B(1/4) = cap_C(1/2) is arithmetic between two values neither of which is that class's ceiling, so it is not a tie condition and does not block the collapse; §3.3.5's box says why.)* Any ε quoted here must name the rung it is measured to, since the runner-up differs between a class's two mod-24 halves even where its value does not.
...
So: **the route yields robustness at floors well below the ceilings, and sharp single-system statements at floors just beneath them.** The closer δ₀ sits to a class ceiling the stronger the arithmetic and the harder the hypothesis — worth stating because it identifies which floor conjecture would be worth proving. A floor of 1/25 gives a 13-way disjunction at odd n (8 above 1/16); a floor just under 7 − 4√3 at n ≡ 11 (mod 12) gives a single Bateman–Horn system. *(A floor of that second kind is what §6.9's conditional theorem supplies, for almost all n — a link rather than a bridge, the two subsections answering different questions under one framework.)*

#### 6.7 The other direction: what a floor forces arithmetically

*§6.6 runs the implication backwards to **systems** — a floor makes some member of a finite explicit list solvable. This subsection runs it backwards to **arithmetic**, and the result is a statement of the same shape as the rungs of §3.6's shifted-prime ladder. It is `enumeration-proof.md` Proposition F.4.*

> Suppose μ(n) ≥ δ₀·C(n,2) for all large n. Then for each such n, **at least one** of: **(a)** n = M·p^b for a prime power p^b with M ≤ 1/δ₀ (up to 1 + O(1/n)); or **(b)** there is a prime **r ≥ δ₀(n−1)/2** with a **prime-power divisor Q of r − 1** satisfying **(r − 1)/Q ≤ D(δ₀) := 2(1 − √δ₀)²/δ₀**, and n decomposes with r as a part. Integers of the form (a) have density zero, so **(b) holds for almost all n**.

**What makes a converse available at all** is that the score is a *minimum* over orbitals, so every part must clear δ₀·C(n,2) on its own; a part does that only by being large **and** carrying a large twist. For a foreign part, r·Q ≥ δ₀n(n−1)/2 with r ≤ n forces Q ≥ δ₀(n−1)/2, and Q | r − 1 ≤ n − 1 then bounds the cofactor by 2/δ₀ — the crude form. Dividing by Q instead of bounding r ≤ n sharpens that: the cofactor is controlled by r's *share* of n, and a configuration with a foreign part has another part of support > √(δ₀n(n−1)), so r ≤ n − √(δ₀n(n−1)) and the bound improves to **D(δ₀) = 2(1 − √δ₀)²/δ₀** (`ep` F.4).

**The dependency, and why the two branches are not symmetric.** Branch (b)'s content is entirely in the word *prime-power*: with Q merely a divisor of r − 1 the statement is vacuous, since Q = r − 1 always works. What confines a **foreign** twist to a prime power is Lemma B′ — AGL(1,r) is nonabelian, so the translations occupy the abelian cyclic layer and the twist is pushed into the top q-group. A **matching** class has no such confinement: its twist is any divisor of c − 1, drawn from the cyclic layer, and may be the full c − 1. *That is why (a) has to be a genuine alternative rather than a weakened form of the same conclusion* — at n = M·p^b the multiplicative engine reaches density ≈ 1/M with no shifted-prime input at all. The layer asymmetry is doing all the work here, and layer-assignment claims have the worst record in this framework (`pending-checks.md` T8), so the reliance on B′ is worth keeping visible even though B′ is proved.

**The "almost all" is load-bearing rather than a hedge, and it is the only thing standing between (b) and vacuity.** Branch (a) is the all-matching case, and it constrains n itself: every class needs c_i > δ₀(n−1), which forces **n = M·p^b with M ≤ 1/δ₀**. Such n have density zero, so (b) covers almost all n — but the decay is slow, of order H_M/log n, and the surviving share is large. Branch (a) is the multiplicative engine's own branch, so by §2.1's identity δ_S2(n) = (Q(n) − 1)/(n − 1) the n it covers at a floor δ₀ are exactly those with Q(n) ≥ δ₀n: at δ₀ = 1/25 and n ≤ 10⁵ that is **43.0%** of integers (43.5% on the divisor form, the two differing by the endpoint convention). So branch (a) is not a sparse technicality at any computable range — it is where the framework needs no shifted prime at all, on nearly half of all n. **So there is no unconditional statement here, only an almost-all one:** at the multiplicative n the framework genuinely needs no shifted prime, and any claim that some weakened divisor condition still holds there would be empty, the twist being an arbitrary divisor.

> **Measured against the table, and the constant that comes back is 12.** Across all 144,299 rows of [6, 159,027] the three predicted inequalities hold without exception: (r − 1)/Q ≤ D(δ) at all **120,556** foreign primes in winning witnesses, and the crude 2/δ with them, r ≥ δ·n at every one — the sharper form, see `ep` F.4's box for why that is not the Proposition's own inequality — and M ≤ 1/δ at all **23,743** rows with no foreign part, branch (a) being keyed on the absence of a foreign block rather than on a part count (`converse_check.py`, in R1). Two of the tightest cases are worth naming. The all-matching bound is essentially *attained* at n = 159,014 = 2·79,507, ratio 0.999994 (0.9996 at n = 2594, the tightest case of the 2,600-frontier table) — the same row §6.1 finds sitting on the feasibility boundary, so two constraints derived by different routes bind at once there. And the largest cofactor (r − 1)/Q occurring anywhere is **exactly 12**, at n = 221 with r = 157, Q = 13. That is (BCG)'s own d ≤ 12, reached from the other direction and with no reference to it: the hypothesis's constant is not an artefact of how it was written down but the value the optima actually use. *(The linearity inequality r ≥ δ·n carries no comparable slack: r ≥ n√(δ(n−1)/n) holds outright, and δ < 1/4 at any configuration with a foreign part turns that into r ≥ 2δn, so the measured factor 2 there is arithmetic rather than headroom — see `ep` F.4's box.) The Proposition's cofactor bound is meanwhile loose by a factor ≈ 2.2 against that* — at the verified floor 0.046210 it gives D(δ₀) = 2(1 − √δ₀)²/δ₀ = 26.1 where 12 suffices (at the conjectured 1/25 it gives 32; the round trip below is quoted at other δ₀ again, so each use names its input) — which is where a further sharpening would start. (Note the slack figure tracks the *floor*, so it is range-dependent on any correct table and a change in it is not a finding.)

> **Why a Zhang-style route does not shortcut this.** The condition is to Sophie Germain what bounded prime gaps are to twin primes (§6.8 makes the parametrisation precise), which invites the hope that a GPY/Maynard argument could deliver bounded D directly. It cannot. Those methods prove that at least 2 of k linear forms are prime **without specifying which**, harmless for bounded gaps because the forms n + h_i are interchangeable — any two give a gap. Here the two primes are asymmetric: one must divide the other minus one. Being handed two primes among {n, 2n+1, …, Dn+1} is useless if n is not one of them, the large prime-power divisor of r − 1 having to come from n itself. A prime at a *designated* form is the twin-prime barrier, which those methods route around rather than break.

**The multiplicative half of (b) isolates as a standalone statement with no reference to n**, which is §6.8's subject. What cannot be isolated is (b)'s second clause — *n decomposes with r as a part* — a Goldbach-type condition on n; the literature supplies that coupling, but at a price in density that §6.8 sets out.

**So (BCG) and a density floor are equivalent up to constants — with the factorisation above as the fine print.** The round trip is stated here at the **note's** constant: (BCG_{1/5}) supplies r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350; the converse takes δ₀ back to a bounded cofactor, so it returns **D ≤ 630** rather than d ≤ 12. *That factor ≈ 52 is an artefact of which δ₀ is fed in, and it is the loosest available choice:* at the conjectured floor 1/25 the same converse returns D ≤ 32, a factor ≈ 2.7 (which is the form `enumeration-proof.md` F.4 quotes, against a measured maximum cofactor of 12), and at the asymptotic ceiling 7 − 4√3 it returns D ≤ 14.9, a factor ≈ 1.24. **The round trip is therefore much tighter than the headline 700 suggests, and any quotation of it should name the δ₀ it used.** **Four differences survive the round trip, and naming them is what keeps "equivalent" honest:**

- the converse yields a prime-**power** divisor where the hypothesis supplies a prime;
- it holds for **almost all** n, matching (BCG-AA) rather than (BCG-AL);
- the cofactor loosens, by a factor between ≈ 1.24 and ≈ 52 depending on the δ₀ fed in;
- what comes back is (SP) *together with* a per-n additive clause, not a single n-free statement — (SP) alone is strictly weaker, and §6.8(iv) makes the gap precise: at a polylogarithmic ρ it reaches n^{2−ε} for almost all n on its own, and the residue is a bounded-cofactor condition on the **companion** n − r.

**What does not degrade is the shape of the statement** — a shifted prime with a large prime-power divisor at *bounded cofactor*, rather than at exponent θ < 1. That is exactly the quantity §3.6's ladder measures, and exactly where Elliott–Halberstam stops short. So the framework is not merely *implied by* a shifted-prime hypothesis: at the θ = 1 end it **demands** one, which is the strongest available reason to think no substantially weaker arithmetic input can deliver a positive density floor.

#### 6.8 Hypothesis (SP): the multiplicative half as a standalone statement

*Branch (b) of Proposition F.4 is, stripped of n, a statement about a set of primes. Fixing its parameters rather than quantifying over them is what lets the four claims below each name which version they concern.*

Write **Q(r)** for the largest prime-power divisor of r − 1, and fix three parameters: an integer **D ≥ 1**, a window ratio **c ∈ (0,1)**, and a positive function **ρ**. Set

> **S_D = { r prime : (r − 1)/Q(r) ≤ D }.**

> **Hypothesis (SP_{D,c,ρ}).** For all large x, **|S_D ∩ [cx, x]| ≥ ρ(x)·π(x)**.

**The window is in the definition rather than derived from a cumulative estimate, and that is deliberate.** The coupling needs members of S_D inside a window proportional to n — branch (b) puts r in [δ₀(n−1)/2, n] — and a cumulative *lower* bound does not deliver one: it is compatible with S_D being empty on [x/2, x] and dense below. Recovering a window count from cumulative counts means bounding the difference |S_D ∩ [1,x]| − |S_D ∩ [1,cx]| below, which needs a lower bound at x **and an upper bound at cx** — the relationship the prime number theorem has to Bertrand's postulate, where the two-sided asymptotic yields π(x) − π(cx) ≫ x/log x and no one-sided statement does. Stating the hypothesis in window form keeps that content while leaving **ρ a genuine lower bound**: (SP_{D,c,ρ}) weakens as ρ shrinks, as c falls and as D grows, so the three parameters index a real family. *(Written as a cumulative asymptotic equality instead, ρ would be pinned to one function and the statement would be a single assertion rather than a family — which is the version to avoid.)*

*A structural remark on D.* Cofactors are essentially even: an odd d needs Q(r) = 2^k, so r = d·2^k + 1 is Fermat-like and such r are very rare — among S_12 below 2·10⁶ the odd cofactors d = 1, 3, 5, 7, 9, 11 contribute 5, 6, 4, 3, 5 and 2 members against 7,755 at d = 2 alone. So D is in effect a bound on the even cofactors, and (BCG)'s d ≤ 12 permits six of them.

**(i) Bounded multiplicative gaps are the weakest case: ρ = 1/π(x).** "Every interval [x, Λx] contains a member of S_D" is exactly **(SP_{D, 1/Λ, 1/π(x)})** — the window ratio is c = 1/Λ and the requirement is one member, not a proportion. Everything below is stated against this baseline.

**(ii) A density floor implies that baseline, with one constant in both roles.** If μ(n) ≥ δ₀·C(n,2) for all large n then **(SP_{D, c, 1/π(x)}) holds with D = D(δ₀) = 2(1 − √δ₀)²/δ₀ and every c < δ₀/2**. Branch (b) supplies a member of S_D in [δ₀(n−1)/2, n] for almost all n; if the window [cx, x] held none, then every n whose whole r-range sits inside it fails (b), and those n fill [2cx/δ₀, x], an interval of positive proportion, which branch (a)'s O(x/log x) integers cannot cover.

> **The strict inequality is the whole of the argument, not a tidying.** At **c = δ₀/2** exactly, an n has its r-range inside the window only if n ≤ x and n ≥ x + 1 — no such n exists, the emptiness implies nothing, and the claim is *unproved at its own endpoint*. Any c < δ₀/2 opens the range to a positive proportion of [cx, x] and the argument runs. So 1/c and 2/δ₀ coincide up to a factor **slightly above** 1 rather than exactly 1, which is the honest form of "one constant in both roles"; D is the separate quantity D(δ₀) = 2(1 − √δ₀)²/δ₀, smaller than 2/δ₀ and not tied to the window at all.

**(iii) Empirics and Bateman–Horn support far more than the baseline: ρ(x) ≍ 1/log x.** The count of r = dQ + 1 with Q a prime power is a Bateman–Horn system for each admissible d, predicting |S_D ∩ [cx,x]| ~ C_D·(li₂(x) − li₂(cx)) with C_D a singular-series sum over the admissible cofactors — hence **ρ(x) = (κ(D,c) + o(1))/log x**. Measured at x = 2·10⁶: κ ≈ **0.700** and **2.285** at (D, c) = (2, 0.04) and (12, 0.04), and **0.333** and **1.080** at c = 0.5, each stable to about 1% across x = 10⁵–2·10⁶. The D = 2 cumulative case is a clean check on the *prediction* rather than on the framework — it is Sophie Germain, whose constant should be 2C₂/2 = C₂ ≈ 0.660 after the r ≤ x versus q ≤ x/2 halving, against 0.692 measured. So the supported hypothesis exceeds the baseline by a factor of order x/log³x.

**(iv) What (SP) at that ρ would give, and where the obstruction actually sits.** With ρ(x) ≍ 1/log x, the route back is the exceptional-set argument of Shparlinski's Theorem 2, which converts a set of shifted primes with large Q into a statement about almost all n — supplying branch (b)'s additive clause. **Read against the proof, the density question answers *yes*, and the obstruction is somewhere else entirely.**

*What the machinery runs on.* Shparlinski's Theorem 2 has as its engine Balog–Sárközy's sumset theorem: for **arbitrary** sets 𝒜, ℬ ⊆ [1, N] with #𝒜·#ℬ ≥ cN log²N, some difference has a prime factor ≫ (#𝒜#ℬ)^{1/2}/log N. Shparlinski plays the exceptional set ℰ against the input set ℛ and reads off #ℰ ≪ x^{2γ}log²x / #ℛ. **The input set enters through its cardinality and nothing else** — no equidistribution, no arithmetic progressions, no circle method.

*So thinness costs one logarithm, not the argument.* At Baker–Harman's #ℛ ≫ x/log x the bound is x^{2γ−1}log³x; at an S_D-type input of relative density 1/log x, #ℛ ≫ x/log²x, and it becomes **x^{2γ−1}log⁴x** — still o(x) for every γ < 1, still polylogarithmic at γ = 1/2. A cardinality hypothesis divides straight through, so ρ ≍ 1/log x is *not* one logarithm short of what this machinery consumes.

*What blocks it is the companion exponent, and it blocks at every input density.* The certified prime factor is capped by (#ℰ·#ℛ)^{1/2}/log x ≤ x/(2√2·log x), which is **sub-linear** — so γ = 1 is unreachable even with a full-density input, the exceptional bound x^{2γ−1}log³x being vacuous there. Now match this against a floor: min{p²k, pkr, qr} ≥ δ₀n² forces p ≥ δ₀n with k ≤ 1/δ₀, i.e. the **companion** n − r must itself have a linear prime factor at bounded cofactor. That is the α = 1 endpoint condition again, on the other side of the equation, and the sumset sieve certifies neither the linear size nor any cofactor bound.

> **The clean demonstration: feed the endpoint set in and nothing improves.** Run Shparlinski's Theorem 2 with ℛ = S_D itself, under (SP) at any polylogarithmic ρ. Then q ≥ (r − 1)/D is linear in r, so the r-side term qr ≫ n²/D reaches Ω(n²) — and the min is still pinned at p²k ≈ n^{1+γ} by the companion. Substituting the endpoint hypothesis on the input side buys **nothing** on the exponent, which localises the whole of (BCG)'s difficulty in the companion clause rather than in S_D's thinness.

> **Which conjecture this is a consumer of.** The only congruence-sensitive input in the whole route is a statement about the single class **1 (mod Q)** — unsurprisingly, since `Q | r − 1` is exactly the condition for AGL(1,r) to carry a twist of order Q, and no other residue class corresponds to a subgroup. That specialisation of Elliott–Halberstam is strictly weaker than EH proper and is where all unconditional progress past level 1/2 lives; it is also what Baker–Harman's α measures. But **it fails at the endpoint exactly as EH does** (Friedlander–Granville, *Limitations III*, for every fixed residue), which is why (BCG) and (SP) have to be counting statements rather than equidistribution ones. `shparlinski-constants.md` §7.5.

*The constants behind this are worked out separately.* `shparlinski-constants.md` carries the argument with the polylogarithms and window constants written out — including the objective function itself, where BBKN's `min{p²k, pkr, qr}` is stated up to constants and two of its three terms differ from the true orbital sizes by a factor near 2 — it is a standalone, lightly audited working note, not part of the main line — and its quantitative form is the useful one: the machinery certifies a companion factor `p ≫ n/(log n)^{3/2}` at a constant-density input and `p ≫ n/(log n)²` at ours, against the `p ≥ δ₀·n` a floor needs. **Thinness costs half a power of log; the endpoint costs two.**

*The conditional this does yield, which is worth stating positively.* **(SP_{D,c,ρ}) at any ρ ≍ 1/log^C x implies f(n) ≫ n^{2−ε} for almost all n, for every ε > 0**, with exceptional set O(x^{1−2ε}log^{C+3}x) — the ladder's limiting exponent, almost-all, from the bounded-cofactor hypothesis alone and with no Baker–Harman input. What it does not give is Ω(n²), and the gap between them is exactly the companion endpoint.

*How thin S_D is, in the currency the counting actually uses.* Counted inside the integers rather than inside the primes, and measured at x = 2·10⁶: the primes have density 0.0745 ≈ 1.08·(1/log x), the Baker–Harman set 0.0253 ≈ 0.37·(1/log x), and **S_12 0.0127 ≈ 2.67·(1/log²x)** — the twin-prime order, one further logarithm down. For the cardinality-only argument of (iv) that costs one power of log x in the exceptional bound and nothing else. It matters against a *different* tool, named below.

*Which requirement belongs to which route — the correction that matters most here.* Equidistribution in arithmetic progressions is what a **circle-method** route would need: major arcs are built from the distribution of the summand sets, minor arcs from an exponential-sum bound, and (SP_{D,c,ρ}) as stated supplies a count and nothing else, so it would be insufficient for that route however large ρ was. **§6.9(c) now settles that in the strong form** — a cardinality bound at exactly this scale is not merely unused by the major arcs but demonstrably insufficient, by an explicit set of primes at the (SP) count for which almost all n are unrepresentable — *for sets treated as arbitrary*. For the actual S_D the question stays open, since the counterexample's concentration is unconditionally ruled out by sieve upper bounds; §6.9(c)'s table keeps the three statuses apart. **Shparlinski's Theorem 2 is not that route** — Balog–Sárközy is a sieve-based sumset theorem whose hypothesis is a pure cardinality condition, so no equidistribution clause is required for the argument of (iv). What *does* have a density appetite is the endpoint tool: to certify a **linear** prime factor, hence a bounded cofactor, the available result is Sárközy–Stewart's dense-sumset theorem, which wants **positive density in the integers on both sets**. S_D at ≈ 1/log²x is two logarithms short of that — and this is the honest form of the "density" obstruction, sitting at the endpoint rather than at ρ ≍ 1/log x. The heuristic count of representations n = c + r with r ∈ S_D remains ≫ n/log³n, and the window is not a difficulty either — at c = 0.04 it holds 92.5% of the cumulative count against 94.7% for the primes (both at x = 2·10⁶, on the same cutoff).

> **What the ladder actually runs out of, and it is not density.** This is the second reason §3.6's ladder stops short of a constant, independent of the level-of-distribution barrier recorded there — but the two quantities to keep apart are the **input** set's α and the **output** exponent γ. Raising α toward 1 thins the input set, and (iv) shows that costs one power of log x in the exceptional set and nothing more. What is capped is γ: the sumset sieve certifies only a sub-linear prime factor in the companion, at any input density, so it delivers n^{1+γ} with γ < 1 and never Ω(n²). So n^{2−ε} and Ω(n²) are not adjacent points on one scale, and (SP) at ρ ≍ 1/log x sits in the gap — reaching the first for almost all n while the second waits on a linear-factor-with-bounded-cofactor statement about the companion.
>
> **What decides the reach, stated positively (§6.9(d)).** The completed circle-method route consumes only the input set's *cardinality* on the minor arcs and its *exact local densities* on the major arcs, and no level of distribution anywhere. So the discriminating property of a hypothesis in this family is **whether it pins local densities**, not how thin its set is: a cardinality bound fails at any density, an exact local structure succeeds at ≈ 1/log²x. That is why the honest reading of the obstruction is the companion clause and not S_D's thinness, and it is the same conclusion (iv) reaches by a different road.

#### 6.9 The circle-method route: what it settles, and with what standing

*This section reports a companion working note, `sp-to-floor.md`, which carries out the route §6.8(iv) identifies as the one a positive-density conclusion would have to take. The findings are tiered by standing, deliberately: two of them are arithmetic and hold whatever becomes of the note's analytic half, one is an elementary counterexample, and the last is a conditional theorem with one reader. Reading them at a uniform level of confidence would be a mistake in both directions.*

**(a) The window optimum is this document's ceiling formula. [ARITHMETIC — independent of the route.]** Optimising the note's reduction over its window gives the constant `δ(k,d) = 1/(√k + √(d/2))²` for a configuration of `k` fused blocks with foreign cofactor `d`. That is **`cap_k(2/d)` identically** — §3.3.5's cap at `F = k`, `η = 2/d` — and the balance point matches `x*` cell for cell. This is a fact about the two formulas, verified algebraically and at all six cells; it does not depend on the circle method, on (BH-SW), or on any step the note takes on citation.

**(b) The two derivations select the same (F, d) at every residue class. [ARITHMETIC.]** More than the formulas agreeing: the note's grid assigns `(1,2)` at `n ≡ 0, 4, 6, 10`, `(1,6)` at 2, 8, `(2,2)` at 1, 9, `(2,4)` at 3, 7, `(2,6)` at 5, and **`(4,6)` at 11** — matching §3.5.3's clause 3 in every cell, including the load-bearing `F = 4`. The two routes share no input. This document's assignment comes from enumerating which Oliver chains exist; the note's comes from solving a congruence for a circle-method window, and its constraints are conditions on `r − 1`'s divisor structure. **So the mod-12 keying is intrinsic to the arithmetic of `r = dQ + 1`, not an artefact of how either table was built** — which is why §3.5.4's gotcha about the shared modulus now stands in a weakened form rather than as stated.

**(c) (SP) as a statement about arbitrary sets is refuted; for the actual S_D it remains open. [ELEMENTARY, and the three-way status is the point.]** The note constructs, at exactly the (SP) scale, a set of primes concentrated in one class mod `m` with `φ(m) ≍ log x` for which almost all `n` are unrepresentable. So:

| statement | before | after |
|---|---|---|
| (SP) suffices, treating S_D as an arbitrary set of that cardinality | open | **refuted** |
| (SP) suffices for the actual S_D | open | **open** |
| the obstruction is the companion exponent, not S_D's thinness | §6.8(iv) | unchanged, reinforced |

The middle row is the one to keep. The counterexample's structure is **unconditionally absent** from S_D — Selberg/Brun–Titchmarsh upper bounds for the pair `{Q, dQ+1}` in a residue class cap its concentration well below the (SP) count — so what is refuted is the *generic* implication, and the route that survives is transference against the sieve majorant (`pending-checks.md` A25). **Reading (c) as "we now know (SP) does not suffice" would overstate it in precisely the direction the note is careful about.**

**(d) The division of labour, sharpening §6.8(iv).** In the completed route the minor arcs consume **only the cardinality** of the input set, the major arcs consume the hypothesis's **exactness** about local densities, and no level of distribution is consumed anywhere. This is (iv)'s finding reached independently, and it sharpens the moral: what decides whether a hypothesis in this family can reach a constant is **whether it pins local densities**, not how thin its set is. A cardinality bound at any density fails; an exact local structure at the same density succeeds.

**(e) The conditional theorem, cited rather than restated. [ONE PASS, ONE READER.]** Assuming Bateman–Horn for the four pairs `{Q, dQ+1}`, `d ∈ {2,4,6,12}`, with Siegel–Walfisz uniformity to polylogarithmic moduli — the note's (BH-SW) — the route yields, for almost all `n`, `μ(n) ≥ (δ(n mod 12) − ε)·C(n,2)`. By (a) and (b) those constants are this document's ceilings, so the conclusion is **not** "a positive constant that beats 1/25" but

> **almost all n attain their own class ceiling, up to ε** — the exact asymptotic half of §5's floor conjecture, meeting the ceiling from below, with binding constant `7 − 4√3` at `n ≡ 11 (mod 12)`.

**Its standing, which should travel with it.** One pass and one reader; two steps used on citation rather than rewritten (a Vinogradov dilation lemma, the Siegel–Walfisz major-arc manipulation); the singular-series completion sketched for squarefree `q` only; the reduction's orbital structure verified by enumeration at five `(p,k)` pairs rather than proved in general; and its constants re-derived once, after an error of a factor 2 in the foreign orbital was found and corrected. The direction and the shape of the conclusion are what to rely on; a second reading is filed as `pending-checks.md` A27 and is worth more now that (a)–(d) are quoted here.

*In §3.5.3's names this is the implication **(BH-SW) ⟹ (BCG-AA)**, and the statement there carries the same standing as this paragraph rather than the standing of the surrounding clauses.*

---

## 7. What this says about the open problems

**The odd-n route above 1/9 is refuted.** It asked for a constant above 1/9 bounding δ from below on odd n, so Theorem E.1 would settle the collapse there wholesale. No such constant exists: a substantial share of the odd n in the computed table have δ(n) < 1/9, and these are exact values of μ, not shortfalls of any family. Worse, the share **grows** across the range. **⟦PENDING-REBUILD⟧** *The shares themselves are table outputs and move with it; the trend is what closes the route, and it is what to read off the rerun.* The route is closed permanently, so **Open Problem 8(b) must be settled by promoting E.3(ii) directly**, which is the only remaining path.
...
**Open Problem 8(a) (k ≤ 3)** is the statement that the four-class cap 1/16 is never the best available, which needs ω(n) ≥ 3 together with no good two- or three-class representation. It has never occurred: no winner in the computed table uses four classes. *The δ ≤ 1/16 tail is now **7 values over the contiguous range — n = 527, 1175, 2075, 2183, 2279, 2303, 2507** — down from 18 under v4, with n = 2759 an eighth if the out-of-range row is counted; it remains a table output and moves with any extension, and `check_doc_figures.py --pass scope` reports the current list, and a sentence naming members is stale the moment the frontier advances.* The branch-and-bound of §5 adds a little: it examined every n ≤ 10⁶ whose lower bound fell below the running floor, and none of them wanted a fourth class either.

**Open Problem 8(b)** lives where the three-class family is the best available. With the above-1/9 route refuted, the only path is the direct one. It also **gets harder as the density floor falls**, since s < 1/√δ − 1: at the ladder's verified floor over 10⁶ the cap is s ≤ 3.65, so s = 4 and s = 5 are out of reach there and E.4's collapse of s = 3 to a single dead pair is the whole remaining content. That the branches with no theorem are unreachable is a **consequence of the current floor and expires with it** — recompute the cap rather than carrying the conclusion forward.
...
2. **Bound the s = 4 and s = 5 branches.** The only item here that is a gap in a *proof* rather than in evidence — and it is now the *whole* of that gap, since Part E‴ closed the s = 2 leftover case that used to sit beside it. Note that these branches are unreachable above δ = 1/25 (the s-ladder) and the fallback branch as a whole is capped below it (Theorem E.5), so the two statements meet at one constant: **what is owed is the regime δ ≤ 1/25 and nothing else.** *Recount after the rebuild:* at n = 3239 and 3059 the density rises sharply under the corrected shape space, so both leave the sub-1/25 set and the branch may narrow without any new theorem. E.1 caps s = 1 by the Mersenne constants and E.3(iii) caps the s = 2 repunit branch; s = 4 has neither, and is not thin enough for an E.4-style collapse. An absolute cap would have to come from the foreign block's twist, as in those two. The search clears it at every computed n, so nothing is unproved — but the gap widens as the floor falls.

3. **Predict the 1/12 shortfall from the singular series.** The odd/even split is **10.0% of odd (6,477 of 64,803) and 0.001% of even (1 of 79,496)** values below 1/12, over [6, 159,027] — at the 2,600 frontier it was 6.7% and 0.08%, so the odd shortfall has *grown* with the range while the even one has all but vanished, which is itself the prediction to test: even n have the 1/4 class with a two-part shape always in supply, odd n pay the fusion count. Both engines' availability is computable heuristically, so this compares the whole framework of this document against measurement rather than testing any single family.
...
6. **When does the single fused class lose to a split?** Over [6, 159,027], S2 wins at 23,743 values and loses to a two-part configuration at the rest; the natural predictor is the cofactor F = n/Q(n), since S2 is worth ≈ 1/F and a split near the class ceiling is worth δ₀ ≈ 0.07–0.25. The distribution of n/Q(n) over the integers is classical, so predicting the S2 win count from it is a clean test. *(The earlier form of this item sliced by ω(n) = 2 — 738 / 472 at the 2,600 frontier, 23,736 / 28,146 here — which was the pre-entangled reading of when a fused class exists; seven S2 winners have ω(n) = 3, and 15,255 ω(n) = 2 values have cofactor above 25, so the slice is the wrong one. Slice by n/Q(n).)*

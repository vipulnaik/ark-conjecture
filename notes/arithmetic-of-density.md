...
> *Verified, with one correction the corrected shape space forces.* Of the 777 one-part winners over the contiguous range to n = 2,600 (2,186 rows), **773 have ω(n) = 2 and four do not** — n = 282, 894, 1434 (all `6x p`) and n = 1490 (`10x149`). **The exceptions are exactly the composite-F fusions**: F = 6 and F = 10 are available only because F_mid need not be a prime power, and n = F·c then carries three prime factors. Under a shape space with F a q-power the statement "every one-part winner has ω(n) = 2" is a theorem — n = F·c with c a p-power has ω(n) ≤ 2 — so this is a place where the correction changes a structural claim and not only a count. Of the 1,210 values with ω(n) = 2, **773** are one-part winners and the other **437** do better with a split.
...
|---|---|---|---|
| **q odd** (cyclic-layer, S7 at F = 2) | 395 | **no congruence: c mod 8 = {1: 92, 3: 100, 5: 101, 7: 96}**, i.e. **193 at c ≡ 1 (mod 4)** against 196 at c ≡ 3, plus 6 at p = 2 | unrestricted |
...
> **Recounted over the completed table**, all 395 cyclic-layer winners rather than the 806-row prefix the provisional figures came from. The c mod 8 distribution is **{1: 92, 3: 100, 5: 101, 7: 96}** — flat to within 5%, and **48.9% now sit at c ≡ 1 (mod 4)** against 9% under v4. The prediction was that the congruence would disappear rather than merely weaken, and at four residues within a few percent of each other over 395 winners it has: what remains is sampling noise, not a residual preference.
...
> - **The second-best shape differs between those halves, though its value does not.** Both fall to (2 − √3)/4 = 0.066987 if the F = 4 optimum is unavailable, but the half n ≡ 11 (mod 24) reaches that at **F = 2** with η = 1/6 while the half n ≡ 23 reaches it at **F = 6** with η = 1/2 — its F = 2 rung being worthless at η = 1/12. Any argument that needs the *runner-up* rather than the winner must therefore split the row; §7's disjunction-collapse is one such.
>
...
> **CONFIRMED over the contiguous range [6, 2600]: 0.048039 at n = 2183 = 37·59** — the global minimum to 10⁶ is lower, 0.0462099 at n = 2759 (§5), which lies outside this range —, witness `6x251 + 1x677*` (v4's floor **0.045742 at n = 1817**, witness `p=389 q=173: 1x1039* + 2x389`, rises to ≥ 0.0594 under the corrected caps),

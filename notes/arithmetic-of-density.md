...
**So (BCG) and a density floor are equivalent up to constants — with the factorisation above as the fine print.** The round trip is stated here at the **note's** constant: (BCG_{1/5}) supplies r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350; the converse takes δ₀ back to a bounded cofactor, so it returns **D ≤ 700** rather than d ≤ 12. *That factor ≈ 58 is an artefact of which δ₀ is fed in, and it is the loosest available choice:* at the conjectured floor 1/25 the same converse returns D ≤ 50, a factor ≈ 4 (which is the form `enumeration-proof.md` F.4 quotes, against a measured maximum cofactor of 12), and at the asymptotic ceiling 7 − 4√3 it returns D ≤ 28, a factor ≈ 2.3. **The round trip is therefore much tighter than the headline 700 suggests, and any quotation of it should name the δ₀ it used.** Four differences survive it, and naming them is what keeps "equivalent" honest: the converse yields a prime-**power** divisor where the hypothesis supplies a prime; it holds for **almost all** n, matching (BCG-AA) rather than (BCG-AL); the cofactor loosens by a factor ≈ 58; and what comes back is (SP) *together with* a per-n additive clause rather than a single n-free statement, since (SP) alone is strictly weaker than (BCG) — §6.8(iv) makes that last gap precise: (SP) at a polylogarithmic ρ reaches n^{2−ε} for almost all n on its own, and the residue is a bounded-cofactor condition on the **companion** n − r. **What does not degrade is the shape of the statement** — a shifted prime with a large prime-power divisor at *bounded cofactor*, rather than at exponent θ < 1. That is exactly the quantity §3.6's ladder measures, and exactly where Elliott–Halberstam stops short. So the framework is not merely *implied by* a shifted-prime hypothesis: at the θ = 1 end it **demands** one, which is the strongest available reason to think no substantially weaker arithmetic input can deliver a positive density floor.

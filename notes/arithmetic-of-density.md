...
> **The condition is to Sophie Germain what bounded prime gaps are to twin primes** — d ≤ D in place of d = 2, with the ladder's θ → 1 corresponding to letting D grow like r^ε. The analogy invites the hope that a Zhang/Maynard-style argument could deliver bounded D without the ladder, and it is worth recording why it does not. Those methods prove that at least 2 of k linear forms are prime **without specifying which**, which is harmless for bounded gaps because the forms n + h_i are interchangeable. Here the two primes are asymmetric — one must divide the other minus one — so being handed two primes among {n, 2n+1, …, Dn+1} is useless if n is not one of them, the large prime-power divisor of r − 1 having to come from n itself. A prime at a *designated* form is the twin-prime barrier, which those methods route around rather than break. What the analogy does leave is a quantitative reading: bounded-cofactor primes have density ≈ C·log D/log x among primes (measured at D = 12: 0.343, 0.235, 0.182 at x = 10⁴, 10⁵, 10⁶), so the set is twin-prime-thin and (H) is a Goldbach condition over a set that thin.

**The multiplicative half isolates as a standalone hypothesis, with no reference to n** — and the right parameter for it is a *density*, not a gap. Write Q(r) for the largest prime-power divisor of r − 1, and S_D for the set of primes with **(r − 1)/Q(r) ≤ D**.

> **Hypothesis (SP_{D,ρ}).** There is a constant D and a function ρ such that, for all large x, the count of S_D up to x is **ρ(x)·π(x)·(1 + o(1))** — a *two-sided* estimate, not a lower bound.

**Why two-sided, which is not fussiness.** A cumulative lower bound is compatible with S_D being empty on [x/2, x] and dense below, and an empty window is exactly what would kill the coupling: branch (b) needs r in a **proportional window**, [δ₀n, (1 − δ₀)n], and lower-bounding
> |S_D ∩ [cx, x]| = |S_D ∩ [1, x]| − |S_D ∩ [1, cx]|
requires a lower bound at x **and an upper bound at cx**. Two-sidedness is what makes the subtraction legal, not a strengthening for its own sake. This is the same relationship the prime number theorem has to Bertrand's postulate: the cumulative asymptotic, being two-sided and tight, yields π(x) − π(cx) ≫ x/log x for fixed c < 1, which no one-sided density statement gives. With a two-sided estimate in hand the window count follows immediately, at ρ(x)(1 − c)x/log x up to the usual factors, and **bounded multiplicative gaps are then the weakest special case** — the window ratio Λ with the count merely required to be ≥ 1, which is why a floor implies the gap version for free while the density version is the substantive hypothesis.

> **The shape S_D actually has, measured.** |S_12| against x/log²x runs 3.57, 2.99, 2.72, 2.67 at x = 10⁴, 10⁵, 10⁶, 2·10⁶ — settling slowly, consistent with a count of order x·log D/log²x and so with ρ(x) ≈ C·log D/log x rather than a constant. The proportional-window behaviour is already benign: at c = 0.04, the window [0.04x, x] holds 89.5%, 92.0% and 92.5% of the cumulative count over the same range, against 96% for the primes themselves. So S_D is not concentrated low, and nothing in the data suggests the window is the obstacle — the obstacle is the size of ρ, not its localisation.

**Why the density and not the gap.** Bounded multiplicative gaps — every [x, Λx] containing a member of S_D — is what a floor *implies*, and it implies it with Λ = D = 2/δ₀ from the single inequality of branch (b): if some [x, (2/δ₀)x] held no member, every n in its upper part would fail (b), and branch (a)'s n are density zero and so cannot fill an interval. But the gap condition is not what a floor would be *recovered from*. The route back is Shparlinski's Theorem 2 machinery, which converts a set of primes with large Q into a statement about almost all n by an exceptional-set argument — supplying exactly the additive coupling that no purely multiplicative statement can. **That machinery consumes relative density**, not gaps: it is applied to the Baker–Harman set {r : Q(r) ≥ r^α}, which has positive relative density for each fixed α < 1.

> **And this is precisely why the ladder tops out where it does.** The relative density of {r : Q(r) ≥ r^α} is ≈ **1 − α** — measured among primes below 10⁶ as 0.650 at α = 0.5, 0.338 at 0.677, 0.173 at 0.8, 0.055 at 0.9, against 1 − α of 0.50, 0.32, 0.20, 0.10. So the input to the coupling **thins to nothing exactly as α → 1**. The bounded-cofactor condition is the α = 1 endpoint, where the set has relative density only ≈ C·log D/log x — at D = 12, measured as 0.343, 0.235, 0.182 over x = 10⁴, 10⁵, 10⁶. **The exponent 2 and the positive density are competing for the same resource:** any α < 1 buys positive density and yields n^{1+α}, while α = 1 buys Ω(n²) and costs the density the coupling runs on. That is a structural account of §3.6's ceiling, not a restatement of it — the ladder does not stop at 2 − ε because nobody has pushed harder, but because the machinery's input vanishes in the limit.

**So the open question is sharper than "prove (SP)".** It is whether the exceptional-set step survives an input whose two-sided relative density is of order 1/log x — one logarithm short of positive, rather than a power short. That is not obviously hopeless: the expected number of representations n = c + r with r ∈ S_D remains ≫ n/log³n, so the heuristic count is comfortable, and what would have to be checked is whether the circle-method error terms tolerate the loss and whether S_D is well enough distributed in arithmetic progressions — the latter being, once again, a level-of-distribution question about the modulus Q, which is where this framework's arithmetic always lands. **Filed as a literature item** (`pending-checks.md` T4), since it is the same reading task as the fixed-residue question and shares its sources.

*What (SP) is not.* It is not sufficient for a floor on its own, and not merely by the almost-all quantifier: branch (b) carries a second clause — *n decomposes with r as a part* — which is Goldbach-type, and a multiplicative hypothesis supplies it only through the coupling step above. So **(H) factors into (SP) and a coupling**, the coupling being available in the literature for positive-density inputs and open at ours. That is the mirror of §3.5's point that (H) is not of Goldbach type alone: it is not of Sophie Germain type alone either.

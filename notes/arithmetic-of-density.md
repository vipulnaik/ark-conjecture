...
|---|---|---|---|
| **q odd** (cyclic-layer, S7 at F = 2) | 396 | **no congruence: c mod 8 = {1: 92, 3: 100, 5: 102, 7: 96}**, i.e. **194 at c ≡ 1 (mod 4)** against 196 at c ≡ 3, plus 6 at c even (p = 2) | unrestricted |
...
> **Recounted over the completed table**, all 396 cyclic-layer winners rather than the 806-row prefix the provisional figures came from. The c mod 8 distribution is **{1: 92, 3: 100, 5: 102, 7: 96}** — flat to within 5%, and **49.0% of the odd-c rows sit at c ≡ 1 (mod 4)** against 9% under v4. The prediction was that the congruence would disappear rather than merely weaken, and at four residues within a few percent of each other over 395 winners it has: what remains is sampling noise, not a residual preference.
...
is proved, the ladder alone supplying every value's lower bound: every one of the 10⁶ values scores at least that, with the minimum at **n = 2759 = 31·89** and the next-lowest entry at 0.04801 (n = 11183). In particular **δ(n) ≥ 1/25 throughout**, with 15.5% to spare. And at the argmin the bound *is* a value: **B(2759) = 175,813** is in the table, equal to the ladder's own score there, so the minimum is determined exactly rather than bounded — which is what §5's box above states and what makes n = 2759 the unique global minimiser rather than merely the lowest entry.

**What the branch-and-bound has left to do.** The worklist holds **45,390** entries — every n whose family score falls below the asymptotic ceiling 7 − 4√3 — but **none below 1/25**, so it has nothing left to eliminate against the conjecture as stated, and the adaptive run against a floor of 0.04 returns an **empty** below-floor list without computing a single B(n). Its remaining use is sharpening: the operative question is **"does B(2759) exceed 0.04621"**, and more generally whether any of the ten lowest entries (2759, 11183, 2183, 6275, 1739, 10595, 5063, 3503, 2639, 15563 — all in the 10³–10⁵ band where the two engines cross) is tight. B is already known at the lowest of them: **B(2759) = 175,813 equals the ladder's score there**, which is what fixes the minimum below 10⁶ exactly; computing B at the other nine would raise the verified floor above 1/25 with real margin.
...
**The global minimum moved down a decade under the corrected scoring.** It was 0.04453 at n = 11183; the entangled correction lifts the [10⁴, 10⁵) decade to 0.04801 — 11183 stays its argmin — while [10³, 10⁴) is lifted less, so the binding value is now **0.04621 at n = 2759**. The decade holding the global minimum and the one after it recover, so the region where neither argument reaches is a single decade wide and its worst value clears 1/25.
...
> Suppose μ(n) ≥ δ₀·C(n,2) for all large n. Then for each such n, **at least one** of: **(a)** n = M·p^b for a prime power p^b with M ≤ 1/δ₀ (up to 1 + O(1/n)); or **(b)** there is a prime **r ≥ δ₀(n−1)/2** with a **prime-power divisor Q of r − 1** satisfying **(r − 1)/Q ≤ 2/δ₀**, and n decomposes with r as a part. Integers of the form (a) have density zero, so **(b) holds for almost all n**.

**What makes a converse available at all** is that the score is a *minimum* over orbitals, so every part must clear δ₀·C(n,2) on its own; a part does that only by being large **and** carrying a large twist. For a foreign part, r·Q ≥ δ₀n(n−1)/2 with r ≤ n forces Q ≥ δ₀(n−1)/2, and Q | r − 1 ≤ n − 1 then bounds the cofactor.
...
**So (BCG) and a density floor are equivalent up to constants — with the factorisation above as the fine print.** The round trip is stated here at the **note's** constant: (BCG_{1/5}) supplies r − 1 = dq with d ≤ 12 and q prime — a prime factor of *linear* size — and yields δ₀ = 1/350; the converse takes δ₀ back to a bounded cofactor, so it returns **D ≤ 700** rather than d ≤ 12. *That factor ≈ 58 is an artefact of which δ₀ is fed in, and it is the loosest available choice:* at the conjectured floor 1/25 the same converse returns D ≤ 50, a factor ≈ 4 (which is the form `enumeration-proof.md` F.4 quotes, against a measured maximum cofactor of 12), and at the asymptotic ceiling 7 − 4√3 it returns D ≤ 28, a factor ≈ 2.3. **The round trip is therefore much tighter than the headline 700 suggests, and any quotation of it should name the δ₀ it used.** **Four differences survive the round trip, and naming them is what keeps "equivalent" honest:**
...
- the cofactor loosens, by a factor between ≈ 2.3 and ≈ 58 depending on the δ₀ fed in;
...
**(ii) A density floor implies that baseline, with one constant in both roles.** If μ(n) ≥ δ₀·C(n,2) for all large n then **(SP_{D, c, 1/π(x)}) holds with D = 2/δ₀ and every c < δ₀/2**. Branch (b) supplies a member of S_D in [δ₀(n−1)/2, n] for almost all n; if the window [cx, x] held none, then every n whose whole r-range sits inside it fails (b), and those n fill [2cx/δ₀, x], an interval of positive proportion, which branch (a)'s O(x/log x) integers cannot cover.

> **The strict inequality is the whole of the argument, not a tidying.** At **c = δ₀/2** exactly, an n has its r-range inside the window only if n ≤ x and n ≥ x + 1 — no such n exists, the emptiness implies nothing, and the claim is *unproved at its own endpoint*. Any c < δ₀/2 opens the range to a positive proportion of [cx, x] and the argument runs. So D and 1/c coincide up to a factor **slightly above** 2 rather than exactly 2, which is the honest form of "one constant in both roles."

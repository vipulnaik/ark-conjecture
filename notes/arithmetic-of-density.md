...
**The multiplicative half isolates as a standalone hypothesis, with no reference to n.** Branch (b) is a statement about a *set of primes*, and it is worth extracting as one, both because it is the cleanest object the converse produces and because it is the natural multiplicative counterpart to a bounded-gap hypothesis. Write Q(r) for the largest prime-power divisor of r − 1.

> **Hypothesis (SP_{D,Λ}).** There are constants D ≥ 2 and Λ > 1 such that the set of primes r with **(r − 1)/Q(r) ≤ D** has **bounded multiplicative gaps**: every interval [x, Λx] with x large contains one.

Two parameters, and both are needed. D alone with "infinitely many" is too weak — a density floor needs a good prime *at every scale*, not merely infinitely often — and Λ is exactly what upgrades infinitude to every scale.

**A density floor gives (SP), and gives it with one constant serving both roles.** Branch (b) supplies, for almost all n, a good prime in [δ₀(n−1)/2, n]. Bounding the *gap* takes no further work: if some interval [x, (2/δ₀)x] held no good prime, every n in the upper part of it would fail (b), and since branch (a)'s n have density zero they cannot fill an interval, so some such n would violate the floor. Hence **D = Λ = 2/δ₀** — the same inequality bounds the cofactor and forbids the gap, which is why one constant does both.

**But (SP) does not imply a density floor, and the gap is not the quantifier.** Branch (b) has a second clause — *n decomposes with r as a part* — which is a Goldbach-type condition on n, and no n-free hypothesis can carry it. Good primes dense at every scale do not make n − r a prime power for any of them, and prime powers are sparse. So the right statement is that **(H) factors into (SP) and an additive coupling**, of which the converse recovers the first n-freely and the second only per n. This is the mirror of §3.5's point that (H) is not of Goldbach type alone: it is not of Sophie Germain type alone either, and (SP) is exactly the Sophie Germain-type factor.

> **How weak (SP) is against the expected truth, measured.** At D = 12 the largest multiplicative gap between successive good primes below 2·10⁶ is **1.041** counting from r > 10³, **1.013** from 10⁴ and **1.003** from 10⁵; at D = 2 — Sophie Germain itself — the same figures are 1.165, 1.029 and 1.009. The gaps are evidently tending to 1, as the density heuristic predicts (≈ 117 good primes at D = 12 in [10⁶, 1.01·10⁶]). So (SP) should hold for **every** Λ > 1 and quite small D, and the version a floor of 1/25 would need — D = Λ = 50 — is enormously weaker than the data suggests. That is the right shape for a hypothesis to be in: what makes (SP) hard is not its constants but that D bounded is the Sophie Germain endpoint of the ladder, where no unconditional method reaches.

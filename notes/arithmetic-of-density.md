...
**Where the verification is hardest is a middle range, and it is bounded.** The lower envelope of achievable density does not fall away as n grows — it dips and then recovers. Minimum bound over the **45,390-entry** worklist, by decade:

| n | entries | minimum bound | attained at |
|---|---|---|---|
| [10², 10³) | 6 | 0.05703 | 527 |
| [10³, 10⁴) | 251 | 0.04574 | 1817 |
*This table is the **worklist scan to 10⁶**, a different artifact from the μ table, and it has now been rerun; the decade rows above are from the corrected-scoring run. What the completed μ table settles is only its overlap: over [10³, 2600] the minimum is **0.048039 at n = 2183**, and the `0.04574 at 1817` entry does not survive the corrected cap, that row now scoring ≥ 0.0594. Whether 1817 remains the decade minimum out to 10⁴ is a question for the rerun, not for this table. The per-block floor table and the block-by-block recovery narrative are outputs of the superseded scan; regenerate them from the rerun before quoting any row.*

> **The worklist is an even split between the two extremal classes, which is the mod-24 prediction landing hard.** Of the 45,390 entries, **22,439 (49.4%) are n ≡ 11 (mod 24) and 22,944 (50.5%) are n ≡ 23**, with 7 spread across the other residues. Those two classes share a ceiling (§3.3.5), and the split is as even as one could ask for. It is worth remembering what a worklist thresholded at a class ceiling can and cannot show: the earlier list, cut at 0.050510, was 99.92% class 23 — not because class 23 is worse but because that number *was* class 23's ceiling under the older unfused reading (it is cap₄(1/6), not cap₂(1/6)), so it appeared on merely falling short while class 11 had to fall 25% below a higher one. **A residue's prominence in a worklist is a fact about the threshold, not about the residue.**
...
> **The per-residue diagnostic saturates early, so extending N does not test it further.** **⟦PENDING-REBUILD⟧** *Where each per-class minimum is attained is a run output; the observed pattern is that all are attained early, so the min-ratio spread stabilises long before the scan ends.* Read it as a statement about small n, and do not expect a wider scan to move it.
...
is proved, the ladder alone supplying every value's lower bound: every one of the 10⁶ values scores at least that, with the minimum at **n = 2759 = 31·89**. ⟦PENDING-LADDER-RERUN⟧ *The margin to the next entry is larger than the worklist shows, the runner-up 0.04801 at n = 11183 being an S7-strip artefact.* In particular **δ(n) ≥ 1/25 throughout**, with 15.5% to spare. And at the argmin the bound *is* a value: **B(2759) = 175,813** is in the table, equal to the ladder's own score there, so the minimum is determined exactly rather than bounded — which is what §5's box above states and what makes n = 2759 the unique global minimiser rather than merely the lowest entry.

**What the branch-and-bound has left to do.** The worklist holds **45,390** entries — every n whose family score falls below the asymptotic ceiling 7 − 4√3 — but **none below 1/25**, so it has nothing left to eliminate against the conjecture as stated, and the adaptive run against a floor of 0.04 returns an **empty** below-floor list without computing a single B(n). Its remaining use is sharpening: the operative question is **"does B(2759) exceed 0.04621"**. ⟦PENDING-LADDER-RERUN⟧ *The rest of the low tail is not a list of hard values but of understated ones: every entry below 0.05 other than 2759 and 2183 carries a fused class at F ≥ 4 whose twist the S7 branch cuts, and rises above 0.056 when it does not — so which values are genuinely worth an exact B is a question the rerun answers and this list does not.* B is already known at the lowest of them: **B(2759) = 175,813 equals the ladder's score there**, which is what fixes the minimum below 10⁶ exactly; computing B at the other nine would raise the verified floor above 1/25 with real margin.
...
The worry motivating §3.5 — that between the computable range and the asymptotic one lies a middle where neither argument reaches — is answerable empirically, and the answer is favourable. Minimum lower bound over each decade of the 45,390-entry worklist:
...
**The binding value is 0.04621 at n = 2759, in the [10³, 10⁴) decade.** ⟦PENDING-LADDER-RERUN⟧ *The other three decade minima are ladder values at n where the S7 branch cuts a fused class's twist, so they are lower bounds on the decade minimum and not the decade minimum: 11183 alone rises above 0.065 on the full twist. What the table establishes as it stands is that **every** decade clears 1/25 with margin, which is what §5 uses; the shape of the descent, and hence "one decade wide", is what the rerun settles.*

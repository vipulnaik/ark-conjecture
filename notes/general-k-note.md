...
| section | standing |
|---|---|
| §1 the completeness theorem | **proved as a bound, conditional on J0a′** (the semilinearity of the block group, stated and reduced in §1's box — it is an assumption, not a consequence of Lemma B); the product form κ_k = τθγ needs joint attainment of the three maxima, verified at k = 3, 4 and open at k ≥ 5 |
...
*Proof sketch.* **Assume the block's group lies in AΓL(1, c)** — see the box below, which is where the real content sits — namely AΓL(1, c) = 𝔽_c ⋊ (𝔽_c^× ⋊ Gal), and this is a chain of subgroups each normal in the next with the displayed quotients. A stabiliser of S therefore has a filtration by its intersections with the three layers, and its order is the product of the three layer contributions **for that S**. **k enters only through which subsets of a given layer can fix a k-set**, never through the layer structure itself, because the layers are a property of the group and not of the arity. ∎
...
> **The semilinearity assumption is J0a, and at k ≥ 3 it is load-bearing rather than cosmetic.** Lemma B of `enumeration-proof.md` does **not** put a block's stabiliser inside AΓL(1, c); it constrains which blocks and twists are admissible, and Part B's own 3^{1+2} ≤ GL(3, 7) exhibits an Oliver-admissible stabiliser outside ΓL(1, 7³) whose orbital beats orb(c, d) by a factor 9 at k = 2. `three-uniform-note.md` §9.3 separates the two questions explicitly, and this proposition conflated them.
>
> **At k = 2 the conflation was harmless; at k ≥ 3 it is not.** There the SAFE score caps a matching class flatly at F·C(c,2), which bounds *any* point stabiliser whatever, so a non-semilinear block cannot make the upper bound false — and the lower bound is an explicit construction, which a richer stabiliser could only supplement. So μ(n) = B(n) is J0a-free at k = 2 in both directions (`enumeration-proof.md`'s J0a entry), and what survives there is a single theorem-side clause, E.3(i) — plus, below density 1/25 only, the possibility that μ sits strictly inside [B_refined, B_safe] on a proper-prime-power stripped block (`enumeration-proof.md` Part E‴'s trichotomy), which J0a′ would also close. *The component argument sketched below has one piece already written at k = 2: `pending-checks.md` A29's eigenvalue-orbit bound gives t ≤ a for the coupled foreign twist with no semilinearity assumption, which is the "m > 1 components" branch in miniature.* No such cap exists at k ≥ 3: C(c, k) sits far above the truth, and the entire content of m\*_k = O(n² log n) is that |Γ_B| is small. The upper bound therefore genuinely requires that no Oliver block group outside AΓL(1, c) does better.
>
> **What is needed is weaker than J0a as usually stated, and looks provable.** Not "every Oliver block stabiliser is semilinear", which is false, but:
>
> > **(J0a′)** For every k, an Oliver-admissible block group H ≤ GL(a, p) that is *not* semilinear has minimum k-orbit no smaller than AΓL(1, c)'s — equivalently, such blocks never win.
>
> A component argument should give it, uniformly in k. H has a normal cyclic C with q-group quotient. If C is irreducible, its centraliser in GL(a, p) is a field, so H ≤ ΓL(1, c) and there is nothing to prove. If C is reducible and H permutes m > 1 homogeneous components, a k-set inside a single component has a short orbit, bounded by m times the per-component count. If m = 1, then H = C·Q with Q a primitive irreducible q-group over C's field; primitivity bounds |C| and forces |Q| to be polylogarithmic in c, so |H| and hence |Γ_B| fall below AΓL(1, c)'s c² log c — and a smaller block group gives a *larger* minimum orbit only when it is transitive enough to matter, which the component bound denies. **Every branch is arity-free**, which is the point: one lemma closes J0a for the minimum-orbit purpose at every k at once, retires the last k = 2 dependency (`enumeration-proof.md` E.3(i) at a ≥ 2), and makes this proposition true as stated. **On the k ≥ 3 side this is the single highest-value item.**

**That is the answer, modulo J0a′, and it is unaffected by the attainment caveat**, since the caveat concerns how the three factors combine and not whether a fourth exists. The k = 2 → 3 step was not the discovery of a new mechanism but the *switching on* of two factors that were inert at k = 2: τ became nontrivial in characteristic 3, and γ became nontrivial in characteristic 2. Both were already present in the decomposition. Beyond k = 3 the same three factors take a wider range of values, and no fourth term can appear, because AΓL(1, c) has exactly three layers.
...
**The threshold, relative to the ambient.** m\*_k is **Ω(n²) and O(n² log n)** at every k — the numerator of the orbit bound never grows with k, because it is |Γ| for a block group inside AΓL(1, c), of order at most c² log c. *(Not Θ(n²): the Galois factor contributes m ≤ a = log_p c, and the gain of §2.3 is the least prime divisor of a, which is unbounded — §6 item 1's c = 32 case realises a gain of 5. The lower bound is what the evasiveness statement uses and is clean; the upper bound carries the log.)* So the statement

...
>
> **At k = 2 the conflation was harmless; at k ≥ 3 it is not.** There the SAFE score caps a matching class flatly at F·C(c,2), which bounds *any* point stabiliser whatever, so a non-semilinear block cannot make the upper bound false. No such cap exists at k ≥ 3: C(c, k) sits far above the truth, and the entire content of m\*_k = O(n² log n) is that |Γ_B| is small. The upper bound therefore genuinely requires that no Oliver block group outside AΓL(1, c) does better.
>

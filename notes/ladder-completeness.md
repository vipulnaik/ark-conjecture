...
> **⟦REQUOTE-ON-EXTENSION⟧ Every measured figure in this document is scoped to the 36,848-row frontier of its writing** — the `offmenu_scan.py` sweep ("efficient-prime pairs with a common q up to 36,848", 729 tabulated n, max 0.1111 at n = 4376) and the `ladder_vs_B.py` agreement ("all 32,861 tabulated values"). **Neither has been rerun since the exact table completed to 10⁶.** The *propositions* are unaffected — they are theorems and a conditional argument, not measurements — and what a rerun would strengthen is the empirical half of Proposition 2: that S6/S11 never actually win. Recorded as `pending-checks.md` R12b.
...
**Turning the proposition into a computation.** Proposition 1 is also an algorithm: above 1/25, B(n) is the maximum of the SAFE scores of the menu shapes and of S6/S11, and each of those is enumerable in O(n/log n). `mu_ladder_exact.py` does exactly that, returns a certified flag iff its maximum clears C(n,2)/25, agrees with `mu_table_exact.csv` at every one of its 32,861 rows, and runs at 89 ms per value at n ≈ 10⁶ — so B(n) is computable at any single n to 10⁶ by theorem, where `mu_exact.py` would take hours. It is what makes "read the ladder as B" unnecessary: one can compute B directly, at the same cost, and know when the theorem does not apply (`pending-checks.md` R7b).
...
**Not licensed.** Nothing here bounds the ladder below 1/25, where Proposition 1's counting fails and the fallback branch reopens; the ladder is a lower bound there and no more. And (BCG-AL) is a hypothesis about *every* large n; under (BCG-AA) — almost all n, the version `bcp-to-floor.md` derives from a fixed system — the conclusion weakens to ladder = B for all but O(x/log^A x) values, which is a different and weaker statement.

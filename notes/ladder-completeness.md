...
| | shape | what it scores |
|---|---|---|
| M1 | one fused class, n = F·c, any F | F·C(c,2) |
| M2 | c + r\*, skipped if r \| c−1 | min(C(c,2), orb(r, best q), cr) |
| M3 | 2c + r\*, three rungs (unfused; cyclic-layer fused; top-layer fused) | the rung scores, with q pinned appropriately |
| M4 | F·c + r\*, 3 ≤ F ≤ 16 (and F = 25 as a tripwire), each (F, q) split | F·orb(c, d) with only the foreign prime stripped |
...
>
> - **(M1)–(M4)** with, in (M2)–(M4), the matching share x = c/n or F·c/n lying in **[0.2, 0.8]**; or
> - **(X)** a shape with **two foreign primes** — r₁\* + r₂\* alone, or c + r₁\* + r₂\* with one matching block — with both r_j − 1 = k_j·q^{e_j} for a **common** top prime q and k_j ≤ 12.
>
> Nothing else.
...
- *Window.* Every part has share > 1/5, so in (M2) the matching share x = c/n lies in (1/5, 4/5) — the foreign part is the complement — which sits inside [0.10, 0.85] with margin on both sides. In (M3) and (M4) the class share F·c/n is below 4/5 for the same reason, and the ladder's per-block c/n is smaller still.
- *Two matching classes merge.* Two classes (F₁, p^a), (F₂, p^b) with a > b share the bottom prime, and the single class of F = F₁p^{a−b} + F₂ blocks of size p^b dominates: its intra F·C(p^b,2) ≥ F₂·C(p^b,2), which was already a term of the old minimum; its within-class cross never binds; its cross terms with every foreign part are unchanged. It is admissible whenever no foreign prime divides F, and a foreign prime r > n/5 cannot divide F ≤ n/p^b unless p^b < 5, which fails the size bound. So no optimum has two matching classes.
- *Fusion count.* cap_F(1) = 1/(1+√F)² is the most (M4) can reach at fusion count F, and it is below 1/25 for F ≥ 17. So F ≤ 16.
...
- *Efficiency of the foreign primes in (X).* A foreign part of share y < 1/2 needs η·y² > 1/25, so η > 4/25 and (r−1)/t < 12.5. ∎

Everything in (M1)–(M4) is in the ladder's menu after the 2026-09 corrections (window right end 0.55 → 0.85; fusion set 3..12 → 3..16). **(X) is not in the menu**, and is the entire content of the question.

## 3. Proposition 2 — the off-menu shapes have an absolute ceiling

> **Proposition 2.** Let X be a two-foreign shape as in Proposition 1. Then, excluding configurations using a Fermat prime (of which finitely many exist, all at n < 200),
>
> - r₁\* + r₂\* has SAFE ≤ **C(n,2)/9**, with equality approached by (2q^e + 1, 4q^e + 1) at q ≥ 5 and by (2·3^e + 1, 4·3^e + 1) — the pair (1459, 2917) at n = 4376 reaches 0.1111;
> - c + r₁\* + r₂\* has SAFE ≤ **C(n,2)/16**;
> - and r₁\* + r₂\* has n **even**, so at **odd n** every off-menu shape is below 1/16.

*Proof sketch.* Two distinct primes r_j = k_j q^{e_j} + 1 with the same odd q have k_j even (r_j odd), so the size ratio ρ = r₂/r₁ is (k₂/k₁)·q^{e₂−e₁} with k_j ∈ {2, 4, 6, 8, 10, 12}, q ∤ k_j, and ρ ≠ 1. The density of r₁\* + r₂\* is min(η₁y₁², η₂y₂², 2y₁y₂) with y₁ + y₂ = 1, y₂ = ρy₁, η_j = 2/k_j. Enumerating (k₁, k₂, e₂ − e₁) the supremum is **1/9**, attained only at (k₁, k₂) = (2, 4) with e₁ = e₂, i.e. r₂ = 2r₁ − 1 and (η₁, η₂) = (1, ½), where y₁ = 1/3 gives min(1/9, 2/9, 4/9). For the hybrid, adding the matching block's x² and the crosses 2xy_j and maximising over x gives **1/16** at the same pair, x = y₁ = 1/4. At q = 2 the primes are k·2^e + 1 with k odd; η = 1/k, and any pair with η > 4/25 for both needs k ∈ {1, 3, 5}: k = 1 is a Fermat prime, and without it the sup drops to 0.047. The Fermat cases give 0.12 (S6) and 0.066 (hybrid) but need 2^{e+1} + 1 and 3·2^e + 1 both prime, which happens only at n = 12. ∎

*Checked by scan* (`offmenu_scan.py`): over all efficient-prime pairs with a common q up to 36,848, the off-menu shapes score at 729 tabulated n, are above 1/25 at 352 of them, reach a **maximum density of 0.1111** (n = 4376), and **never reach B(n)** — closest 0.842 of B at n = 56 and 0.834 at n = 4376, both S6 at the (2·3^e+1, 4·3^e+1) pair. So on the table, (X) never wins.

## 4. Proposition 3 — why (X) never wins for large n, conditionally

The off-menu ceilings sit strictly below every class ceiling of `arithmetic-of-density.md` §3.3.5:

| where (X) lives | its ceiling | the menu's ceiling there | margin |
|---|---|---|---|
| even n, classes 0, 4, 6, 10 (mod 12) | 1/9 = 0.1111 | 1/4 | 0.139 |
| even n, classes 2, 8 (mod 12) | 1/9 = 0.1111 | 0.13397 (S3 at η = 1/3, or F = 3) | **0.023** |
| odd n, any class | 1/16 = 0.0625 | ≥ 7 − 4√3 = 0.0718 | **≥ 0.0093** |
...
*Proof.* Propositions 1 and 2: the optimum is in the menu or is an (X) with density ≤ 1/9 (n even) or ≤ 1/16 (n odd). Under (BCG-AL) the ceiling-setting menu shape reaches (1 − ε)·δ₀(class), which exceeds 1/9 at every even class and 1/16 at every odd class once ε ≤ 0.129. So for n large the menu beats every (X), and the optimum is in the menu; the ladder scores it at SAFE by E.5. ∎

**Where "sufficiently large" bites, and it is not asymptotic hand-waving.** The binding *absolute* margin is class 2/8 against S6's 1/9: 0.023. At the approach rate of `approach-rate-note.md`, the shortfall ≈ 0.3·log³n/n falls below 0.023 around **n ≈ 10⁴** — which is exactly where n = 4376 sits (S6 at 0.834 of B, the closest approach on the table). So the table's clean verdict at n ≤ 36,848 and Proposition 3's "large n" overlap, with no gap between them: below ~10⁴ the check is the table, above it the check is (BCG-AL), and both say the same thing. The odd-n margin is smaller in absolute terms (0.0093 at class 11) but the odd-n off-menu shape needs a third part and a Cunningham-like pair, and the table shows it reaching only 0.0478 (n = 323, 1175) against a class ceiling of 0.0718 or more.

## 5. What this licenses, and what it does not

**Licensed.** Reading a ladder value at n > 36,848 as B(n) — hence as μ(n), by Corollary E.6 wherever it exceeds 1/25 — is correct *unless* an (X)-shape wins at that n, which requires the menu shape setting n's class ceiling to fall 0.023 (even n) or 0.0093 (odd n) short of it. That is a specific, checkable event: `offmenu_scan.py` extended to the range in question would find it. **The extrapolation is not "the ladder is probably tight"; it is "the ladder is tight unless a two-foreign configuration at a common top prime beats it, and here is the script that looks for one."**
...
- The menu's (M3)/(M4) branches pin q in ways that are stricter than necessary (`pending-checks.md` risk item 3); that only under-scores, so it cannot break ladder = B on the table, but it means a *future* shortfall could arise from a pinning rule rather than from (X). `ladder_vs_B.py`'s shape breakdown is what would tell the two apart.

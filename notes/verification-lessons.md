...
- **A script that is only correct under a start-up flag is a trap.** Defining a global whose name an autoloaded package already owns aborts at the assignment — read-only — with a message naming the *variable* and not the file, so it reads as a problem with the environment rather than with the script. The flag that suppresses autoloading makes it go away for whoever knows to pass it, and fails for everyone else. **Rename the global instead**, and check the collision up front: a loop over the names the file defines, testing bound-and-read-only, converts a cryptic abort two hundred lines later into a diagnosis at the top. The instance was `Orb`, owned by the `orb` package.

...
**The additive count is not enumerated but counted directly.** With every F_i = 1 a shape is determined by its part count k ≤ K = ⌊L⌋ and its number of foreign parts i ∈ {1,…,k}, so **N_add(δ₀) = Σ_{k≤K} k = K(K+1)/2**, quadratic in L and hence ≈ 1/(2δ₀). *(The sum runs from k = 1, so it counts the one-part shape with its single part foreign — n = r itself, a prime, which is outside the domain. Every N_add figure here and in `aod` §6.4 is therefore one high; the convention is kept because the growth rate is what both documents use, but the two must keep it in step.)* **This was the count `aod` §6.6 quoted, and it is the wrong one.** The fusion shapes were dropped from the covering set as "density-zero", which is true only of shapes with *no* foreign part (n = F·c); a fused class beside a foreign prime, n = F·c + r, needs c a prime power and not n, and is available at positive density. The operative count is the number of feasible shapes **with at least one foreign part**, N_foreign = N − #{multisets} = **9, 25, 42, 66** at the four floors above (odd/even split 6/3, 11/14, 22/20, 37/29, parity being that of the total fusion count), which `ladder-completeness.md` Proposition 1 then cuts to **13 systems per parity at 1/25** and 8 above 1/16. See `aod` §6.4 as corrected. N_add is retained here because the growth-rate contrast of §2 still uses it.
...
**Why this asymptotic is not the operative one.** It counts the raw set, whereas the covering statement runs over the shapes with a foreign part — the same growth rate, since dropping the no-foreign multisets removes a lower-order term — and above 1/25 `ladder-completeness.md` Proposition 1 replaces the count by a fixed list of 13 systems per parity regardless of how many shapes are feasible. *(An earlier form of this paragraph said the fusion shapes cover a density-zero set of n and the covering statement runs over purely additive shapes; that holds only for shapes with no foreign part — see §1.)* The raw growth rate matters only for the negative claim — that halving the floor does not square the disjunction — and for the contrast with the partition-factor growth below.

...
>
> **KSS at prime powers is not an alternative to it**, despite covering infinitely many n: the prime powers are a density-zero set, so KSS leaves cofinitely many n untouched and reduces the eventual problem not at all. **Parity and orbital shape are finite patches by construction** — parity is a coincidence of one counting sequence at one n, and orbital shape requires inspecting the actual subgraphs at that n, a hand computation with no uniform version. Neither could scale to all large n even in principle: nothing makes a fixed orbital shape recur, and nothing governs the parity of A066537.
>
> **And the patches are downstream of the eventual route, not independent of it.** BBKN's Ω(n log n) gives eventual evasiveness with no explicit threshold; what our μ table adds is turning that into *fails at exactly six n*, and only then is the residue small enough to enumerate by hand. Sharpening the asymptotic route is what made the finite arguments possible, so the honest count is **one route, plus mopping up**.
>

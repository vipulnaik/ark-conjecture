...
*Verified by explicit orbit enumeration.* For the matching part, building `𝔽_p^k ⋊ (C_{p−1} diagonal × C_k rotation)` and enumerating its pair-orbits gives intra-block orbitals of exactly `k·C(p,2)` at (p,k) = (5,2), (7,2), (7,3), (11,2), (5,3); the cross-block orbitals — which f omits entirely — total `p²·C(k,2)` pairs, always **larger** than the intra term, so omitting them is safe and the intra term is genuinely the minimum. *(They form **one** orbit only for `k ≤ 3`, which is every pair enumerated above; from `k = 4` the rotation `C_k` has `⌊k/2⌋` orbits on 2-subsets of `ℤ/k` and the smallest is the antipodal one, of size `(k/2)p²` — see `sp-to-floor.md` §2, which records `2p²` at `k = 4`. The direction is safe either way, since `(k/2)p² > k·C(p,2)`, but the single-orbit formula must not be evaluated at large `k`, and in Theorem 2's regime `k ≈ n^{1−γ}` is exactly where it would be.)* For the foreign part, the stabiliser of a pair `{0,x}` in `C_r ⋊ C_Q` is nontrivial exactly when `−1 ∈ C_Q`, i.e. when `2 | Q`; at (r,Q) = (13,4), (13,3), (31,5), (31,2), (41,8), (11,5) the orbit sizes are `rQ` for odd Q and `rQ/2` for even Q, with no exceptions.
...
- **The `qr` term is essentially exact**, since q is prime and so odd except at q = 2. This matters for `ep` F.4, which bounds a foreign class's contribution by `F·r·Q` — **that is the true orbital for odd Q, so F.4's foreign term is tight and cannot be sharpened by recovering a factor 2.** *(F.4 as a whole **has** since been sharpened, but on the other summand: the cofactor bound improves from `2/δ₀` to `2(1−√δ₀)²/δ₀` by using the companion part's support rather than `r ≤ n` — §6 below. Nothing about the orbital changes.)* The natural-looking sharpening — recover the halving that the other terms concede — is unavailable here, because it applies only at even Q, and `aod` §6.8's structural remark (cofactors are essentially even, hence Q essentially odd) says that is the rare case.
...
- **foreign intra:** `rq ≥ δ₀n(n−1)/2` with `r ≤ n` gives `q ≥ δ₀(n−1)/2`, i.e. cofactor **`(r−1)/q ≤ 2/δ₀`** — which is `ep` F.4's branch (b) in its crude form, as it must be.

> **And the sharper form of F.4 is this section's own thesis, applied on the other summand.** Bounding `r ≤ n` throws away the fact that the configuration also carries the companion `kp`, which by the bullet above has `p ≳ δ₀n` — more precisely every part has support `> √(δ₀n(n−1))`. So `r ≤ n − √(δ₀n(n−1))`, and dividing `rq ≥ δ₀n(n−1)/2` by `q` rather than bounding `r ≤ n` gives
>
> **`(r−1)/q ≤ D(δ₀) := 2(1 − √δ₀)²/δ₀`**,
>
...
> **This is one of three independent statements that the difficulty is in the companion, and they are worth reading together.** (i) *This section:* the sumset route caps the certified companion factor at `n/(log n)²` for structural reasons (§3(b)), and substituting the endpoint hypothesis on the input side improves the answer not at all. (ii) *`sp-to-floor.md` §6.1:* a cardinality hypothesis on the input set cannot give the representation `n = kp + r` even for generic sets — the counterexample attacks the **zero** class, where the companion's richness is helpless. (iii) *The shape-space side:* Oliver's chain admits no ternary shape with density bounded below — matching parts share `p`, foreign parts need distinct primes at a common `q` — so every admissible shape is binary in the additive variable and the companion is always a single `kp` (`solvable-relaxation.md` §4, which prices this by contrast: the solvable relaxation *does* admit three free parts and therefore *does* have unconditional floors). Three routes, three different objects — a sieve, a circle-method hypothesis, a group-theoretic chain — and the same conclusion.
...
- `S_12`: density `≈ 2.67/log²x` **as measured cumulatively at x = 2·10⁶** (25,353 primes; recomputed). That figure is inflated by lower-order terms — in the window `[x/2, x]` it is already 2.33, and the Bateman–Horn constant for the union over even `d ≤ 12` is `≈ 2`. Any of the three serves §7's point; the constant is not load-bearing. Two logarithms from order N.
...
2. **A cofactor-controlled version of any of the above.** Note that even a linear-factor result gives `p ≫ N` with an unspecified constant, whereas a floor needs `p ≥ δ₀n` with δ₀ *ours*; the constant in the sumset conclusion becomes the constant in the floor. So the round trip that `aod` §6.7 records — `d ≤ 12` out, `D ≤ 630` back under F.4's sharpened constant — would acquire a third loss here.
...
- **Sárközy–Stewart: checked against secondary sources, and Shparlinski's characterisation is accurate.** Two independent restatements give the hypothesis as **positive relative density**: for `A, B ⊆ {1,…,N}` with `#A, #B ≥ c₁N`, some `a + b` has `P(a + b) ≥ c₂N` with `c₂` depending only on `c₁` (Mérai, *On divisors of sums of polynomials*, arXiv:2112.03607, §1; Stewart's own abstract for the multiplicative analogue speaks of dense subsets). So "cardinalities of order N" is right, and **the linear conclusion is what positive density buys** — the nuance being that the Sárközy–Stewart papers do treat sets that are merely "not too small", but those yield correspondingly *weaker* lower bounds for `P(a + b)`, not the linear one §7 needs.
  - **Consequence: §7 does not change and §8 item 1 is not answered.** The density obstruction is real. Baker–Harman's set at `≈ 0.37/log x` and `S_12` at `≈ 2.67/log²x` are one and two logarithms respectively from the hypothesis, and no reading of the endpoint tool relieves that. This was flagged as the highest-value open item in this document; it is now closed **negatively**, which is the outcome that leaves §7's accounting standing rather than the one that would have improved it.
  - *Not read: the original paper.* Two secondary statements agreeing on the hypothesis is enough to retire the question of whether Shparlinski mischaracterised it; it is not enough to rule out a variant theorem elsewhere in the series (I–V) with a weaker density and a linear conclusion. That residual is small and is no longer this document's highest-value item.

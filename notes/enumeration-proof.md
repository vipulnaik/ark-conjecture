...
| **C** — a middle-layer twist sharing an outside prime couples the two | Part D, opening box | Part D, opening box, **proved at every a for a semilinear stabiliser** (automatic at a = 1; J0a at a ≥ 2); exclusion is Corollary C′'s domination | `--refined`, attainment, **and the collapse certificate's condition (4)**; not B_safe |
...
| **S7** | **middle**-layer-fused matching + outside; **F = 2 is the odd-n fused rung B** | enumerated | **396** at F = 2 (18.1%); **58** at F = 4; **102** at F = 3 and **14** at F = 6 | F = 2: 11.5 → 20.0 → **21.3%**; F = 4: 1.6 → 2.4 → **3.6%** | **F = 2: exists → all odd n**, **wins → five of the six odd classes** — n ≡ 1, 3, 5, 7, 9 (mod 12), whose class ceiling it attains. At n ≡ 11 (mod 12) the ceiling belongs to F = 4. **F ≥ 3: exists → every n; wins → one odd class**, n ≡ 11 (mod 12), all of it at F = 4. Split by the parity of F. **Odd F** needs F·c even, hence c = 2^a at odd n — O(log n) sizes per n, an escape, winning nowhere: at the even classes where S3 is unobstructed its cap 1/4 beats F = 3's 0.13397 outright, and at n ≡ 2, 8 (mod 12) — where S3 is itself cut to 0.13397 — the two only tie, since cap₃(1) = cap₁(1/3). **Even F** has no such restriction: at odd n, F·c is automatically even and c ranges over all prime powers, so the supply is a full Hardy–Littlewood system, and **F = 4 attains the class ceiling at n ≡ 11 (mod 12)** (`aod` §3.3.5) | cap_F(η) = η/(1 + √(Fη))². F = 2: **0.17157 / 0.12500 / 0.10102 / 0.06699** at η = 1, ½, ⅓, ⅙. F = 4: **0.11111 / 0.08579 / 0.07180 / 0.05051** at the same η. Decreasing in F, so a larger F pays only when it unlocks a better η | `aod` §3.2, §3.3, §4.3 |
...
Γ₂ is a p-group by construction; Γ₁/Γ₂ is cyclic **whenever** the independent orders in it are pairwise coprime, which is what makes this an existence argument. *That is a sufficient condition on the construction and not a necessary one on admissible configurations* — an entangled generator supplies an F_mid alongside a twist from one cyclic subgroup, and a diagonal generator supplies several classes' twists at once, so the enumeration does not impose it (Part 0 step 1's box). What the enumeration does enforce is the one genuine subgroup condition, that the foreign primes be distinct. Γ/Γ₁ is a q-group as a product of q-groups on disjoint supports. And the resulting orbital sizes are *forced*, not chosen: the intra-orbital of a class is Fᵢ·orb(cᵢ, dᵢ) because the block permutation fuses the Fᵢ copies; the within-class cross is (Fᵢ or Fᵢ/2)·cᵢ² because the block-permuting group of the construction is the regular C_F, whose minimum pair-orbital is exactly **Fᵢ/2 for even Fᵢ and Fᵢ for odd Fᵢ** — for a transitive group of prime-power degree ℓ^a every orbital has ℓ-power size, and the ℓ-part of C(F,2) = F(F−1)/2 is F for odd ℓ and F/2 for ℓ = 2 since F−1 is coprime to ℓ, so if every orbital exceeded that value they would all be divisible by ℓF and so would their sum, contradicting the ℓ-part of the sum.
...
>
> **F.1 is tight and cannot be improved to k ≤ 3 by counting.** Running the same argument through the *smallest* part reproduces k ≤ 1/√δ exactly, so the equal-parts case saturates it, and no tightening of constants does better: at the density floor (0.048039 at n = 2183 over [6, 2600]) F.2 gives k ≤ 4.29 against F.1's k ≤ 4.68 — still 4, not 3, and a higher floor only lowers both. Since no winning configuration anywhere in the computed range uses more than three parts (Part I), the residual factor is arithmetic, not metric: any proof of k ≤ 3 must use the prime-power structure and the Diophantine constraint n = Σ F_i c_i. This is Part J item 1.
...
Winning configuration shapes, over the completed range: **853** use one p-characteristic part with one foreign prime (S3); **743** a single fused class (S2); **590** a fused class with a foreign prime (S5 and S7 at every F); and **none** uses either two p-characteristic parts with a foreign prime (S4) or two foreign primes (S6) — both families are empty. The last is worth watching rather than dismissing: the branch-and-bound of `arithmetic-of-density.md` §5 found a second instance at **n = 3059** = 1511\* + 907\* + 641, and it is the global density minimum. So the two-foreign shape is not merely rare — it is concentrated at the extremes, appearing where nothing else is available. These are the same five shapes reported at n ≤ 1306 (553 / 542 / 127 / 46 / 1) — **no sixth shape appears** in the extended range, which is the more informative fact. The last two are the ones a hand-built family menu is most likely to miss, since they combine features that are natural to treat separately. (The only known two-foreign instance is the worklist value n = 3059, beyond the contiguous range and the global density minimum. Candidates inside the range are exceeded by fused readings — n = 1175 = 641\* + 277 + 257\* by `2x233 + 1x709*` at B = 41,831.)
...
**What replaces the census's three-part row.** S3 (unfused matching plus foreign) takes 853 winners and S2 (fused single class) 743; the fused-with-foreign rungs take S7f2 = 396, S7f3 = 102, S7f4 = 58, S7f6 = 14, with S5 at 21. **S1, S4 and S6 have no winners anywhere on the table** — S4 for the reason just given, S6 (two foreign parts) having been a single row at n = 1175 that is now the fused `2x233 + 1x709*`.
...
Of the 897 odd values in the contiguous range, 837 reach density 1/12 or better. Among those:

| route | count |
|---|---|
| **no even part at all** | **348** |
| one even part, a 2-power p-characteristic block (the 2^a + r shape) | 55 |
| one even part, odd block size with even fusion count | 434 |

and by part count, 348 use a **single fused class** and 489 use two parts, with nothing above. So the dominant route for a strong odd n is a fused class beside a foreign block — the even fusion count supplying the parity that a 2-power block used to have to supply — and after that the single fused class of §2.1 of `arithmetic-of-density.md` with both factors odd. **The third row is the largest because an even fusion count at odd c supplies the parity by itself**, which is what makes a 2-power block unnecessary at most odd n.

Restricting to the values where the single fused class is *unavailable* is where that diagnosis applies, and the corrected shape space has narrowed what it describes. Of the **217 odd values with ω(n) ≥ 3**, every one now uses a **two-part** configuration — a fused class beside a foreign block — and their median density is 0.1284, above the old three-part cap of 1/9 rather than sitting at it. So the 2^a + r shortage still limits the *unfused* two-block family exactly as the argument says, but that family is no longer what serves these n.
...
1. **Minimality: k ≤ 3 in the low-density tail.** Corollary F.3 makes this free wherever δ > 1/16 — 99.68% of the computed table, 7 rows falling below — so what is open is the tail below that threshold, measured in Part I over the low-density tail (**⟦PENDING-REBUILD⟧**). (How much of n beyond the computed range lies below 1/16 is itself governed by the open density questions, so this is a per-n statement, not a global one.)

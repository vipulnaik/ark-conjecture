...
> **What this buys the certificates, and the one qualification to carry with it.** `fb_common.py`'s condition (4) caps a leftover p-characteristic part by stripping the foreign primes from its twist. That strip is **not** a necessary condition on admissible configurations — the witnesses above are admissible and violate it. It **is** necessary among configurations scoring above n·log₂n, by Corollary C′, and that is the only necessity the certificates use, since they evaluate candidates against thresholds of order δ·C(n,2) ≫ n·log₂n. So the strip is justified **at every a**, with the justification being coupling-plus-threshold rather than an unconditional lemma, and the threshold must be stated wherever the condition is defended: a diagnostic hunting for configurations *below* n·log₂n could not use it.

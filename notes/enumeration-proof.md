...
*Companion to `orbital-evasiveness-notes.md` §2. Classifies the possible orbit-and-twist structures of an Oliver group, enumerates them, and establishes an upper bound on μ(n). Implemented in `mu_enumerate_v2.py`.*
...
> - **B_refined(n) ≤ B_safe(n)** by design. `B_safe` deliberately over-counts a p-characteristic part at F·orb(c, dmax) even where Lemma C reduces its twist further, precisely so that no assumption is made about which configuration is realisable.
...
| **C** — a middle-layer twist avoids outside primes | Part D, opening box | Part D, opening box | `--refined`, attainment, **and the collapse certificate's condition (4)**; not B_safe |
| **D1** — bottom-layer copies of a matching block absorb | Part D2 | Part D2, proved | the block-count split F = F_mid·F_top |
| **D2** — outside blocks are never fused, by any layer | Part D2 | Part D2, **proved** | the block-count split F = F_mid·F_top |
...
**Lemma C is proved only for prime blocks**, the prime-power case being open. B_safe does not use it, so the *bound* is unaffected — but the **collapse certificate does**: condition (4) of `fb_common.py` caps a leftover p-characteristic part by stripping the foreign primes from its twist, which is Lemma C's conclusion. That strip is therefore scoped to prime blocks in the code, and closing the a > 1 case would both tighten the certificate and remove the last obstacle to replacing B_safe by B_refined outright.
...
| **S9** | fused outside block, any layer | killed, **D2** | — | — | — | never exists, so neither limit is defined | translations forced diagonal ⇒ a class of at most \|O\|/2 | Part D2 |
| **S10** | outside block with r = q | killed | — | — | — | never exists, so neither limit is defined | normality kills the twist ⇒ the block is worth r | Part D2 |
...
*Case E below is a worked instance of D2 rather than an independent case, and is kept because the mis-scoring it guards against is easy to repeat.*
...
> **GAP — the last step of this proof does not cover every admissible block-permuting group, and the conclusion is load-bearing.** The divisibility argument needs the block-permuting group to be a **ℓ-group of prime-power degree**. Under the corrected shape space the block count is F = F_mid·F_top and need not be a prime power, and the permuting group need not be an ℓ-group — it need only be a solvable group compatible with the chain. **Witness:** AGL(1,5) = C₅⋊C₄, with C₅ in the cyclic layer and C₄ on top, is solvable, sits inside the chain, and is **2-transitive on 5 blocks**, so it has a *single* pair-orbital of size **10 = C(5,2)** — computed directly, not conjectured. Against the claimed bound of F·r = 5r, the offset-0 class is then 10r = 2|O|, and "at most |O|/2" does not follow.
>
> This is the same scoping error as the within-class cross coefficient in Part E, one level down: a statement exact for the regular C_F action, asserted as a bound over all admissible permuters. **What is at stake is larger here.** Part E's coefficient was harmless because the intra term binds first; D2's conclusion supports **Corollary D2′ and hence the block-count split F = F_mid·F_top**, i.e. the corrected shape space itself. If outside blocks can be fused after all, the enumeration is missing shapes, and the failure mode is the one that sank the q-power count: **μ ≤ B_safe, not the constructions.**
>
> **What is *not* in doubt.** The diagonal-translation step is untouched: Γ₁/Γ₂ cannot contain C_r^F for F ≥ 2, so translations are diagonal and offset is invariant. The r = q case below is untouched. And no fused-outside-block configuration has ever appeared in the table — but the table is computed from an enumeration that excludes them, so that is not evidence.
>
> **What would settle it, in order of cost.** (i) Check whether a 2-transitive block-permuting group is actually admissible here — the twist and the translations must also fit the chain alongside it, and that may fail independently. (ii) Failing that, bound the *other* classes: with a 2-transitive permuter the nonzero-offset classes may be small enough to restore m\* ≤ |O|/2 by a different route. (iii) Failing both, enumerate fused-outside-block configurations explicitly and check whether any beats B(n). Tracked as **A18** in `pending-checks.md`.

*Why no threshold is needed, and the comparison that suggests one is.* Stating the lemma as an asymptotic domination invites an apparent counterexample at chunk size 15 (three copies of a 5-block, same-position family 15 against an alternative of 10). That comparison is against the wrong alternative: 10 is the score of a *different* configuration on 5 points, not of this one, and this one's own score is 15 out of an orbit of 15 — a density of 1, but on an orbit that is a vanishing fraction of any n where it competes. The lemma as stated bounds m\* by n/2 outright, which beats every configuration of density bounded below at every n where the bound is nontrivial. No threshold appears.
...
> **Corollary D2′** *(subject to the gap above).* Fusion is available to matching blocks only. Combined with D1, the block count of any class is F = F_mid · F_top, with no contribution from Γ₂ for matching blocks (D1) and no fusion at all for outside blocks (D2).
...
1. **Minimality: k ≤ 3 in the low-density tail.** Corollary F.3 makes this free wherever δ > 1/16 — 97.7% of the computed table — so what is open is the tail below that threshold, measured in Part I at 45 of 1,921 values. (How much of n beyond the computed range lies below 1/16 is itself governed by the open density questions, so this is a per-n statement, not a global one.)
   ...
   Three findings constrain any proof there, all recorded elsewhere: Proposition F.2 shows counting saturates at four parts, so the argument must be arithmetic; the wide B₃/B₂ margins of Part I rule out a perturbation or small-domination route, since a proof must instead *produce* a strong ≤3-part decomposition at the given n; and n = 551 = 256 + 167\* + 128 (Part I) forbids assuming the two p-parts are equal, since distinct powers of one prime are admissible together and §6.2 of `arithmetic-of-density.md` shows they need no coprimality between their twists. *(Status of this witness: it is the **v2** winner at n = 551, worth 8,128. Under the corrected shape space v4 wins that n with the equal-block `3x128 + 1x167*` at 13,861, so 551 no longer witnesses distinct powers actually **winning** — only their admissibility, which is what the argument uses. A replacement winning witness has not been looked for; if one is wanted, `validate_table.py --baseline` reports shape migrations and is where to look.)* The supply question that remains has the same shifted-prime character as the density questions, which is why this item and item 2 now sit in the same regime.
...
2a. **Bound the s = 4 and s = 5 branches.** *Recount before relying on the numbers here: under the corrected shape space n = 3239 and 3059 reach 0.043570 and 0.083906, so both leave the sub-1/25 set and the branch may narrow without any new theorem.* Against the v2 floor of 0.026117, four values sit below 1/25 (n = 2291, 2303, 3059, 3239) and one below 1/36 — these being the values at which the certificate reports an s ≥ 4 branch. Note that the *threshold* for s = 4 to become reachable at all is δ ≤ 1/16, not δ ≤ 1/25 (corrected in the Corollary after E.3); the set of values below 1/16 is much larger (45 of 1,921, Part I), so why the certificate reports only four s ≥ 4 branches rather than several dozen is not explained by the threshold and should be re-derived from the certificate's own output. Neither branch is thin enough for an E.4-style collapse — c − 1 = 4r and c − 1 = 5r carry no parity or congruence forcing — so an absolute cap would have to come from the foreign block's twist, as in E.1 and E.3(iii). The search clears both at every computed n, so nothing is unproved; the gap is theorem-side and widens as the floor falls.

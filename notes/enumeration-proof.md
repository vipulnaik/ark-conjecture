...
**F. n = 3239 — a cyclic-layer block count at a record-relevant value.** Seven copies of a 256-block plus an outside block of 1447 with twist 241. Terms 228,480 / 458,752 / 2,593,024 / 348,727 → score **228,480**, density **0.043570**. Seven is a power of neither the home prime 2 nor the top prime 241, so a q-power-only count values this n at 136,957 (density 0.026117) instead. The branch-and-bound's pre-repair "minimum ≥ 0.026117 over n ≤ 10⁶" remains valid but is not attained here, and the argument for where the minimum sits has since been redone (`arithmetic-of-density.md` §5.1): v4 scores this n at **289,230 (density 0.055155)** via `6x311 + 1x1373*`, so the value is no longer record-relevant and this case is kept only for the block-count phenomenon it exhibits.
...
Against that, B₀ is **loose exactly where it would matter**. Its density floor below 3000 is 0.123 (at n = 551), against B's 0.045742 (n = 1817, the minimum over the contiguous range n ≤ 2600 and over the worklist rows above it alike; the pre-repair figure was 0.026117 at n = 3239, which the corrected shape space lifts to 0.055155), and it sits near 1/4 generically — B₀(200,000) has density 0.2494. So it does not identify arithmetically weak n, and it cannot be used to prune the enumeration. It is a cheap outer bracket, not a tool.
...
> **s = 4 is not reachable in range, and would have no theorem if it were.** The corollary below bounds s by 1/√δ − 1, so a lower density admits a larger s. Under the pre-repair shape space n = 2291 sat at δ = 0.037524, giving s ≤ 4 and one branch the certificate could dispatch only by search; the corrected shape space lifts that value to 0.066767, and at the floor of 0.045742 (n = 1817) the bound is s ≤ 3.68, so **the largest permitted s over the table is 3** — and that holds over the worklist rows above n = 2600 too, since they set no lower minimum and this branch is empty rather than merely cleared. What follows is why it would still want a theorem if the floor fell. Unlike s = 3, the branch is **not** thin: c − 1 = 4r with c a prime power and r prime carries no parity or congruence forcing — 33 such pairs exist with c < 4000, beginning (9,2), (13,3), (29,7), (53,13), (125,31) — so no analogue of E.4 is available and an absolute cap would have to come from the foreign block's twist, as in E.1 and E.3(iii). The search still clears it (0 candidates at every computed n), so nothing is unproved; but the theorem-side coverage now has a hole that widens as the density floor falls. This is the first branch to appear since the framework was set up, and it appeared because the computed minimum dropped from 0.041107 to 0.037524.
...
> **Corollary (the fallback question below 1/9, reduced).** Since s ≤ 1/√δ − 1, **δ > 1/16 forces s ≤ 3**, **δ > 1/25 forces s ≤ 4**, **δ > 1/36 forces s ≤ 5**, and so on. *(The ladder is easy to write shifted by one — "δ > 1/25 forces s ≤ 3" — which does not follow from s ≤ 1/√δ − 1; the thresholds are 1/(s+1)². The applications below are computed from the inequality directly, so they do not depend on reading the ladder correctly.)* **Over v4 this covers the whole table.** The floor is **0.045742 (n = 1817)**, so s ≤ 1/√δ − 1 = 3.68 and the largest permitted s is **3**; **no computed value falls below 1/25**, and the four that did under the pre-repair shape space have all lifted (n = 2291 to 0.066767, 2303 to 0.059833, 3239 to 0.055155; 3059 lies beyond the contiguous frontier). Recount this after every extension; it is what `check_doc_figures.py --pass scope` exists for — and note that the recount is only sound if the surrounding sentence names the *current* argmin, since the s-bound is arithmetic on the floor and nothing else.
>
...
> *Figures below are a mix: those explicitly marked v4 are current, and the rest are measurements of the **pre-repair** table, hence of B_safe over the too-small shape space — the frame of this Part has not been rewritten, only its individual bullets updated, so read every unmarked figure as v2-era. They remain accurate as descriptions of what the program computes, and are not descriptions of μ(n). The n = 10 and n = 12 GAP comparisons are the exception and are now more valuable than before: they are the only check that would have caught a missing shape, and they did not fire at those degrees — which says the missing shape needs a larger n to appear, consistent with the smallest known instance being n = 143.*

From `mu_table_safe_v2.csv` — n = 6 … 2298, unconditional mode, 1,921 rows.

> *Reconciling the several row counts quoted across these documents, since they look inconsistent and are not.* `mu_table_safe_v2.csv` holds **2,047** rows in all, reaching n = 3239 because the branch-and-bound of `arithmetic-of-density.md` §5 added sparse values above the contiguous range. Of those, **1,848** have n ≤ 2212 (the figure the notes' overview quotes), **1,921** have n ≤ 2298 (the contiguous frontier, and the basis of every count in this Part), and **2,008** are the certified values Part E′ reports over. Different windows on one file, not four different files.

> **The table is a contiguous prefix plus a biased tail, and every aggregate here is over the prefix.** `mu_enumerate_v2.py` has computed **every** non-prime-power n ≤ **2,600** — 2,186 rows, and that is the range any distributional figure describes. Rows above 2,600 exist but are not a continuation of it: they are values pulled off `ladder_weak.txt`, i.e. n selected *because* the ladder scored them low, so they are a **low-density subsample**. Measured on the current file, median density is 0.1994 below the frontier against 0.0662 above it, and 0.8% of rows sit below 1/16 against 31.9%. **So medians, shares, part-count splits and tail counts are quoted over n ≤ 2,600 and nowhere else.** The exceptions are the quantities that remain valid on any superset — the **density floor**, and statements of the form “no computed value falls below X” — which take the whole file, since a worklist row is exactly where a new minimum would first appear. `check_doc_figures.py` enforces the split: it detects the contiguous frontier and computes every aggregate over it. **All figures in this Part are v2-era**, i.e. from before both the shape-space correction and the SAFE tightening. They remain accurate as descriptions of what the pre-repair program computed; they are not descriptions of μ(n), and the shape counts in particular will move — S4's 257 is the clearest case, since v3 sent it to 0 and the tightened v4 brings it back. Recompute against v4 before citing.
...
2. **The collapse for odd n at low density: the global promotion.** Part E′ closes every branch but one *within the δ > 1/16 regime* — s = 3 outright (E.4), the s = 2 repunit family by absolute cap (E.3(iii)), s = 1 by Cap(a) (E.1), and everything at δ > 1/9 (E.1); below 1/16, s = 4 and s = 5 open up and have no theorem either (see the box after E.3, and item 2a below) — and E″ certifies the collapse at **every** composite non-prime-power n ≤ 100,000 from lower bounds alone, 90,299 of 90,299. The surviving branches are **s = 2 with c a safe prime** — within which E.3(ii) resolves the bare pair outright, leaving only the case **with a leftover**, **247 instances over v4's range**. Under the corrected shape space the largest permitted s is 3, so the s = 4 and s = 5 branches that the v2 floor admitted — and which have no theorem at all — are not reachable there.
   ...
   The alternative route — raise the odd-n guarantee above 1/9 and bring odd n under Theorem E.1 wholesale — is **closed**. It would need a constant above 1/9 bounding μ(n)/C(n,2) from below on odd n, and no such constant exists: **34.8% of the odd n in the computed table already fall below 1/9** — 312 of the 897 odd values at n ≤ 2600 — with the share growing across the range (23.6% below 800, 33.5% in [800, 1600), 43.3% in [1600, 2600]). The figure 54.3% quoted here previously was v2-era; `arithmetic-of-density.md` §7 carries the current one, and the two documents now agree. So the direct promotion is the only path.

...
> Over the range last certified — the 1,848 values at n ≤ 2212 — **no candidate at any value, 0 inconclusive**, with **1,390 (75.2%) settled by Theorem E.1 alone** and the other 458 by the search — every one of those 458 uncovered because δ ≤ 1/9, never because a Cap(a) binds. (Rerunning after each extension is item A6 of `pending-checks.md`.) So μ(n) = B(n) is proved at each computed n, independently of tie-breaking, and B_refined = B_safe follows rather than being separately measured.

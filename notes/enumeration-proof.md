...
**Lemma B′'s socle argument has had a second, independent reading and was confirmed** — Step 0 does need irreducibility plus C_G(V) = V, exactly as written. A further reading remains worth more than another pass by either reader, per the track record noted above.
...
Winning configuration shapes, over the contiguous range **⟦PENDING-REBUILD⟧** (v4 counts — 289 rows are known low and the S4 row in particular reverts, so recount after the rebuild): **900** use one p-characteristic part with one foreign prime (S3); **777** a single class, fused or not (S1/S2); **493** a fused class with a foreign prime (S5 and S7 at every F); **16** two p-characteristic parts with one foreign prime (S4); and **none** uses two foreign primes. The last is worth watching rather than dismissing: the branch-and-bound of `arithmetic-of-density.md` §5 found a second instance at **n = 3059** = 1511\* + 907\* + 641, and it is the global density minimum. So the two-foreign shape is not merely rare — it is concentrated at the extremes, appearing where nothing else is available. These are the same five shapes reported at n ≤ 1306 (553 / 542 / 127 / 46 / 1) — **no sixth shape appears** in the extended range, which is the more informative fact. The last two are the ones a hand-built family menu is most likely to miss, since they combine features that are natural to treat separately. (An older frontier had a two-foreign winner at n = 1175 = 641\* + 277 + 257\*; the fused reading `1x619* + 4x139` now exceeds it — see the three-part box below — so the only known two-foreign instance is the worklist value n = 3059, beyond the contiguous range and the global density minimum.)

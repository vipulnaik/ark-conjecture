...
> - **B_refined(n) ≤ B_safe(n)** by design. `B_safe` deliberately over-counts a p-characteristic part at **F·C(c,2)** even where its twist cannot reach that at all, precisely so that no assumption is made about which configuration is realisable.
> - **μ(n) ≤ B_safe(n)**, because F·C(c,2) caps any point stabiliser whatever and Part 0's shape space covers every Oliver group. **This is the inequality with the worst track record** — it fails the moment a realisable shape is missing from the enumeration, and it has failed that way once, at n = 308 (see the gotcha in Part 0, step 3). It rests on Part 0's completeness and on the cap F·C(c,2) being valid, and on nothing else.
...
>
> **B_safe = B_refined wherever δ exceeds 1/25**, which is now a **theorem** (Part E‴, Corollary E.6) rather than a per-n certificate: no fallback configuration other than a bare safe-prime pair and fifteen listed small-n instances has SAFE score above C(n,2)/25, so above that line the endpoints coincide, **B_refined = B_safe = B**, and hence **μ(n) = B(n)**. Since `ladder_verify.py` puts δ ≥ 0.04621 unconditionally on every n ≤ 10⁶, and a ladder score is a lower bound on B_safe, **μ(n) = B(n) at every eligible n ≤ 10⁶ without computing a single B(n)**. Every construction, every "μ(n) ≥ …" statement, and §5's floor rest on that. *The certificates remain the independent route to the same conclusion — a different mechanism testing the same claim, which is worth keeping — but they are no longer what carries it. See `pending-checks.md` R1.*
...
```
  ...
  +--------------------------------------------------------------------+
   ...
   +---------------+      +----------------+      +--------------------+
                                |
             realisable when every demanded prime is distinct
             here 3, 52 and 149 share nothing, so it stands
             (distinctness is SUFFICIENT, not necessary -- see below)
```
...
> **But read the coupling as a sufficient condition for building, not as a necessary one for existing.** Distinct demanded primes let the arrangement be assembled; they are *not* required of every admissible arrangement, and imposing them on the enumeration would be anti-permissive. Two of the three collisions dissolve for the same quotient-versus-subgroup reason as everywhere else in this document — a copy count colliding with another chunk's twist is supplied by one diagonal generator, and two copy counts sharing a prime by one diagonal rotation. The third, a copy count sharing a prime with an outside block, dissolves by counting instead: an outside block of size r matters only if it clears the score, forcing r ≳ √(2·score), while r | F_mid makes the class at least r·c, which overruns n. So no colliding arrangement can be the best one, and `mu_enumerate_v3.py` deliberately enumerates them all — its `value()` checks only that the outside block sizes are distinct. **The prose here and the code differ on purpose, and the code is the permissive one.**
...
```
     ...
     at most                         NOT bounded by copies x (block
     copies x (pairs in a block)     size) squared -- a 2-homogeneous
                                     block permuter beats that.  The
                                     score never uses this term: the
                                     line above always binds first.
```
...
**Lemma C — a middle-layer twist sharing a prime with an outside block couples the two.** *Must show:* that the top layer's conjugation is a single power map on the cyclic layer, so its Frobenius exponent on the matching block and its multiplier on the outside block agree mod r; hence the outside twist divides ord_r(p), which divides a. **Status: proved at every a (Part D) for a semilinear stabiliser, Γ|_O ≤ AΓL(1, c) — which is what the proof's Frobenius-exponent step needs, automatic at a = 1 and an open assumption at a ≥ 2 (item J0a). Shares are admissible — there are explicit Oliver groups at n = 28, 21 and 10 — so this is a coupling, not an exclusion, and what removes such configurations is Corollary C′'s domination, whose range-scoped half expires silently.** Does not affect B_safe, which caps every matching block at the flat F·C(c,2) without reference to it. **And it no longer bears on the collapse either:** `fb_common.py`'s condition (4) is the flat F·C(c,2), with no twist strip anywhere in the necessary conditions, so neither this lemma nor Corollary C′ nor J0a is in the certificates' trusted base. The strip survives only as an unread diagnostic (`set_strip_trace`), and what it would bound is the minimum orbital an actual *group* realises — which yields μ = B_refined, not B_refined = B_safe.
...
| **C** — a middle-layer twist sharing an outside prime couples the two | Part D, opening box | Part D, opening box, **proved at every a for a semilinear stabiliser** (automatic at a = 1; J0a at a ≥ 2); exclusion is Corollary C′'s domination | `--refined` and attainment only. **Not** B_safe, and **not** the collapse certificates — condition (4) is the flat cap |
...
| **E.5** — the fallback branch caps at δ = 1/25 | Part E‴ | Part E‴, **proved**; 15 explicit exceptions at n ≤ 63 | **the collapse above 1/25, by theorem** |
| **E.6** — B_refined = μ = B_safe above δ = 1/25 | Part E‴ | Part E‴, from E.5 + E.3(ii) | μ(n) = B(n) at every n ≤ 10⁶, via the ladder floor |
...
**Lemma C is a coupling rather than an exclusion**, and holds at every a. Neither B_safe nor the collapse certificates use it: condition (4) of `fb_common.py` is the flat F·C(c,2), because the question the certificates ask is whether a configuration's **SAFE** score can reach B_safe(n), and a strip of the foreign prime tests a smaller number — anti-permissive, and invisible in the output. So the lemma's reach is `--refined` and attainment. What it *would* buy, if the weaker statement μ = B_refined were the target, is the leftover twist cap of E″, and that inherits Corollary C′'s licence and J0a at a ≥ 2.
...
**C. A collision that is not one — kept as a gotcha, since the reading it corrects is the natural one.** Three copies of a 7-block, twist dividing 6. It is tempting to say the copies have already spent the prime 3 in the middle layer, so the twist may only divide 2 and the class is worth 3 × 7 = 21 rather than 3 × C(7,2) = 63. **That is the quotient-versus-subgroup error of step 3 one level down.** The block-rotation image C₃ is a *quotient* of the cyclic layer, not a subgroup of it, and an entangled generator z of order 18 with z³ the full twist supplies both from one cyclic subgroup: the class is worth **63**, and gcd(F_mid, d) > 1 costs nothing. Both shipped enumerators agree — n = 78 wins with `6x13` at 6·C(13,2) = 468, where gcd(6, 12) = 6 — and the counterexample groups at n = 33, 78 and 105 are builds of exactly this element. Nothing here makes the corrected B smaller: the entangled repair only *enlarges* the admissible space, so B moves upward at every n it moves at all.
...
| **S2** | fused matching class, n = F·c with **F > 1** | enumerated | **743** (34.0%) | 45.0 → 31.3 → **28.4%** | **exists = 1**, **wins → 0** — the one shape where the two limits come apart completely. It **exists at every non-prime-power n** (F is an arbitrary integer after the entangled correction, so take c = Q(n)); it *wins* only when the cofactor n/Q(n) is small, since δ_S2(n) = (Q(n) − 1)/(n − 1) | clusters at **1/F**: 0.4995, 0.3326, 0.2492, 0.1992, 0.1420, 0.1103. Holds every δ > 1/4 | `aod` §4.1 |
| **S3** | matching + outside, n = c + r\* | enumerated | **853** (39.0%) | 35.1 → 41.5 → **40.1%** | **exists → all even n *plus a positive proportion of odd n***, **wins → all six even classes**. All even n conjecturally; at odd n the shape survives with c = 2^a, i.e. n = 2^a + r\*, which Romanov puts at positive lower density and Erdős's covering congruences keep bounded away from all of odd n. The only row where existence exceeds winning by a positive **density** rather than merely by a rate — the odd instances exist but almost never sit well enough near the balance point to win. On the even side the two limits agree and existence still converges faster | → class cap. Medians 0.220/0.221/0.218/0.213 at n ≡ 0,4,6,10 (cap **1/4**); 0.1265/0.1272 at 2,8 (cap **0.13397**) | `aod` §3.1 |
| **S4** | two matching + outside, n = 2c + r\* | enumerated | **0** — the family is empty on the completed table, the whole of it having migrated to the fused two-part rung (the v4 baseline had 16, all c ≡ 1 mod 8) | — | **exists → 12/24** (all odd n), **wins → 0.** The cyclic-layer fused rung beats it at every c, since 2·C(c,2) > C(c,2) with no congruence in play (`aod` §3.2.3), so it is dominated inside the three-part family as well as outside it. The widest existence/winning gap in the census: available almost everywhere, beaten by the fused rung where that rung is reachable and by F = 4 where it is not — see `arithmetic-of-density.md` §3.2 and `three-part-family-split.md` | → class cap. Medians 0.1033/0.0997 at 1,9 (**1/9**); 0.0782/0.0765 at 3,7 (**0.08579**); 0.0695 at 5 (**0.0718**); 0.052 at 11 (**0.05051**) | `aod` §3.2, §3.3.5 |
| **S5** | **top-layer**-fused matching + outside; forces q = 2, hence η = 1/u | enumerated | **21** (1.0%) | 2.5 → 0.5 → **0.2%** | **exists → 12/24** (all odd n — q = 2 fusion is always buildable, just usually at a useless η), **wins → 0**. An escape: winning needs η = 1/u with u small, i.e. r = 2^a·u + 1, which is O(log n) candidates per n. *The share moves in **bursts**: one usable r serves a window of n proportional to r, so the trend column can rise over a third of the range (0.3% → 0.8% on the exact table, 59 of the 80 winners on the exact table at the single prime r = 12289 = 3·2¹² + 1, against 16 at r = 257 and 5 at r = 769) without contradicting the verdict. A monotone-decline test on this row is measuring the spacing of the family, not its limit* | cap₂(η) = 0.17157 at η = 1, but η = 1 needs a **Fermat** prime; at η = 1/3 it is 0.10102, already below S4's 1/9. Max seen 0.1614 (n = 639) | `aod` §3.2, §3.3, §4.3 |
| **S6** | two outside blocks | enumerated | **0** | — | **exists → 12/24** (every even n with a Goldbach representation); **wins → 0**, by local obstruction rather than scarcity — the cap 1/(√m₁+√m₂)² reaches 1/4 and 0.17157 only on families with ω(3) = 3, leaving n = 26 and n = 20 | ceiling is 1/4 but unreachable: it needs both rᵢ − 1 = 2q^{aᵢ} for a **common** q with a₁ ≠ a₂, an O(N^{1/3}) set | `aod` §4.2 |
| **S7** | **middle**-layer-fused matching + outside; **F = 2 is the odd-n fused rung B** | enumerated | **395** at F = 2 (18.1%); **58** at F = 4; **102** at F = 3 and **14** at F = 6 | F = 2: 11.5 → 20.0 → **21.3%**; F = 4: 1.6 → 2.4 → **3.6%** | **F = 2: exists → all odd n**, **wins → five of the six odd classes** — n ≡ 1, 3, 5, 7, 9 (mod 12), whose class ceiling it attains. At n ≡ 11 (mod 12) the ceiling belongs to F = 4. **F ≥ 3: exists → every n; wins → one odd class at F = 4** (n ≡ 11 (mod 12)) **and two even classes at F = 3** (n ≡ 2, 8 (mod 12)). Split by the parity of F, and at odd F again by the parity of **n**. **Odd F at odd n** needs F·c even, hence c = 2^a — O(log n) sizes per n, an escape, winning nowhere. **Odd F at even n** has c an ordinary odd prime and a full Hardy–Littlewood supply: it loses at the even classes where S3 is unobstructed, whose cap 1/4 beats F = 3's 0.13397 outright, but at n ≡ 2, 8 (mod 12) — where S3 is itself cut to η = 1/3 — cap₃(1) = cap₁(1/3) exactly, so F = 3 **ties for the ceiling and co-wins those two classes**, a positive winning share rather than 0. Odd F ≥ 5 is out at even n by cap₅(1) = 0.09549 < 0.13397. **Even F** has no restriction at either parity: at odd n, F·c is automatically even and c ranges over all prime powers, so the supply is a full Hardy–Littlewood system, and **F = 4 attains the class ceiling at n ≡ 11 (mod 12)** (`aod` §3.3.5) | cap_F(η) = η/(1 + √(Fη))². F = 2: **0.17157 / 0.12500 / 0.10102 / 0.06699** at η = 1, ½, ⅓, ⅙. F = 4: **0.11111 / 0.08579 / 0.07180 / 0.05051** at the same η. Decreasing in F, so a larger F pays only when it unlocks a better η | `aod` §3.2, §3.3, §4.3 |
...
| **S11** | one matching class + **two** outside blocks, n = c + r₁\* + r₂\* | enumerated | **0** | — | **exists → thin** (both r_j − 1 = k_j·q^{e_j} at a **common** q, k_j ≤ 12, so a two-term exponential family); **wins → 0** by cap: 1/16 at the best pair (η₁, η₂) = (1, ½), r₂ = 2r₁ − 1, against a class ceiling ≥ 7 − 4√3 everywhere. **The one live shape the ladder's menu does not contain**, with S6 — `ladder-completeness.md` | cap ≤ **1/16**; closest approach on the table 0.0478 (n = 323, 1175) | `ladder-completeness.md` §3 |
| **S12** | two matching classes of **distinct** block size, same p | enumerated; **dominated** | **0** | — | **exists** (551 = 256 + 167\* + 128 is admissible); **wins → 0**, and never wins at any n with δ > 1/25: the merged single class of F = F₁p^{a−b} + F₂ blocks of size p^b scores at least as much on every term and is admissible whenever no foreign prime divides F, which above 1/25 is automatic | dominated by S2 / S7 with the merged F | `ladder-completeness.md` §2 |

> **One computed instance of each live shape**, from the current table (**⟦REQUOTE-ON-EXTENSION⟧** — first instances are small-n and in practice stable). The starred part is the foreign block; each is the first value of n at which that shape is the winner.
>
...
**S7 splits at F = 2, and the F ≥ 3 half vanishes only where the parity forces it to.** Write the shape as n = F·c + r with r an odd prime. At **F = 2** the shape is the odd-n **fused rung B**: it carries 5/12 of all n asymptotically, is the sole winner at the five odd classes n ≡ 1, 3, 5, 7, 9 (mod 12), and is the commonest single shape among the odd rows. It does not vanish and is analysed in `aod` §§3.2–3.3 alongside S4, not here. What follows is about **F ≥ 3**, whose fate differs by parity — and **the split that decides it is the parity of F together with the parity of n**, not of F alone.

- **n odd** forces F·c even. With F **odd** this forces **c to be a power of 2**, so there are only O(log n) choices of c *per n* — the escape, giving O(n/log n) values by `aod` §4.3. With F **even** there is no such restriction, and F = 4 sets the class ceiling at n ≡ 11 (mod 12).
- **n even** forces c odd at odd F, and the supply is then a full Hardy–Littlewood system — the escape argument does not reach this case at all. At the eight unobstructed even classes S3's cap 1/4 beats F = 3's 0.13397 and the shape loses. **At n ≡ 2, 8 (mod 12) it does not lose**: the ℓ = 3 obstruction cuts S3 to η = 1/3, and cap₃(1) = cap₁(1/3) = 0.13397 identically, so F = 3 at full efficiency **ties for the ceiling and co-wins the class**. On the exact table 1,261 of 1,352 F = 3 winners are even n, all at these two classes, all but three at a safe prime, holding ≈ 37% of them near n = 2×10⁴ and rising.

So S7 **at F ≥ 3** is an escape in one regime only — odd F at odd n — and elsewhere a competing family: even F carries the ceiling at n ≡ 11 (mod 12), and odd F co-carries it at n ≡ 2, 8 (mod 12). *Gotcha, and it has bitten twice: a sentence beginning "F·c even forces c = 2^a" is true only for odd F at odd n, while the surrounding claim quantifies over all F ≥ 3 and all n. Check both parities before reading such a clause as a supply bound.* Among F ≥ 3 only F = 3, 4, 5 and 6 occur at all: larger fusion counts shrink F·C(c,2) faster than the foreign term can compensate, and cap_F(1) = 1/(1 + √F)² already excludes F ≥ 8 with no arithmetic input.

> **S7 at F = 2 versus S5, since the two are easy to confuse.** Both fuse two equal c-blocks alongside a foreign block, and they differ only in which layer holds the swap. In the **cyclic** layer (S7 at F = 2) the swap is carried by an entangled generator, so the twist is **full at every c** and no congruence on c is involved (`arithmetic-of-density.md` §3.2.3); the top prime is free, so the foreign efficiency η is unconstrained. In the **top** layer (S5) the twist is untouched and the intra term is 2·C(c,2) for every odd prime power c — but q is forced to 2, so η = 1/u with u the odd part of r − 1. The first is a carrier of the odd-n asymptotics; the second is supply-limited to r = 2^a·u + 1 with u small and is therefore an escape. Over [6, 10⁶], **306,544** winners are the cyclic-layer rung at F = 2 and **2,012** are S5 (1,883 of them on r = 65,537, the Fermat prime, 108 on 12,289, 16 on 257 and 5 on 769 — the burst structure `validate_table_v3.py` reports), matching the census rows in this document; recount both together whenever the range moves, since a figure taken at an earlier frontier drifts from the census without either looking wrong. Full treatment in `arithmetic-of-density.md` §§3.2–3.3.

**`ladder_verify.py` reaches both.** Its S7 loop runs over F ∈ {3, …, 12, 16, 25} — F = 2 is deliberately absent from it, because at F = 2 the shape is not an escape but the odd-n fused rung and belongs with the family it competes against. Both F = 2 readings are therefore scored in the three-part branch instead: **rung B** (cyclic-layer swap, **full twist at every c**, q restricted to odd primes) and **rung B′** (top-layer swap, full twist, η the 2-part of r − 1). Its `CAP` table is keyed on the **mod-12** ceilings of `arithmetic-of-density.md` §3.3, which is what the rung values are measured against. **The gotcha the script guards against:** scoring rung B with a twist cut by the block count understates it at every c, and since the script takes a max over families it would report a floor that is too low rather than an error.
...
> **Why per n rather than per shape, which is a design choice and not a shortfall — and what got past the dilemma in the end.** *Read this box as the reason the intermediate attempts failed. The way through was neither per-shape nor per-n but **per-density**: Theorem E.5 bounds the SAFE score of every fallback configuration regardless of shape or supply, which a per-shape argument could not do and a per-n one did not attempt.* The fallback configurations are shapes in the ordinary sense — the shapes of `arithmetic-of-density.md` §6 already link their parts, since §3.3's system is three conditions in one variable — so the natural question is why the collapse is not dispatched shape by shape. The answer is in §3.5.6: written in their natural variable, the top prime q, these families split by the exponent e in the foreign twist t = q^e. At **e = 1** the family is linear in q and behaves like any other, but at **e ≥ 2** the supply of admissible foreign blocks is sparse — about N^{1/e} up to N — so such a configuration is available only at a **density-zero set of n**, and at those n it may well be the optimum. A per-shape argument cannot dispatch a family that is available at almost no n yet decisive where it is available. **The per-n certificate is the right instrument for that, not a stopgap for a missing theorem.** The refined formula is therefore only ever strictly below C(c,2) on configurations that lose anyway.
...
> **Corollary C′ (what a share costs).** No configuration containing a share attains the maximum wherever δ(n)·C(n,2) > n·log₂n. With the ladder's unconditional δ ≥ 0.04621 on n ≤ 10⁶ that holds for **n ≥ 371**; below it, direct scoring of the bound against every tabulated value clears with worst ratio 0.7000 (at n = 15), so the exclusion covers the computed range and everything to 10⁶ with overlap. *(The threshold moves with the floor and nothing else — the pre-repair floor 0.02516 gave n ≥ 763 — so recompute it rather than quoting a remembered value.)* Beyond 10⁶ it needs only δ(n) ≫ log n / n — the weakest arithmetic input anywhere in the framework.

> **What this would buy a certificate that used it — and none does.** `fb_common.py` once capped a leftover p-characteristic part by stripping the foreign primes from its twist; **condition (4) is now the flat F·C(c,2) and applies no strip anywhere**, so what follows describes the licence the strip *would* need rather than a live dependency. That strip is **not** a necessary condition on admissible configurations — the witnesses above are admissible and violate it. It **is** necessary among configurations scoring above n·log₂n, by Corollary C′, which is the only necessity such a version would use, since candidates are evaluated against thresholds of order δ·C(n,2) ≫ n·log₂n. So the strip would be justified **at every a wherever B exceeds the sharing bound**, with the justification being coupling-plus-licence rather than an unconditional lemma — and the licence is local, needing no threshold on n at all: **min(r·ord_r(p), C(r,2)) < B**, evaluated per (p, a, r). At a = 1 the coupling forces ord_r(p) = 1 and the licence reads r < B. Crude global forms of the same statement — B(n) > n·log₂n, say, which fails at 34 tabulated values below n = 118 — are implied by it and are not needed.

*How this interacts with B_safe.* SAFE scores every p-characteristic part at the flat F·C(c,2), which accounts for neither the layer constraint nor the coupling, so **B_safe does not use Lemma C at all** — deleting the stripping from `value()` reproduces the table exactly at every n ≤ 400. The lemma enters through `--refined`, through the `fallback` column and through the Part E′ collapse argument. Note also that dropping a necessary condition can only *enlarge* the configuration space, so the max could only rise, and in SAFE mode it does not rise at all; while Part E's construction uses coprimality only as a *sufficient* condition, which is an existence argument. Both **μ(n) ≤ B_safe(n)** and its attainment are therefore independent of this lemma; what it buys is the sharpness of the search and the collapse.
...
> **Lemma D2 (fused outside blocks are dominated).** Let O be an orbit whose finest block is an outside block of prime size r, with F ≥ 2 copies permuted transitively by some subgroup of Γ — **or, equally, let O₁, …, O_F be F ≥ 2 distinct foreign orbits of the same prime r, not permuted at all**; the proof below never uses the permutation, only that each block is a foreign orbit of Γ₁. Then
>
...
> **The non-permuted case, since it is what the admissibility bullet in Part E rests on.** Read the proof with σ = 1 throughout. Step 1's "transitivity forces every component nonzero" needs only that each Oᵢ is an orbit, so its induced group is transitive on r points and its r-part comes from T\*; Step 2's conjugation argument is literally the same, with the multipliers aᵢ forced equal because one element z of the cyclic layer is being conjugated by one power map; Step 3 is unchanged. So **F distinct foreign orbits of one prime share a diagonal C_r and carry an invariant class of C(F,2)·r pairs** — at F = 2, exactly r. This is why the enumeration may take foreign primes pairwise distinct: not because two equal ones cannot be built (they can), but because a configuration containing them scores at most r. *The earlier justification — "C_r × C_r is not cyclic" — was the subgroup-versus-shared-element error one level up from the entangled generator, and it was stated as fact at three sites; all three now point here.*
...
> **Corollary D2′ (what fusion of outside blocks costs).** Fused outside blocks **exist**, but no configuration containing one attains the maximum wherever δ(n)·C(n,2) > n^{3/2}/2, i.e. **√n < δ(n)·(n − 1)** — and δ here may be any construction lower bound, which the fused-outside question cannot touch. With the ladder's unconditional δ ≥ 0.04621 on n ≤ 10⁶ this holds for **n ≥ 471**; below that, direct scoring of the bound against every tabulated value clears it with worst ratio 0.8276 (at n = 56), so the exclusion covers the whole computed range and everything to 10⁶ with overlap. *(As at C′, the threshold is a function of the floor alone — the pre-repair 0.02516 gave n ≥ 1582 — and `a18_verify.py` recomputes it from whichever floor is fed in.)* Beyond 10⁶ it needs only δ(n) ≫ n^{−1/2}, weaker than every density statement the framework runs on. `a18_verify.py` reproduces the witness, its chain and the range check.
...
- **Foreign primes are pairwise distinct — by domination, not by non-existence, and the distinction has bitten before.** The tempting argument is that two foreign parts of the same prime r would place C_r × C_r inside the cyclic layer. It would not: Lemma B′ Case 1 puts each orbit's translations *inside the cyclic layer*, which has a unique subgroup of order r, so the two orbits share **one** C_r acting diagonally — the shared-element pattern of the entangled generator, one level over. Such configurations exist. What removes them is Lemma D2 with its permutation hypothesis dropped (see the remark after D2's proof): the shared generator is conjugated by a single power map, so every h ∈ Γ induces the *same* multiplier on both orbits and the same-position class {(x, x)} is invariant, of size r. So two equal foreign parts are worth at most r to any group, and the enumeration omits them as it omits fused foreign classes — because they lose, not because they cannot be built. `mu_enumerate`'s `value()` and `fb_common.py` both enforce distinctness; both are sound for this reason.
...
What does **not** reduce is multisets of distinct part sizes and the choice of bottom prime: both are genuinely distinct configurations, and the measured data shows both matter — **no winner uses two p-characteristic classes at all** on the completed table — the family that made the case is empty (Part I's census), every member having migrated to a fused two-part configuration. The reduction still must not merge distinct part sizes, because that is a statement about which configurations are *distinct*, not about which ones win; but the measured support for it is now historical. The v4 baseline had 257 such winners, with the two p-blocks equal in 255 and the exceptions n = 551 and n = 2015 carrying two distinct 2-powers; both are now fused two-class winners.
...
- each foreign twist, a q-power, lies in the top q-group, as does each class's **F_top** factor of block permutation; a class's **F_mid** factor is carried by an **entangled cyclic-layer generator** — one element z rotating the F_mid blocks whose step-multipliers have product a generator of the twist, so z^{F_mid} is the full twist and the rotation and twist come from a single cyclic subgroup. The carrier's order must be coprime to the **foreign primes**; it need not be coprime to any F_mid, and a construction demanding that would reject configurations it can in fact realise. The n = 33, 78 and 105 regression witnesses of the battery are builds of exactly this element, and n = 255 (`73+73+109*` at q = 3) is a battery row whose F = 2 is entirely cyclic-layer.

Γ₂ is a p-group by construction; Γ₁/Γ₂ is cyclic **whenever** the independent orders in it are pairwise coprime, which is what makes this an existence argument. *That is a sufficient condition on the construction and not a necessary one on admissible configurations* — an entangled generator supplies an F_mid alongside a twist from one cyclic subgroup, and a diagonal generator supplies several classes' twists at once, so the enumeration does not impose it (Part 0 step 1's box). What the enumeration does enforce is that the foreign primes be distinct — a **domination** exclusion (Lemma D2, non-permuted form), not a subgroup condition: equal foreign parts exist, sharing one diagonal C_r, and score at most r. Γ/Γ₁ is a q-group as a product of q-groups on disjoint supports. And the resulting orbital sizes are *forced*, not chosen: the intra-orbital of a class is Fᵢ·orb(cᵢ, dᵢ) because the block permutation fuses the Fᵢ copies; the within-class cross is (Fᵢ or Fᵢ/2)·cᵢ² because the block-permuting group of the construction is the regular C_F, whose minimum pair-orbital is exactly **Fᵢ/2 for even Fᵢ and Fᵢ for odd Fᵢ** — for a transitive group of prime-power degree ℓ^a every orbital has ℓ-power size, and the ℓ-part of C(F,2) = F(F−1)/2 is F for odd ℓ and F/2 for ℓ = 2 since F−1 is coprime to ℓ, so if every orbital exceeded that value they would all be divisible by ℓF and so would their sum, contradicting the ℓ-part of the sum.
...
Since orb(c, c−1) = C(c,2) identically (Part B), the two endpoints coincide exactly when the optimum does not invoke the fallback. So attainment reduces to excluding fallback configurations from the optimum, and that is provable rather than merely observable. *Part E‴ now does exactly that above δ = 1/25, for the whole branch at once; what follows — the s-ladder and the per-branch theorems — is how that proof is organised and remains the route any statement about **all** n must take.*
...
> **s < 1/√δ − 1**, strictly — so **s ≤ 2 at δ > 1/16**, and **s = 1 at δ > 1/9** (sharp form: s ≤ 1 needs only r > (n−1)/3, which r² > δn(n−1) delivers once δ > (n−1)/9n). This is the s-ladder in its canonical form; every threshold quoted below is read off it and nothing restates it independently.
...
>
> It does **not** extend past the bare pair, and the obstruction is sharp rather than technical. With a leftover the re-reading must re-type the leftover parts too, and the commonest case **L = c fails outright**: two blocks of the same prime c would be two *equal* foreign parts, which share a single diagonal C_c and are therefore worth at most c by Lemma D2 in its non-permuted form (the admissibility bullet above); fusing them is dominated by the same lemma, for the same reason. So the promotion is complete for L = 0 and open for L > 0.
>
...
> **Corollary (the fallback question below 1/9, reduced).** **The s-ladder, canonically: s < 1/√δ − 1, hence s ≤ K whenever δ > 1/(K+2)², and the branch s is reachable only when δ ≤ 1/(s+1)².** So **δ > 1/9 forces s ≤ 1**, **δ > 1/16 forces s ≤ 2**, **δ > 1/25 forces s ≤ 3**; equivalently s = 3 opens at δ ≤ 1/16, s = 4 at δ ≤ 1/25, s = 5 at δ ≤ 1/36. *(Two ways of indexing one ladder, and they are easy to conflate: 1/(K+2)² is the threshold that **guarantees** s ≤ K, while 1/(s+1)² is where branch s **opens**. Strictness carries the whole difference — s = 4 needs 1/√δ − 1 ≥ 4, i.e. δ ≤ 1/25 — so a version reading ≤ at the boundary is one branch too generous at every rung and contradicts Theorem E.1, which settles δ > 1/9 at s = 1. Quote this sentence rather than re-deriving it; three editing passes have each repaired one indexing and left another standing.)* **This covers the whole table so long as the floor stays above 1/25.** *The floor is 0.04621 (ladder, to 10⁶) and 0.048039 (table, to 2600); `check_doc_figures.py --pass scope` exists to recount them if the range moves.* **The gotcha is that the recount is arithmetic on the floor and nothing else**, so a sentence naming a floor or an argmin that is no longer current still reads as true — state the bound without naming one, or requote both together.
>
> So at a computed n with δ > 1/16 the fallback question reduces by theorem to: s = 1 (Mersenne, Cap(a)); s = 2 with a ≥ 2 (repunit, Cap′(a)); s = 2 with a = 1 (safe prime; bare pair resolved by E.3(ii)); s = 3 (dead by E.4) — and there the **only branch without an absolute cap is s = 2, a = 1**, where E.3(ii) resolves the bare pair outright and only the leftover cases remain. Below the sharper thresholds — **δ ≤ 1/25 for s = 4, δ ≤ 1/36 for s = 5** (s ≤ 1/√δ − 1; Corollary F.3's box keeps the two ladders apart) — those branches become reachable and **neither has a theorem of its own**; the search clears them at every computed value. **But the leftover case is no longer part of the residue, and neither is anything else above 1/25:** Theorem E.5 caps the *whole* fallback branch, s = 2 leftovers included, at δ = 1/25, so the theorem-side reduction is complete on exactly the range the s ≥ 4 branches are unreachable on. The two statements meet at the same constant, and what is left of Part J item 2 is the single regime **δ ≤ 1/25**.

> **The same holds for `wide_cert.py`.** Its pass 2 calls `branch_settled` and skips the dispatched branches, so the same worry applies there, and is tested the same way, via `--no-theorems`. **⟦PENDING-CERT-RUN⟧** *Coverage figures are run outputs; requote after the certificates are rerun.* The collapse at that scale rests on the necessary conditions, on Lemma D2's unfused-foreign scoring, and — at the bare pairs — on E.3(ii); the s-branch dispatch settles nothing there (every live pair has s ∈ {2, 4, 6}), so the mode comparison that carries weight for E.1, E.3(iii) and E.4 is `fallback_cert.py`'s.
>
> **The hardest shape in the certificate, worked at its two extreme instances.** The shape that resists longest is 2c + r\* with c a safe prime, r = (c−1)/2, s = 2 and a leftover L = c — exactly the case E.3(ii) declines to cover, since the (r, r) re-reading would re-type the leftover c-block as a *second* foreign part equal to the first, which Part E forbids. Its two extreme instances below 10⁵ are **n = 50,817 = 2·20327 + 10163** and **n = 89,697 = 2·35879 + 17939**. What settles them depends on which statement is wanted, and the two part company precisely here.

> - For **μ(n) = B_refined(n)** the leftover twist lemma of E″ below settles them outright: it bounds any p-characteristic class's intra term by F·orb(c, dmax) with dmax stripped of the licensed foreign prime, and at L = c with c − 1 = 2r that leaves dmax | 2, so the leftover's intra caps at orb(c, 2) = c, far below B — the F = 2 reading dying the same way. (That inherits the lemma's own scope: a ΓL(1)-type stabiliser, and J0a at a ≥ 2.)
> - For **B_refined = B_safe**, which is what the collapse needs, the cap is unavailable: SAFE credits the same class the flat F·C(c,2), so only the search can close the shape, and at these two n it does not. Both survive as candidates, at s = 2 with the leftover L = c.
>
> **What actually closes them is a better lower bound, not the true B(n) — and the amount needed is tiny.** The candidate at each dies on condition (3) as soon as B exceeds the foreign block's own cap: orb(10163, 5081) = 51,638,203 at n = 50,817, and orb(17939, 8969) = 160,894,891 at n = 89,697. In density those are **0.039994** and **0.039996** — a hair under 1/25. So the question at these two n was never "what is B(n)" but "does this n clear the conjectured floor", and the fused F = 2 rung answers it with room: `2x14879 + 21059*` gives δ ≥ **0.1715** and `2x26387 + 36923*` gives δ ≥ **0.1694**. At those bounds both candidate lists are empty and s_max drops to 1. **Neither value needs the enumerator**, which at this size would be out of reach anyway.
>
> *Why they looked hard, which is the reusable part.* `wide_cert.py`'s B_lo scored the three-part shape **unfused** only — census S4, the reading that wins nowhere — so at these n it reported 0.0392 and 0.0368 where the fused rung reaches 0.17. The fused rung is the odd-n carrier and is worth a factor of two on the intra term; its absence is the same missing shape, for the same reason, as the S7 branch `ladder_verify.py` used to cut. A weak B_lo is permissive and so cost nothing but two false residues — but it is worth recording that **a family can be present in a menu and still contribute nothing**, since the fused rung balances at x\* = (2 − √2)/2 rather than the unfused 1/3, and a scan anchored at the wrong balance point returns zero silently.
...
>
> So *as established by this route*, over the certified range, the collapse μ(n) = B(n) rests on the eight necessary conditions being necessary, on Part 0's shape space, and on Lemma D2's domination of fused foreign classes — and on nothing else. **⟦Read this as a statement about the certificate, not about the collapse.⟧** *Part E‴ establishes the same conclusion over a far wider range by a different route, whose base is Part 0's shape space and counting on SAFE terms — not the eight conditions. So the two routes now share only Part 0, which is the right shape for corroboration: an error in the conditions cannot take the collapse with it above δ = 1/25.* In particular it does **not** rest on Lemma C, on Corollary C′ or on J0a: the conditions cap a p-characteristic part at the flat F·C(c,2), which is what SAFE assigns, so no twist strip is applied anywhere in them. E.1, E.3(ii), E.3(iii) and E.4 explain why the search is cheap and are what any statement about *all* n would have to go through, but they carry no weight in the per-n proof as it currently stands. This is worth stating plainly, because it is a much smaller trusted base than the section's structure suggests: the hardcoded `MERSENNE` and `REPUNIT3` tables, Lemma E.2's bound on L(a), E.4's uniqueness scan to a = 200 and E.3(ii)'s re-reading are all, for the computed range, commentary.

*The certificate.* `fallback_cert.py` enumerates the tuples (p, q, F, c, r) satisfying eight necessary conditions for a fallback configuration to score B(n) — c a p-power; r prime with r | c−1 and r ≠ p; F **any integer** with Fc + r ≤ n (F = F_mid·F_top with only F_top a q-power — see the permissive-block-count box below); each of the p-part intra, foreign intra, cross and within-class-cross terms at least B; and the leftover L = n − Fc − r either 0 or large enough to be a legal part carrying an intra-orbital of size B.

> **Which quantity the conditions must bound, which decides what "necessary" means here.** The collapse needs B_refined(n) = B_safe(n), which reduces to: no share-carrying configuration has its **SAFE score** ≥ B_safe(n). SAFE values a p-characteristic part at the flat **F·C(c,2)**, so that is what a condition on such a part must test. Capping it instead by F·orb(c, d) with the foreign prime stripped — the leftover twist cap below — tests a *smaller* number and can reject a configuration whose SAFE score does reach B: anti-permissive, and invisible in the output. The strip is not wrong; it bounds a different quantity, namely the minimum intra-orbital an actual *group* of that shape realises, which yields μ(n) = B_refined(n) rather than B_refined(n) = B_safe(n), and which is itself valid only for a ΓL(1)-type stabiliser (Part B's extraspecial counterexample). So the conditions use the flat cap, and the strip appears only as a diagnostic. **The consequence is a smaller trusted base than the strip version had:** with flat caps and `--no-theorems`, the candidate list is still empty at every row, so the collapse over the computed range rests on neither Lemma C, nor Corollary C′, nor J0a. Further parts only lower the minimum, so the conditions are necessary and an empty list is a proof at that n. Cost O(n log n) per value. It also reports which values Theorem E.1 settles outright.

> **The certificate returns no candidate at any value, and 0 inconclusive**, over all **2,187** rows to n = 2759. Requoted from the run on the completed table: theorem-settled all-branches **1,940 of 2,187 (88.7%)**; s-branches dispatched **2,195 of 2,442 (89.9%)**; E.3(ii) residue **247**, pairwise-only with the global promotion open; **largest permitted s = 3**. *Every count here is a run output and expires on any extension — requote, do not carry forward.* What the run establishes when it is clean is that μ(n) = B(n) is proved at each computed n, independently of tie-breaking, with B_refined = B_safe following rather than being separately measured. The theorem-side residue *at the time of that run* was a single class: the branches where E.3(ii) is pairwise only and the global promotion open. **Part E‴ has since removed it above δ = 1/25**, so the 247 figure now measures how often the dispatch defers to the search rather than how much is unproved. The run is also repeated with `--no-theorems`, which stubs every dispatch and rests the result on the eight necessary conditions alone.
>
> **And it rests only on the eight necessary conditions — measured, not assumed.** `--no-theorems` dispatches nothing and disables the E.3(ii) resolution, so every branch reaches the search; the candidate list is **still empty**, at 0 of 2,187 settled by theorem against 1,940 in the normal run. **That gap is what makes the comparison evidence rather than a tautology**: 89.9% of s-branches really are dispatched normally, so switching them off moves genuine work into the search. The check is only worth as much as the dispatch's firing rate at the range in question, which is why the rate is quoted beside it. *Contrast `wide_cert.py` at NMAX = 10⁵, where the dispatch settles **nothing** — the foreign-cap filter removes the s = 1 and s = 3 branches the theorems cover before the dispatch sees them — so a `--no-theorems` run there agrees trivially and is no evidence at all.* E.1, E.3(ii), E.3(iii), E.4, Lemma E.2's bound and the hardcoded `MERSENNE` / `REPUNIT3` tables are therefore commentary for the per-n proof — they explain why the search is cheap, and they are what any statement about *all* n must go through.
>
> *The same comparison at `wide_cert.py` reads differently, and the difference is worth keeping.* There B_lo replaces B, so the foreign-cap filter removes the s = 1 and s = 3 branches before `branch_settled` sees them, and the **s-branch dispatch** settles nothing at any NMAX in reach. What does fire is **E.3(ii)**, which is not a branch dispatch but a per-candidate resolution: at NMAX = 10⁵ it accounts for the whole difference between the modes, five bare pairs. So the two modes do *not* agree there, and should not be expected to — the disagreement is the E.3(ii) clause doing its work, and any difference beyond those five is the thing to chase. The evidential weight for the *branch* theorems still sits with this file, where B is the true value.
...
> **Result: the collapse B_refined(n) = B_safe(n) = B(n) is certified from proven lower bounds alone, far past the computed table's reach.** **⟦PENDING-CERT-RUN⟧** *Coverage counts are run outputs and are requoted after each certificate run; see `pending-checks.md` R1. The last complete run predates the fused rung's addition to B_lo and left two values open, both since closed at those n individually; the headline count, the weakest B_lo density and the permitted s all want requoting from a full rerun.*
>
> *The two statements have different coverage past the table, and the run reports the one that matters.* A value closed only by the leftover twist cap is closed for μ(n) = B_refined(n) and not for B_refined = B_safe. Under the flat-cap conditions the certificate answers the second, which is why the coverage is 99.998% rather than the 100% the strip-gated conditions reported: that figure was an artefact of testing the wrong quantity, not a stronger result. The cost is minutes of arithmetic at 10⁵: the lower-bound pass dominates at roughly 150 s per 10⁵ and is cached, so the next decade is an afternoon rather than a research problem.

> **The leftover twist lemma, and what it is and is not an instrument for.** The shape that resisted longest is s = 2 with c a safe prime and **leftover L = c** — two equal c-blocks plus the foreign r — since that is precisely where the (r, r) re-reading is unavailable, two blocks of the same prime being unable to both be foreign.
>
> **Read the lemma against the right quantity.** It bounds the minimum intra-orbital of a *group* whose configuration keeps the share, which is what μ(n) = B_refined(n) needs. It does **not** bound the SAFE score of that configuration, which is the flat F·C(c,2), so it cannot be used as a necessary condition inside a certificate for B_refined = B_safe — using it there would discard configurations whose SAFE score reaches B, silently. The certificates therefore run on flat caps and close this shape by search instead; the lemma remains the right tool for the weaker group-level statement and for any argument about *all* n, where a search is unavailable.
>
...
>
> Every ingredient is an already-proven necessary condition or a licensed strip, so this is the leftover analogue of the coupling argument. *(The licence is Corollary C′, whose Frobenius-exponent step is written inside AΓL(1, c) — automatic at a = 1, and at a ≥ 2 inheriting J0a. `fb_common.set_strip_trace` records what the strip would decide, so the exposure can be measured rather than argued about.)* **At L = c the shape dies on the strip alone:** c − 1 = 2r, so the r goes and, for the odd q that survive the foreign gate (q ∤ 2r), the q-part is 1 — leaving d | 2 and an intra term of orb(c, 2) = c ≪ B. The q = 2 reading dies earlier still, at orb(r, 2) = r < B. The same cap applied to the *main* class kills the F = 2 reading: there too c − 1 = 2r, the strip removes r, and what is left is d | 2.
>
> **What that argument does and does not close, which is the scoping the certificates turn on.** It bounds what a *group* of this shape realises, hence μ(n) = B_refined(n). It does not bound the shape's **SAFE score**, which is the flat F·C(c,2) with no strip, so it cannot serve as a necessary condition for B_refined = B_safe — the two questions differ exactly here. The certificates accordingly run on flat caps and close this shape by search; the lemma is the instrument for the group-level statement and for any argument about *all* n, where no search is available. **No F_mid strip is used anywhere in either reading, and none is available** — which matters, because an earlier route to the same conclusion leaned on one. **The hardest instances of this shape below 10⁵ — n = 50,817 with (c, r) = (20327, 10163) and n = 89,697 with (35879, 17939) — are closed by exactly this cap. ⟦PENDING-CERT-RUN⟧** The foreign-leftover analogue falls out the same way: a foreign leftover of size c needs q | c − 1 = 2r, forcing q = 2, which is dead at the same gate.
...
>
> *Measured over the table*, on the 49,552 foreign parts passing the gate: at **e = 1 — 90.4% of them (44,816, against 4,080 at e ≥ 2 and 656 at q = 2) — the median number of admissible positions is 0 and the maximum is 9**, against the ceiling 2/δ ≈ 43. So on the main branch the mechanism is not merely present, it is usually total.
>
> **It becomes conditional at step 1, and only there.** Every later bound is proportional to δ, so an unconditional q-pinning lemma needs an unconditional floor δ ≥ δ₀ — which is Hypothesis (BCG-AL), or `arithmetic-of-density.md` §5's conjecture. Inside the verified range the floor is computed and the argument is unconditional; beyond it, this route is conditional on the same hypothesis the density ceilings are. That is not circular, since B(n) ≥ δ₀·C(n,2) comes from the constructions rather than from the certificate, but it does mean the unconditional version dies with the asymptotic floor.
>
...
>
> **And pinning reaches only half the leftover.** It constrains *foreign* leftover parts. A p-characteristic leftover part is not pinned by q at all; what bounds it is the leftover twist cap below, which is Lemma C's coupling together with Corollary C′ — proved at every a for a semilinear stabiliser (automatic at a = 1; at a ≥ 2 it inherits J0a), so this half is unconditional wherever J0a is. What remains conditional is the pinning half, which fails at q = 2 and large e.
>
...
> **So the realistic target is not "eliminate the fallback branch unconditionally"** but: *conditional on δ ≥ δ₀, the fallback branch reduces to the q = 2 and large-e cases together with the a > 1 case* — a named finite residue rather than an open problem. That is a statable lemma and it is closer than it looks. **⟦SUPERSEDED⟧** *It has since been stated and proved, with a better constant than this framing anticipated and with no conditionality on a pinning argument at all: Part E‴'s Theorem E.5 caps the whole branch at δ = 1/25 by counting on SAFE terms, so the q = 2 and large-e residue described above is not a residue of the collapse question. What it remains a residue of is the sharper statement E″ is actually about — replacing B_safe by B_refined **below** 1/25 — which is where the analysis in this box still applies.*
...
## Part E‴. The fallback branch has a ceiling at density 1/25

*This Part closes the collapse question above one explicit density, by theorem rather than by certificate. It supersedes the "realistic target" framed at the end of E″ — that target has been reached, and reached with a better constant than the framing anticipated. Nothing below uses Lemma C, Corollary C′, J0a or any statement about a group's realised orbitals: every step is counting on **SAFE terms** plus the arithmetic of q-power twists, which is the same trusted base the certificates run on.*

**What was left open, and why the certificates could not close it.** E.3(ii) resolves the bare safe-prime pair and declines the leftover, so the theorem-side residue was the s = 2, a = 1 branch **with a leftover**, at *every* density — not confined to a low-density tail by anything written down. The certificates dispatch it per n and always find nothing; the two values that resisted `wide_cert.py` at NMAX = 10⁵, n = 50,817 and n = 89,697, are exactly this shape. What follows explains why they resisted and why nothing else will: the branch has an absolute density ceiling of 1/25, the two values sat at 0.039994 and 0.039996, and the margin to the line is Θ(1/n).

> **Theorem E.5 (the fallback sup).** Let W be a fallback configuration at n — a p-characteristic class (F, c) with c = p^a, a foreign prime r | c − 1 with r ≠ p, a top prime q, and a leftover L = n − Fc − r composed of admissible parts. If W is **not** a bare safe-prime pair (F = 1, L = 0, c = 2r + 1), then
>
> **SAFE(W) < C(n,2)/25,**
>
> with exactly **15 exceptions, all at n ≤ 63**: the r = 2 and small-Mersenne instances at c ∈ {3, 4, 8, 16, 32} and the repunit instance (c, r) = (27, 13), listed in the table below and each closed by direct evaluation.

*Proof.* Write B = SAFE(W), δ = B/C(n,2), and s = (c−1)/r. Every part of W has size at least ℓ(B) := min{m : C(m,2) ≥ B}, and the foreign part satisfies orb(r,t) ≤ C(r,2), hence **r(r−1) ≥ 2B**. Take the s-branches in turn.

- **s ≥ 4.** The s-ladder of E′ gives s < 1/√δ − 1 strictly, so δ < 1/(s+1)² ≤ 1/25.
- **s = 3.** Theorem E.4: the branch is the single pair (16, 5) with SAFE ≤ 10, which is below C(n,2)/25 at every n ≥ 21 where the pair fits.
- **s = 1, and s = 2 with a ≥ 2.** Theorems E.1 and E.3(iii) give absolute caps Cap(a) and Cap′(a). Each fails to clear C(n,2)/25 outside a bounded window of n, and Lemma E.2's L(a) ≤ 2^{(a−1)/2} + 1 makes the windows empty from a = 13 (Mersenne) and a = 7 (repunit) upward. The surviving windows are the 15 exceptions.
- **s = 2, a = 1**, the branch E.3(ii) declines. Here c = 2r + 1 is a safe prime and B ≤ min(F·C(c,2), orb(r,t)) ≤ r(r−1)/2. Four sub-cases, and each is a counting bound on n:
  - **F ≥ 2, any leftover.** n ≥ 2c + r = 5r + 2, so δ ≤ [r(r−1)/2] / C(5r+2, 2) = (r−1)/(5(5r+1)) < **1/25**.
  - **F = 1, leftover containing a p-characteristic part.** That part's block size is a power of p = c, hence ≥ c = 2r + 1, so again n ≥ c + r + c = 5r + 2 and the same bound applies. *This is the L = c shape, the one E.3(ii)'s promotion could not re-type, and it is the sharp case: δ = 1/25 − Θ(1/r).*
  - **F = 1, leftover of two or more parts.** Each has size ≥ ℓ(B), and with B = θ·C(r,2) one has ℓ(B) ≥ √θ·√(r(r−1)) approximately, so n ≥ c + r + 2ℓ ≥ (3 + 2√θ)·√(r(r−1)) up to the usual n/(n−1). Hence δ ≤ θ/(3 + 2√θ)², which is ≤ 1/25 on θ ∈ (0,1] with equality only at θ = 1, where the exact count is strict.
  - **F = 1, leftover exactly one foreign part r′.** The only genuinely new case, and the one the certificates were doing by hand. Here n − 1 = 3r + r′. Suppose δ ≥ 1/25. Then r, r′ > (n−1)/5, which with 3r + r′ = n − 1 forces **r′/r ∈ (3/4, 2)**. Both foreign blocks must clear B, so their twists — powers of the **same** q, by Lemma B′ — satisfy (r−1)/t ≤ 3 and (r′−1)/t′ ≤ 3 (t′ even) or ≤ 7 (t′ odd). For **q odd** that gives r = 2q^e + 1, and the only value in the ratio window with the same q is r′ = 4q^e + 1 = 2r − 1, whence n = 5r exactly and δ ≤ [r(r−1)/2]/C(5r,2) = (r−1)/(5(5r−1)) < 1/25. For **q = 2**, r ∈ {2^e + 1, 3·2^e + 1}; but c = 2r + 1 is divisible by 3 when r = 3·2^e + 1, and by 5 for every Fermat prime r ≥ 17, so c is not prime and only r ∈ {3, 5} survive, i.e. n ∈ {15, 23}, where SAFE ≤ 7 < C(n,2)/25.

  In every sub-case δ < 1/25, strictly. ∎

> **The 15 exceptions, and why they are not a qualification.** Enumerating *every* fallback configuration at every n ≤ 3000 — any (F, c), any r | c − 1, any top prime, leftover of zero, one or two admissible parts — and taking the sup of SAFE per n, the configurations reaching C(n,2)/25 are these and nothing else (`fallback_sup.py`):
>
> | n | c | r | F | L | q | s | a | SAFE |
> |---|---|---|---|---|---|---|---|---|
> | 5, 8, 10 | 3 | 2 | 1 | 0, 3, 5 | — | 1 | 1 | 2 |
> | 7, 11, 12 | 4 | 3 | 1 | 0, 4, 5 | 2 | 1 | 2 | 3 |
> | 9 | 7 | 2 | 1 | 0 | — | 3 | 1 | 2 |
> | 15, 23, 27, 28, 31 | 8 | 7 | 1 | 0, 8, 12, 13, 16 | 3 | 1 | 3 | 18–21 |
> | 21 | 16 | 5 | 1 | 0 | 2 | 3 | 4 | 10 |
> | 40 | 27 | 13 | 1 | 0 | 3 | 2 | 3 | 39 |
> | 63 | 32 | 31 | 1 | 0 | 5 | 1 | 5 | 155 |
>
> Every row is a Mersenne pair (E.1), the repunit pair (E.3(iii)) or the r = 2 degeneracy, every one is at a proper prime power c, and every one is inside the computed table, where the certificate closes it directly. So the theorem's exception list is finite, explicit and already discharged — not an open tail.

> **Corollary E.6 (the collapse above 1/25).** If **B_safe(n) > C(n,2)/25** then
>
> **B_refined(n) = μ(n) = B_safe(n).**
>
> *Proof.* Let W attain B_safe(n). If W is fallback-free the endpoints coincide, since orb(c, c−1) = C(c,2). If W is a fallback configuration then SAFE(W) = B_safe(n) > C(n,2)/25, so by E.5 it is either a bare safe-prime pair or one of the 15 listed instances. A bare pair is dominated by its own (r, r) re-reading, which is fallback-free and scores at least as much (E.3(ii)), so B_refined(n) ≥ B_safe(n); the 15 instances are checked directly. In each case the SAFE optimum is realised by a fallback-free configuration, which Part E constructs, and Part E′'s sandwich closes. ∎

**What this changes, stated carefully.**

- **The collapse over the computed range no longer rests on the certificates.** `ladder_verify.py` gives δ ≥ 0.04621 > 1/25 unconditionally on every n ≤ 10⁶, and a ladder score is a lower bound on B_safe; so **μ(n) = B(n) at every non-prime-power n ≤ 10⁶ by theorem**. `fallback_cert.py` and `wide_cert.py` become corroboration — valuable corroboration, since they test the same claim by a completely different route — rather than the proof. *The reduction is to the ladder, not to the table: no B(n) is computed anywhere in it.*
- **E.3(ii)'s global promotion is no longer owed.** The leftover case it declined cannot attain B above 1/25, so nothing needs promoting there. What survives of the old residue is the statement *below* 1/25, which is where the s ≥ 4 branches live too — one regime rather than two open items.
- **The two thresholds coincide for a reason.** The conjectured floor is 1/25 and the fallback ceiling is 1/25, and the coincidence is structural rather than lucky: both are the "five parts of size ≈ n/5" configuration, reached from opposite sides — the floor by the balanced family it would take to fall that far, the ceiling by n = 5r + 2 and n = 5r. **So the collapse holds exactly as far as the floor does, and the two will stand or fall together.**

> **Sharpness, and what a counterexample would have to be.** E.5's constant cannot be improved: the shape n = 5r + 2 with c = 2r + 1 and leftover L = c reaches ratio **0.9955 of the 1/25 line at n = 1797**, 0.9936 at 1257, 0.9911 at 897, 0.9808 at 417 — every one an n = 5r + 2 with (q, 2q+1, 4q+3) a **Cunningham chain of length 3**, and the ratio tends to 1. So *B_refined = B_safe at every n* is now **equivalent** to: at no Cunningham-chain n = 5r + 2, and no n = 5r with 2q^e+1 and 4q^e+1 both prime, does every fallback-free configuration fall below 1/25 − O(1/n). That is a Hardy–Littlewood statement of exactly the kind §5 of `arithmetic-of-density.md` already assumes, and no weaker: at the 21 chain values below 24,597 the table's own δ runs 0.12–0.33, a factor of three to seven clear, but the clearance is supply and not structure.

> **If the sandwich ever did open, where would μ sit? The answer is not symmetric, and it is worth recording because it is the question the sandwich was built to leave open.**
>
> - **At a = 1 — which is all of the s = 2 branch and the sharp shape above — μ sits at the LOWER end.** The c-block's stabiliser is a subgroup of 𝔽_c^× with c prime, so there is no exotic option: it is cyclic of some order d | c − 1, and the refined score is exact for it. If r | d then Lemma C at a = 1 — automatic, needing no semilinearity — forces the foreign twist trivial (or q = r, where D2q does), so the foreign block is worth r; if r ∤ d then d | 2 and the c-block is worth at most c. Either way every group realising the partition has m\* at most the refined value, so **μ(n) = B_refined(n) < B_safe(n)** there, provably, and B_safe would simply be loose.
> - **Strictly in between is possible only at a ≥ 2, and only below the caps.** The refined scoring gives a stripped block a **cyclic** twist and so ignores the Galois layer, which at k = 2 is a real orbital source: at c = 343 with foreign prime 19, the group C₁₈ ⋊ Frob₃ ≤ ΓL(1, 343) sits in an Oliver chain at q = 3 and realises minimum intra-orbital 3087, against orb(343, 18) = 3087 at the unstripped twist but 3·orb(343, 6) at the stripped one — the same phenomenon Part B's extraspecial example exhibits outside ΓL(1) entirely. So μ could exceed B_refined on a proper-prime-power stripped block. But a ≥ 2 fallbacks are confined by E.1/E.3(iii)'s caps to δ = O(n^{−1/2}), or to s ≥ 4 below 1/25.
>
> **So the trichotomy resolves:** above 1/25 the two ends coincide (E.6); below it, wherever the stripped block is a **prime**, μ is pinned to the lower end; and "strictly in between" requires simultaneously a proper prime power c, a stripped twist, and a density below the caps. *The natural repair for the third case is to score the Galois layer — a refined bound **B_refined⁺** taking the twist inside ΓL(1, c) rather than inside the Singer cycle — after which the only remaining source of an interior μ is J0a proper, i.e. a stabiliser outside ΓL(1) altogether.* That is a small, well-posed addition to `mu_enumerate`'s `orb` and is filed as such.

> **A dependency worth naming, because it removes J0a from one more place.** Lemma C's *coupling* conclusion t | ord_r(p) is proved via a Frobenius exponent and so assumes a semilinear stabiliser at a ≥ 2 (J0a). The **bound** it is used for — orb(r, t) ≤ r·a, which is all Corollary C′ and E.3(i) consume — needs no such assumption. The r-element z′ of the cyclic layer acts on V = 𝔽_p^a with eigenvalues that are r-th roots of unity, closed under Frobenius, hence lying in at most a/ord_r(p) Frobenius orbits. Conjugation by any h ∈ Γ is a power map on the cyclic layer, z′ ↦ z′^{m_h}, whose reduction mod r is h's induced multiplier on the foreign block; the multiplier group M ≤ (ℤ/r)^× therefore permutes those orbits, and M⟨p⟩/⟨p⟩ acts freely on the cosets, giving |M| ≤ |M ∩ ⟨p⟩|·a/ord_r(p) ≤ a. Hence **t ≤ a for any stabiliser whatsoever**, and E.3(i)'s "worth at most c" weakens only to "at most max(c, r·a)" — still linear in n, which is all that clause needs. *One reading only; it wants a second before anything is rested on it, and nothing above is.*
...
*The inequality is strict and carries an n/(n−1), which matters at the boundary.* F.1 gives k < √(n/((n−1)δ)) rather than k < 1/√δ, so a row sitting just above a threshold can still permit the next part count: at **n = 1775**, δ = 0.062519 is above 1/16 yet √(n/((n−1)δ)) = 4.0006, so k ≤ 4 rather than k ≤ 3 there. The same fudge sits in E′'s s-bound and in F.4's branch (a), and the three should be quoted in the same form. *The search is small in practice.* The floor over the contiguous range [6, 10⁶] is **175813/3804661 = 0.046210 at n = 2759**, giving k ≤ 4 (as did v4's 0.045742 and the 2,600-frontier floor 0.048039); at the median density 0.1713 the bound is k ≤ 2. And wherever the ladder keeps δ above 1/25 — which it does throughout n ≤ 10⁶, at floor 0.04621 — **no admissible configuration in that range has more than four orbits**, and most have two. (**Gotcha:** a family-menu figure is not a density, and reading 0.0147 at n = 4917 as one gives k ≤ 8. That figure is a *lower bound* on μ rather than a density — see the two-δ caution in Part H — and the ladder's floor supersedes it in both directions.)
...
>
> **F.1 is tight and cannot be improved to k ≤ 3 by counting.** Running the same argument through the *smallest* part reproduces k ≤ 1/√δ exactly, so the equal-parts case saturates it, and no tightening of constants does better: at the density floor (0.046210 at n = 2759 over [6, 10⁶]) F.2 gives k ≤ 4.37 against F.1's k ≤ 4.65 — still 4, not 3, and a higher floor only lowers both. Since no winning configuration anywhere in the computed range uses more than three parts (Part I), the residual factor is arithmetic, not metric: any proof of k ≤ 3 must use the prime-power structure and the Diophantine constraint n = Σ F_i c_i. This is Part J item 1.
...
>
> **(b) Shifted-prime branch.** There is a prime r ≥ δ₀(n−1)/2 with a **prime-power divisor Q of r − 1** satisfying **(r − 1)/Q ≤ D(δ₀) := 2(1 − √δ₀)²/δ₀**, up to a 1 + O(1/n). The cruder **(r − 1)/Q ≤ 2/δ₀** follows and is what a bound r ≤ n gives; the sharper form uses the fact that a configuration carrying a foreign part carries a second part as well.
>
> Integers of the form (a) have density zero, so **(b) holds for almost all n**. Branch (b)'s multiplicative content extracts as an n-free hypothesis on the set of primes — bounded cofactor, plus a condition on how densely that set sits among the primes — which `arithmetic-of-density.md` §6.8 states as Hypothesis (BCP_{D,c,ρ}) and locates against the literature's almost-all machinery. Bounded multiplicative gaps, at any **Λ > 2/δ₀** (strictly — at Λ = 2/δ₀ exactly the argument is degenerate, §6.8(ii)), also follow from the inequality above. Neither gaps nor density is what a route *back* to a floor runs out of: §6.8(iv) shows the almost-all machinery consumes cardinality alone and tolerates a thin set, and that what it cannot deliver at any input density is the **companion** clause's bounded cofactor.
>
> *Proof.* The score is a minimum over orbitals, so **every** part with support s_i ≥ 2 has its intra term at least δ₀·C(n,2) = δ₀n(n−1)/2 — this is what makes a converse available at all, and it is the same feature that makes the forward direction hard. (A part of support 1 has no intra orbital and so is not covered by that sentence; it needs no separate branch, since its *cross* orbitals have size at most n, which fails the floor outright for large n. So a configuration with a fixed point is excluded rather than classified.)
>
> If some part is **foreign**, it is a class of F ≥ 1 blocks of prime size r whose twist Q is a q-power — forced, not assumed: AGL(1,r) is nonabelian, so the translations occupy the abelian cyclic layer and the twist is pushed into the top q-group, which is Lemma B′'s content. The class contributes at most F·r·Q, and **F·r ≤ n jointly** — the same care the matching branch needs — so F·r·Q ≥ δ₀n(n−1)/2 gives Q ≥ δ₀n(n−1)/(2·F·r) ≥ δ₀(n−1)/2, and Q ≤ r − 1 ≤ n − 1 then gives both r ≥ δ₀(n−1)/2 and (r − 1)/Q ≤ 2/δ₀. That is (b) in its crude form.
>
> *The cofactor bound sharpens, and the sharpening is free.* Dividing the same inequality by Q rather than bounding r ≤ n gives (r − 1)/Q ≤ 2F·r(r−1)/(δ₀n(n−1)) ≤ 2r(r−1)/(δ₀n(n−1)) at F = 1, so the cofactor is controlled by r's **share** of n rather than by n itself. A configuration containing a foreign part is not the whole of n: it has at least one other part, and every part has support > √(δ₀n(n−1)) by the same floor, so F·r ≤ n − √(δ₀n(n−1)) and hence
>
> > **(r − 1)/Q ≤ 2(1 − √δ₀)²/δ₀**, up to a 1 + O(1/n).
>
> That is D(δ₀) as stated. It is a strict improvement at every δ₀ — D = 25.4 against 42 at the table floor, 32 against 50 at the conjectured 1/25, 14.9 against 28 at the asymptotic ceiling — and it costs nothing beyond Proposition F.1's own part-size bound, which the proof has already used. The per-row form 2F·r(r−1)/(δn(n−1)) is **attained** wherever the foreign term binds, so it is the form to measure against. (The F ≥ 2 case is shape S9, which Lemma D2 shows is dominated and which wins nowhere in range; the bound is stated for it anyway, since domination is not non-existence and F.4 quantifies over configurations rather than over winners.)
>
> If **no part is foreign**, every part is a matching class of F_i blocks of size c_i = p^{a_i}, all sharing the chain prime p (Part B). *Why they must share it, given the chain:* a block class of size s^k with k ≥ 2 and s ≠ p has elementary-abelian non-cyclic translations, which need a home in the chain. At **s ∉ {p, q}** there is none: every s-element maps into the cyclic layer, so the Sylow s-subgroup is cyclic and cannot contain C_s^k. At **s = q** that argument does not apply — the top layer holds non-cyclic q-groups perfectly well — and the conclusion comes from Lemma B′ instead, whose **Case 2** is exactly this branch: with π_O(Γ₁) = 1 the block group is a transitive q-group acting primitively, hence regular of prime degree, so k = 1 there too. Either way a block class at a second prime has k = 1, which is the foreign case and exits to (b). *(The s = q branch is the one a compressed "only Γ₂ can hold it" sentence gets wrong, since it is the one case where a second layer can.)* The dichotomy is therefore exhaustive **per chain**, and since the argument fixes one chain first (Part G.0) and needs only *some* chain, nothing is lost by a group admitting several. From F_i·c_i²/2 > δ₀n(n−1)/2 with F_i c_i ≤ n we get c_i > δ₀(n−1) and F_i < n/(δ₀(n−1)); writing b = min a_i, n = Σ F_i p^{a_i} = p^b·(Σ F_i p^{a_i − b}) with p^b ≥ min c_i, so the cofactor is bounded by the same quantity. That is (a). ∎
>
> **Checked shape by shape against the Part B census, which is a different kind of check from the numerical one.** The proof splits on whether a foreign part is present, so the census is a finite audit of that split. **S3** (c + r\*), **S4** (2c + r\*), **S5** (top-fused matching + outside), **S6** (two outside blocks) and **S7** (middle-fused matching + outside) all contain a foreign part and land in (b) — S6 twice over, either block serving. **S9** (fused outside class) also lands in (b), and is the shape the split is easiest to get wrong on: it needs the joint bound F·r ≤ n rather than r ≤ n, exactly as the matching branch does. **S8** and **S10** are killed (D1 and D2q), so no configuration of those shapes exists to quantify over — S10 instructively, its twist being forced into the cyclic layer beside the translations and hence trivial, which is the degenerate boundary of the very confinement (b) relies on. **S1** and **S2** are the multiplicative engine and land in (a).
>
> *S1 and S2 are kept separate, and the boundary is F.* S1 is the unfused single class (F = 1, n = c a prime power); S2 is F > 1. So S2's domain is the non-prime-powers and S1's is the prime powers, and between them they cover every n — which is why the floor conjecture's domain, "composite non-prime-power n", is exactly S2's domain and not an arbitrary exclusion (`aod` §5).
>
...
> **Branch (b) rests entirely on Q being a prime power** — with Q merely a divisor of r − 1 the conclusion is vacuous, since Q = r − 1 always qualifies, which is precisely what happens in branch (a). The confinement is **Lemma B′'s**, so this is a layer-assignment claim resting on a proved lemma rather than on an unstated assumption — but layer-assignment claims are the category with the worst record here, and B′ is itself the one structural lemma still inviting further scrutiny (`pending-checks.md` T1, T8). Two independent readings of the Proposition; B′ itself has had one.
>
> **Checked against the table.** All three inequalities hold at every one of the 2,186 contiguous rows: (r − 1)/Q ≤ 2/δ at all 1,443 foreign primes appearing in winning witnesses, r ≥ δ·n at every one, and M ≤ 1/δ at all 743 winners with no foreign part. *Two scoping notes on what `converse_check.py` tests.* The second inequality is checked in the **sharper** form r ≥ δ·n, which is not the one proved above (that gives r ≥ δ₀(n−1)/2); the factor 2 comes from combining the foreign intra cap F·C(r,2) with F ≤ n/r, so a violation under 2× would contradict the sharper form without touching the Proposition. And the third is keyed on the **absence of a foreign part** rather than on a part count, because branch (a) is wider than the one-part shapes — a multi-class all-matching row such as n = 640 = 1·256 + 3·128 belongs to it, and keying on `parts == 1` would let such a row reach neither branch. No such row exists in v4 or in the v5 prefix; the check is live against a rebuild producing one. The all-matching bound is nearly attained at n = 2594 = 2·1297 (ratio 0.9996), which is also §6.1's tightest feasibility row. The largest cofactor (r − 1)/Q anywhere in the table is **12** — (BCG)'s own constant, recovered from the opposite direction — against D(δ₀) = 25.4 at the table floor (26.1 at the ladder floor), so the Proposition is loose by a factor ≈ 2.1 against what optima use. *(The crude 2/δ₀ gives 42 and a factor 3.5; the sharper constant is the one to quote, and `converse_check.py` reports both.)* **The second inequality carries no headroom at all, and its apparent factor of 2 is not evidence of one.** Any foreign part has orb(r, t) ≤ C(r,2), so r ≥ n√(δ(n−1)/n) outright — a theorem, not a slack observation; and any configuration carrying a foreign part has k ≥ 2 and hence δ < 1/4, which turns that into r ≥ 2δn automatically. So the measured "r/(δn) = 2.003" is √δ/δ at δ ≈ 1/4 and says nothing about F.4. The tightest instance of the sharp form r ≥ √(δ·n(n−1)) is **ratio 1.0004**, at n = 2580; `converse_check.py` tests that form rather than r ≥ δ·n, which it implies. *Only the cofactor slack in (1) bears on the sharpening question, which is therefore one question about one inequality.* These are requoted from `converse_check.py` on the completed table, and the headline constant 12 is attained twice — at n = 221 (r = 157, Q = 13) and again at the range minimiser n = 2759 (r = 1453, Q = 121). The slack figure moves with the floor and so is range-dependent even on a correct table.
>
> **What the second branch costs, and why it is not a defect of the statement.** Integers of the form M·p^b with M bounded have density → 0, so for **almost all** n the all-matching branch is unavailable and the conclusion is the clean one: *a prime r linear in n whose r − 1 has a prime-power divisor of bounded cofactor*. But the decay is only of order H_M/log n, and the surviving share is large: **the all-matching branch is exactly the multiplicative engine's, and by `aod` §2.1's identity δ_S2(n) = (Q(n) − 1)/(n − 1) it certifies δ ≥ 1/25 by itself precisely when Q(n) ≥ n/25**. Measured at n ≤ 10⁵ that is **43.0%** of integers on the exact form, or **43.5%** on the divisor form (some M ≤ 25 with n/M a prime power), the two differing only in the endpoint convention. So this is an asymptotic exclusion and not one any computation will exhibit — and the 43% is not a loose plausibility check on the floor: it is the statement that the universal fused shape certifies the floor unaided on nearly half of all n, and that the whole difficulty of the conjecture lives in the complementary set where the largest prime-power divisor is small.
...
**G.3a The full orbital decomposition.** *The orbital **count** also appears in `orbital-evasiveness-notes.md` §9.7 (t = Σᵢ[(cᵢ−1)/|±Tᵢ| + ⌊Fᵢ/2⌋] + C(k,2)); this section adds the sizes and shapes, which let the count be checked against C(n,2).* G.3 names one class of each *type* — the ones the score's minimum ranges over — without multiplicities, so its sizes do not sum to C(n,2). The complete decomposition is uniform by part type. Let part i be a vertex orbit of Fᵢ blocks of prime size cᵢ carrying a multiplier group of order dᵢ on each block, and put hᵢ = |⟨twist, −1⟩| (= dᵢ if dᵢ is even, 2dᵢ if odd) and sᵢ = Fᵢcᵢ. Then the orbitals are:
...
From these the size of the search follows. The admissible parts are pairs (F, c) with c a prime power in [δn, n] and F any integer at most 1/δ (F = F_mid·F_top, only F_top a q-power), so their number is
...
- **certification level, not orbit count.** The `certified_K` column records the K at which the Part F iteration halts, which is one more than the part count it rules out. Its distribution is {2: 394, 3: 1546, 4: 239, 5: 7}, but the **actual part counts are {1: 743, 2: 1443}** — no winner uses more than *two* parts. **Reading `certified_K` as an orbit count is the trap here**, and it invites the conclusion that "K = 5 is the predicted ceiling, reached by the hardest rows" — wrong twice over. At the weakest density in the contiguous range (0.048039 at n = 2183) Proposition F.1 permits **four** orbits and the winner there uses **two**. Nothing in range has five orbits; the certified_K distribution and part counts here are v4 figures awaiting the rebuild;
- what *is* tight is Part G.4 on the individual axes, and tight on a single row. Under the v2 table the instance was n = 575 with witness `23x25` at δ = 0.041812: the fusion bound 1/δ = 23.9 met by F = 23, and the block-size bound δn = 24.0 by c = 25 — both within one unit simultaneously. ⟦REQUOTE-ON-EXTENSION⟧ That n is won by `4x103 + 1x163*` at δ = 0.080006 in the v4 table, so the instance is pending recomputational and the current one is below. **The tightest row is n = 2594 = `2x1297`**, where both G.4 ratios sit at 1.0004 and the feasibility criterion Σ√Fᵢ ≤ 1/√δ has slack 0.0003 — the same row and the same figure `aod` §6.1 reports, and the same row at which F.4's branch-(a) bound is nearly attained, so three constraints derived by different routes bind together there. That the same row saturates all three is structural rather than a coincidence: a single fused class n = F·c has δ = (c−1)/(Fc−1), so c = δn, F = 1/δ and Σ√Fᵢ = 1/√δ all hold to O(1/n) at once, and the three bounds are one bound read on three axes;
- over the contiguous range [6, 10⁶] the minimum is **175813/3804661 = 0.046210 at n = 2759** (`2x653 + 1x1453*`; the 2,600-frontier minimum was 0.048039 at n = 2183, and v4's 0.045742 at n = 1817 rises to ≥ 0.0594 under the corrected caps), the maximum 0.499999 at n = 999,958 = 2·499,979 (Theorem 2.1's family), and the median 0.1713. **A falling floor is what makes a larger s-branch reachable**, so it is the figure to recheck after any extension — and it moves in both directions, a missing shape depressing it and a repair lifting it;
- **Theorem settles 1,940 of the 2,187 values (88.7%) outright**, with no search needed; per s-branch it is 2,195 of 2,442 (89.9%). The residue is a single class, **247 branches on E.3(ii)'s open global promotion** — the s = 4 and s = 5 branches of the v2 figures having gone, since the largest permitted s over the current table is 3. The Mersenne branch of Theorem E.1 never binds: no n in range has Cap(a) ≥ B(n) for any applicable exponent. Only Cap(2) = 6, Cap(3) = 21, Cap(5) = 155 and Cap(7) = 1143 arise, with tightest margins B/Cap = 1.43 at n = 15, 1.63 at n = 63 and 1.90 at n = 20. So the entire residual difficulty is low density, not the Mersenne coincidence.

Winning configuration shapes, over [6, 10⁶]: **373,661** use one p-characteristic part with one foreign prime (S3); **124,502** a single fused class (S2); **423,102** a fused class with a foreign prime (S5 and S7 at every F, the largest group); and **none** uses either two p-characteristic parts with a foreign prime (S4) or two foreign primes (S6) — both families are empty, over a range four hundred times the one on which they were first reported empty. The S6 emptiness is now a theorem above 1/25 rather than a watch item: `ladder-completeness.md` Proposition 2 caps two foreign primes at a common top prime at 1/9 (even n) or 1/16 (odd n), and `offmenu_scan.py` finds the shape's best score on the table at 0.111 (n = 4376), never equal to B. *(An earlier form of this paragraph cited n = 3059 = 1511\* + 907\* + 641 as a two-foreign winner and the global density minimum; both were pre-repair readings — under the corrected shape space 3059 scores 0.083906 on `3x512 + 1x1523*`, nearly double the floor, and the minimum is n = 2759.)* These are the same shapes reported at n ≤ 1306 and at n ≤ 2600 — **no sixth shape appears** in a range extended four-hundred-fold, which is the more informative fact, and Proposition 1 of `ladder-completeness.md` says none can above 1/25.
...
**The three-part family is now empty, and the prediction that it would nearly empty was an understatement.** On the completed table there are **no three-part winners at all** — part counts are 743 one-part and 1,443 two-part, with nothing above. The v4 census had 16, of which 15 were already known to be exceeded by entangled two-class readings; the sixteenth, n = 1529, went the same way once the whole table was rescored. So the shape class that carried its own subsection is gone.

> **Why it emptied completely rather than nearly.** All 16 v4 three-part winners had one shape — one foreign prime plus two unfused p-characteristic parts of equal block size — and that shape is precisely what a cyclic-layer fusion supersedes: the two equal parts merge into a single fused class of twice the block count at full twist, which is a *two*-part configuration scoring strictly higher. The correction does not chip away at the family, it rewrites every member of it. This is the same mechanism as the S4 → S7f2 migration and it has the same endpoint.

**What replaces the census's three-part row.** S3 (unfused matching plus foreign) takes 853 winners and S2 (fused single class) 743; the fused-with-foreign rungs take S7f2 = 395, S7f3 = 102, S7f4 = 58, S7f6 = 14, with S5 at 21. **S1, S4 and S6 have no winners anywhere on the table** — S4 for the reason just given, S6 (two foreign parts) having been a single row at n = 1175 that is now the fused `2x233 + 1x709*`.

**There is no three-part population left to measure against two-part configurations.** The family is empty on the completed table, every member having been rewritten by cyclic-layer fusion into a two-part configuration scoring higher, so the B₃/B₂ margins that once ruled out the perturbation route to k ≤ 3 no longer exist as data. *That does not restore the perturbation route:* what closed it was that a proof must **produce** a strong ≤3-part decomposition rather than perturb a weaker one, and the empty family says the same thing more strongly — there is nothing to perturb from. Part J item 1 is stated in those terms.
...
**How odd n are actually served, and the correct scope of the two-block diagnosis.** One diagnosis of the odd-n shortfall runs by arguing that the strong two-block family needs exactly one even block, that the block must be the p-characteristic one since the other has prime degree and 2 is the only even prime, and hence that odd n reach the strong family only through n = 2^a + r with r prime — about log₂n candidate splits against ~n/2 for even n. The reasoning is sound about that *family*, and it is the binding constraint for odd n with **ω(n) ≥ 3 that use two parts** — which is precisely the weak tail. What it does not describe is how most odd n are served, because most *strong* odd n have ω(n) = 2 and use a single fused class, where no additive representation enters at all.

Of the 421,284 odd values in [6, 10⁶], 372,101 reach density 1/12 or better. Among those:

| route | count |
|---|---|
| **no even part at all** | **58,844** |
| one even part, a 2-power p-characteristic block (the 2^a + r shape) | 6,187 |
| one even part, odd block size with even fusion count | **307,070** |

and by part count, 58,844 use a **single fused class** and 313,257 use two parts, with nothing above. *(At the 2,600 frontier the rows read 348 / 55 / 434 and the third was already the largest; at 10⁶ it is 83% of the strong odd values, the 2-power route having fallen from 6.6% to 1.7%.)* So the dominant route for a strong odd n is a fused class beside a foreign block — the even fusion count supplying the parity that a 2-power block used to have to supply — and after that the single fused class of §2.1 of `arithmetic-of-density.md` with both factors odd. **The third row is the largest because an even fusion count at odd c supplies the parity by itself**, which is what makes a 2-power block unnecessary at most odd n.

Restricting to the values where the single fused class is *unavailable* is where that diagnosis applies, and the corrected shape space has narrowed what it describes. Of the **222,249 odd values with ω(n) ≥ 3** in [6, 10⁶], every one uses a **two-part** configuration — a fused class beside a foreign block — and their median density is 0.1248, above the old three-part cap of 1/9 rather than sitting at it. So the 2^a + r shortage still limits the *unfused* two-block family exactly as the argument says, but that family is no longer what serves these n.
...
Fermat primes achieve this at q = 2, safe primes with t = q odd and 2t = r−1, and the general family is r = 2qᵉ + 1 (so 163 = 2·3⁴+1 and 251 = 2·5³+1 qualify without being either). **The Fermat case is the one that defeats the ℓ = 2 obstruction** of `arithmetic-of-density.md` §3.3: it has r − 1 a pure 2-power, so 4 | r − 1 yet eff = 1, and it is why the 16 winners of shape `2×c + 257*` (§3.3.2 of `arithmetic-of-density.md` lists them) sit in classes 3 and 7 mod 12 at densities up to 0.16138 against those classes' generic cap of 0.08579. Across winning configurations about 77% of the foreign blocks used have efficiency 1, the commonest being 487, 257, 347 and 383. (The notes' glossary quotes 74.8% with a different top-five; that figure is over n < 685 and this one over the full range — same measurement, different windows.) The Fermat prime 257 is frequently the *binding* orbital, its full C₂₅₆ twist giving 257·128 = 32,896.
...
**J0a. The stabiliser question — closed for μ(n) = B(n), live only in the theorem-side clauses.** The framework's *constructions* take a matching block's twist inside the multiplicative group of the field, whereas the stabiliser of a primitive affine group of degree p^a may be any irreducible subgroup of GL(a, p) — possibly larger, and of an order dividing neither c − 1 nor anything the field supplies. Part B's own 3^{1+2} ≤ GL(3, 7) is such a group, beating orb(c, d) by a factor 9.

> **It cannot affect either direction of μ(n) = B(n), and the reason is the flat cap.**
>
> - *Upper bound.* B_safe credits a matching class F·C(c,2) and a pair of orbits sᵢsⱼ. **Both bound any point stabiliser and any pair of orbits whatever**, semilinear or not — they are counting bounds on the pair set, not statements about the group. So μ(n) ≤ B_safe(n) holds for every Oliver group regardless of J0a, and no stabiliser however exotic can push a value above the table.
> - *Lower bound.* μ(n) ≥ B(n) is Part E's explicit construction, which uses field twists. A richer stabiliser would only add groups to the maximum, never remove the one exhibited.
>
> **So every computed value is J0a-free**, and so is the collapse over the certified range: with the flat-cap conditions and `--no-theorems` the candidate list is empty, so neither Lemma C, nor Corollary C′, nor J0a enters the per-n proof at all.

**What remains of J0a is where `orb` is used as an upper bound on a *real* group**, which is a different use from scoring and the only one the flat cap does not cover. Two sub-cases:

> **(a) A non-field stabiliser of the same order cannot beat the field one.** Any orbit of H ≤ GL(a, p) on the c − 1 nonzero vectors has size at most |H|, by orbit–stabiliser; the multiplicative subgroup of the field of that order is **semiregular**, so its orbits all have size exactly |H| and it attains the bound everywhere at once. The same holds for the ± version governing pair orbitals. This is `pending-checks.md` T2, and it is a proof rather than a measurement.
>
> **(b) A non-field stabiliser of *larger* order is the genuine residue**, since (a) compares at fixed order and an H whose order divides no field subgroup's is outside its reach — 3^{1+2} ≤ GL(3, 7) again, |H| = 27 with 27 ∤ 342 = 7³ − 1. **This is the whole of what is still open, and it bears on exactly one clause in this document: E.3(i)'s "worth at most c to any actual group", which reads Lemma C on a block at a ≥ 2.** That clause dispatches the s = 2, a ≥ 2 branch by theorem; the certificates do not use it, so the exposure is to statements about *all* n and not to any computed value. The branch is also arithmetically tiny — base-3 repunit primes, three values of c below 4×10⁶.

**At k ≥ 3 the same residue is load-bearing rather than incidental**, because there is no flat cap to fall back on: C(c, k) sits far above the truth and the content of m\*_k = O(n² log n) is precisely that the block group is small. `general-k-note.md` §1 states the needed form — **(J0a′)**, that a non-semilinear Oliver-admissible block group never wins the minimum k-orbit — and reduces it to a component argument uniform in k. Proving that closes (b) at every arity at once, including k = 2's E.3(i).
...
2. **The collapse for odd n at low density.** **⟦LARGELY CLOSED — read the rest of this item for what survives.⟧** *Part E‴'s Theorem E.5 caps the SAFE score of every fallback configuration, the s = 2 leftover shapes included, at C(n,2)/25, and Corollary E.6 turns that into B_refined = μ = B_safe wherever δ > 1/25 — hence at every n ≤ 10⁶ via the ladder floor. The global promotion this item asked for is therefore **not owed**: the leftover case it was needed for cannot attain B above the line. What survives is the regime **δ ≤ 1/25**, which is item 2a's regime too, so the two are now one question. The analysis below is retained because it is what identified the shape that turned out to be sharp — the L = c reading of n = 5r + 2 at a Cunningham chain, which reaches 0.9955 of the 1/25 line and is what makes E.5's constant unimprovable.* **The global promotion, as originally posed:** Part E′ closes every branch but one *within the δ > 1/16 regime* — s = 3 outright (E.4), the s = 2 repunit family by absolute cap (E.3(iii)), s = 1 by Cap(a) (E.1), and everything at δ > 1/9 (E.1); below 1/25 the s = 4 branch opens (1/36 for s = 5) with no theorem either — the sharp thresholds, per item 2a below; between 1/16 and 1/25 the reduction stays complete at s ≤ 3 — and E″ certifies the collapse per n from lower bounds alone, far beyond the table (**⟦PENDING-CERT-RUN⟧** for the coverage count). The surviving branches are **s = 2 with c a safe prime** — within which E.3(ii) resolves the bare pair outright, leaving only the case **with a leftover**. Wherever the density floor keeps the largest permitted s at 3, the s = 4 and s = 5 branches — which have no theorem at all — are not reachable.

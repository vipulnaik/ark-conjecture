...
>
> If **no foreign part** is present, every part is a matching class of F_i blocks of size c_i = p^{a_i}, all sharing the chain prime p (Part B). The intra term is F_i·orb(c_i, d_i) ≤ F_i·c_i·d_i, and here **F_i c_i must be kept together**: it is bounded by n directly, so F_i c_i d_i ≥ δ₀n²/2 gives d_i ≥ δ₀n²/(2·F_i c_i) ≥ δ₀n/2, whence (c_i − 1)/d_i ≤ 2/δ₀ — the same bound as the foreign branch, with d_i a q-power dividing c_i − 1. *Splitting the product instead — bounding F_i ≤ 1/δ₀ and c_i ≤ n separately — spends δ₀ twice and yields only 2/δ₀².* This branch also constrains n itself: from F_i c_i² > δ₀n² with F_i c_i ≤ n we get c_i > δ₀n and F_i < 1/δ₀, so writing b = min a_i, n = Σ F_i p^{a_i} = p^b·(Σ F_i p^{a_i − b}) with p^b ≥ δ₀n, and the cofactor is at most 1/δ₀. ∎
>
> **The whole statement rests on Q being a prime power** — with Q merely a divisor of r − 1 the conclusion is vacuous, since Q = r − 1 always qualifies. That confinement is the top layer's, and it is a **layer-assignment claim**, which is the category with the worst record in this framework (`pending-checks.md` T8). One reading so far.
>

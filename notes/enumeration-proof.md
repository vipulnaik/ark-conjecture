...
| **S7** | **middle**-layer-fused matching + outside (**includes F = 2**, the odd-n fused rung B) | enumerated | **338** at F = 2 (15.5%); **50** at F = 4; 53/3/17/2 at F = 3/5/6/8 | 2.7 → 2.3 → **3.4%** | **F = 2: exists → 12/24** (all odd n), **wins → 8/24 ≈ 33.3%** — the odd residues 1, 3, 5, 9, 13, 17, 19, 21 mod 24, whose class ceiling it attains. At the other four odd residues the ceiling belongs to F = 4. **F ≥ 3: exists → 24/24, wins → 4/24 ≈ 16.7%**, all of it at F = 4. Split by the parity of F. **Odd F** needs F·c even, hence c = 2^a at odd n — O(log n) sizes per n, an escape, winning nowhere since S3's cap 1/4 beats F = 3's 0.13397 at even n. **Even F** has no such restriction: at odd n, F·c is automatically even and c ranges over all prime powers, so the supply is a full Hardy–Littlewood system, and **F = 4 attains the class ceiling at 7, 11, 15 and 23 mod 24** (`aod` §3.3.5) | cap_F(η) = η/(1 + √(Fη))². F = 2: **0.17157 / 0.12500 / 0.10102 / 0.06699** at η = 1, ½, ⅓, ⅙. F = 4: **0.11111 / 0.08579 / 0.07180 / 0.05051** at the same η. Decreasing in F, so a larger F pays only when it unlocks a better η | `aod` §3.2, §3.3, §4.3 |
...
> **s = 4 is not reachable in range, and would have no theorem if it were.** The corollary below bounds s by 1/√δ − 1, so a lower density admits a larger s. At the floor of **⟦EG-TENTATIVE⟧** tentatively 0.048039 at n = 2183 the bound is s ≤ 3.56 (v4's 0.045742 gave 3.68), so **the largest permitted s over the table is 3** either way — and that holds over the worklist rows above the contiguous frontier too, since they set no lower minimum. The branch reopens only below **δ = 1/25 = 0.04**, so the current margin is 0.0457 against 0.0400: narrow enough that one extension could close it, which is why what follows records why the branch would want a theorem if the floor ever fell. Unlike s = 3, the branch is **not** thin: c − 1 = 4r with c a prime power and r prime carries no parity or congruence forcing — 33 such pairs exist with c < 4000, beginning (9,2), (13,3), (29,7), (53,13), (125,31) — so no analogue of E.4 is available and an absolute cap would have to come from the foreign block's twist, as in E.1 and E.3(iii). The search still clears it (0 candidates at every computed n), so nothing is unproved; but the theorem-side coverage would have a hole that widens as the density floor falls. The branch first drew attention under the v2-era table, whose computed minimum fell to 0.037524 — below 1/25 and hence into s = 4 territory; under the corrected shape space that value has lifted and the branch is out of reach again.
...
> **Corollary (the fallback question below 1/9, reduced).** Since s ≤ 1/√δ − 1, **δ > 1/9 forces s ≤ 2**, **δ > 1/16 forces s ≤ 3**, **δ > 1/25 forces s ≤ 4**, and so on: **the thresholds are 1/(s+2)²**, so the branch at a given s first becomes reachable once δ ≤ 1/(s+2)². *(The sharp reading of the s = 4 line is "δ > 1/25 forces s ≤ 3", and it does follow — 1/√δ − 1 < 4 exactly when δ > 1/25. Quoting the s = 4 threshold as 1/16 is slack by one and understates how far the floor must fall before that branch opens. The applications below are computed from the inequality directly, so they do not depend on reading the ladder correctly.)* **This covers the whole table.** The floor is **⟦EG-TENTATIVE⟧** tentatively 0.048039 at n = 2183, so s ≤ 1/√δ − 1 = 3.56 (3.68 under v4's 0.045742) and the largest permitted s is **3**; **no computed value falls below 1/25** — and the correction only raises values, so this scope claim survives the rebuild a fortiori. Recount after every extension; it is what `check_doc_figures.py --pass scope` exists for. **The gotcha is that the recount is arithmetic on the floor and nothing else**, so a sentence naming an argmin that is no longer the argmin still reads as true — name the current one, or state the bound without naming one.
>
...
> **Result: the collapse B_refined(n) = B_safe(n) = B(n) is certified at every composite non-prime-power n ≤ 10,000 (8,719 of 8,719), and at 90,299 of the 90,299 such n ≤ 100,000 — from proven lower bounds alone.** That is fifty times the range of the computed table, for about three minutes of arithmetic; the lower-bound pass dominates the cost at roughly 150 s per 10⁵ and is cached, so the next decade is an afternoon rather than a research problem.
...
*The search is small in practice.* The floor over the computed table is **⟦EG-TENTATIVE⟧** tentatively 0.048039 at n = 2183, giving k ≤ 4 (as did v4's 0.045742); at the median density 0.1994 the bound is k ≤ 2. And since the ladder puts δ ≥ 0.04453 at every composite non-prime-power n ≤ 10⁶, **no admissible configuration below 10⁶ has more than four orbits**, and most have two. (An earlier form of this paragraph quoted 0.0147 at n = 4917 and concluded k ≤ 8. That figure is a family-menu *lower bound* on μ rather than a density — see the two-δ caution in Part H — and the ladder's floor supersedes it in both directions.)
...
>
> **F.1 is tight and cannot be improved to k ≤ 3 by counting.** Running the same argument through the *smallest* part reproduces k ≤ 1/√δ exactly, so the equal-parts case saturates it, and no tightening of constants does better: at the density floor (**⟦EG-TENTATIVE⟧** tentatively 0.048039 at n = 2183; v4's 0.045742 shown here) F.2 gives k ≤ 4.29 against F.1's k ≤ 4.68 — still 4, not 3, and a higher floor only lowers both. Since no winning configuration anywhere in the computed range uses more than three parts (Part I), the residual factor is arithmetic, not metric: any proof of k ≤ 3 must use the prime-power structure and the Diophantine constraint n = Σ F_i c_i. This is Part J item 1.
...
From `mu_table_safe_v4.csv`, unconditional mode, over the contiguous range n = 6 … 2600 — 2,186 rows.
...
2. **The collapse for odd n at low density: the global promotion.** Part E′ closes every branch but one *within the δ > 1/16 regime* — s = 3 outright (E.4), the s = 2 repunit family by absolute cap (E.3(iii)), s = 1 by Cap(a) (E.1), and everything at δ > 1/9 (E.1); below 1/16, s = 4 and s = 5 open up and have no theorem either (see the box after E.3, and item 2a below) — and E″ certifies the collapse at **every** composite non-prime-power n ≤ 100,000 from lower bounds alone, 90,299 of 90,299. The surviving branches are **s = 2 with c a safe prime** — within which E.3(ii) resolves the bare pair outright, leaving only the case **with a leftover**, **247 instances over v4's range**. Under the corrected shape space the largest permitted s is 3, so the s = 4 and s = 5 branches that the v2 floor admitted — and which have no theorem at all — are not reachable there.
   ...
   The alternative route — raise the odd-n guarantee above 1/9 and bring odd n under Theorem E.1 wholesale — is **closed**. It would need a constant above 1/9 bounding μ(n)/C(n,2) from below on odd n, and no such constant exists: **34.8% of the odd n in the computed table already fall below 1/9** — 312 of the 897 odd values in the contiguous range — with the share growing across it (23.6% below 800, 33.5% in [800, 1600), 43.3% above). `arithmetic-of-density.md` §7 carries the same figure. So the direct promotion is the only path.

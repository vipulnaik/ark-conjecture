...
>
> **F.1 is tight and cannot be improved to k ≤ 3 by counting.** Running the same argument through the *smallest* part reproduces k ≤ 1/√δ exactly, so the equal-parts case saturates it, and no tightening of constants does better: at the density floor (0.048039 at n = 2183 over [6, 2600]; v4's 0.045742 shown here) F.2 gives k ≤ 4.29 against F.1's k ≤ 4.68 — still 4, not 3, and a higher floor only lowers both. Since no winning configuration anywhere in the computed range uses more than three parts (Part I), the residual factor is arithmetic, not metric: any proof of k ≤ 3 must use the prime-power structure and the Diophantine constraint n = Σ F_i c_i. This is Part J item 1.
...
**What replaces the census's three-part row.** S3 (unfused matching plus foreign) takes 853 winners and S2 (fused single class) 743; the fused-with-foreign rungs take S7f2 = 395, S7f3 = 102, S7f4 = 58, S7f6 = 14, with S5 at 21. **S1, S4 and S6 have no winners anywhere on the table** — S4 for the reason just given, S6 (two foreign parts) having been a single row at n = 1175 that is now the fused `1x619* + 4x139`.
...
Fermat primes achieve this at q = 2, safe primes with t = q odd and 2t = r−1, and the general family is r = 2qᵉ + 1 (so 163 = 2·3⁴+1 and 251 = 2·5³+1 qualify without being either). **The Fermat case is the one that defeats the ℓ = 2 obstruction** of `arithmetic-of-density.md` §3.3: it has r − 1 a pure 2-power, so 4 | r − 1 yet eff = 1, and it is why the 18 winners of shape `2×c + 257*` (§3.3.2 of `arithmetic-of-density.md` lists them) sit in classes 3 and 7 mod 12 at densities up to 0.16138 against those classes' generic cap of 0.08579. Across winning configurations about 77% of the foreign blocks used have efficiency 1, the commonest being 487, 257, 347 and 383. (The notes' glossary quotes 74.8% with a different top-five; that figure is over n < 685 and this one over the full range — same measurement, different windows.) The Fermat prime 257 is frequently the *binding* orbital, its full C₂₅₆ twist giving 257·128 = 32,896.

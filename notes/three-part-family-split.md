...
>
> | residues (mod 24) | η | fused cap_B | unfused cap_C | winner |
> |---|---|---|---|---|
> | 1, 9, 13, 21 | 1 | 0.171573 | 0.111111 | fused |
> | 3, 7, 15, 19 | 1/2 | 0.125000 | 0.085786 | fused |
> | 5, 17 | 1/3 | 0.101021 | 0.071797 | fused |
> | 11, 23 | 1/6 | 0.066987 | 0.050510 | fused |
>
> **The within-family split is 100 / 0 / 0 at all twelve odd residues.** S4's share is zero, the tie's share is zero, and the 1 : 1 : 2 supply computation of §1 predicts the outcome of a contest that is now settled by inequality. The two special cases the document is built around are both void: the coincidence cap_B(1/4) = cap_C(1/2) at residues 7 and 15 arose only because the cut forced the fused rung down to η = 1/4, and at 23 the fused rung's supposed inability to reach the ceiling (η = 1/12, cap 0.042020) was the same artifact. See `aod` §3.2.3 and §3.3.4.
>
> **Three things in here do survive, and are the reason to keep the file:**
>
> 1. **The exact agreement of the two singular series** (§1.1, and the third box in §1.2). 𝔖_D = 𝔖_{2D} identically, for the structural reason that both collision conditions — h ≡ 0 and h ≡ −1/2 (mod ℓ) — are conditions on h alone and mention no D. That is a fact about the systems, not about the competition, and it is unaffected.
> 2. **§1.2's account of Bateman–Horn's limits** — that the neglected secondary term is of relative order 1/log n, the same order as the effect being predicted, so a heuristic read at leading order cannot be separated from a nearby limit by any reachable computation. That is a caution about method and stands on its own; it is item T5a in `pending-checks.md`.
> 3. **The Friedlander–Granville discussion** in §2.4, for the same reason.
>
> **Read everything else as a record of how the framework looked under the pre-correction twist law.** The arithmetic is internally correct given that law; it is the law that was wrong. In particular, do not quote §1.3's or §1.4's share tables, §2's measured splits, or the "S7 at F = 4 takes 7, 11, 15, 23" line — F = 4 now takes only 11 and 23 (`aod` §3.3.5).

---

*Extracted from `arithmetic-of-density.md`. It was kept as a separate note because its conclusions are about **runners-up** rather than about which shape wins at n, so nothing in the main line depended on it — while the arithmetic it contains was worth preserving. It is now deprecated for the reason above; the framing below is preserved as written.*
...
**⟦DEPRECATED-EG⟧** **The absolute shares differ, and S4's is zero.** *(The conclusion — S4 wins nowhere — happens to remain true, but by a different and simpler route: the fused rung now beats it at every c, not only at the residues where F = 4 intervenes.)* At 7, 11, 15 and 23 mod 24 the class ceiling belongs to the two-part F = 4 shape (§3.3.5), which beats every reading of n = 2c + r; at the other eight odd residues the fused rung attains it, taking 100% of the family there by congruence. S4's entire within-family share was earned at 7, 15 and 23 — exactly the residues F = 4 takes — so it survives nowhere. Combining:

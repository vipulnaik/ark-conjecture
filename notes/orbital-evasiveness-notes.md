...
**One caution about the hand template**, since §2.4's implementation note refers to it. `TemplateGroup` places the block rotation in the **cyclic middle layer**, requiring the twist, the foreign primes and the rotation order to be pairwise coprime, and demands a prime block count. Theorem 2.4 places the rotation in the top q-group instead, whence any d | c−1 is admissible and the block count need only be a prime power. Consequently the template misses μ(10) = 20 (k = 2, d = 4) and μ(12) = 18 (k = 4), and anything selected through `candidate_groups` is drawn from a pool that excludes both optima. The GAP path has no such restriction and supersedes that enumerator.

...
> **2026-08 correction, applying to §§2, 3 and 5.** Theorem 2.3's block-count step (G.2 of `enumeration-proof.md`) is **false**: the group permuting the blocks of an orbit may sit in the *cyclic layer* of the Oliver chain rather than the top q-group, so the block count need not be a q-power. It need only be coprime to everything else the cyclic layer carries. The smallest witness is n = 308, where Γ₀(53, Z₃) × Γ(149, 37) — BBKN's own construction — achieves 4134 against B(308) = 3775.
>
> **Consequences.** B(n) is no longer an upper bound on μ(n). It is not simply a lower bound either: `B_safe` deliberately over-counts per configuration (scoring a p-characteristic part at F·C(c,2) even where Lemma C reduces the twist), so B_safe and μ are **incomparable** in general. What holds is `B_refined ≤ μ`, and `B_safe = B_refined` wherever the collapse certificate applies — the whole computed table, and all but two composite non-prime-power n ≤ 10⁵ — so on that range the tabulated value is ≤ μ. The table is not exact; "attained" claims stand but "the maximum" claims do not. §5's global floor **survives as an inequality** (every step used the table from below) but is no longer tight, and its argmin has moved: at n = 3239 the corrected shape space gives 0.043570 rather than 0.026117. The class caps of §5.3 are ceilings for the unfused shapes only.
>
> **What is unaffected.** B₀ and Theorem 2.3's *inequality* μ(n) ≤ B₀(n), whose proof never used the block-count classification; every construction and every lower bound; the whole of §4; and the number-theoretic analysis of §§5–6, which is about families rather than about completeness. `enumeration-proof.md` Part 0 has the corrected shape space, the repair's lemma inventory, and a census of all ten configuration types.
...
> **Lemma B (foreign orbits).** Let O have full capacity — so Γ|_O is 2-homogeneous, hence primitive affine of degree s = p₀^a — and suppose p₀ ≠ p. Then
>
> **an orbit of foreign characteristic attains full capacity only if s is prime with s − 1 ∈ {qᵉ, 2qᵉ}.**
>
> The mechanism, in one line: the image of the bottom p-group is a normal p-subgroup of a primitive group of degree coprime to p, hence trivial, which forces the whole twist up into the top q-group and the degree down to a = 1. Proof in Part B of the companion.

> **Lemma C (the cyclic layer).** For a p-characteristic part with twist order d_i and a foreign prime part of size r_j,
>
> **gcd(d_i, r_j) = 1** — in particular gcd(m−1, r) = 1 between a p-block of size m and a foreign block of size r.
>
> What forces it is the **conjugation action of the top q-group** on the cyclic layer: that action induces the twist on the foreign part and the identity on the p-characteristic one, which is impossible if a common prime divides both images. Twists acting *diagonally* on several p-blocks are not independent and carry no coprimality condition — this is what Theorem 2.1's construction exploits. Proof in Part D of the companion, together with the pitfall that the shorter "disjoint supports generate a direct product, which must be cyclic" argument does **not** establish it and was carried here for several revisions.

These are exactly the conditions §5 imposes on its ladder, now derived rather than assumed. Lemma B is the origin of the safe-prime hypothesis: the top rung wants a foreign block of prime size r with a twist of order ∼r/2, and Lemma B says the twist must be a q-power, so r − 1 ∈ {qᵉ, 2qᵉ} — with e = 1 in the 2qᵉ case being precisely "r is a safe prime." Both lemmas are validated in Part I of the companion, against our own construction tables and — the non-circular test — against the exhaustively enumerated GAP battery at n = 10, where every full-capacity orbit is of prime-power size and every prime-sized one satisfies Lemma B's condition, with no exceptions in either case.

**2.5 Lemma B′, and what carrying the bottom prime buys.**
...
*Effect on the bound.* Carrying the bottom prime through the capacity recursion — so that only p-power orbits receive C(s,2), foreign primitive orbits receive Lemma B′'s q-power-twist value, and everything else recurses over q-power block counts — is exactly what turns B₀ into a bound the constructions meet. That is the step from B₀ to B, and it is why the gap between them is structural rather than numerical.

**Consequence for the extremality claim.** With Lemmas A–C in hand, what §3 states as Theorem 3.1 is no longer a classification problem about solvable permutation groups. The configuration space they permit is finite and explicitly bounded, every member of it is realised by an explicit group, and the resulting bound is attained. The structural gap once inherited from the Singer step (§3) is closed: it bore only on configurations that lose anyway, and the companion's Part E′ now proves that. What is left is whether the enumeration can be pruned to a shortest sufficient list — a question of cost, free above density 1/16 and open only in a low-density tail (Open Problem 8).

**What §2 has and has not settled.** The recursive *shape* once merely asserted is proved: non-prime-power degree forces a block system, the block system forces the ceiling n(c−1)/2, and that ceiling is keyed to the divisor lattice of n. §2.4 adds the coherence conditions, and §3 assembles both into Theorem 3.1. So: **block recursion, theorem; coherence conditions, theorem; extension to nested configurations, theorem (tower depth absorbs into F — Part G.2); extension to partial capacity, theorem in the only form the bound needs (Part E′).** What the twist-coherence half still carries is not a structural question but an arithmetic one, and it is the same low-density arithmetic that limits everything else in Part II.
...
**2.6 The configuration enumeration.** Lemmas A–C cut the space of admissible group shapes down to something finite and explicitly listable. An Oliver group on n points is described by its chain primes (p, q) together with orbits

> **n = Σᵢ Fᵢ·cᵢ**, each Fᵢ a power of q, each cᵢ a prime power, and each orbit either *p-characteristic* (cᵢ a power of p, twist any divisor of cᵢ−1) or *foreign* (cᵢ prime, twist a power of q by Lemma B′), subject to Lemma C.

Its orbitals are: **Fᵢ·orb(cᵢ, dᵢ)** within an orbit; **(Fᵢ or Fᵢ/2)·cᵢ²** between the fused blocks of one orbit when Fᵢ > 1; and at most **sᵢ·sⱼ** between distinct orbits. Two admissibility constraints are easy to overlook and both inflate the bound if omitted: foreign primes must be **pairwise distinct**, since two orbits of the same prime r would place C_r × C_r in the cyclic layer; and foreign orbits are **never fused**, because diagonal translations — the only fusion the cyclic layer permits — preserve differences, so the pairs with equal coordinates form a small cross class that the twist cannot merge and that then binds.

Every construction used in these notes is a special case. In particular:

> **Theorem 2.4 (fused blocks).** For n = k·m with **k and m both prime powers**, Γ = 𝔽_m^k ⋊ (C_{m−1} diagonal × T), with T a transitive q-group on the k blocks, is an Oliver group with minimum orbital **k·C(m,2) = n(m−1)/2**. Verified by orbit computation at n = 12, 20, 28, 44, 45, 56, 117. Theorem 2.1 is the case k = 2. Note that k need only be a prime *power*: a transitive q-group has q-power degree, so k = 4, 8, 9 and so on are available.
>
> **Theorem 2.5 (one p-part with foreign primes).** For n = m + r + s with m a prime power and r ≠ s primes, q a prime dividing both r−1 and s−1, and d the largest divisor of m−1 coprime to rs, there is an Oliver group with chain 𝔽_m ◁ 𝔽_m ⋊ (C_d × 𝔽_r × 𝔽_s) ◁ Γ and minimum orbital min(orb(m,d), orb(r, q-part of r−1), orb(s, q-part of s−1), mr, ms, rs). The two foreign twists need only be powers of a **common** q; no relation between r and s beyond that is required.
...
**Notation.** Write **B(n)** for the maximum, over all configurations admissible in the above sense, of the configuration's score — the minimum over its intra-orbital, fused-cross and between-orbit terms. This is the quantity the enumeration computes and the `mu_bound` column of the table holds, and it is an upper bound on μ(n) because every Oliver group on n points realises some admissible configuration. It is the sharpening of Theorem 2.3's B₀(n) promised there: μ(n) ≤ B(n) ≤ B₀(n), with the second inequality typically strict for the reason set out after Theorem 2.3. Two scorings are in play, differing only on a p-characteristic part whose twist Lemma C strictly reduces: **B_safe** values such a part by its unconditional capacity F·C(c,2), while **B_refined** values it by F·orb(c, dᵢ), which is what the explicit construction reaches. B(n) means B_safe unless stated otherwise; the two coincide wherever the companion's certificate applies, and there B(n) = μ(n).

The enumeration is implemented in `mu_enumerate.py` and analysed in the companion document `enumeration-proof.md`, which classifies the admissible structures and bounds the resulting search: at most 1/√δ orbits, blocks of size at least δn, fusion counts at most 1/δ, and termination certified at the first K for which no larger configuration can beat B_K.

> **Reading the output.** Its value is an **unconditional lower** bound on μ(n) — it was believed to be an upper bound until the 2026-08 correction — computed for n ≤ 2376 plus two larger values (2,008 rows) at about n^2.9 per value, and **attained**: every admitted configuration is realised by an explicit group whose orbital sizes are forced rather than chosen, so μ(n) = B(n). The one configuration shape at which the scoring exceeds what the construction reaches — a p-characteristic part whose twist Lemma C strictly reduces — is excluded from the optimum by Part E′ of the companion: by theorem wherever the density exceeds 1/9, and elsewhere by a per-n certificate cheap enough to run to **n = 100,000** without computing B(n) at all (where it succeeds at all but two values, n = 50,817 and n = 89,697). Two cautions about the table are set out in Part I of the companion and easy to get wrong: the `certified_K` column is a stopping level, not an orbit count (no winner anywhere in range uses more than three parts), and the bounds that are genuinely tight are the per-axis ones of G.4 rather than the part count. The classification behind all of this has had no independent scrutiny, and one step originally asserted as a sketch turned out to be false, so the numerical agreement should be read partly as evidence that the reasoning is sound rather than only as a check on the code.
...
What the framework once had to assume about extremal groups is, in its structural content, now a theorem. This section states it, keeps the two clauses that a careless derivation gets wrong, records the one step of the original argument that is false, and says what is left.
...
>
> **n = Σᵢ Fᵢcᵢ**, each Fᵢ a power of q and each cᵢ a prime power,
>
> in which each orbit is either **p-characteristic** — cᵢ a power of p, twist any divisor of cᵢ−1 — or **foreign** — cᵢ prime, twist a power of q — subject to **gcd(dᵢ, rⱼ) = 1** between every p-characteristic twist order dᵢ and every foreign prime rⱼ. Imprimitive tower depth contributes nothing beyond Fᵢ. The orbital sizes are then *forced*, not chosen: **Fᵢ·orb(cᵢ, dᵢ)** within a fused class, **(Fᵢ or Fᵢ/2)·cᵢ²** between the blocks of one class, and **sᵢsⱼ** between distinct orbits.
>
> Conversely, **every configuration meeting these conditions is realised by an explicit group** with exactly that orbital data.
>
> *Sources.* Solvability and the primitive-affine reduction give the decomposition; Lemma A gives inheritance of (p, q); Lemma B′ types the foreign orbits; Lemma C gives the coprimality; Part G.2 of the companion absorbs tower depth into Fᵢ; Part E constructs the converse. None of it is conditional on number theory.
...
*The mechanisms below are what the constructions do; which n they are available at, and why the ceilings take the values they do, is the subject of `arithmetic-of-density.md`. In its terms this section is the additive engine, whose caps are 1/k² at k parts, and §5.5 records what the multiplicative engine adds on the thin set where it applies.*
...
> **δ₀^{even} = 1/4** (density relative to C(n,2); equivalently μ ≥ n²/8) **for n ≢ 2 (mod 3)**; the ℓ = 3 obstruction applies to even n as well as odd, giving 1/(1+√3)² ≈ 0.13397 on n ≡ 2 (mod 3). Full table by residue class in §5.3.
...
**Odd n: two blocks are parity-blocked, and the repair uses a repeated block.** Two odd primes sum to an even number, so the even-n template does not transfer. The tempting fix — an auxiliary bounded block — fails instructively: a bounded block B has cross-orbitals of size O(|B|·n), collapsing m\* from density Θ(1) to O(1/n), the same mechanism that makes a fixed point fatal (Theorem 3.1's block floor). The correct repair keeps every block large and uses **two equal p-characteristic blocks together with one foreign prime**:

> **n = 2c + r**, c an odd prime power, r an odd prime; Γ has bottom 𝔽_c acting diagonally on both c-blocks, the foreign translations 𝔽_r in the cyclic layer, and the r-twist in the top q-group. The two c-blocks share a *diagonal* twist, so Lemma C imposes no coprimality between them — only between each and r.

Writing x = c/n and η = orb(r,t)/C(r,2) for the foreign block's efficiency, the orbital sizes are C(c,2) ≈ x²n²/2 for each intra term, c² for the cross between the two c-blocks, cr for the cross to the foreign block, and e·C(r,2) for the foreign intra term. Hence

> **Proposition 5.3 (the odd-n ceiling, closed form).** δ(x) = min( x², 2x(1−2x), η(1−2x)² ), maximised at the balance point. At **η = 1** this gives **1/9** at x = 1/3; at **η = 1/2**, **1/(2+√2)² ≈ 0.0858** at x = 1/(2+√2); at **η = 1/3**, **≈ 0.0718**.

Which efficiency is available is a local question, and it splits n by residue class rather than by parity alone. Full efficiency needs the foreign twist to have order (r−1)/2, and Lemma B′ forces that to be a power of q, so **(r−1)/2 must be a prime power**; with r prime and c = (n−r)/2 a prime power this is a Bateman–Horn system in the single variable r, of the same three-condition shape as the even case. Its local densities are computed by `local_solubility.py`:
...
- At **ℓ = 2**, full efficiency wants (r−1)/2 odd, i.e. r ≡ 3 (mod 4); then c = (n−r)/2 is odd only if **n ≡ 1 (mod 4)**. For n ≡ 3 (mod 4) the alternative r ≡ 1 (mod 4) forces 4 | r−1, capping the efficiency at 1/2 — *unless r is a Fermat prime*, where r − 1 is a pure 2-power and the efficiency is 1 after all. That escape is O(1)-sparse but occurs in range.

> **Conjecture (global density floor).** For every composite non-prime-power n, **μ(n) ≥ C(n,2)/50**, i.e. δ(n) ≥ 0.02 — and asymptotically **δ(n) ≥ (5 − 2√6)/2 − o(1) = 0.050510…**, the class-11 cap. The extremal class is n ≡ 11 (mod 12), the one carrying both local obstructions; and below 10⁶ the finite half is no longer conjectural: an exhaustive branch-and-bound over the 48,729 candidates gives **min μ(n)/C(n,2) = 0.026117, attained at n = 3239** (witness `1x1511* + 1x907* + 1x821`). Every value that set the running floor along the way — 575, 2183, 2291, 3059, 3239 — was n ≡ 11 (mod 12), the doubly-obstructed class. Derivation and the falsification test in `arithmetic-of-density.md` §5.

> **The balanced additive family, by residue class mod 12.** These are the δ₀ of Prop. 5.2′ — the most the balanced two-class (even n) or three-class (odd n) shape extracts, hence a guaranteed lower bound on μ(n) whenever n admits that representation. "Observed" is the largest density reached by a winner of this family at its class's generic efficiency.
>
> **They are not upper bounds on δ(n).** A single fused class reaches 1/F and exceeds every row; at odd n the shape 2^a + r\* avoids the three-class balance altogether (n = 1015 = 512 + 503\* gives 0.24534 against 0.08579 for class 7). In the computed table 91 values of class 11 exceed 0.05051, the largest 0.20168.
>
...
>
> The efficiency then follows from r − 1 = 2^a·u (u odd, L the largest prime power dividing u): η = max(1/u, L/(2^{a−1}u)), so **η = 1 exactly when either u = 1 or a = 1 with u a prime power** — equivalently r − 1 ∈ {qᵉ, 2qᵉ}, i.e. r Fermat or safe-prime-like. ℓ = 2 forces a ≥ 2, hence η ≤ 1/2 **provided u > 1**; ℓ = 3 forces 3 | u, hence η ≤ 1/3 unless u is a power of 3; both give η ≤ 1/6 subject to the same provisos. The u = 1 branch is not vacuous: a Fermat foreign block escapes the ℓ = 2 cap entirely, which in range produces 20 winners of shape 2×c + 257\* in classes 3 and 7 at densities up to 0.16138. Being tied to the five known Fermat primes it is O(1)-sparse and leaves the asymptotic constants untouched; see the boxed note in `arithmetic-of-density.md` §3.3.
>
> **Fusion raises these caps, and the table does not model it.** The odd-n rows are derived for the *unfused* shape 2c + r\*. When q = 2 the two equal c-blocks can be fused, and reduction (R1) of the companion says they should be. For a fused class of F blocks of size c plus one foreign prime the balance gives **cap_F(η) = Fη/(√F + F√η)²** at x\* = √η/(√F + F√η) — reducing to the k = 1 row at F = 1, and giving **0.17157 at (F, η) = (2, 1)** and **0.10102 at (2, 1/3)** against the tabulated 1/9 and 0.0718. Fusion at F = 2 forces q = 2, so η = 1 requires the foreign block's twist to be the full 2-part of r − 1, i.e. r a Fermat prime — which is why all 20 winners of this shape use r = 257 and sit in classes 3 and 7. Against the data, 0 of 58 fused-class winners exceed cap_F(η), the maximum being 0.16138 at n = 639. Derivation in `arithmetic-of-density.md` §3.3.
>
> Consequently **194 of the 1,108 two- and three-class one-foreign winners exceed their own class's δ₀** — 57 via 2^a + r\*, 43 via the fused route, 94 via an η above the class generic — and in class 11 the median winner sits at 1.24× the tabulated value. Read the rows as guarantees for one specific unfused shape rather than as generic behaviour; what does bound every row without exception is cap_F(η) at the configuration's own F and η.
>
> Validated two ways, since a class maximum alone would only show the cap is attainable. **Upper:** every one of the 1,167 two- and three-part winners satisfies δ ≤ cap(its own efficiency), with zero exceptions (1,108 of them re-checked automatically, covering every unfused one-foreign row). **Lower:** normalising each class by its cap, all twelve reach 0.98–1.00 with medians 0.84–0.94 and minima 0.45–0.81 — indistinguishable between obstructed and unobstructed classes, so no residual class-dependent effect remains unexplained.

Two remarks. The ℓ = 3 obstruction has a **sparse escape**: if (r−1)/2 or c is itself a power of 3, full efficiency returns. In range this lifts n ≡ 5 (mod 12) to a maximum of 0.10975, but the escape is thin — 22 of the 35 such rows use the *same* foreign prime r = 487, where (r−1)/2 = 243 = 3⁵ — so it should be read as a feature of the computed range rather than a mechanism, and the generic ceiling 0.0718 is the one to quote asymptotically. Second, the ceilings above are those of *this* family; the ratios in the table show the constructions meet them to within 2%, so as with the even case the wall is real rather than a sampling shortfall. Improving on them requires a structurally different family (Open Problem 1).
...
**5.5 The ledger against μ.** An earlier version of this section measured the *family menu* of `mu_fast.py` to n = 10⁴ against a 1/12 diagnostic threshold; its figures overstated the odd shortfall about threefold, and the measurements now live in Part I of `enumeration-proof.md`. That version also explained the parity gap by the scarcity of splits n = 2^a + r, which is **correct but incompletely scoped**: it is the binding constraint for odd n with ω(n) ≥ 3 that use two parts, and those are indeed the weak tail, but most *strong* odd n have ω(n) = 2 and are served by a single fused class, where no additive representation is involved at all.

The full account is the subject of **`arithmetic-of-density.md`**, and its thesis reframes this whole section: the framework has **two arithmetic engines**, a multiplicative one (a single fused class, n = F·c with both factors prime powers, density exactly **1/F**) and an additive one (k balanced parts, density **1/k²**), and they serve complementary sets of n. Three things follow that are worth carrying here. Density above 1/4 is purely multiplicative and needs ω(n) ≤ 2. The thresholds 1/4, 1/9 and 1/16 recurring through these documents are one sequence — the additive caps at k = 2, 3, 4 — rather than independently chosen constants. And the multiplicative engine covers a **density-zero** set, thinning like log log n / log n, so it props up more than half the present table but vanishes asymptotically, which is why the δ₀ constants below rather than the fused family's 1/2 are the right asymptotic quantities.

What follows is the measurement against μ itself.
...
*The ladder's odd ceilings are met to within 2%.* Prop. 5.3's values are attained at ratios 0.98–0.998 in every residue class, so they are ceilings of the mechanism rather than artefacts of search. They are not floors for μ — 27 of 679 odd values fall below 0.0718 and the global minimum is 0.0418 at n = 575 — because the family needs a representation and not every n has one. That is the conditional half of the ladder, not a defect in the constants.

*The parity signal is threshold-dependent, which is why 1/12 was the diagnostic.* Measured instead against δ₀^even = 1/4 — attained only when a balanced safe-prime split exists — **81.3% of even and 81.7% of odd** values fall short, and the parity signal vanishes entirely. So statements about "how many n are weak" say as much about the threshold as about n; the useful thresholds are the ones with mechanisms attached, which is what the next paragraph uses.

*Three thresholds line up by parity, and this is why the odd case is hard everywhere at once.* Below 1/9 — the threshold above which the companion's Theorem E.1 settles the collapse outright — sit 4.8% of even values against **54.3% of odd**. That last figure closes a route rather than merely describing one: since these are exact values of μ, **no constant above 1/9 can bound μ(n)/C(n,2) from below on odd n**, so Theorem E.1 can never be made to cover odd n wholesale by improving a ladder constant. The share also grows with n — 37.0% below 800, 59.9% in [800, 1600), 64.8% in [1600, 2299) — so this is permanent, and Open Problem 8(b) must be settled by promoting E.3(ii) directly. Below 1/16, the threshold above which Corollary F.3 gives k ≤ 3 for free, sit 0.1% of even against 5.6% of odd. So both residues of Open Problem 8 are essentially even-free and concentrate in exactly the odd, low-density regime that Open Problem 1 is about; they are one obstruction viewed three ways, and δ₀^odd > 1/9 would clear all three.
...
> **Proposition 5.2′ (top rung, both parities).** Unconditionally: μ(n) ≥ δ₀(n)·C(n,2) for every n admitting the relevant representation — two blocks for even n, n = 2c + r for odd n — with δ₀(n) the constant for n's residue class mod 12 tabulated in §5.3, ranging from 1/4 down to 0.05051. What each n actually achieves is **computable directly** in O(n/log n), against n^2.9 for μ(n) itself, and `ladder_verify.py` has done so for every composite non-prime-power n ≤ 10⁶: the minimum is 0.02504 at n = 3239, and nothing falls below 0.02.
...
8. **(Two arithmetic residues in the enumeration — both confined to low density.)** The configuration enumeration of §2.6 is complete, finite and realised, and the bound it computes is **attained**: μ(n) = B(n), proved at every computed value and, via the lower-bound form of the certificate, at every composite non-prime-power n ≤ 10⁵. The apparatus is Parts E′–E″ and F of `enumeration-proof.md`. Two questions remain, and both sit in the same low-density regime.
   ...
   **(b) The fallback exclusion as a theorem, not a certificate.** Every branch is closed by theorem except one: above density 1/9 the question dies (Theorem E.1), the s = 3 branch is a single dead pair (E.4), the s = 2 repunit branch is absolutely capped (E.3(iii)), and the s = 2 safe-prime branch is resolved for the bare pair (E.3(ii)). What is open is that last branch **with a leftover**, sharply obstructed at leftover = c where the re-reading would need two equal foreign parts. The route through the odd-n ceiling is **closed** — see §5.5 — so the direct promotion of E.3(ii) is the only path.
   ...
   For orientation on how much of the range is in play: the global minimum of μ(n)/C(n,2) over all composite non-prime-power n ≤ 10⁶ is **0.026117 at n = 3239**, established by exhaustive branch-and-bound (`arithmetic-of-density.md` §5), and every value that set the running floor was n ≡ 11 (mod 12).

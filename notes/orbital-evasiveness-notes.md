...
|---|---|---|---|---|---|---|
| **Oliver** | **k = 2** | **binds.** A block's twist must be a q-power, so a prime r contributes η = 2/D of r − 1; this is the whole source of the eight mod-24 ceilings | Parts B–D: matching blocks p-power, foreign blocks prime, F = F_mid·F_top, coprimality budget in the cyclic layer | available but **rung-dependent**: reachable at nine odd residues, obstructed at 7, 15, 23 | cap_F(η) = η/(1+√(Fη))², balance points per residue | every prime power (AGL(1,c), 2-homogeneous) — infinite |

...
|---|---|---|---|---|---|---|
| **Oliver** | **k = 2** | **binds.** A block's twist must be a q-power, so a prime r contributes η = 2/D of r − 1; this is the whole source of the six mod-12 ceilings | Parts B–D: matching blocks p-power, foreign blocks prime, F = F_mid·F_top, coprimality budget in the cyclic layer | available at every odd residue with full twist (a cyclic-layer fusion costs nothing on the matching side — `arithmetic-of-density.md` §3.2.3) | cap_F(η) = η/(1+√(Fη))², balance points per residue | every prime power (AGL(1,c), 2-homogeneous) — infinite |
...
> **A third axis the table does not vary: the invariance group.** Both rows above hold the property S_n-invariant. Weakening that to **A_n** gives the *chiral* graph properties of `chiral-graph-properties.md` — properties that may separate isomorphic labelled graphs when every isomorphism between them is odd. Nothing collapses: the sign map has index ≤ 2, so μ_chi(n) ≥ μ(n)/2 and Θ(n²) survives. What is qualified is the **prime-power theorem**: AGL(1,c) is not inside A_c for odd c, its full-order twist being odd, so δ_chi = 1 at c = 2^a and at **c ≡ 3 (mod 4)** but only 1/2 at **c ≡ 1 (mod 4)**. Away from the prime powers the factor is not paid: a **fused** matching class is realised by one entangled generator, which is even at full twist for every even F and every odd c, so ε = 1 at every ceiling-setting odd row and the mod-12 ceilings carry over unscaled. Dropping graph structure altogether — arbitrary monotone functions under a transitive group — is `monotone-transitive-note.md`, and there the apparatus contributes *nothing*, because the group already acts on the coordinates and the argument closes at one orbital. `johnson-presentations.md` explains why those two are the same axis seen twice.
...
>
> **Measured over the table (⟦PENDING-REBUILD⟧ — the orbital count t is not a column of the CSV, so this distribution has not been regenerated against the completed range and is the one figure here still carrying pre-rebuild values):** t = 2 (20.4%), 3 (46.1%), 4 (18.0%), 5 (10.1%), 6 (4.0%), with a tail to 10; mean 3.36, and **t ≤ 4 covers 84.5%**, t ≤ 5 covers 94.6%.
>
...
- **Bipartiteness** evasive at n = 3m (m ≥ 3); **planarity** at n = 2m and 3m for m ≥ 5; **acyclicity** at n = km for m ≥ 3, k ∈ {2,3}; **C₄-freeness** at both, for **m ≥ 4** — the block size must carry a C₄ inside a single K_m, and 2K₃ and 3K₃ are C₄-free; **K₃,₃-freeness** at n = 18 and 21.

...
**The same implication has a second, sharper reading, at the level of arithmetic rather than of systems.** Because a configuration is scored by the *minimum* over its orbitals, every part must clear δ₀·C(n,2) on its own, and a part can do that only by being large and carrying a large twist. Chasing that through gives, for almost all n, a prime r linear in n whose r − 1 carries a **prime-power divisor of bounded cofactor** — the bound depending on δ₀ alone. This is worth stating because it is the same quantity §5's ladder measures: the ladder's rungs supply a prime factor of r − 1 of size r^θ for θ < 1, whereas a *positive density floor demands one of linear size*. **So the shifted-prime condition is not merely a convenient sufficient input to this framework; at the θ = 1 end it is necessary for one**, which is the strongest available reason to expect that no substantially weaker arithmetic will give a density floor. `enumeration-proof.md` Proposition F.4 and `arithmetic-of-density.md` §6.7 carry the statement with its constants and its exceptional set. *Recent, with one reading; the exact constants and the precise form of the divisor condition may still move, and the argument rests on a step about which layer of the chain supplies a foreign block's twist (`pending-checks.md` T8), which is this framework's least reliable kind of step. The direction of the implication is what to rely on here, not its numerals.*

...
> **The balanced additive family, by residue class mod 12.** *Superseded: the ceilings are mod 24, and nine of the twelve odd residues are higher than shown. See `arithmetic-of-density.md` §3.3 for the corrected table with closed forms and balance points. The rows below remain correct as the values of the **unfused** rung.* These are the δ₀ of Prop. 5.2′ — the most the balanced two-class (even n) or three-class (odd n) shape extracts, hence a guaranteed lower bound on μ(n) whenever n admits that representation. "Observed" is the largest density reached by a winner of this family at its class's generic efficiency.
>
...
2. **(Number theory — push the shifted-prime exponent.)** The route's exponent is governed by a single parameter. Writing θ for the size of prime factor one can guarantee in r − 1 — that is, P(r − 1) > r^θ — the family delivers roughly **n^{1+θ}**, since the foreign block contributes qr ≥ r^{1+θ}. The whole ladder is that identity:

   | input | θ | quantifier | exponent |
   |---|---|---|---|
   | Bombieri–Vinogradov | 1/4 | all large n | n^{5/4+o(1)} (Shparlinski, Thm 1) |
   | Chowla-type | 1/2 | all large n | n^{3/2−ε} (BBKN, conditional) |
   | Baker–Harman, positive density | 0.677 | almost all n | n^{1.677} (Shparlinski, Cor. 3) |
   | Elliott–Halberstam | → 1 | all large n | n^{2−o(1)} |

   Hypothesis (H) is the **θ = 1 endpoint**. So the concrete questions are: **raise θ unconditionally**; shrink the exceptional set at fixed θ; or reach the endpoint without passing through Elliott–Halberstam.

   **Is θ = 0.677 a natural ceiling, in the way Chowla's 1/2 is?** Not in the same way, and the difference matters. Chowla's 1/2 is a *conjecture's* value — it is what that conjecture buys, and beating it means assuming something else. Baker–Harman's exponent is the current output of a *method*, and the method's limit is a level-of-distribution barrier: results of this shape rest on Brun–Titchmarsh on average, i.e. on controlling primes in progressions to moduli beyond x^{1/2}, which is exactly where Bombieri–Vinogradov stops and Elliott–Halberstam begins. The exponent moves whenever that control improves, and it has: **Runbo Li (2025) raises it to 0.679** using Maynard's triple-convolution estimates, in the same lineage that took Bombieri–Friedlander–Iwaniec's x^{29/56} to Maynard's x^{11/21} and Lichtman's x^{17/32}.

   So the ceiling on this route is the level of distribution, and θ → 1 is precisely θ → (level → 1) = Elliott–Halberstam. **It is the shifted-prime condition that imposes this.** Binary Goldbach in the almost-all regime needs no such input, which is why Montgomery–Vaughan gives it unconditionally with a power-saving exceptional set; adding "and r − 1 has a large prime factor" couples the problem to primes in progressions to large moduli and puts it behind that barrier.

   Separately and still open: effectivize anything (à la Helfgott for ternary Goldbach), and record the computational verification of the covering-chain representations up to a large explicit bound.

...
2. **(Number theory — the attackable frontier.)** *Largely settled in the literature; the frontier has moved.* The problem as previously posed here — μ(n) ≥ n^{1.67−o(1)} for almost all n, unconditionally, via binary Goldbach restricted to the positive-density set of primes p with P(p−1) > p^θ — is **Shparlinski (2014), Corollary 3**: all but O(x^{0.354}(log x)⁴) integers n ≤ x satisfy the bound with exponent 1.677, unconditionally, on exactly the Baker–Harman input the problem identified. His Corollary 4 goes further in the same direction, giving c·n^{3/2} for all but O((log x)⁴) integers — so the Chowla-saturated pointwise 3/2 is already beaten in the almost-all sense, with a polylogarithmic exceptional set.

   What is actually open is **pushing θ past 0.677**. Shparlinski's Theorem 2 is parameterised by that exponent; Baker–Harman supplies 0.677 unconditionally, Elliott–Halberstam gives any θ < 1, and he records that the standard heuristic expects any θ < 1. Our hypothesis (H) is the **θ = 1 endpoint** of that ladder, which is a more informative way to state its cost than calling it a barrier. So the live questions are: improve the unconditional θ; or shrink the exceptional set at fixed θ; or find a route to the endpoint that does not pass through Elliott–Halberstam. See `literature-findings.md` §2.

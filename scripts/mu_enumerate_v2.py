...
# ---------------------------------------------------------------- parts
class Part:
    """One class of the configuration: F blocks of size c.

    2026-08.  The block count now splits as F = Fmid * Ftop, one factor per
    layer that can supply the block-permuting group:

      Ftop   a power of the top prime q, from Gamma/Gamma_1.  The only case the
             enumeration used to allow, because G.2 wrongly asserted the block
             count had to be a q-power.
      Fmid   ANY integer, from the cyclic layer Gamma_1/Gamma_2.  A transitive
             subgroup of a cyclic group is cyclic, so Fmid is unrestricted in
             itself; what constrains it is that the whole cyclic layer must stay
             cyclic, which is a GLOBAL condition across the configuration and is
             enforced in `value`, not here.
      Fbot   always 1, by Lemmas D1 and D2 -- bottom-layer copies of a matching
             block make the orbit a prime power already enumerated at (1, |O|)
             and scored higher there, and outside blocks cannot be fused at all.

    The within-class cross term takes the coefficient F or F/2 according to the
    parity of F, not of q: the relevant fact is the minimum pair-orbital of a
    transitive group of degree F, which is F/2 for even F and F for odd.
    """
    __slots__ = ("F", "Fmid", "Ftop", "c", "foreign", "size", "cap", "cb")

    def __init__(self, Fmid, Ftop, c, foreign, q, p, spf):
        F = Fmid * Ftop
        self.F = F
        self.Fmid = Fmid
        self.Ftop = Ftop
        ...
        self.cb = ((F if F % 2 else F // 2) * c * c) if F > 1 else None


def _fusions(fmax, q):
    """All (Fmid, Ftop) with Fmid*Ftop <= fmax.  Ftop runs over powers of q --
    the top layer is a q-group, so a transitive subgroup on the blocks has
    q-power degree.  Fmid runs over every integer, since a transitive subgroup
    of the cyclic layer is cyclic of that order and nothing local restricts it;
    the restriction is global (see `value`) and cannot be applied part by part.

    Yielded largest-first so the caller's size-descending sort is cheap."""
    out = []
    Ftop = 1
    while Ftop <= fmax:
        for Fmid in range(1, fmax // Ftop + 1):
            out.append((Fmid, Ftop))
        if q < 2:
            break
        Ftop *= q
    return out
...
def value(sel, p, spf, q=None):
    ...
    # The cyclic layer Gamma_1/Gamma_2 carries, all at once: the translation
    # group C_r of every foreign part, and the block-permuting group C_Fmid of
    # every class fused there.  A cyclic group has a unique subgroup of each
    # order, so all of these must be PAIRWISE COPRIME.  This is the global
    # condition that the pre-2026-08 enumeration did not have, because with
    # every block count a q-power there was nothing for the foreign primes to
    # collide with.  Two foreign parts of the same prime r are the special case
    # that was already excluded.
    if len(foreigns) != len(set(foreigns)):
        return None
    if not _coprime_ok(foreigns + [t.Fmid for t in sel]):
        return None
    terms = []
    for t in sel:
        if t.foreign:
